...
local int	procdata(struct proginfo *,char *,int,uint *,int,
			MEMFILE *,bfile *, cchar *) ;
...
int progfile(PI *pip,uint hasha,int nhash,memfile *mfp,bfile *nfp,
		ccchar *fname) noex {
	cint	size = (pip->pagesize * 4) ;
	int	rs ;
	int	llen = (size-1) ;
	char	*lbuf ;
...
cchar	fname[] ;
{
	...
	len = iceil(szof(postentry),szof(int)) ;
