...
/* revision history:
	...
	This code was originally written but it was probably started
	from any one of the numerous subroutine which perform a
	similar "file-processing" fron end.
...
/*******************************************************************************
	...
	Description:
	This program will read the input file, separate one paragraph
	from another and then post each paragraph using the MSGS
	program.
...
extern "C" {
    extern int	procfile(proginfo *,bfile *,char *,int) noex ;
}


/* forward references */

local int	helpfile(cchar *,bfile *) noex ;
...
/* get our configuration information from the PCS-wide configuration */
	...
	rs = pcsconf(pip->pr,nullptr,&cs,&sets,&extras,
...
/* log us */
	...
	if ((rs = logfile_open(&lh,buf,0,0666,u.logid)) >= 0) {
	    ...
	    buf[0] = '\0' ;
	    if (u.fullname != nullptr)
	        strcpy(buf,u.fullname) ;

	    else if (u.name != nullptr)
	        strcpy(buf,u.name) ;

	    else if (u.gecosname != nullptr)
...
/* OK, check to see if what we have as working parameters look OK so far */
	...
	if ((pip->prog_rbbpost != nullptr) && (pip->prog_rbbpost[0] != '/') &&
	    ((rs = pcsgetprog(pip->pr,buf,pip->prog_rbbpost)) >= 0)) {

		if (rs > 0) {

	    if (pip->prog_rbbpost != nullptr)
	...
	if ((pip->prog_msgs != nullptr) && (pip->prog_msgs[0] != '/') &&
	    ((rs = pcsgetprog(pip->pr,buf,pip->prog_msgs)) >= 0)) {

		if (rs > 0) {

	    if (pip->prog_msgs != nullptr)
	        free(pip->prog_msgs) ;
...
/* defaults */
	...
	if (pip->newsgroup == nullptr)
	    pip->newsgroup = DEFNEWSGROUP ;

	if (pip->spooldname == nullptr)
...
local int helpfile(cchar *f,bfile *ofp) noex {
	bfile	file, *ifp = &file ;
	int	rs ;
	int	rs1 ;
