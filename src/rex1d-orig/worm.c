...
/* execute some stuff on the remote which is supposed to be there already */
	...
	if ((ruser == NULL) || (ruser[0] == '\0'))
	    ...
	    wip->offset = bseek(wip,0L,SEEK_CUR) ;
...
/* OK, now send over the stuff that is fixed */
	...
	if (g.f.remote)
	    bprintf(wfp,"ORIGHOSTNAME=%s.%s\n",
	        u.nodename, u.domainname) ;

	else
	    bprintf(wfp,"ORIGHOSTNAME=%s\n",
	        u.nodename) ;

	bprintf(wfp,"export ORIGHOSTNAME\n") ;

	bprintf(wfp,"ORIGLOGNAME=%s\nexport ORIGLOGNAME\n",
	    u.username) ;
...
int		len ;
{
	...
	if (wip == NULL) return BAD ;

	if ((wip->offset < 0) || (wip->wfd < 0)) return BAD ;
	...
	if (rs >= 0) {
	    ...
	    if ((rs = write(wip->wfd,logname,len)) < 0) rs = (- errno) ;
...
struct worm	*wip ;
{
	...
	if (wip->wfd < 0) return BAD ;

	if ((rs = close(wip->wfd)) < 0) rs = (- errno) ;

	if (wip->f_delete) unlink(wip->wormfname) ;
