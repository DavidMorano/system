...
#if	CF_DEBUG
	        if (DEBUGLEVEL(2))
	            debugprintf("progfile: breadln() rs=%d\n",rs) ;
#endif
	...
	while ((rs >= 0) && (ll > 0)) {
	    ...
	    if (DEBUGLEVEL(3)) {
	        debugprintf("process/procline: li=%u olenr=%u\n",
	            ldp->li,ldp->olenr) ;
	        debugprintf("process/procline: tl=%u token=>%r<\n",
	            cl,cp,cl) ;
	    }
#endif /* CF_DEBUG */
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(3))
	            debugprintf("process/procline: cl=%u cp=>%r<\n",
	                cl,cp,cl) ;
#endif
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(3))
	        debugprintf("process/procline: copy-down cl=%u cp=>%r<\n",
	            cl,cp,cl) ;
#endif
