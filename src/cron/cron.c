...
#define	TMPSTDFNIN	"/tmp/crinXXXXXX"  /* file to put stdin in for cmd  */
#define	TMPDIR		"/tmp"
#define	PFX		"crout"
...
initialize(int firstpass)
{
	...
	/* stdout is log file */
	if (frdebugopen(ACCTFILE, "a", stdout) == NULL)
	...
	/* null for stdin */
	(void) frdebugopen("/dev/null", "r", stdin);
...
ex(struct event *e)
{
	...
	if (e->etype == CRONEVENT) {
		...
		if (e->of.ct.input != NULL) {
			if ((tmpfile = strdup(TMPSTDFNIN)) == NULL) {
