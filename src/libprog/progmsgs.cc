...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	                debugprintf("mailmsgstage_gathermsg: line=>%r<\n",
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(5))
	            debugprintf("progmsgs/procmsgenver: env a=>%r<\n",cp,cl) ;
#endif
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(5))
	            debugprintf("progmsgs/procmsgenver: env d=>%r<\n",cp,cl) ;
#endif
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(5))
	            debugprintf("progmsgs/procmsgenver: env r=>%r<\n",cp,cl) ;
#endif
...
static int procmsghdrs(PROGINFO *pip,PROCDATA *pdp)
{
	...
	for (i = 0 ; msghdrgets[i] != nullptr ; i += 1) {
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(2))
	            debugprintf("progmsgs/procmsg: i=%u r=>%r<\n",
...
#if	CF_DEBUG
	                if (DEBUGLEVEL(2))
	                    debugprintf("progmsgs/procmsg: j=%u r=>%r<\n",
...
static int procmsghdr_replyto(PROGINFO *pip,PROCDATA *pdp)
{
	...
	if ((rs = mailmsg_hdrval(msgp,hdr,&vp)) >= 0) {
	    ...
	    if (DEBUGLEVEL(2)) {
	        ...
	        debugprintf("progmsgs/procmsg: REPLYTO v=>%r<\n",
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("progmsgs/procmsghdr_errorsto: v=%r\n",vp,vl) ;
#endif
...
static int procmsgouthdrs(PROGINFO *pip,PROCDATA *pdp)
{
	...
	for (i = 0 ; msgouthdrs[i] != nullptr ; i += 1) {
#if	CF_DEBUG
	    if (DEBUGLEVEL(5))
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("progmsgs/procmsgouthdr_remaining: hdr=%r\n",
	            kp,kl) ;
#endif

	    strwcpy(hdrname,kp,MIN(kl,hdrlen)) ;
	    rs1 = vecstr_search(hlp,hdrname,cmpheadname,nullptr) ;
...
static int procmsgouthdr_cc(PROGINFO *pip,PROCDATA *pdp)
{
	...
	if ((rs = mailmsg_hdrval(msgp,hdr,&vp)) > 0) {
	    ...
	    pdp->tlen += rs ;
	    if (rs >= 0)
	        rs = procmsglogaddr(pip,nullptr,hdr,vp,vl) ;
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("progmsgs/procmsgouthdr_articleid: a=%r\n",
			cp,ml) ;
#endif
...
static int procmsgouthdr_deliveredto(PROGINFO *pip,PROCDATA *pdp)
{
	...
	if (mailmsg_hdrval(msgp,hdr,nullptr) == SR_NOTFOUND) {
	    VECOBJ	*rlp = pdp->rlp ;
	    RECIP	*rp ;
...
/* copy the body of the input to the output */
	...
	if (mip->clen >= 0) {
	    ...
	    while ((rs >= 0) && (lenr > 0)) {
	        ...
	        if (DEBUGLEVEL(4)) {
	            debugprintf("progmsgs/procmsgoutbody: ll=%u body=>%r<\n",
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("progmsgs/procmsgct: hdr=>%r<\n",hp,hl) ;
#endif
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("progmsgs/procmsgct: v=>%r<\n",vp,vl) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("progmsgs/procmsgce: hdr=>%r<\n",hp,hl) ;
#endif
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("progmsgs/procmsgce: v=>%r<\n",vp,vl) ;
#endif
...
int		*valp ;
{
	...
	if (hname == nullptr) return SR_FAULT ;
	if (pdp == nullptr) return SR_FAULT ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("procmsglogaddr: ent a=>%r<\n",
