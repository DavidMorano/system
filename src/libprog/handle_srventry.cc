...
#define	CF_AWAYSDEFAULT	0		/* always use default access */
...
extern int	processargs(PI *,char *,vecstr *) noex ;
extern int	execute(PI *,int,char *,char *,vecstr *,vecstr *) noex ;
extern int	badback(int,int,char *) noex ;
...
/* quote any service arguments given w/ the request into a single buffer */
	...
	for (i = 0 ; vecstr_get(&svcargs,i,&ap) >= 0 ; i += 1) {
	    ...
	    if ((qlen = quoteshellarg(qbuf,BUFLEN,ap,-1,nullptr)) >= 0)
...
/* check if this connection is allowed based on the service access */
	...
	rs = SR_OK ;
	if (cnp->fl.inet && ((ese.access != nullptr) || (pip->defacc != nullptr))) {
	    FIELD	af ;
	    ...
	    cp = (ese.access != nullptr) ? ese.access : pip->defacc ;
...
/* process the server access list */
	    ...
	    while ((fl = field_get(&af,nullptr)) >= 0) {
...
/* try our own netgroups */
	    ...
	    if (pip->fl.acctab)
	        rs = acctab_anyallowed(atp,&netgroups,&names,nullptr,nullptr) ;
...
/* get a program if we do not have one already */
	...
	if (program == nullptr) {
	    rs = SR_NOEXIST ;
	    if (ese.srvargs == nullptr) goto badnoserver ;
	    program = arg0 ;
...
/* can we execute this service daemon ? */
	...
	if ((rs = findfilepath(nullptr,programpath,program,X_OK)) < 0) {
...
/* do we want to change user or group? (more involved than you think!) */
	...
	} /* end if (username was not nullptr) */
	if (sep->ngroups >= 0) {
	...
	sl = bufprintf(lognamebuf,LOGBUFLEN,"LOGNAME=%s",
	    ((f_switched && (username)) ? username : pip->username)) ;
