...
#pragma	ident	"@(#)switch_utils.c	1.9	99/07/07 SMI"
 ...
 * If PAM_LDAP, PAM_NIS or PAM_NISPLUS is defined, then the
...
#ifdef PAM_LDAP
	case PAM_REP_LDAP:
		nss_search(&db_root, nss_ldap_passwd, NSS_DBOP_PASSWD_BYNAME,
		    &arg);
		break;
#endif
#ifdef PAM_NISPLUS
...
#endif /* (PAM_LDAP || PAM_NIS || PAM_NISPLUS) */
	...
	pam_handle_t *pamh;
{
	...
	"    passwd: files"));
	snprintf(messages[2], sizeof (messages[2]), PAM_MSG(pamh, 163,
	"    passwd: files ldap"));
	snprintf(messages[3], sizeof (messages[3]), PAM_MSG(pamh, 3,
	"    passwd: files nis"));
	snprintf(messages[4], sizeof (messages[4]), PAM_MSG(pamh, 4,
	"    passwd: files nisplus"));
	snprintf(messages[5], sizeof (messages[5]), PAM_MSG(pamh, 5,
	"    passwd: compat"));
	snprintf(messages[6], sizeof (messages[6]), PAM_MSG(pamh, 6,
	"    passwd: compat AND"));
	snprintf(messages[7], sizeof (messages[7]), PAM_MSG(pamh, 164,
	"    passwd_compat: ldap OR"));
	snprintf(messages[8], sizeof (messages[8]), PAM_MSG(pamh, 7,
	"    passwd_compat: nisplus"));
	snprintf(messages[9], sizeof (messages[9]), PAM_MSG(pamh, 8,
	...
	int nowarn;
{
	...
	/*
	 * XXX: Currently we do not support more than 2 services
	 */
		...
		if (strcmp(lkp->service_name, "files") == 0) {
			...
			if (confcomp == NULL) {
				...
				if (strcmp(confcomp->lookups->service_name,
					...
					"ldap") == 0) {
					/* Service is ldap */
					rep = PAM_REP_FILES | PAM_REP_LDAP;
					return (rep);
				} else {
