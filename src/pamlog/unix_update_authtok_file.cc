 ...
 * All rights reserved.
 */
#pragma	ident	"@(#)unix_update_authtok_file.c	1.14	99/07/14 SMI"
	...
	int nowarn;
{
	...
	orig_mask = umask(S_IAMB & ~(buf.st_mode & S_IRUSR));
	if ((tsfp = fopen(SHADTEMP, "w")) == NULL) {
		...
		(void) ulckpwdf();
		/* Set umask to the orig_mask before return */
		...
		while (*data != NULL) {
			...
			if ((value = attr_match("AUTHTOK_MAXAGE", *data))
				...
				if (maxdate == -1) {	/* turn off aging */
					...
					/*
					 * Also, this has to done only if
					 * the passwd entry is NOT empty.
					 * Else, gives an impression that
					 * empty passwd is OK and login
					 * succeeds with empty passwd.
					 */
#ifdef _LP64
					unix_sp->sp_lstchg = (time32_t)DAY_NOW;
#else
					unix_sp->sp_lstchg = (time_t)DAY_NOW;
#endif
				}
	...
	int		nowarn;
{
	...
	orig_mask =  umask(S_IAMB & ~(stat_buf.st_mode & S_IRUSR));
	if ((tpfp = fopen(PASSTEMP, "w")) == NULL) {
		...
		/* Set umask to orig_mask before return */
		(void) umask(orig_mask);
		return (PAM_AUTHTOK_ERR);
	}
