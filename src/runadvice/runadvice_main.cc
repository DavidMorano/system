/* runadvice_main SUPPORT (Run Advice) */
/* charset=ISO8859-1 */
/* lang=C++20 */
...
/* revision history:
	...
	This program was pretty extensively modified to take much
	more flexible combinations of user supplied paramters.
...
/*******************************************************************************
	...
	controlled substitution of various parameters in the ADVICE
	input and circuit files.
...
/* some other global initialization */
	...
	g.envv = envv ;
	g.cwd = nullptr ;
	if (((cp = getenv(VARPWD)) != nullptr) && (access(cp,X_OK) == 0)) {
	    g.cwd = mallocstrw(cp,-1) ;
	}

	if (g.cwd == nullptr) {
	    g.cwd = getcwd(nullptr,0) ;
	}

	g.runfname = DEF_RUNFNAME ;
	g.ofname = DEF_STDFNOUT ;
#ifdef	COMMENT
	...
	if ((cp = getenv("RUNADVICE_ADVICE")) != nullptr)
	    g.prog_advice = cp ;

/* other initialization */

	for (i = 0 ; i < 3 ; i += 1) fpa[i] = nullptr ;

#ifdef	COMMENT
	configfname = "config.ra" ;
#endif
	if ((cp = getenv("RUNADVICE_CONFIG")) != nullptr)
	    configfname = cp ;
...
/* set up the csignalandlers */
	...
	sigaction(SIGTERM,&ss,nullptr) ;

	sigaction(SIGHUP,&ss,nullptr) ;
...
#endif
	sigaction(SIGPIPE,&ss,nullptr) ;

	bflush(g.efp) ;

/* other general initialization */

	userinfo(&u,userbuf,USERINFO_LEN,nullptr) ;
...
/* make sure that we are a "real" slave program */
	    ...
	    if ((cp = getenv("RUNMODE")) == nullptr) goto badslave ;
...
/* make the first log entry */
	...
	logfile_printf(&g.lh,"%02d%02d%02d %02d%02d:%02d M     %-14s %s/%s\n",
	    ...
	    timep->tm_min,
	    timep->tm_sec,
	    g.progname,(g.fl.sysv_ct) ? "SYSV" : "BSD",VERSION) ;

	buf[0] = '\0' ;
	if (u.mailname != nullptr) 
...
/* handle TMPDIR */
	        ...
	        if ((g.tmpdir == nullptr) && (ch.tmpdir != nullptr) &&
	            ((cp = strchr(ch.tmpdir,'=')) != nullptr))
	            g.tmpdir = mallocstrw((cp + 1),-1) ;

/* what about a default ADVICE program ? */

	        if ((g.prog_advice == nullptr) && (ch.advice != nullptr) &&
	            ((cp = strchr(ch.advice,'=')) != nullptr))
	            g.prog_advice = mallocstrw((cp + 1),-1) ;

/* ADVICE files */

	        if ((g.confname == nullptr) && (ch.control != nullptr) &&
	            ((cp = strchr(ch.control,'=')) != nullptr))
	            g.confname = mallocstrw((cp + 1),-1) ;

	        if ((g.paramfname == nullptr) && (ch.params != nullptr) &&
	            ((cp = strchr(ch.params,'=')) != nullptr))
	            g.paramfname = mallocstrw((cp + 1),-1) ;

	        if ((g.maifname == nullptr) && (ch.main != nullptr) &&
	            ((cp = strchr(ch.main,'=')) != nullptr))
...
/* what is the "run" file that we will be using ? */
	...
	if ((g.runfname == nullptr) || (g.runfname[0] == '-') || 
	    (g.runfname[0] == '\0')) {
...
/* perform a final check on the ADVICE data output file name */
	...
	if ((g.ofname == nullptr) || (g.ofname[0] == '\0') || 
	    (g.ofname[0] == '-'))
...
/* file name defaults */
	...
	if (g.confname == nullptr)
	    g.confname = DEF_CONCKT ;

	if (g.paramfname == nullptr)
	    g.paramfname = DEF_PARAMS ;

	if (g.maifname == nullptr)
	    g.maifname = DEF_MAINCKT ;
...
/* add default parameters where they were not specified by the user */
	...
	for (i = 0 ; dpa[i].paramname != nullptr ; i += 1) {
...
/* print out the accumulated run parameter information */
	...
	if (g.debuglevel > 1)
	    ...
	    n = 1 ;
	    i = 0 ;
	    for (pp = g.plist ; pp != nullptr ; pp = pp->next) {

	        if (pp->top != nullptr) pp->current = pp->top ;

	if (g.fl.verbose || (g.debuglevel > 1))
	        bprintf(efp,"%s: p=\"%s\"\n",
	            g.progname,pp->name) ;

	        j = 0 ;
	        for (vp = pp->top ; vp != nullptr ; vp = vp->next) {
...
completed:
	if ((g.debuglevel > 0) || g.fl.verbose) {
...
/* csignalandlers */
local void int_signal(int signo) noex {
    	(void) signo ;
	g.f_signal = true ;
}

local void int_pipe(int signo) noex {
    	(void) signo ;
	g.f_pipe = true ;
} /* end csignalandlers */

/* is the provided load OK ? */
local int loadok(int f_i,long spare) noex {
	if (f_i) {
