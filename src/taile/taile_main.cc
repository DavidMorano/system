...
constexpr const pivars	initvars = {
	VARPROGRAMROOT1,
	VARPROGRAMROOT2,
	VARPROGRAMROOT3,
	PROGRAMROOT,
...
/* any filter files? */
	...
	if (pip->have.ssfile || pip->have.sxfile) {
	    rs = linefilter_start(&sf,pip->ssfname,pip->sxfname) ;
	    if (rs < 0)
	        goto badfilterinit ;
	    sfp = &sf ;
...
static int procopts(proginfo *pip,keyopt *kop) noex {
	...
	if (rs >= 0) {
	    if (keyopt_cur cur ; (rs = keyopt_curbegin(kop,&cur)) >= 0) {
	        uint	uv ;
	        int	oi ;
	        int	kl, vl ;
	        cchar	*kp, *vp ;
