...
/* revision history:
	...
	Sun does not support LOGDIR or LOGNAME at this time.  There
	...
	user information besides the name of the user and login
	home directory.
...
/* grab the names that are in the argument file */
	...
	if ((rs >= 0) && (afname != NULL) && (afname[0] != '\0')) {
	    ...
	    if (rs >= 0) {
	        ...
	        while ((rs = breadln(&ifile,linebuf,LINELEN)) > 0) {
		    len = rs ;
