...
local int	procopts(PI *) ;
local int	procfts(PI *) ;
local int	procname(PI *,cchar *) ;
local int	procdir(PI *,cchar *,ustat *) ;
local int	procother(PI *,cchar *,ustat *) ;

local int	procsufbegin(PI *) ;
local int	procsufhave(PI *,cchar *,int) ;
local int	procsufend(PI *) ;

local int	procloadsuf(PI *,int,cchar *,int) ;

local int	procprintfts(PI *,cchar *) ;
local int	procprintsufs(PI *,cchar *) ;

local int	proclinkbegin(PI *) ;
local int	proclinkadd(PI *,ino64_t,cchar *) ;
local int	proclinkhave(PI *,ino64_t,LI **) ;
local int	proclinkend(PI *) ;

local int	procsize(PI *,cchar *, ustat *,FI *) ;
local int	proclink(PI *,cchar *, ustat *,FI *) ;
local int	procsync(PI *,cchar *, ustat *,FI *) ;
local int	procrm(PI *,cchar *, ustat *,FI *) ;
...
local int	linkinfo_start(LI *,ino64_t,cchar *) ;
local int	linkinfo_finish(LI *) ;

local int	mkpdirs(cchar *,mode_t) ;
local int	istermrs(int) ;
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("main: fin mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
local int procdir(PI *pip,cchar *name,ustat *sbp) noex {
	int		rs = SR_OK ;
	int		sz ;
	int		nl ;
	int		c = 0 ;
...
local int procother(PI *pip,cc *name,ustat *sbp) noex {
	fileinfo	ck{}, *ckp = &ck ;
...
local int proclinkbegin(PI *pip)
{
	hdb		*dbp = &pip->links ;
...
ino64_t		ino ;
{
	...
	hdb_dat	key, val ;
	LINKINFO	*lip ;
	cint	sz = szof(LINKINFO) ;
	int	rs ;
	if ((rs = lm_mall(sz,&lip)) >= 0) {
	    rs = linkinfo_start(lip,ino,fp) ;
	    if (rs >= 0) {
	        key.buf = &lip->ino ;
	        key.len = szof(ino64_t) ;
	        val.buf = lip ;
	        val.len = sz ;
...
local int proclinkhave(PI *pip,ino64_t ino,LI **rpp) noex {
	hdb		*dbp = &pip->links ;
	hdb_dat		key, val ;
	int	rs ;
...
local int procsize(PI *pip,cc *name,ustat *sbp,fileinfo *ckp) noex {
	ulong	bytes ;
	ulong	sz ;
	int	rs ;
...
local int proclink(pip,name,sbp,ckp)
struct proginfo	*pip ;
cchar	name[] ;
ustat	*sbp ;
struct fileinfo	*ckp ;
...
local int procsync(pip,name,sbp,ckp)
struct proginfo	*pip ;
cchar	name[] ;
ustat	*sbp ;
struct fileinfo	*ckp ;
...
local int procsynclink(pip,name,sbp,lip)
struct proginfo	*pip ;
cchar	name[] ;
ustat	*sbp ;
LINKINFO	*lip ;
...
local int procsyncer(pip,name,sbp)
struct proginfo	*pip ;
cchar	name[] ;
ustat	*sbp ;
{
...
local int procsyncer_reg(pip,name,sbp)
struct proginfo	*pip ;
cchar	name[] ;
ustat	*sbp ;
{
...
#if	CF_DEBUG
	                if (DEBUGLEVEL(3))
	                    debugprintf("main/procsyncer_reg: filetime()\n") ;
#endif
	                uc_fileutime(dstfname,&ut) ;
	            }
	        }
	        } else if (rs == SR_NOENT) {
			rs = SR_OK ;
...
local int procsyncer_dir(pip,name,sbp)
struct proginfo	*pip ;
cchar	name[] ;
ustat	*sbp ;
{
...
/* update (or create) the target file */
	...
	if ((rs >= 0) && ((duid < 0) || (duid == pip->euid))) {
	    f_updated = true ;
	    if (f_mode) {
		rs = u_chmod(dstfname,nm) ;
	    }
	    if ((rs >= 0) && f_mtime) {
		u_filetime(dstfname,&ut) ;
	    }
	} /* end if */
...
local int procsyncer_fifo(pip,name,sbp)
struct proginfo	*pip ;
cchar	name[] ;
ustat	*sbp ;
{
...
local int procsyncer_sock(pip,name,sbp)
struct proginfo	*pip ;
cchar	name[] ;
ustat	*sbp ;
{
...
local int procrm(pip,name,sbp,ckp)
struct proginfo	*pip ;
cchar	name[] ;
ustat	*sbp ;
struct fileinfo	*ckp ;
...
local int linkinfo_start(LI *lip,ino64_t ino,cchar *fp) noex {
	int	rs ;
	cchar	*cp ;
	lip->ino = ino ;
	rs = lm_strw(fp,-1,&cp) ;
...
local int istermrs(int rs) noex {
	int	f = false ;
...
local int linkcmp(struct linkinfo *e1p,struct linkinfo *e2p,int len) noex {
	const int64_t	d = (e1p->ino - e2p->ino) ;
	int		rc = 0 ;
	if (d > 0) {
	    rc = 1 ;
	} else if (d < 0) {
