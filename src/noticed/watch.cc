...
#define	CF_REQUESTS	1		/* handle FIFO requests, usually on */
...
/*****************************************************************************
	...
	connection.  This subroutine is called with the folowing
	arguments:
...
#define	TI_POLL		4		/* u_poll(2) interval */
...
#if	CF_DEBUG
	    if (pip->debuglevel > 1)
	        debugprintf("watch: TCP listening on FD=%d\n",sip->fd_listentcp) ;
#endif
...
#if	CF_DEBUG
	                    if (pip->debuglevel > 1)
	                        debugprintf("watch: bad u_accept rs=%d\n",
	                            ns) ;
#endif
