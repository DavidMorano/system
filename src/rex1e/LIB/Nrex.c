...
/* initialize some other common user stuff */
	...
	if ((rs = userinfo(&u,userbuf,USERBUFLEN)) < 0)
	    if (u.domainname == NULL) u.domainname = "" ;

#if	F_DEBUG
	eprintf("rex: got user information\n") ;
#endif
...
#if	F_DEBUG
	            eprintf("rex: service specified port=%d\n",
	                port) ;
#endif
...
#if	F_DEBUG
	eprintf("rex: creating remote command\n") ;
#endif
...
#if	F_DEBUG
	    eprintf("rex: executing REXEC on supplied password\n") ;
#endif

#if	F_DEBUG
	    eprintf("rex: supplied m=\"%s\" u=\"%s\" p=\"%s\"\n",
...
#if	F_DEBUG
	    eprintf("rex: back from REXECL w/ rs=%d\n",
	        rs) ;
#endif
...
#if	F_DEBUG
	eprintf("rex: about to process the NETRC files\n") ;
#endif
...
#if	F_DEBUG
	        eprintf("rex: looking at entry m=\"%s\"\n",
	            mp->machine) ;
#endif
...
#if	F_DEBUG
	            eprintf("rex: trying entry m=\"%s\" u=\"%s\"\n",
	                mp->machine,mp->login) ;
#endif
...
#if	F_DEBUG
	    eprintf("rex: no matching NETRC entries\n") ;
#endif
...
#if	F_DEBUG
	    eprintf("rex: executing RCMDU u=%s\n",username) ;
#endif
...
#if	F_DEBUG
	eprintf("rex: executing REXEC with blank password\n") ;
#endif
...
#if	F_DEBUG
	    eprintf("rex: couldn't find a NETRC entry \n") ;
#endif
...
#if	F_DEBUG
	    eprintf("rex: use our own username w/ RCMDU\n") ;
#endif
...
#if	F_DEBUG
	eprintf("rex: failed all authorization attempts\n") ;
#endif
