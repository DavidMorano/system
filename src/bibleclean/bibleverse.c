...
static int	bibleverse_objloadbegin(BIBLEVERSE *,
...
{
	return bibleverse_read(op,qp,vbuf,vlen) ;
}
/* end subroutine (bibleverse_get) */


int bibleverse_read(BIBLEVERSE *op,BIBLEVERSE_Q *qp,char *vbuf,int vlen)
{
	BIBLEVERSES_Q	sq ;
	int		rs ;
...
#if	CF_DEBUGS
	debugprintf("bibleverse_get: q=%u:%u:%u\n",
		qp->b,qp->c,qp->v) ;
#endif
	...
	sq.c = qp->c ;
	sq.v = qp->v ;
	rs = (*op->call.get)(op->obj,&sq,vbuf,vlen) ;

#if	CF_DEBUGS
	debugprintf("bibleverse_get: ret rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("bibleverse_objloadbegin: modload_open() rs=%d\n",rs) ;
#endif
