...
#define	CF_DEBUGMALL	1		/* debug memory-allocationcs */
...
static int	procname(cchar *,cchar *) ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("main: un=%s project=%s\n",un,projname) ;
#endif

	if (projname == NULL)
	    goto badnoproj ;

	if ((un == NULL) || (strcmp(un,"-") == 0)) {
	    un = usernamebuf ;
...
static int usage(PROGINFO *pip)
{
	...
	fmt = "%s: USAGE> %s <project> [<username>] [-q]\n" ;
...
static int procname(cchar *un,cchar *projname)
{
