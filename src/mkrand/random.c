...
/* standard C-language library random object */
...
/*******************************************************************************
	...
	This is a knock-off of the UNIX® System |random(3c)| library
...
constexpr int degrees[MAX_TYPES] = {
	DEG_0, DEG_1, DEG_2, DEG_3, DEG_4 
} ;

constexpr int seps[MAX_TYPES] =	{
...
int stdrandom_start(stdrandom *rp,int type,uint seed) noex {
	ulong		hi, lo ;
	uint		ihi, ilo, uiw ;
	int		i ;
...
/* end subroutine (stdrandom_start) */

int stdrandom_finish(stdrandom *rp) noex {
	int		rs = SR_FAULT ;
	if (rp) {
	    rs = SR_OK ;
	} /* end if (non-null) */
...
/* end subroutine (stdrandom_finish) */

int stdrandom_getuint(stdrandom *rp,uint *uip) noex {
	uint		rv ;
...
/* end subroutine (stdrandom_getuint) */

int stdrandom_getint(stdrandom *rp,int *ip) noex {
	int		rv ;
...
/* end subroutine (stdrandom_getint) */

int stdrandom_getulong(stdrandom *rp,ulong *ulp) noex {
	ulong		rv, hi, lo ;
	uint		ihi, ilo ;
...
/* end subroutine (stdrandom_getulong) */
