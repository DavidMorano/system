...
#define	NDEBFNAME	"opensvcprojinfo.deb"
...
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"opensvc_projinfo: 0 un=%s\n",un) ;
#endif
...
/* process this user */
	...
	if (rs >= 0) {
	if ((rs = pcsgetprojinfo(pr,lbuf,llen,un)) >= 0) {
	    ll = rs ;
	    if (ll > cols) ll = cols ;
	    if (ll > 0) {
	        lbuf[ll++] = '\n' ;
