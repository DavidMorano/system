...
local int	procdefs_begin(PROGINFO *) ;
local int	procdefs_end(PROGINFO *) ;

local int	procenvs_begin(PROGINFO *) ;
local int	procenvs_end(PROGINFO *) ;

local int	procexps_begin(PROGINFO *,cchar *) ;
...
local int	procenvextra(PROGINFO *) ;
local int	procenvdef(PROGINFO *) ;
local int	procenvuser(PROGINFO *) ;
local int	procenvsort(PROGINFO *) ;
local int	procenvname(PROGINFO *,cchar *,cchar *,cchar *) ;

#if	CF_SYSVAR
local int	procenvsysvar(PROGINFO *,cchar *) ;
#endif

local int	procsched_begin(PROGINFO *) ;
local int	procsched_end(PROGINFO *) ;

local int	pvarstart(PROGINFO *,cchar **,cchar *) ;
local int	pvarfinish(PROGINFO *) ;

local int	loadtz(PROGINFO *) ;
local int	loadgroupname(PROGINFO *) ;
local int	loadaux(PROGINFO *) ;
local int	loadhz(PROGINFO *) ;
local int	loadncpu(PROGINFO *) ; /* may not be thread-safe */
local int	loadnodeinfo(PROGINFO *) ;
local int	loadorganization(PROGINFO *) ;
local int	loadcooks(PROGINFO *) ;
local int	loaddefsfile(PROGINFO *,cchar *) ;
local int	loaddefs(PROGINFO *,cchar **) ;
local int	loadxfile(PROGINFO *,cchar *) ;
local int	loadxsched(PROGINFO *,cchar **) ;
...
local int	ourconf_defprog(OURCONF *,cchar *,int) ;
local int	ourconf_nopreload(OURCONF *,cchar *,int) ;
local int	ourconf_getdefprog(OURCONF *,cchar **) ;
local int	ourconf_ifnopreload(OURCONF *) ;
...
/* gather the arguments */
	...
	argr = (argc - 1) ;
	argi = ai ;
	while ((rs >= 0) && (program == nullptr) && (argr > 0) && f_envset) {
...
/* continue */
	...
	if (DEBUGLEVEL(2)) {
	    ...
	    debugprintf("main: next ai=%u arg=>%s<\n",
	        ai,
	        (((ai < argc) && (argv[ai] != nullptr)) ? argv[ai] : "*NA*")) ;
	    debugprintf("main: remaining args>\n") ;
	    {
	        int	i ;
	        for (i = argi ; argv[i] != nullptr ; i += 1) {
	            debugprintf("main: arg[%u]=>%s<\n",i,argv[i]) ;
	        }
	    }
	}
...
/* initialization */
	...
	if (pip->cfname == nullptr) pip->cfname = getenv(VARCFNAME) ;
	if (pip->dfname == nullptr) pip->dfname = getenv(VARDFNAME) ;
	if (pip->xfname == nullptr) pip->xfname = getenv(VARXFNAME) ;
	...
	if (rs >= 0)  {
	    if ((rs = proginfo_rootname(pip)) >= 0) {
		    ...
		    if ((pip->niceval == 0) && (argval != nullptr)) {
...
int progdefprog(PROGINFO *pip,cchar **rpp)
{
	...
	*rpp = nullptr ;
	...
	if ((rs >= 0) && (pip->defprog == nullptr)) {
	    if ((vp = getenv(VARDEFPROG)) != nullptr) {
	        if (vp[0] != '\0') {
...
local int procenvextra(PROGINFO *pip)
{
	NULSTR		ns ;
...
local int procenvdef(PROGINFO *pip)
{
	vecstr		*elp = &pip->exports ;
	...
	for (i = 0 ; envdefs[i] != nullptr ; i += 1) {
	    ename = envdefs[i] ;
	    if ((rs = vecstr_search(elp,ename,vstrkeycmp,nullptr)) == nrs) {
...
#if	CF_SYSVAR
local int procenvsysvar(PROGINFO *pip,cchar *sysvardb)
{
	SYSVAR		sv ;
	SYSVAR_CUR	cur ;
	int		rs ;
...
local int procenvuser(PROGINFO *pip)
{
	vecstr		*elp = &pip->exports ;
	cint	nrs = SR_NOTFOUND ;
	int		rs = SR_OK ;
...
/* environment LOCALDOMAIN */
	...
	if (rs >= 0) {
	    ...
	    if ((rs = vecstr_envget(elp,varlocaldomain,nullptr)) == nrs) {
...
/* environment TZ */
	...
	if (rs >= 0) {
	    cchar	*vartz = VARTZ ;
	    if ((rs = vecstr_envget(elp,vartz,nullptr)) == nrs) {
	        rs = SR_OK ;
	        vp = pip->tz ;
...
/* environment NAME */
	...
	if (rs >= 0) {
	    rs = procenvname(pip,VARNAME,NAMECNAME,nullptr) ;
	    c += rs ;
	}
...
local int procenvname(pip,varname,fname,prename)
PROGINFO	*pip ;
cchar		*varname ;
cchar		fname[] ;
cchar		*prename ;
{
	...
	if (varname == nullptr) return SR_FAULT ;
	...
	if ((rs = vecstr_envget(elp,varname,nullptr)) == nrs) {
	    cchar	*np = nullptr ;
	    int		nl = -1 ;
	    if ((prename != nullptr) && (prename[0] != '\0')) {
	        rs = SR_OK ;
	        np = prename ;
	    } else {
...
local int procenvsort(PROGINFO *pip)
{
	int		rs ;
...
local int procexpcooks_begin(PROGINFO *pip)
{
	int		rs ;
...
local int procexpcooks_end(PROGINFO *pip)
{
	int		rs = SR_OK ;
	int		rs1 ;
...
local int procourconf_cfname(PROGINFO *pip)
{
...
/* configuration file */
	...
	if ((cfn == nullptr) || ((cfn[0] != '\0') && (cfn[0] != '-'))) {
	    ...
	    if (rs1 == nrs) {
	        int	i ;
	        cchar	*apr ;
	        for (i = 0 ; (rs >= 0) && (altprs[i] != nullptr) ; i += 1) {
	            apr = altprs[i] ;
...
local int procourconf_getdefprog(PROGINFO *pip,cchar **rpp)
{
	int		rs = SR_OK ;
	if (pip->config != nullptr) {
	   OURCONF	*ocp = pip->config ;
...
local int procdefs_begin(PROGINFO *pip)
{
	vecstr		*dlp = &pip->defs ;
	int		rs ;
	if ((rs = vecstr_start(dlp,DEFNDEFS,0)) >= 0) {
	    pip->open.defs = true ;
	    if ((pip->dfname != nullptr) && (pip->dfname != '\0')) {
...
local int procdefs_end(PROGINFO *pip)
{
	int		rs = SR_OK ;
	int		rs1 ;
	if (pip->open.defs) {
...
local int procenvs_begin(PROGINFO *pip)
{
	ENVS		*xlp = &pip->xenvs ;
	int		rs ;
	if ((rs = envs_start(xlp,DEFNXENVS)) >= 0) {
	    pip->open.envs = true ;
	    if ((pip->xfname != nullptr) && (pip->xfname[0] != '\0')) {
...
local int procenvs_end(PROGINFO *pip)
{
	int		rs = SR_OK ;
	int		rs1 ;
	if (pip->open.envs) {
...
local int procexps_begin(PROGINFO *pip,cchar *pvfname)
{
...
local int procexps_end(PROGINFO *pip)
{
	int		rs = SR_OK ;
	int		rs1 ;
	rs1 = pvarfinish(pip) ;
...
local int procloadinfo(PROGINFO *pip)
{
	int		rs ;
	if ((rs = loadtz(pip)) >= 0) {
...
local int processor(PROGINFO *pip,ARGINFO *aip,cchar *prog) noex {
	int		rs ;
	char		pbuf[MAXPATHLEN+1] ;
	if ((prog != nullptr) && hasonlypm(prog,-1)) {
	    if ((! pip->fl.progdash) && (hasonlymi(prog,-1))) {
...
local int procnopreload(PROGINFO *pip)
{
	int		rs = SR_OK ;
	int		f = FALSE ;
#if	CF_DEBUG
	...
	if (pip->have.nopreload) {
		...
		cchar		*k = "LD_PRELOAD" ;
	    	if ((rs = vecstr_search(elp,k,vstrkeycmp,nullptr)) >= 0) {
#if	CF_DEBUG
		    if (DEBUGLEVEL(5))
...
local int pvarstart(PROGINFO *pip,cchar **pathvars,cchar *fname)
{
...
local int pvarfinish(PROGINFO *pip)
{
	vecstr		*pvp = &pip->pvars ;
	int		rs = SR_OK ;
	int		rs1 ;
...
local int loadpvars(PROGINFO *pip,cchar **sched,cchar *fname)
{
	VECSTR		*pvp = &pip->pvars ;
	cint	tlen = MAXPATHLEN ;
	int		rs = SR_OK ;
...
local int loadpvarsdef(PROGINFO *pip,cchar **pnames)
{
	vecstr		*pvp = &pip->pvars ;
	int		rs ;
	int		i = 0 ;
...
local int loadtz(PROGINFO *pip)
{
	...
	if ((pip->tz == nullptr) || (pip->tz[0] == '\0') || (sid == pip->pid)) {
	    USERATTR	ua ;
	    cint	alen = TZLEN ;
	    int		al = 0 ;
	    char	abuf[TZLEN+1] = { 0 } ;
	    ...
	    if (rs >= 0) {
	        ...
	        if ((pip->tz == nullptr) || (pip->tz[0] == '\0')) {
	            if (abuf[0] == '\0') {
	                rs = inittimezone(abuf,alen,nullptr) ;
	                al = rs ;
	            }
	        } /* end if */
...
local int loadgroupname(PROGINFO *pip)
{
	...
	if ((pip->groupname == nullptr) || (pip->groupname[0] == '\0')) {
...
local int loadaux(PROGINFO *pip)
{
	UINFO_AUX	ua ;
	int		rs ;
	...
	if ((rs = uinfo_aux(&ua)) >= 0) {
	    ...
	    for (i = 0 ; (rs >= 0) && (i < 5) ; i += 1) {
	        ...
	        if ((cp = getourenv(pip->envv,vn)) != nullptr) vp = cp ;
	        if (vp != nullptr) {
	            rs = proginfo_setentry(pip,vpp,vp,-1) ;
	        }
	    } /* end for */
...
local int loadncpu(PROGINFO *pip)
{
	int		rs ;
...
local int loadnodeinfo(PROGINFO *pip)
{
	int		rs = SR_OK ;
	...
	if ((cluster == nullptr) || (system == nullptr)) {
	    cchar	*nn = pip->nodename ;
	    if ((rs = getnodeinfo(pip->pr,cbuf,sbuf,nullptr,nn)) >= 0) {
	        if (cluster == nullptr) cluster = cbuf ;
	        if (system == nullptr) system = sbuf ;
	    } else if (isNotPresent(rs) || (rs == SR_OVERFLOW))
	        rs = SR_OK ;
	} /* end if (getnodeinfo) */

	if ((rs >= 0) && (cluster != nullptr)) {
	    cchar	**vpp = &pip->clustername ;
	    rs = proginfo_setentry(pip,vpp,cluster,-1) ;
	}

	if ((rs >= 0) && (system != nullptr)) {
...
local int loadorganization(PROGINFO *pip)
{
...
local int loaddefsfile(PROGINFO *pip,cchar *dfname)
{
	ustat	sb ;
	int		rs = SR_OK ;
	int		rs1 ;
...
local int loaddefs(PROGINFO *pip,cchar *sched[])
{
	cint	tlen = MAXPATHLEN ;
	int		rs ;
	int		f = FALSE ;
...
local int loadxfile(PROGINFO *pip,cchar xfname[])
{
	ustat	sb ;
	int		rs = SR_OK ;
	int		rs1 ;
...
local int loadxsched(PROGINFO *pip,cchar *sched[])
{
	cint	tlen = MAXPATHLEN ;
	int		rs ;
	int		f = FALSE ;
...
local int ourconf_finish(OURCONF *ocp)
{
	int		rs = SR_OK ;
	int		rs1 ;
...
local int ourconf_setentry(OURCONF *lip,cchar **epp,cchar *vp,int vl)
{
	int		rs = SR_OK ;
	int		len = 0 ;
...
local int ourconf_read(OURCONF *ocp)
{
	...
	if ((rs = paramfile_open(&pf,ev,cfname)) >= 0) {
	    ...
	    for (oi = 0 ; cparams[oi] != nullptr ; oi += 1) {
...
local int ourconf_defprog(OURCONF *ocp,cchar *vp,int vl)
{
	int		rs = SR_OK ;
	if ((vp != nullptr) && (ocp->defprog == nullptr)) {
	    PROGINFO	*pip = ocp->pip ;
	    char	tbuf[MAXPATHLEN+1] ;
...
local int ourconf_getdefprog(OURCONF *ocp,cchar **rpp)
{
	int		rs = SR_OK ;
	if (rpp != nullptr) {
	    *rpp = ocp->defprog ;
