...
/*******************************************************************************
  	...
  	Description:
	This progrm displays UTMP entries.
...
local int	process(PI *,bfile *,pid_t) noex ;
local int	zaputx(utmpx *) noex ;
...
cint		utsz_id		= utmpsize.id ;
cint		utsz_user 	= utmpsize.user ;
cint		utsz_line 	= utmpsize.line ;
cint		utsz_home 	= utmpsize.host ;
...
local int process(PI *pip,bfile *ofp,pid_t pid) noex {
	utmpx		uc ;
	utmpx		*up ;
	LI		*lip = pip->lip ;
...
/* loop */
	...
	while ((up = getutxent()) != nullptr) {
	    if (lip->fl.zap && (lip->searchline != nullptr)) {
		cchar	*searchline = lip->searchline ;

	        if (strncmp(up->ut_line,searchline,utsz_line) == 0)
	            break ;

	    } else if ((up->ut_type == utmptype.procuser) &&
	        	(up->ut_pid == sid)) {
	        break ;
	    } /* end if */
	...
	if (up != nullptr) {
	    strwcpy(lognamebuf,up->ut_user,utsz_user) ;
	}
	...
	if (lip->fl.zap && (lip->searchline != nullptr)) {
	...
	} else if ((! lip->fl.optyes) && (! lip->fl.optno)) {
	    fmt = "id=%-4t line=%-12t user=%-12t sid=%-5u ses=%u host=%r\n" ;

	    if (up != nullptr) {

	        bprintf(ofp,fmt,
	            up->ut_id,strnlen(up->ut_id,utsz_id),
	            up->ut_line,strnlen(up->ut_line,utsz_line),
	            up->ut_user,strnlen(up->ut_user,utsz_user),
	            up->ut_pid,
	            up->ut_session,
	            up->ut_host,strnlen(up->ut_host,utsz_host)) ;

	    } else {
	        bprintf(ofp,"not logged in\n") ;
	    }

	} else if (lip->fl.optyes) {
		...
		cint	termlen = TERMBUFLEN ;
	        ...
	        if ((rs = termdevice(termbuf,termlen,FD_STDIN)) >= 0) {
	            char	idbuf[utsz_id + 1] ;
	            tl = rs ;
	            uc = {} ;
	            mkutmpid(idbuf,utsz_id,termbuf,tl) ;

	            strncpy(uc.ut_id,idbuf,utsz_id) ;

	            strncpy(uc.ut_line,(termbuf + 5),utsz_line) ;

	            if (lognamebuf[0] == '\0') {
	                getusername(lognamebuf,USERNAMELEN,-1) ;
		    }

	            strncpy(uc.ut_user,lognamebuf,utsz_user) ;

	            if (pip->hostname != nullptr) {

	                strncpy(uc.ut_host,pip->hostname,utsz_host) ;

	                cl = strnlen(pip->hostname,utsz_host) ;
	            ...
	            gettimeofday(&uc.ut_tv,nullptr) ;

	            uc.ut_pid = sid ;
	            uc.ut_exit.e_termination = 0 ;
	            uc.ut_exit.e_exit = 0 ;
	            uc.ut_type = utmptype.procuser ;
...
local int zaputx(utmpx *up) noex {
    	cnullptr	np{} ;
	int		rs ;
	int		rs1 ;
	int		ei = 0 ;
	if (tmpx ut ; (rs = tmpx_open(&ut,np,O_RDWR)) >= 0) {
	    if (tmpx_cur cur ; (rs = tmpx_curbegin(&ut,&cur)) >= 0) {
		tmpx_ent	ue ;
		bool		f = false ;
	        while ((ei = tmpx_curenum(&ut,&cur,&ue)) >= 0) {

	            f = ((strncmp(up->ut_id,ue.ut_id,utsz_id) == 0) &&
	                (strncmp(up->ut_line,ue.ut_line,utsz_line) == 0)) ;
