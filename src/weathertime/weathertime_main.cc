...
/* check up on the arguments */
	...
	if ((obfname == nullptr) || (obfname[0] == '\0'))
	    goto badnofile ;
	...
	if (rs >= 0) {
	    ...
	    while ((len = breadln(&infile,linebuf,LINELEN)) > 0) {
	...
	debugprintf("main: len=%d linebuf=>%r<\n",
	    ...
	    if (len > 0) {
	        ...
	        field_get(&of,nullptr) ;
	        sl = field_get(&of,nullptr) ;
