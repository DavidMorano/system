...
/*******************************************************************************
	...
	This object provides a crude access manager and cache for UNIX®
	...
	there may be one or two places where a sorted map might have
	been useful.  But (so called) "n" is always relatively small
	so using structure sorted maps everywhere might have actually
	slowed things down also.

	2. The use of mapping the stored local 'passwd' data file can
	be changed to simple file reading if the need ever arises.
	We did not use the FILEMAP object (because at first I thought
	it too simple for the requirement), but this could be hacked
	in later (as indeed it turns out that object is appropriate)
	if you want to make the code look cleaner.
...
int grmems_lookup(op,curp,gnp,gnl)
GRMEMS		*op ;
GRMEMS_CUR	*curp ;
const char	gnp[] ;
int		gnl ;
{
	...
	if (op->recs == NULL) rs = grmems_starter(op) ;
...
int grmems_lookread(op,curp,ubuf,ulen)
GRMEMS		*op ;
GRMEMS_CUR	*curp ;
char		ubuf[] ;
int		ulen ;
	...
	if (rs >= 0) {
	    ...
	    if (ri >= 0) {
	        ...
	        if ((rs = recarr_get(rlp,ri,&ep)) >= 0) {
	            if (i < ep->nmems) {
	                if ((rs = sncpy1(ubuf,ulen,ep->mems[i])) >= 0) {
	                    curp->i = i ;
	                }
	            } else
	                rs = SR_NOTFOUND ;
...
int grmems_invalidate(op,gnp,gnl)
GRMEMS		*op ;
const char	gnp[] ;
int		gnl ;
{
...
/* loop checking all cache entries */
	...
	if (op->recs != NULL) {
	    ...
	    for (i = 0 ; recarr_get(rlp,i,&ep) >= 0 ; i += 1) {
	        ...
	        if ((rs = record_isold(ep,daytime,op->ttl)) > 0) {
	            f = TRUE ;
...
static int grmems_mkrec(op,daytime,epp,gnp,gnl)
GRMEMS		*op ;
time_t		daytime ;
GRMEMS_REC	**epp ;
const char	gnp[] ;
	...
	if ((rs = recarr_count(op->recs)) >= 0) {
	    ...
	    if (n >= op->max) {
	        ...
	        if ((rs = pq_rem(&op->lru,&pep)) >= 0) {
	            ...
	            if ((rs = grmems_recdel(op,ep)) >= 0) {
	                rs = grmems_recstart(op,daytime,ep,gnp,gnl) ;
	                ri = rs ;
	            }
	        ...
	        if ((rs = grmems_newrec(op,daytime,epp,gnp,gnl)) >= 0) {
	            ri = rs ;
	            if (*epp != NULL) {
	                pep = (PQ_ENT *) *epp ;
...
static int grmems_newrec(op,dt,epp,gnp,gnl)
GRMEMS		*op ;
time_t		dt ;
GRMEMS_REC	**epp ;
const char	gnp[] ;
...
static int grmems_recstart(op,dt,ep,gnp,gnl)
GRMEMS		*op ;
time_t		dt ;
GRMEMS_REC	*ep ;
const char	gnp[] ;
...
static int grmems_recrefresh(op,dt,ep)
GRMEMS		*op ;
time_t		dt ;
GRMEMS_REC	*ep ;
{
...
static int grmems_recusers(op,dt,ulp,gid)
GRMEMS		*op ;
time_t		dt ;
vecelem		*ulp ;
gid_t		gid ;
...
static int grmems_mkugload(op,dt,ulp)
GRMEMS		*op ;
vecelem		*ulp ;
time_t		dt ;
{
...
static int grmems_mkugstore(op,dt,ulp)
GRMEMS		*op ;
vecelem		*ulp ;
time_t		dt ;
{
	...
	if ((rs = vecelem_count(ulp)) >= 0) {
	    ...
	    if ((rs = uc_malloc(size,&p)) >= 0) {
	        ...
	        for (i = 0 ; vecelem_get(ulp,i,&ugp) >= 0 ; i += 1) {
	            if (ugp == NULL) continue ;
#if	CF_DEBUGS
	            debugprintf("grmems_mkugstore: un=%t gid=%d\n",
...
static int grmems_recaccess(op,dt,ep)
GRMEMS		*op ;
time_t		dt ;
GRMEMS_REC	*ep ;
{
...
static int grmems_recdel(op,ep)
GRMEMS		*op ;
GRMEMS_REC	*ep ;
{
	int		rs = SR_OK ;
...
static int record_access(ep,daytime)
GRMEMS_REC	*ep ;
time_t		daytime ;
{
	int		rs = SR_OK ;
...
static int record_isold(GRMEMS_REC *ep,time_t daytime,int ttl)
{
	int		f_old ;

	if (ep == NULL) return SR_FAULT ;

	f_old = ((daytime - ep->ti_create) >= ttl) ;
	return f_old ;
}
...
static int pwentparse(lbuf,llen,gp)
const char	lbuf[] ;
int		llen ;
gid_t		*gp ;
{
