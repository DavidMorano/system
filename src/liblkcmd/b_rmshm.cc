...
static int	procname(PROGINFO *,const char *) ;
...
int b_rmshm(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
/* end subroutine (b_rmshm) */


int p_rmshm(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	return mainsub(argc,argv,envv,contextp) ;
}
/* end subroutine (p_rmshm) */
...
static int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_rmshm: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_rmshm: final mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
static int usage(PROGINFO *pip)
{
	...
	fmt = "%s: USAGE> %s [<shmname(s)> ...] [-af <afile>]\n" ;
...
/* end subroutine (lockinfo_finish) */
...
static int procname(PROGINFO *pip,cchar *name)
{
	int		rs = SR_OK ;
	int		f ;
	const char	*np = name ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_rmshm/procname: name=%s\n",name) ;
#endif
	...
	if (name[0] != '\0') {
	    if (name[0] != '/') {
	        sncpy2(namebuf,MAXPATHLEN,"/",name) ;
	        np = namebuf ;
	    }
	    rs = uc_unlinkshm(np) ;
	}
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_rmshm/procname: ret rs=%d\n",rs) ;
#endif
