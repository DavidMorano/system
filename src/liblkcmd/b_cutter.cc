...
static int	procfile(PROGINFO *,SEARCHINFO *,void *,cchar *) ;
...
int b_cutter(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
int p_cutter(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
...
static int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_cutter: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_cutter: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_cutter: searchstr=>%s< fn=%d \n",
	        searchstr,fn) ;
#endif
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_cutter: final mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
/* process a file */
static int procfile(PROGINFO *pip,SEARCHINFO *sip,void *ofp,cchar fname[])
{
	LOCINFO		*lip = pip->lip ;
...
#if	CF_DEBUG
	        if (DEBUGLEVEL(3))
	            debugprintf("procfile: >%r<\n",lbuf,len) ;
#endif

/* fast forward past the search string */

	        sl = -1 ;
	        if ((sip->searchstr != NULL) &&
	            (sip->searchstr[0] != '\0')) {

	            if ((cl = sfsub(lbuf,len,sip->searchstr,&cp)) > 0) {
	                sp = (cp + cl) ;
	                sl = (lbuf + len) - sp ;
	            }
...
#if	CF_DEBUG
	            if (DEBUGLEVEL(3))
	                debugprintf("procfile: remaining >%r<\n",sp,sl) ;
#endif

/* skip forward past and field terminators or white space */
