...
static int mainsub(int argc,mainv argv,mainv envv,void *contextp) noex {
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_uuidgen: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_uuidgen: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	debugprintf("b_uuidgen: f_help=%u\n",f_help) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	debugprintf("b_uuidgen: exiting ex=%u (%d)\n",ex,rs) ;
#endif
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_uuidgen: final mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
