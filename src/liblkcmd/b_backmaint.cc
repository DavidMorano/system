/* b_backmaint SUPPORT (KSH builtin) */
/* charset=ISO8859-1 */
...
local int	locinfo_addacl(LOCINFO *,aclinfo_t *) ;
local int	locinfo_mksol(LOCINFO *) ;

local int	aclents_match(aclent_t *,int,aclinfo_t *) ;
...
int b_backmaint(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
int p_backmaint(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
...
local int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("backmaint: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("backmaint: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
/* some initialization */
	...
	if ((rs = procacls(pip,aclspec)) >= 0) {
	    if ((rs = locinfo_procopts(lip)) >= 0) {
	        if ((rs = locinfo_ftypes(lip)) >= 0) {
	...
	if (rs >= 0) {
	    ...
	    if ((rs = userinfo_start(&u,NULL)) >= 0) {
			...
			if ((rs = proguserlist_begin(pip)) >= 0) {
			    ...
			    cchar	*afn = afname ;
			    cchar	*ofn = ofname ;
			    if ((rs = process(pip,aip,bop,afn)) >= 0) {
...
/* done */
	if ((rs < 0) && (ex == EX_OK)) {
	    ex = mapex(mapexs,rs) ;
	    if (! pip->fl.quiet) {
	        shio_printf(pip->efp,"%s: processing error (%d)\n",
	            pip->progname,rs) ;
	    }
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("backmaint: exiting ex=%u (%d)\n",ex,rs) ;
#endif
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("backmaint: fin mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
local int process(PI *pip,ARGINFO *aip,bits *bop,cchar *afn) noex {
	int		rs ;
	int		rs1 ;
	int		c = 0 ;

	if ((rs = ids_load(&pip->id)) >= 0) {
	    cint	sz = (pip->n * szof(aclent_t)) ;
	    char	*bp ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_backmaint/procname: ent fname=%s\n",fname) ;
#endif
...
/* apply the specified perms */
	    ...
	    for (i = 0 ; vecobj_get(&lip->acls,i,&ap) >= 0 ; i += 1) {
	        ...
	        f_idtype = false ;
	        if ((j = aclents_match(aclbuf,nacls,ap)) >= 0) {
#if	CF_DEBUG
	            if (DEBUGLEVEL(4))
...
/* default mask */
	        ...
	        if ((j = aclents_defmaskneed(aclbuf,nacls)) < -1) {
...
/* regular mask */
	        ...
	        if ((j = aclents_maskneed(aclbuf,nacls)) < -1) {
...
/* re-calculate the "mask" entry */
	            ...
	            if (rs >= 0) {
	                if ((j = aclents_maskmat(aclbuf,nacls)) >= 0) {
	                    if (aclbuf[j].a_perm != perm_mask) {
	                        f_changed = true ;
			    }
			}
	            } /* end if */
...
/* finly set the actual ACL (as needed) on the file */
...
/* do we need to change the owner? */
	...
	if ((rs >= 0) && (lip->uid_owner >= 0)) {
	    if (sbp->st_uid != lip->uid_owner) {
	        if (fd >= 0) {
	            rs = u_fchown(fd,lip->uid_owner,-1) ;
	        } else {
	            rs = u_chown(fname,lip->uid_owner,-1) ;
		}
	    }
	} /* end if (ownership) */
...
local int locinfo_start(LOCINFO *lip,PI *pip) noex {
	...
	if ((rs = keyopt_start(&lip->akopts)) >= 0) {
	    ...
	    if ((rs = paramopt_start(&lip->aparams)) >= 0) {
	        cint	esz = szof(aclinfo_t) ;
	        pip->open.aparams = true ;
	        if ((rs = vecobj_start(&lip->acls,esz,10,0)) >= 0) {
	            lip->fl.acls = true ;
	        }
	        if (rs < 0) {
...
local int locinfo_procopts(LOCINFO *lip) noex {
	PI		*pip = lip->pip ;
	keyopt		*kop = &lip->akopts ;
	keyopt_cur	kcur ;
	int		rs = SR_OK ;
...
local int locinfo_ftypes(LOCINFO *lip)
{
	PROGINFO	*pip = lip->pip ;
	paramopt_cur	cur ;
	int		rs = SR_OK ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("backmaint/locinfo_ftypes: ent\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("backmaint/locinfo_ftypes: ret rs=%d c=%u\n",rs,c) ;
#endif
...
local int locinfo_isftype(LOCINFO *lip,cchar *fname,ustat *sbp) noex {
	cmode		m = sbp->st_mode ;
	int		f = false ;
	if (fname == NULL) return SR_FAULT ;
	if (S_ISREG(m)) {
...
local int locinfo_addacl(LOCINFO *lip,aclinfo_t *aip) noex {
...
/* aclents (ACL entries) */
local int aclents_match(aclent_t *aclbuf,int nacls,struct aclinfo *ap) noex {
...
local int aclents_minmax(aclent_t *aclbuf,int nacls,int *pminp,
		int *pmaxp) noex {
	int		c = 0 ;
	*pminp = -1 ;
	*pmaxp = -1 ;
...
local int aclents_print(aclent_t aclbuf[],int nacls) noex {
	...
	if ((textbuf = acltotext(aclbuf,nacls)) != NULL) {
	    ...
	    if (*sp != '\0')
	        debugprintf("backmaint/aclents_print: | %s\n",sp) ;
	    uc_free(textbuf) ;
	} else
	    debugprintf("backmaint/aclents_print: couldn't get text\n") ;
	return 0 ;
}
...
local int parseperms(cchar *psp,int psl) noex {
	...
	for (int i = 0 ; (rc >= 0) && (i < psl) && (psp[i] != '\0') ; i += 1) {
	    switch (cint ch = (psp[i] & UCHAR_MAX) ; ch) {
	    case 't':
	        rc |= T_OK ;
	        break ;
	    case 's':
