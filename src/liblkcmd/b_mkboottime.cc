...
int b_mkboottime(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
int p_mkboottime(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
...
static int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_mkboottime: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_mkboottime: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_mkboottime: final mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
static int procline(PROGINFO *pip,void *ofp,cchar *lbuf,int llen)
{
	...
	if ((rs = field_start(&fsb,lbuf,llen)) >= 0) {
	    ...
	    while ((fl = field_get(&fsb,aterms,&fp)) >= 0) {
	        if (fl > 0) {
	            if (fl > MAXPATHLEN) fl = MAXPATHLEN ;
...
static int procspec(PROGINFO *pip,void *ofp,cchar *fp,int fl)
{
	...
	if (rs >= 0) {
	    ...
	    if ((rs = nulstr_start(&ns,fp,fl,&fname)) >= 0) {
	        ...
	        if (pip->debuglevel > 0) {
	            fmt = "%s: file=%s (%d)\n" ;
	            bprintf(ofp,fmt,pn,fname,rs) ;
	        }

	        if (pip->verboselevel > 0) {
	            fmt = "file=%s (%d)\n" ;
	            bprintf(ofp,fmt,fname,rs) ;
	        }
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("b_mkboottime/procspec: ret rs=%d\n",rs) ;
#endif
...
/* end subroutine (locinfo_boottime) */
