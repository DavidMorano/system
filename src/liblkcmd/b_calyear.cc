...
local int procargs(PROGINFO *pip,ARGINFO *aip,bits *bop,cchar *ofn,cchar *afn)
{
	...
	if ((rs = shio_open(ofp,ofn,"wct",0666)) >= 0) {
	    ...
	    if ((rs >= 0) && (! f_a) && lip->fl.apm) {
		const int	f_apm = lip->fl.apm ;
	    ...
	    if ((rs >= 0) && (! f_a) && (lip->ncites == 0) && lip->fl.defnull) {
	...
	if ((rs = paramfile_curbegin(pfp,&cur)) >= 0) {
	    ...
	    while (rs >= 0) {
	        ...
	        case param_logfile:
	            ...
	            break ;
	        case param_caldirs:
	            if ((! lip->finval.caldirs) && (el > 0)) {
	                rs = locinfo_loaddirs(lip,ebuf,el) ;
	            }
	            break ;
	        case param_calnames:
	            if ((! lip->finval.calnames) && (el > 0)) {
