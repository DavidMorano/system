/* b_homepage SUPPORT (KSH builtin) */
/* charset=ISO8859-1 */
/* lang=C++20 */

/* program to create a "home" webpage in corporate environment */
/* version %I% last-modified %G% */

#define	CF_DEBUG	0		/* compile-time debugging */
#define	CF_DEBUGMALL	1		/* debug memory-allocations */
#define	CF_DEBUGMOUT	0		/* debug memory-allocations */
#define	CF_PROCARGS	0		/* |procargs()| */
#define	CF_STACKCHECK	1		/* check stacks afterwards */
#define	CF_UCREADE	1		/* use |uc_reade(3uc)| */
#define	CF_UGETPW	1		/* use |ugetpw(3uc)| */
#define	CF_TMPGROUP	1		/* set group on TMP dir */
#define	CF_MNTCHECK	0		/* perform mount-check */

/* revision history:

	= 2000-05-14, David A­D­ Morano
	Originally written for Rightcore Network Services.

*/

/* Copyright © 2000 David A­D­ Morano.  All rights reserved. */

/*******************************************************************************

  	Name:
	b_homepage

	Description:
	This program creates a web page that is (generally) meant to
	be a home page for a browser in a corporate environment.

	Synopsis:
	$ homepage [-V]

	Arguments:
	-V		print program version to standard-error and then exit

*******************************************************************************/

#include	<envstandards.h>	/* MUST be first to configure */

#if	defined(SFIO) || defined(KSHBUILTIN)
#undef	CF_SFIO
#define	CF_SFIO	1
#else
#ifndef	CF_SFIO
#define	CF_SFIO	0
#endif
#endif

#if	CF_SFIO
#include	<shell.h>
#endif

#include	<sys/types.h>
#include	<sys/param.h>
#include	<sys/mman.h>
#include	<unistd.h>
#include	<climits>		/* CSTD */
#include	<csignal>		/* CSTD */
#include	<cstddef>		/* CSTD */
#include	<cstdlib>		/* CSTD */
#include	<cstring>		/* CSTD */
#include	<clanguage.h>		/* LIBU */
#include	<usysbase.h>		/* LIBU */
#include	<usyscalls.h>		/* LIBU */
#include	<pta.h>			/* LIBU */
#include	<ptm.h>			/* LIBU */
#include	<ptc.h>			/* LIBU */
#include	<upt.h>			/* LIBU */
#include	<ascii.h>		/* LIBU */
#include	<intceil.h>		/* LIBU */
#include	<findbit.h>		/* LIBU */
#include	<getpwx.h>		/* LIBUC */
#include	<bufsizeget.h>		/* LIBUC */
#include	<estrings.h>		/* LIBUC */
#include	<bits.h>		/* LIBUC */
#include	<keyopt.h>		/* LIBUC */
#include	<paramopt.h>		/* LIBUC */
#include	<userinfo.h>		/* LIBUC */
#include	<vecobj.h>		/* LIBUC */
#include	<vecstr.h>		/* LIBUC */
#include	<vecpstr.h>		/* LIBUC */
#include	<filer.h>		/* LIBUC */
#include	<paramfile.h>		/* LIBUC */
#include	<expcook.h>		/* LIBUC */
#include	<svcfile.h>		/* LIBUC */
#include	<nulstr.h>		/* LIBUC */
#include	<vechand.h>		/* LIBUC */
#include	<bwops.h>		/* LIBUC */
#include	<termout.h>		/* LIBUC */
#include	<spawner.h>		/* LIBUC */
#include	<lfm.h>			/* LIBUC */
#include	<tmtime.hh>		/* LIBUC */
#include	<querystr.h>		/* LIBUC */
#include	<strn.h>		/* LIBUC |strnrbrk(3uc)| */
#include	<sfx.h>			/* LIBUC */
#include	<strwcpy.h>		/* LIBUC */
#include	<rmdirfiles.h>		/* LIBUC */
#include	<exitcodes.h>		/* LIBU */
#include	<localmisc.h>		/* LIBU */

#include	"shio.h"
#include	"kshlib.h"
#include	"b_homepage.h"
#include	"defs.h"
#include	"proglog.h"
#include	"cgi.h"
#include	"htm.h"
#include	"svckv.h"
#include	"svcent.h"

#pragma		GCC dependency		"mod/libutil.ccm"

import libutil ;			/* |memclear(3u)| */
import findbit ;

/* local defines */

#if	CF_UGETPW
#define	GETPW_NAME	ugetpw_name
#else
#define	GETPW_NAME	getpw_name
#endif /* CF_UGETPW */

#define	STACKSIZE	(1*1024*1024)
#define	STACKGUARD	(2*1024)

#define	HBUFLEN		120		/* header buffer length */

#define	LI		struct locinfo

#define	CONFIG		struct config
#define	CONFIG_FILE	struct config_file
#define	CONFIG_MAGIC	0x23FFEEDD

#define	GATHER		struct gather

#ifndef	PI
#define	PI		proginfo
#endif

#define	LI		locinfo

#define	FILEDAT		struct filedat
#define	FILEDAT_DSIZE	(3*1024) ;

#define	PO_OPTION	"option"

#define	SVCLEN		(MAXNAMELEN)
#define	SVCENTLEN	(2*LINEBUFLEN)

#ifndef	ENVBUFLEN
#define	ENVBUFLEN	2048
#endif

#define	MEMPATTERN	0x5A5A5A5A


/* typedefs */

#ifndef	TYPEDEF_CCHAR
#define	TYPEDEF_CCHAR	1
typedef cchar	cchar ;
#endif

extern "C" {
    typedef	int (*tworker)(void *) noex ;
}


/* external subroutines */

extern int	printhelp(void *,cchar *,cchar *,cchar *) noex ;
extern int	proginfo_setpiv(PI *,cchar *,const pivars *) noex ;
extern int	proginfo_rootname(PI *) noex ;

extern int	proguserlist_begin(PI *) noex ;
extern int	proguserlist_end(PI *) noex ;


/* external variables */

extern char	**environ ;		/* definition required by AT&T AST */


/* local structures */

struct gather {
	vechand		ents ;
	ptm		mx ;
	ptc		c ;
	proginfo	*pip ;
	cchar		*termtype ;
	int		nout ;
	int		cols ;
} ; /* end struct */

struct filedat {
	proginfo	*pip ;
	cchar		*a ;
	cchar		*fname ;
	cchar		*svc ;
	cchar		*termtype ;
	char		*dbuf ;
	caddr_t		saddr ;
	caddr_t		maddr ;
	size_t		ssize ;
	size_t		msize ;
	pthread_t	tid ;
	int		dsize ;
	int		dl ;
	int		to ;
	int		lines ;
	int		cols ;
	int		f_running ;
	int		f_stackcheck ;
} ; /* end struct */

struct locinfo_fl {
	uint		stores:1 ;
	uint		lbuf:1 ;
	uint		trunc:1 ;
	uint		start:1 ;
	uint		basedname:1 ;
	uint		dbfname:1 ;
	uint		hfname:1 ;
	uint		reqfname:1 ;
	uint		mntfname:1 ;
	uint		msfname:1 ;
	uint		svcs:1 ;
	uint		pidlock:1 ;
	uint		intcache:1 ;
	uint		intspeed:1 ;
	uint		intconf:1 ;
	uint		intsvcs:1 ;
	uint		intwait:1 ;
	uint		intmaint:1 ;
	uint		defpage:1 ;
	uint		defsvcs:1 ;
	uint		quietlock:1 ;
	uint		force:1 ;
	uint		maint:1 ;
	uint		s:1 ;			/* SVCFILE */
	uint		cooks:1 ;
} ; /* end struct */

struct locinfo {
	vecstr		stores ;
	locinfo_fl	have, f, changed, finval ;
	locinfo_fl	init, open ;
	SVCFILE		s ;
	GATHER		g ;
	LFM		pidlock ;
	expcook		cooks ;
	proginfo	*pip ;
	void		*svcs ;
	cchar		*tmpourdname ;		/* work-dir */
	cchar		*basedname ;		/* base-dir (for DB) */
	cchar		*dbfname ;
	cchar		*wfname ;		/* log-welcome file-name */
	cchar		*hfname ;		/* HEAD file-name */
	cchar		*sfname ;		/* SVC file-name */
	cchar		*reqfname ;
	cchar		*mntfname ;
	cchar		*msfname ;
	cchar		*termtype ;
	cchar		*copyright ;
	cchar		*webmaster ;
	gid_t		gid_rootname ;
	int		start ;
	int		nproc ;
	int		cols ;
	int		year ;
	int		intcache ;		/* interval cache */
	int		intspeed ;
	int		intconf ;
	int		intsvcs ;
	int		intwait ;
	int		intmaint ;
	int		to_cache ;
	int		to_lock ;
} ; /* end struct */

struct config_file {
	cchar		*fname ;
	time_t		mtime ;
} ; /* end struct */

struct config {
	uint		magic ;
	proginfo	*pip ;
	paramopt	*app ;
	vecobj		files ;
	uint		f_p:1 ;
} ; /* end struct */


/* forward references */

local int	mainsub(int,cchar **,cchar **,void *) noex ;

local int	usage(PI *) noex ;

local int	locinfo_start(LI *,PI *) noex ;
local int	locinfo_finish(LI *) noex ;
local int	locinfo_setentry(LI *,cchar **,cchar *,int) noex ;
local int	locinfo_basedir(LI *,cchar *,int) noex ;
local int	locinfo_tmpourdname(LI *) noex ;
local int	locinfo_tmpgroup(LI *,cchar *) noex ;
local int	locinfo_gidrootname(LI *,PASSWD *,char *,int) noex ;
local int	locinfo_defreg(LI *) noex ;
local int	locinfo_defdaemon(LI *) noex ;
local int	locinfo_dbinfo(LI *,cchar *,cchar *) noex ;
local int	locinfo_wfname(LI *,cchar *) noex ;
local int	locinfo_sethead(LI *,cchar *,int) noex ;
local int	locinfo_svclistbegin(LI *) noex ;
local int	locinfo_svclistadds(LI *,cchar *,int) noex ;
local int	locinfo_svclistadd(LI *,cchar *,int) noex ;
local int	locinfo_svclistend(LI *) noex ;
local int	locinfo_svclistget(LI *,int,cchar **) noex ;
local int	locinfo_svclistdel(LI *,int) noex ;
local int	locinfo_svclistdelall(LI *) noex ;
local int	locinfo_svclistcount(LI *) noex ;
local int	locinfo_svclistfinal(LI *) noex ;
local int	locinfo_copyright(LI *,cchar *,int) noex ;
local int	locinfo_defs(LI *) noex ;
local int	locinfo_defsvc(LI *) noex ;
local int	locinfo_defpfname(LI *) noex ;
local int	locinfo_qs(LI *,cchar *) noex ;
local int	locinfo_finalize(LI *) noex ;
local int	locinfo_svcsbegin(LI *) noex ;
local int	locinfo_svcsend(LI *) noex ;
local int	locinfo_svcscheck(LI *,time_t) noex ;
local int	locinfo_gatherbegin(LI *) noex ;
local int	locinfo_gatherbeginall(LI *) noex ;
local int	locinfo_gatherbeginsome(LI *) noex ;
local int	locinfo_gatherbeginer(LI *,SVCFILE_ENT *) noex ;
local int	locinfo_gatherend(LI *) noex ;

local int	locinfo_lockbegin(LI *) noex ;
local int	locinfo_lockend(LI *) noex ;
local int	locinfo_lockbeginone(LI *,LFM *,cchar *) noex ;
local int	locinfo_lockcheck(LI *,lfm_check *) noex ;

local int	locinfo_cookbegin(LI *) noex ;
local int	locinfo_cookend(LI *) noex ;
local int	locinfo_cookexp(LI *,int,char *,int,cchar *,int) noex ;

local int	config_start(CONFIG *,PI *,paramopt *,cchar *) noex ;
local int	config_addfile(CONFIG *,cchar *) noex ;
local int	config_addfins(CONFIG *) noex ;
local int	config_check(CONFIG *) noex ;
local int	config_load(CONFIG *,cchar *) noex ;
local int	config_havefile(CONFIG *,vecstr *,char *,cchar *,cchar *) noex ;
local int	config_read(CONFIG *,cchar *) noex ;
local int	config_reader(CONFIG *,paramfile *) noex ;
local int	config_finish(CONFIG *) noex ;

local int	procopts(PI *,keyopt *) noex ;

local int	procuserinfo_begin(PI *,userinfo *) noex ;
local int	procuserinfo_end(PI *) noex ;
local int	procuserinfo_logid(PI *) noex ;

local int	procourconf_begin(PI *,paramopt *,cchar *) noex ;
local int	procourconf_end(PI *) noex ;

local int	process(PI *,cchar *,cchar *) noex ;
local int	procregular(PI *,cchar *,cchar *) noex ;
local int	procbackinfo(PI *) noex ;
local int	procback(PI *) noex ;
local int	procbacks(PI *) noex ;
local int	procbackcheck(PI *) noex ;
local int	procbackmaint(PI *) noex ;
local int	procbacker(PI *,cchar *,cchar **) noex ;
local int	procbackenv(PI *,SPAWNER *) noex ;
local int	procmntcheck(PI *) noex ;
local int	procdaemon(PI *) noex ;
local int	procdaemons(PI *) noex ;
local int	procdaemoner(PI *) noex ;
local int	procdaemoncheck(PI *) noex ;

local int	procpidfname(PI *) noex ;
local int	procbackdefs(PI *) noex ;
local int	procdaemondefs(PI *) noex ;
local int	procdaemoninfo(PI *) noex ;

#if	CF_PROCARGS
local int	procargs(PI *,ARGINFO *,bits *,cchar *,cchar *,cchar *) noex ;
#endif

local int	procresp(PI *,cchar *,cchar *) noex ;
local int	procexecname(PI *,char *,int) noex ;

local int	procdoc(PI *,cchar *) noex ;
local int	procdocbody(PI *,HTM *) noex ;
local int	procdocbodyhdr(PI *,HTM *) noex ;
local int	procdocbodyhdrfile(PI *,char *,int,cchar *) noex ;
local int	procdocbodymain(PI *,HTM *) noex ;
local int	procdocbodyfooter(PI *,HTM *) noex ;
local int	procdocbodyfooterleft(PI *,HTM *) noex ;
local int	procdocbodyfooterext(PI *,HTM *) noex ;
local int	procdocbodymain_svcs(PI *,HTM *) noex ;
local int	procdocbodymain_svcer(PI *,HTM *,GATHER *,SVCFILE_ENT *) noex ;
local int	procdocbodymain_svcerhdr(PI *,HTM *,SVCFILE_ENT *) noex ;
local int	procdocbodymain_svcerinc(PI *,HTM *,SVCFILE_ENT *) noex ;
local int	procdocbodymain_svcerincer(PI *,HTM *,
			SVCFILE_ENT *,cchar *) noex ;
local int	procdocbodymain_svcerfile(PI *,HTM *,GATHER *,
			SVCFILE_ENT *) noex ;
local int	procdocbodymain_svcerfiler(PI *,HTM *,GATHER *,
			SVCFILE_ENT *,int) noex ;
local int	proclockcheck(PI *) noex ;
local int	proclockprint(PI *,cchar *,lfm_check *) noex ;
local int	proclocklog(PI *,cchar *,lfm_check *,cchar *) noex ;

local int	procdocbodymain_svcprint(PI *,HTM *) noex ;
local int	prochdrs(PI *,CGI *,int) noex ;

local int	procpage_begin(PI *,char *) noex ;
local int	procpage_end(PI *,char *) noex ;
local int	procpage_make(PI *,char *) noex ;

local int	mkourname(PI *,char *,cchar *,cchar *,int) noex ;
local int	mkincfname(PI *,char *,cchar *,int) noex ;

local int gather_start(GATHER *,PI *,cchar *,int) noex ;
local int gather_finish(GATHER *) noex ;
local int gather_file(GATHER *,cchar *,int,cchar *,int) noex ;
local int gather_entfins(GATHER *) noex ;
local int gather_getlines(GATHER *,cchar *) noex ;
local int gather_getbuf(GATHER *,cchar *,cchar **) noex ;

local int filedat_start(FILEDAT *,PI *,cchar *,int,cchar *,cchar *,int) noex ;
local int filedat_worker(FILEDAT *) noex ;
local int filedat_workread(FILEDAT *,int) noex ;
local int filedat_workreadreg(FILEDAT *,int) noex ;
local int filedat_workreadterm(FILEDAT *,int) noex ;
local int filedat_workreadtermline(FILEDAT *,TERMOUT *,char *,cc *,int) noex ;
local int filedat_getlines(FILEDAT *) noex ;
local int filedat_getbuf(FILEDAT *,cchar **) noex ;
local int filedat_havesvc(FILEDAT *,cchar *) noex ;
local int filedat_finish(FILEDAT *) noex ;
local int filedat_stackbegin(FILEDAT *) noex ;
local int filedat_stackend(FILEDAT *) noex ;
local int filedat_stackfill(FILEDAT *) noex ;
local int filedat_stackused(FILEDAT *) noex ;


/* local variables */

enum argopts {
	argopt_version,
	argopt_verbose,
	argopt_tmpdir,
	argopt_help,
	argopt_root,
	argopt_sn,
	argopt_af,
	argopt_ef,
	argopt_of,
	argopt_if,
	argopt_cf,
	argopt_lf,
	argopt_wf,
	argopt_db,
	argopt_qs,
	argopt_daemon,
	argopt_svcs,
	argopt_overlast
} ; /* end enum */

constexpr cpcchar	argopts[] = {
	"VERSION",
	"VERBOSE",
	"TMPDIR",
	"HELP",
	"ROOT",
	"sn",
	"af",
	"ef",
	"of",
	"if",
	"cf",
	"lf",
	"wf",
	"db",
	"qs",
	"daemon",
	"svcs",
	nullptr
} ; /* end array */

constexpr PIVARS	initvars = {
	VARPROGRAMROOT1,
	VARPROGRAMROOT2,
	VARPROGRAMROOT3,
	PROGRAMROOT,
	VARPRLOCAL
} ; /* end array */

constexpr MAPEX		mapexs[] = {
	{ SR_NOENT, EX_NOUSER },
	{ SR_AGAIN, EX_TEMPFAIL },
	{ SR_DEADLK, EX_TEMPFAIL },
	{ SR_NOLCK, EX_TEMPFAIL },
	{ SR_TXTBSY, EX_TEMPFAIL },
	{ SR_ACCESS, EX_NOPERM },
	{ SR_REMOTE, EX_PROTOCOL },
	{ SR_NOSPC, EX_TEMPFAIL },
	{ 0, 0 }
} ; /* end array */

enum akonames {
	akoname_print,
	akoname_add,
	akoname_inc,
	akoname_base,
	akoname_cf,
	akoname_lf,
	akoname_wf,
	akoname_log,
	akoname_head,
	akoname_svcs,
	akoname_welcome,
	akoname_daemon,
	akoname_pidfile,
	akoname_mntfile,
	akoname_msfile,
	akoname_to,
	akoname_intrun,
	akoname_intidle,
	akoname_intpoll,
	akoname_intconf,
	akoname_intsvcs,
	akoname_intcache,
	akoname_intwait,
	akoname_intmaint,
	akoname_quiet,
	akoname_quietlock,
	akoname_force,
	akoname_maint,
	akoname_overlast
} ; /* end enum */

constexpr cpcchar	akonames[] = {
	"print",
	"add",
	"inc",
	"base",
	"cf",
	"lf",
	"wf",
	"log",
	"head",
	"svcs",
	"welcome",
	"daemon",
	"pidfile",
	"mntfile",
	"msfile",
	"to",
	"intrun",
	"intidle",
	"intpoll",
	"intconf",
	"intsvcs",
	"intcache",
	"intwait",
	"intmaint",
	"quiet",
	"quietlock",
	"force",
	"maint",
	nullptr
} ; /* end array */

constexpr cpcchar	csched[] = {
	"%p/etc/%n/%n.%f",
	"%p/etc/%n/%f",
	"%p/etc/%n.%f",
	nullptr
} ; /* end array */

enum cparams {
	cparam_workdir,
	cparam_basedir,
	cparam_basedb,
	cparam_logfile,
	cparam_logsize,
	cparam_head,
	cparam_svcs,
	cparam_copyright,
	cparam_webmaster,
	cparam_intrun,
	cparam_intidle,
	cparam_intpoll,
	cparam_intconf,
	cparam_intlock,
	cparam_intmark,
	cparam_intcache,
	cparam_intwait,
	cparam_intmaint,
	cparam_overlast
} ; /* end enum */

static const char	*cparams[] = {
	"workdir",
	"basedir",
	"basedb",
	"logfile",
	"logsize",
	"head",
	"svcs",
	"copyright",
	"webmaster",
	"intrun",
	"intidle",
	"intpoll",
	"intconf",
	"intlock",
	"intmark",
	"intcache",
	"intwait",
	"intmaint",
	nullptr
} ; /* end array */

constexpr cpcchar	isexecs[] = {
	"so",
	"p",
	"a",
	nullptr
} ; /* end array */

enum svcopts {
	svcopt_termout,
	svcopt_overlast
} ; /* end enum */

constexpr cpcchar	svcopts[] = {
	"termout",
	nullptr
} ; /* end array */

constexpr int	sigignores[] = {
	SIGPIPE,
	SIGPOLL,
	SIGHUP,
	0
} ; /* end array */


/* exported variables */


/* exported subroutines */

int b_homepage(int argc,con mainv argv,void *contextp) noex {
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;

	if ((rs = lib_kshbegin(contextp,nullptr)) >= 0) {
	    cchar	**envv = (cchar **) environ ;
	    ex = mainsub(argc,argv,envv,contextp) ;
	    rs1 = lib_kshend() ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (ksh) */

	if ((rs < 0) && (ex == EX_OK)) ex = EX_DATAERR ;

	return ex ;
}
/* end subroutine (b_homepage) */

int p_homepage(int argc,con mainv argv,con mainv envv,void *contextp) noex {
	return mainsub(argc,argv,envv,contextp) ;
}
/* end subroutine (p_homepage) */


/* local subroutines */

/* ARGSUSED */
local int mainsub(int argc,con mainv argv,con mainv envv,void *contextp) noex {
	proginfo	pi, *pip = &pi ;
	LI		li, *lip = &li ;
	ARGINFO		ainfo ;
	bits		pargs ;
	keyopt		akopts ;
	paramopt	aparams ;
	SHIO		errfile ;

#if	(CF_DEBUG || CF_DEBUG) && CF_DEBUGMALL
	uint		mo_start = 0 ;
#endif

	int		argr, argl, aol, akl, avl, kwi ;
	int		ai, ai_max, ai_pos, ai_continue ;
	int		rs, rs1 ;
	int		v ;
	int		ex = EX_INFO ;
	int		f_optminus, f_optplus, f_optequal ;
	int		f_usage = false ;
	int		f_version = false ;
	int		f_help = false ;

	cchar		*argp, *aop, *akp, *avp ;
	cchar		*argval = nullptr ;
	cchar		*pr = nullptr ;
	cchar		*sn = nullptr ;
	cchar		*afname = nullptr ;
	cchar		*efname = nullptr ;
	cchar		*ofname = nullptr ;
	cchar		*cfname = nullptr ;
	cchar		*wfname = nullptr ;
	cchar		*dbfname = nullptr ;
	cchar		*qs = nullptr ;
	cchar		*un = nullptr ;
	cchar		*cp ;


#if	CF_DEBUG || CF_DEBUG
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != nullptr) {
	    rs = debugopen(cp) ;
	    debugprintf("b_homepage: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUG */

#if	(CF_DEBUG || CF_DEBUG) && CF_DEBUGMALL
	uc_mallset(1) ;
	uc_mallout(&mo_start) ;
#endif

	rs = proginfo_start(pip,envv,argv[0],VERSION) ;
	if (rs < 0) {
	    ex = EX_OSERR ;
	    goto badprogstart ;
	}

	if ((cp = getourenv(envv,VARBANNER)) == nullptr) cp = BANNER ;
	rs = proginfo_setbanner(pip,cp) ;

/* early things to initialize */

	pip->verboselevel = 1 ;
	pip->daytime = time(nullptr) ;
	pip->to_open = -1 ;
	pip->fl.logprog = OPT_LOGPROG ;

	pip->lip = lip ;
	if (rs >= 0) rs = locinfo_start(lip,pip) ;
	if (rs < 0) {
	    ex = EX_OSERR ;
	    goto badlocstart ;
	}

/* process program arguments */

	if (rs >= 0) rs = bits_start(&pargs,0) ;
	if (rs < 0) goto badpargs ;

	rs = keyopt_start(&akopts) ;
	pip->open.akopts = (rs >= 0) ;

	if (rs >= 0) {
	    rs = paramopt_start(&aparams) ;
	    pip->open.aparams = (rs >= 0) ;
	}

	ai_max = 0 ;
	ai_pos = 0 ;
	ai_continue = 1 ;
	argr = argc ;
	for (ai = 0 ; (ai < argc) && (argv[ai] != nullptr) ; ai += 1) {
	    if (rs < 0) break ;
	    argr -= 1 ;
	    if (ai == 0) continue ;

	    argp = argv[ai] ;
	    argl = xstrlen(argp) ;

	    f_optminus = (*argp == '-') ;
	    f_optplus = (*argp == '+') ;
	    if ((argl > 1) && (f_optminus || f_optplus)) {
	        cint	ach = MKCHAR(argp[1]) ;

	        if (isdigitlatin(ach)) {

	            argval = (argp + 1) ;

	        } else if (ach == '-') {

	            ai_pos = ai ;
	            break ;

	        } else {

	            aop = argp + 1 ;
	            akp = aop ;
	            aol = argl - 1 ;
	            f_optequal = false ;
	            if ((avp = strchr(aop,'=')) != nullptr) {
	                f_optequal = true ;
	                akl = avp - aop ;
	                avp += 1 ;
	                avl = aop + argl - 1 - avp ;
	                aol = akl ;
	            } else {
	                avp = nullptr ;
	                avl = 0 ;
	                akl = aol ;
	            }

	            if ((kwi = matostr(argopts,2,akp,akl)) >= 0) {

	                switch (kwi) {

/* version */
	                case argopt_version:
	                    f_version = true ;
	                    if (f_optequal)
	                        rs = SR_INVALID ;
	                    break ;

/* verbose */
	                case argopt_verbose:
	                    pip->verboselevel = 2 ;
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl) {
	                            rs = optvalue(avp,avl) ;
	                            pip->verboselevel = rs ;
	                        }
	                    }
	                    break ;

/* temporary directory */
	                case argopt_tmpdir:
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl)
	                            pip->tmpdname = avp ;
	                    } else {
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                pip->tmpdname = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                    }
	                    break ;

	                case argopt_help:
	                    f_help = true ;
	                    break ;

/* program-root */
	                case argopt_root:
	                    if (argr > 0) {
	                        argp = argv[++ai] ;
	                        argr -= 1 ;
	                        argl = xstrlen(argp) ;
	                        if (argl)
	                            pr = argp ;
	                    } else
	                        rs = SR_INVALID ;
	                    break ;

/* program searcn-name */
	                case argopt_sn:
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl)
	                            sn = avp ;
	                    } else {
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                sn = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                    }
	                    break ;

	                case argopt_af:
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl)
	                            afname = avp ;
	                    } else {
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                afname = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                    }
	                    break ;

/* error file name */
	                case argopt_ef:
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl)
	                            efname = avp ;
	                    } else {
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                efname = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                    }
	                    break ;

	                case argopt_of:
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl)
	                            ofname = avp ;
	                    } else {
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                ofname = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                    }
	                    break ;

/* configuration file-name */
	                case argopt_cf:
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl)
	                            cfname = avp ;
	                    } else {
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                cfname = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                    }
	                    break ;

/* log file-name */
	                case argopt_lf:
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl)
	                            pip->lfname = avp ;
	                    } else {
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                pip->lfname = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                    }
	                    break ;

/* log welcome-name */
	                case argopt_wf:
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl)
	                            wfname = avp ;
	                    } else {
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                wfname = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                    }
	                    break ;

/* database file */
	                case argopt_db:
	                    cp = nullptr ;
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl) {
	                            cp = avp ;
	                        }
	                    } else {
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                cp = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                    }
	                    if ((rs >= 0) && (cp != nullptr)) {
	                        lip->finval.dbfname = true ;
	                        lip->have.dbfname = true ;
	                        dbfname = cp ;
	                    }
	                    break ;

/* quert string */
	                case argopt_qs:
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl)
	                            qs = avp ;
	                    } else {
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                qs = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                    }
	                    break ;

/* daemon mode */
	                case argopt_daemon:
	                    pip->have.daemon = true ;
	                    pip->finval.daemon = true ;
	                    pip->fl.daemon = true ;
	                    if (f_optequal) {
	                        f_optequal = false ;
	                        if (avl) {
	                            rs = cfdecti(avp,avl,&v) ;
	                            pip->intrun = v ;
	                        }
	                    }
	                    break ;

/* services */
	                case argopt_svcs:
	                    if (argr > 0) {
	                        argp = argv[++ai] ;
	                        argr -= 1 ;
	                        argl = xstrlen(argp) ;
	                        if (argl) {
	                            rs = locinfo_svclistadds(lip,argp,argl) ;
	                        }
	                    } else
	                        rs = SR_INVALID ;
	                    break ;

/* default action and user specified help */
	                default:
	                    rs = SR_INVALID ;
	                    break ;

	                } /* end switch (key words) */

	            } else {

	                while (akl--) {
	                    cint	kc = MKCHAR(*akp) ;

	                    switch (kc) {

	                    case 'V':
	                        f_version = true ;
	                        break ;

	                    case 'C':
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                cfname = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                        break ;


	                    case 'D':
	                        pip->debuglevel = 1 ;
	                        if (f_optequal) {
	                            f_optequal = false ;
	                            if (avl) {
	                                rs = optvalue(avp,avl) ;
	                                pip->debuglevel = rs ;
	                            }
	                        }
	                        break ;

/* quiet */
	                    case 'Q':
	                        pip->fl.quiet = true ;
	                        break ;

/* program-root */
	                    case 'R':
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                pr = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                        break ;

/* terminal-type */
	                    case 'T':
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                lip->termtype = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                        break ;

/* daemon mode */
	                    case 'd':
	                        pip->finval.background = true ;
	                        pip->fl.background = true ;
	                        if (f_optequal) {
	                            f_optequal = false ;
	                            if (avl) {
	                                cchar	ch = MKCHAR(*avp) ;
	                                if (isdigitlatin(ch)) {
	                                    pip->finval.intrun = true ;
	                                    pip->have.intrun = true ;
	                                    rs = cfdecti(avp,avl,&v) ;
	                                    pip->intrun = v ;
	                                } else if (tolc(ch) == 'i') {
	                                    pip->intrun = INT_MAX ;
	                                } else
	                                    rs = SR_INVALID ;
	                            }
	                        }
	                        break ;

	                    case 'f':
	                        lip->finval.force = true ;
	                        lip->have.force = true ;
	                        lip->fl.force = true ;
	                        if (f_optequal) {
	                            f_optequal = false ;
	                            if (avl) {
	                                rs = optbool(avp,avl) ;
	                                lip->fl.force = (rs > 0) ;
	                            }
	                        }
	                        break ;

/* options */
	                    case 'o':
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl) {
	                                keyopt	*kop = &akopts ;
	                                rs = keyopt_loads(kop,argp,argl) ;
	                            }
	                        } else
	                            rs = SR_INVALID ;
	                        break ;

/* quiet */
	                    case 'q':
	                        pip->verboselevel = 0 ;
	                        break ;

/* starting offset */
	                    case 's':
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl) {
	                                ulong	ulw ;
	                                rs = cfdecmful(argp,argl,&ulw) ;
	                                lip->start = ulw ;
	                            }
	                        } else
	                            rs = SR_INVALID ;
	                        break ;

/* truncate */
	                    case 't':
	                        lip->fl.trunc = true ;
	                        if (f_optequal) {
	                            f_optequal = false ;
	                            if (avl) {
	                                rs = optbool(avp,avl) ;
	                                lip->fl.trunc = (rs > 0) ;
	                            }
	                        }
	                        break ;

	                    case 'u':
	                        if (argr > 0) {
	                            argp = argv[++ai] ;
	                            argr -= 1 ;
	                            argl = xstrlen(argp) ;
	                            if (argl)
	                                un = argp ;
	                        } else
	                            rs = SR_INVALID ;
	                        break ;

/* verbose output */
	                    case 'v':
	                        pip->verboselevel = 2 ;
	                        if (f_optequal) {
	                            f_optequal = false ;
	                            if (avl) {
	                                rs = optvalue(avp,avl) ;
	                                pip->verboselevel = rs ;
	                            }
	                        }
	                        break ;

	                    case '?':
	                        f_usage = true ;
	                        break ;

	                    default:
	                        f_usage = true ;
	                        rs = SR_INVALID ;
	                        break ;

	                    } /* end switch */
	                    akp += 1 ;

	                    if (rs < 0) break ;
	                } /* end while */

	            } /* end if (individual option key letters) */

	        } /* end if */

	    } else {

	        rs = bits_set(&pargs,ai) ;
	        ai_max = ai ;

	    } /* end if (key letter/word or positional) */

	    ai_pos = ai ;

	} /* end while (all command line argument processing) */

	if (efname == nullptr) efname = getourenv(envv,VAREFNAME) ;
	if (efname == nullptr) efname = STDFNERR ;
	if ((rs1 = shio_open(&errfile,efname,"wca",0666)) >= 0) {
	    pip->efp = &errfile ;
	    pip->open.errfile = true ;
	    shio_control(&errfile,SHIO_CSETBUFLINE,true) ;
	} else if (! isFailOpen(rs1)) {
	    if (rs >= 0) rs = rs1 ;
	}

	if (rs < 0) goto badarg ;

	if (pip->debuglevel == 0) {
	    if ((cp = getourenv(envv,VARDEBUGLEVEL)) != nullptr) {
	        if (hasnonwhite(cp,-1)) {
	            rs = optvalue(cp,-1) ;
	            pip->debuglevel = rs ;
	        }
	    }
	} /* end if */

#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_homepage: debuglevel=%u\n",pip->debuglevel) ;
#endif

	if (f_version) {
	    shio_printf(pip->efp,"%s: version %s\n",pip->progname,VERSION) ;
	}

/* get the program root */

	if (rs >= 0) {
	    if ((rs = proginfo_setpiv(pip,pr,&initvars)) >= 0) {
	        rs = proginfo_setsearchname(pip,VARSEARCHNAME,sn) ;
	    }
	}

	if (rs < 0) {
	    ex = EX_OSERR ;
	    goto retearly ;
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(4)) {
	    debugprintf("b_homepage: pr=%s\n",pip->pr) ;
	    debugprintf("b_homepage: sn=%s\n",pip->searchname) ;
	}
#endif

	if (pip->debuglevel > 0) {
	    shio_printf(pip->efp,"%s: pr=%s\n", pip->progname,pip->pr) ;
	    shio_printf(pip->efp,"%s: sn=%s\n", pip->progname,pip->searchname) ;
	} /* end if */

	if (f_usage)
	    usage(pip) ;

/* help file */

	if (f_help) {
#if	CF_SFIO
	    printhelp(sfstdout,pip->pr,pip->searchname,HELPFNAME) ;
#else
	    printhelp(nullptr,pip->pr,pip->searchname,HELPFNAME) ;
#endif
	}

	if (f_version || f_help || f_usage)
	    goto retearly ;


	ex = EX_OK ;

/* check a few more things */

	if (afname == nullptr) afname = getourenv(envv,VARAFNAME) ;

	if (qs == nullptr) qs = getourenv(envv,VARQS) ;
	if (qs == nullptr) qs = getourenv(envv,VARquerystr) ;

	if ((rs >= 0) && (pip->n == 0) && (argval != nullptr)) {
	    rs = optvalue(argval,-1) ;
	    pip->n = rs ;
	}

	if (rs >= 0) {
	    if ((rs = locinfo_dbinfo(lip,nullptr,dbfname)) >= 0) {
	        if ((rs = locinfo_finalize(lip)) >= 0) {
	            if ((rs = locinfo_qs(lip,qs)) >= 0) {
	                if ((rs = locinfo_finalize(lip)) >= 0) {
	                    if ((rs = procopts(pip,&akopts)) >= 0) {
	                        rs = locinfo_finalize(lip) ;
	                    }
	                }
	            }
	        }
	    }
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_homepage: 2 rs=%d\n",rs) ;
#endif

	if (cfname == nullptr) cfname = getourenv(envv,VARCFNAME) ;

	if (pip->lfname != nullptr) pip->lfname = getourenv(envv,VARLFNAME) ;

	if (pip->tmpdname == nullptr) pip->tmpdname = getourenv(envv,VARTMPDNAME) ;
	if (pip->tmpdname == nullptr) pip->tmpdname = TMPDNAME ;

	if (lip->tmpourdname == nullptr) {
	    lip->tmpourdname = getourenv(envv,VARWORKDNAME) ;
	}

	if (wfname == nullptr) wfname = getourenv(envv,VARWFNAME) ;
	if (wfname == nullptr) wfname = LWFNAME ;

	if (rs >= 0) {
	    rs = locinfo_wfname(lip,wfname) ;
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_homepage: 3 rs=%d\n",rs) ;
#endif

	if (pip->debuglevel > 0) {
	    cchar	*pn = pip->progname ;
	    cchar	*fmt ;
	    fmt = "%s: cf=%s\n" ;
	    shio_printf(pip->efp,fmt,pn,cfname) ;
	    fmt = "%s: lf=%s\n" ;
	    shio_printf(pip->efp,fmt,pn,pip->lfname) ;
	    fmt = "%s: wf=%s\n" ;
	    shio_printf(pip->efp,fmt,pn,lip->wfname) ;
	    fmt = "%s: force=%u\n" ;
	    shio_printf(pip->efp,fmt,pn,lip->fl.force) ;
	    fmt = "%s: maint=%u\n" ;
	    shio_printf(pip->efp,fmt,pn,lip->fl.maint) ;
	}

/* go */

	memclear(&ainfo) ;
	ainfo.argc = argc ;
	ainfo.ai = ai ;
	ainfo.argv = argv ;
	ainfo.ai_max = ai_max ;
	ainfo.ai_pos = ai_pos ;
	ainfo.ai_continue = ai_continue ;

	if (rs >= 0) {
	    userinfo	u ;
	    if ((rs = userinfo_start(&u,un)) >= 0) {
	        if ((rs = procuserinfo_begin(pip,&u)) >= 0) {
	            if (cfname != nullptr) {
	                if (pip->euid != pip->uid) u_seteuid(pip->uid) ;
	                if (pip->egid != pip->gid) u_setegid(pip->gid) ;
	            }
	            if ((rs = procourconf_begin(pip,&aparams,cfname)) >= 0) {
	                if ((rs = proglog_begin(pip,&u)) >= 0) {
	                    if ((rs = proguserlist_begin(pip)) >= 0) {
	                        cchar	*ofn = ofname ;
	                        cchar	*afn = afname ;
	                        if ((rs = process(pip,ofn,afn)) >= 0) {
	                            if (pip->debuglevel > 0) {
	                                cchar	*pn = pip->progname ;
	                                cchar	*fmt = "%s: updates=%u\n" ;
	                                shio_printf(pip->efp,fmt,pn,rs) ;
	                            }
	                        }
	                        rs1 = proguserlist_end(pip) ;
	                        if (rs >= 0) rs = rs1 ;
	                    } /* end if (proguserlist) */
	                    rs1 = proglog_end(pip) ;
	                    if (rs >= 0) rs = rs1 ;
	                } /* end if (progloger) */
	                rs1 = procourconf_end(pip) ;
	                if (rs >= 0) rs = rs1 ;
	            } /* end if (procourconf) */
	            rs1 = procuserinfo_end(pip) ;
	            if (rs >= 0) rs = rs1 ;
	        } /* end if (procuserinfo) */
	        rs1 = userinfo_finish(&u) ;
	        if (rs >= 0) rs = rs1 ;
	    } else {
	        cchar	*pn = pip->progname ;
	        cchar	*fmt = "%s: userinfo failure (%d)\n" ;
	        ex = EX_NOUSER ;
	        shio_printf(pip->efp,fmt,pn,rs) ;
	    }
	} else if (ex == EX_OK) {
	    cchar	*pn = pip->progname ;
	    cchar	*fmt = "%s: invalid argument or configuration (%d)\n" ;
	    ex = EX_USAGE ;
	    shio_printf(pip->efp,fmt,pn,rs) ;
	    usage(pip) ;
	} /* end if (ok) */

	if ((rs < 0) && (! pip->fl.quiet)) {
	    cchar	*pn = pip->progname ;
	    cchar	*fmt ;
	    if (rs == SR_TIMEDOUT) {
	        fmt = "%s: timed-out (%d)\n" ;
	    } else if (rs == SR_EXIT) {
	        fmt = "%s: program terminated (%d)\n" ;
	    } else {
	        fmt = "%s: could not process (%d)\n" ;
	    }
	    shio_printf(pip->efp,fmt,pn,rs) ;
	}

/* done */
	if ((rs < 0) && (ex == EX_OK)) {
	    switch (rs) {
	    case SR_INVALID:
	        ex = EX_USAGE ;
	        break ;
	    case SR_NOENT:
	        ex = EX_CANTCREAT ;
	        break ;
	    case SR_AGAIN:
	        ex = EX_TEMPFAIL ;
	        break ;
	    default:
	        ex = mapex(mapexs,rs) ;
	        break ;
	    } /* end switch */
	} else if ((rs >= 0) && (ex == EX_OK)) {
	    if ((rs = lib_sigterm()) < 0) {
	        ex = EX_TERM ;
	    } else if ((rs = lib_sigintr()) < 0) {
	        ex = EX_INTR ;
	    }
	} /* end if */

retearly:
	if (pip->debuglevel > 0) {
	    shio_printf(pip->efp,"%s: exiting ex=%u (%d)\n",
	        pip->progname,ex,rs) ;
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_homepage: exiting ex=%u (%d)\n",ex,rs) ;
#endif

	if (pip->efp != nullptr) {
	    pip->open.errfile = false ;
	    shio_close(pip->efp) ;
	    pip->efp = nullptr ;
	}

	if (pip->open.aparams) {
	    pip->open.aparams = false ;
	    paramopt_finish(&aparams) ;
	}

	if (pip->open.akopts) {
	    pip->open.akopts = false ;
	    keyopt_finish(&akopts) ;
	}

	bits_finish(&pargs) ;

badpargs:
	locinfo_finish(lip) ;

badlocstart:
	proginfo_finish(pip) ;

badprogstart:

#if	(CF_DEBUG || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mi[12] ;
	    uint	mo ;
	    uint	mdiff ;
	    int		f_out = CF_DEBUGMOUT ;
	    uc_mallout(&mo) ;
	    mdiff = (mo-mo_start) ;
	    debugprintf("b_homepage: final mallout=%u\n",mdiff) ;
	    if ((mdiff > 0) || f_out) {
	        UCMALLREG_CUR	cur ;
	        UCMALLREG_REG	reg ;
	        cint	size = (10 * szof(uint)) ;
	        cchar		*ids = "main" ;
	        uc_mallinfo(mi,size) ;
	        debugprintf("b_homepage: MIoutnum=%u\n",mi[ucmallreg_outnum]) ;
	        debugprintf("b_homepage: MIoutnummax=%u\n",
	            mi[ucmallreg_outnummax]) ;
	        debugprintf("b_homepage: MIoutsize=%u\n",
	            mi[ucmallreg_outsize]) ;
	        debugprintf("b_homepage: MIoutsizemax=%u\n",
	            mi[ucmallreg_outsizemax]) ;
	        debugprintf("b_homepage: MIused=%u\n",mi[ucmallreg_used]) ;
	        debugprintf("b_homepage: MIusedmax=%u\n",
	            mi[ucmallreg_usedmax]) ;
	        debugprintf("b_homepage: MIunder=%u\n",mi[ucmallreg_under]) ;
	        debugprintf("b_homepage: MIover=%u\n",mi[ucmallreg_over]) ;
	        debugprintf("b_homepage: MInotalloc=%u\n",
	            mi[ucmallreg_notalloc]) ;
	        debugprintf("b_homepage: MInotfree=%u\n",
	            mi[ucmallreg_notfree]) ;
	        ucmallreg_curbegin(&cur) ;
	        while (ucmallreg_curenum(&cur,&reg) >= 0) {
	            debugprintf("b_homepage: MIreg.addr=%p\n",reg.addr) ;
	            debugprintf("b_homepage: MIreg.size=%u\n",reg.size) ;
	            debugprinthexblock(ids,80,reg.addr,reg.size) ;
	        }
	        ucmallreg_curend(&cur) ;
	    }
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */

#if	(CF_DEBUG || CF_DEBUG)
	debugclose() ;
#endif

	return ex ;

/* bad stuff comes here */
badarg:
	ex = EX_USAGE ;
	shio_printf(pip->efp,"%s: invalid argument specified (%d)\n",
	    pip->progname,rs) ;
	usage(pip) ;
	goto retearly ;

} /* end subroutine (mainsub) */

local int usage(PI *pip) noex {
	int		rs ;
	int		wlen = 0 ;
	cchar		*pn = pip->progname ;
	cchar		*fmt ;

	fmt = "%s: USAGE> %s [-of <outfile>] [-d[=<runint>]]\n" ;
	rs = shio_printf(pip->efp,fmt,pn,pn) ;
	wlen += rs ;

	fmt = "%s:  [-af <afile>] [-svcs <svc(s)>]\n" ;
	rs = shio_printf(pip->efp,fmt,pn) ;
	wlen += rs ;

	fmt = "%s:  [-Q] [-D] [-v[=<n>]] [-HELP] [-V]\n" ;
	rs = shio_printf(pip->efp,fmt,pn) ;
	wlen += rs ;

	return (rs >= 0) ? wlen : rs ;
} /* end subroutine (usage) */

/* process the program ako-options */
local int procopts(PI *pip,keyopt *kop) noex {
	LI		*lip = pip->lip ;
	int		rs = SR_OK ;
	int		rs1 ;
	int		c = 0 ;
	cchar		*cp ;

	if ((cp = getourenv(pip->envv,VAROPTS)) != nullptr) {
	    rs = keyopt_loads(kop,cp,-1) ;
	}

	if (rs >= 0) {
	    keyopt_cur	kcur ;
	    if ((rs = keyopt_curbegin(kop,&kcur)) >= 0) {
	        int	oi ;
	        int	kl, vl ;
	        cchar	*pr = pip->progname ;
	        cchar	*sn = pip->searchname ;
	        cchar	*kp, *vp ;

	        while ((kl = keyopt_curenumkeys(kop,&kcur,&kp)) >= 0) {

	            if ((oi = matostr(akonames,2,kp,kl)) >= 0) {
	                int	v = 0 ;

	                vl = keyopt_fetch(kop,kp,nullptr,&vp) ;

	                switch (oi) {
	                case akoname_base:
	                    if (! lip->finval.basedname) {
	                        if (vl > 0) {
	                            lip->finval.basedname = true ;
	                            lip->have.basedname = true ;
	                            rs = locinfo_basedir(lip,vp,vl) ;
	                        }
	                    }
	                    break ;
	                case akoname_cf:
	                    if ((pip->cfname == nullptr) && (vl > 0)) {
	                        cchar	*lc = LOGCNAME ;
	                        char	tbuf[MAXPATHLEN+1] ;
	                        rs = prmkfname(pr,tbuf,vp,vl,true,lc,sn,"") ;
	                        if (rs >= 0) {
	                            cchar	**vpp = &pip->cfname ;
	                            rs = proginfo_setentry(pip,vpp,tbuf,rs) ;
	                        }
	                    }
	                    break ;
	                case akoname_lf:
	                    if ((pip->lfname == nullptr) && (vl > 0)) {
	                        cchar	*lc = LOGCNAME ;
	                        char	tbuf[MAXPATHLEN+1] ;
	                        rs = prmkfname(pr,tbuf,vp,vl,true,lc,sn,"") ;
	                        if (rs >= 0) {
	                            cchar	**vpp = &pip->lfname ;
	                            rs = proginfo_setentry(pip,vpp,tbuf,rs) ;
	                        }
	                    }
	                    break ;
	                case akoname_wf:
	                    if ((lip->wfname == nullptr) && (vl > 0)) {
	                        cchar	*lc = LOGCNAME ;
	                        char	tbuf[MAXPATHLEN+1] ;
	                        rs = prmkfname(pr,tbuf,vp,vl,true,lc,sn,"") ;
	                        if (rs >= 0) {
	                            cchar	**vpp = &lip->wfname ;
	                            rs = locinfo_setentry(lip,vpp,tbuf,rs) ;
	                        }
	                    }
	                    break ;
	                case akoname_log:
	                    if (! pip->finval.logprog) {
	                        pip->fl.logprog = true ;
	                        if (vl > 0) {
	                            pip->finval.logprog = true ;
	                            pip->have.logprog = true ;
	                            rs = optbool(vp,vl) ;
	                            pip->fl.logprog = (rs > 0) ;
	                        }
	                    }
	                    break ;
	                case akoname_quiet:
	                    if (! pip->finval.quiet) {
	                        pip->fl.quiet = true ;
	                        if (vl > 0) {
	                            pip->finval.quiet = true ;
	                            pip->have.quiet = true ;
	                            rs = optbool(vp,vl) ;
	                            pip->fl.quiet = (rs > 0) ;
	                        }
	                    }
	                    break ;
	                case akoname_quietlock:
	                    if (! lip->finval.quietlock) {
	                        lip->fl.quietlock = true ;
	                        if (vl > 0) {
	                            lip->finval.quietlock = true ;
	                            lip->have.quietlock = true ;
	                            rs = optbool(vp,vl) ;
	                            lip->fl.quietlock = (rs > 0) ;
	                        }
	                    }
	                    break ;
	                case akoname_force:
	                    if (! lip->finval.force) {
	                        lip->fl.force = true ;
	                        if (vl > 0) {
	                            lip->finval.force = true ;
	                            lip->have.force = true ;
	                            rs = optbool(vp,vl) ;
	                            lip->fl.force = (rs > 0) ;
	                        }
	                    }
	                    break ;
	                case akoname_maint:
	                    if (! lip->finval.maint) {
	                        lip->have.maint = true ;
	                        lip->fl.maint = true ;
	                        if (vl > 0) {
	                            lip->finval.maint = true ;
	                            rs = optbool(vp,vl) ;
	                            lip->fl.maint = (rs > 0) ;
	                        }
	                    }
	                    break ;
	                case akoname_head:
	                    if (! lip->finval.hfname) {
	                        if (vl > 0) {
	                            lip->finval.hfname = true ;
	                            lip->have.hfname = true ;
	                            rs = locinfo_sethead(lip,vp,vl) ;
	                        }
	                    }
	                    break ;
	                case akoname_svcs:
	                    if (! lip->finval.svcs) {
	                        if (vl > 0) {
	                            lip->have.svcs = true ;
	                            rs = locinfo_svclistadds(lip,vp,vl) ;
	                        }
	                    }
	                    break ;
	                case akoname_daemon:
	                    if (! pip->finval.daemon) {
	                        c += 1 ;
	                        pip->finval.daemon = true ;
	                        pip->have.daemon = true ;
	                        pip->fl.daemon = true ;
	                        if (vl > 0) {
	                            rs = cfdecti(vp,vl,&v) ;
	                            pip->intrun = v ;
	                            if (v == 0) {
	                                pip->fl.daemon = false ;
	                            }
	                        }
	                    } /* end if */
	                    break ;
	                case akoname_pidfile:
	                    if (! pip->finval.pidfname) {
	                        if (vl > 0) {
	                            cchar	**vpp = &pip->pidfname ;
	                            pip->finval.pidfname = true ;
	                            pip->have.pidfname = true ;
	                            rs = proginfo_setentry(pip,vpp,vp,vl) ;
	                        }
	                    }
	                    break ;
	                case akoname_to:
	                case akoname_intrun:
	                case akoname_intidle:
	                case akoname_intpoll:
	                case akoname_intconf:
	                case akoname_intsvcs:
	                case akoname_intcache:
	                case akoname_intwait:
	                case akoname_intmaint:
	                    {
	                        if (vl > 0) {
	                            rs = cfdecti(vp,vl,&v) ;
	                        }
	                        if ((rs >= 0) && (vl > 0)) {
	                            switch(oi) {
	                            case akoname_to:
	                                if (! pip->finval.to) {
	                                    c += 1 ;
	                                    pip->finval.to = true ;
	                                    pip->have.to = true ;
	                                    pip->to = v ;
	                                }
	                                break ;
	                            case akoname_intrun:
	                                if (! pip->finval.intrun) {
	                                    c += 1 ;
	                                    pip->finval.intrun = true ;
	                                    pip->have.intrun = true ;
	                                    pip->intrun = v ;
	                                }
	                                break ;
	                            case akoname_intidle:
	                                if (! pip->finval.intidle) {
	                                    c += 1 ;
	                                    pip->finval.intidle = true ;
	                                    pip->have.intidle = true ;
	                                    pip->intidle = v ;
	                                }
	                                break ;
	                            case akoname_intpoll:
	                                if (! pip->finval.intpoll) {
	                                    c += 1 ;
	                                    pip->finval.intpoll = true ;
	                                    pip->have.intpoll = true ;
	                                    pip->intpoll = v ;
	                                }
	                                break ;
	                            case akoname_intconf:
	                                if (! lip->finval.intconf) {
	                                    c += 1 ;
	                                    lip->finval.intconf = true ;
	                                    lip->have.intconf = true ;
	                                    lip->intconf = v ;
	                                }
	                                break ;
	                            case akoname_intsvcs:
	                                if (! lip->finval.intsvcs) {
	                                    c += 1 ;
	                                    lip->finval.intsvcs = true ;
	                                    lip->have.intsvcs = true ;
	                                    lip->intsvcs = v ;
	                                }
	                                break ;
	                            case akoname_intcache:
	                                if (! lip->finval.intcache) {
	                                    c += 1 ;
	                                    lip->finval.intcache = true ;
	                                    lip->have.intcache = true ;
	                                    lip->intcache = v ;
	                                }
	                                break ;
	                            case akoname_intwait:
	                                if (! lip->finval.intwait) {
	                                    c += 1 ;
	                                    lip->finval.intwait = true ;
	                                    lip->have.intwait = true ;
	                                    lip->intwait = v ;
	                                }
	                                break ;
	                            case akoname_intmaint:
	                                if (! lip->finval.intmaint) {
	                                    c += 1 ;
	                                    lip->finval.intmaint = true ;
	                                    lip->have.intmaint = true ;
	                                    lip->intmaint = v ;
	                                }
	                                break ;
	                            } /* end switch */
	                        } /* end if (have) */
	                    } /* end block */
	                    break ;
	                } /* end switch */
	                c += 1 ;
	            } else {
	                cchar	*pn = pip->progname ;
	                cchar	*fmt = "%s: invalid key=>%r<\n" ;
	                shio_printf(pip->efp,fmt,pn,kp,kl) ;
	                rs = SR_INVALID ;
	            }
	            if (rs < 0) break ;
	        } /* end while (looping through key options) */
	        rs1 = keyopt_curend(kop,&kcur) ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (cursor) */
	} /* end if (ok) */

	return (rs >= 0) ? c : rs ;
} /* end subroutine (procopts) */

local int procuserinfo_begin(PI *pip,userinfo *uip) noex {
	int		rs = SR_OK ;
	pip->nodename = uip->nodename ;
	pip->domainname = uip->domainname ;
	pip->username = uip->username ;
	pip->gecosname = uip->gecosname ;
	pip->realname = uip->realname ;
	pip->name = uip->name ;
	pip->fullname = uip->fullname ;
	pip->mailname = uip->mailname ;
	pip->org = uip->organization ;
	pip->logid = uip->logid ;
	pip->pid = uip->pid ;
	pip->uid = uip->uid ;
	pip->euid = uip->euid ;
	pip->gid = uip->gid ;
	pip->egid = uip->egid ;

	if (rs >= 0) {
	    cint	hlen = MAXHOSTNAMELEN ;
	    char	hbuf[MAXHOSTNAMELEN+1] ;
	    cchar	*nn = pip->nodename ;
	    cchar	*dn = pip->domainname ;
	    if ((rs = snsds(hbuf,hlen,nn,dn)) >= 0) {
	        cchar	**vpp = &pip->hostname ;
	        rs = proginfo_setentry(pip,vpp,hbuf,rs) ;
	    }
	}

	if (rs >= 0) {
	    rs = procuserinfo_logid(pip) ;
	} /* end if (ok) */

	if (rs >= 0) {
	    LI	*lip = pip->lip ;
	    TMTIME	t ;
	    if ((rs = tmtime_timegm(&t,pip->daytime)) >= 0) {
	        lip->year = (t.year + TMTIME_YEARBASE) ;
	        rs = locinfo_cookbegin(lip) ;
	    }
	}

	if (pip->debuglevel > 0) {
	    shio_printf(pip->efp,"%s: username=%s\n",
	        pip->progname,pip->username) ;
	}

	return rs ;
} /* end subroutine (procuserinfo_begin) */

local int procuserinfo_end(PI *pip) noex {
	int		rs = SR_OK ;
	int		rs1 ;

	if (pip == nullptr) return SR_FAULT ;

	{
	    LI	*lip = pip->lip ;
	    rs1 = locinfo_cookend(lip) ;
	    if (rs >= 0) rs = rs1 ;
	}

	return rs ;
} /* end subroutine (procuserinfo_end) */

local int procuserinfo_logid(PI *pip) noex {
	int		rs ;
	if ((rs = lib_runmode()) >= 0) {
	    if (rs & KSHLIB_RMKSH) {
	        if ((rs = lib_serial()) >= 0) {
	            cint	s = rs ;
	            cint	plen = LOGIDLEN ;
	            cint	pv = pip->pid ;
	            cchar	*nn = pip->nodename ;
	            char	pbuf[LOGIDLEN+1] ;
	            if ((rs = mklogidpre(pbuf,plen,nn,pv)) >= 0) {
	                cint	slen = LOGIDLEN ;
	                char		sbuf[LOGIDLEN+1] ;
	                if ((rs = mklogidsub(sbuf,slen,pbuf,s)) >= 0) {
	                    cchar	**vpp = &pip->logid ;
	                    rs = proginfo_setentry(pip,vpp,sbuf,rs) ;
	                }
	            }
	        } /* end if (lib_serial) */
	    } /* end if (runmode-KSH) */
	} /* end if (lib_runmode) */
	return rs ;
} /* end subroutine (procuserinfo_logid) */

local int procourconf_begin(PI *pip,paramopt *app,cchar *cfname) noex {
	cint		figsize = szof(CONFIG) ;
	int		rs ;
	void		*p ;

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("b_homepage/procourconf_begin: ent\n") ;
#endif

	if ((rs = lm_mall(figsize,&p)) >= 0) {
	    CONFIG	*csp = p ;
	    pip->config = csp ;
	    if ((rs = config_start(csp,pip,app,cfname)) >= 0) {
	        LI	*lip = pip->lip ;
	        if (lip->copyright == nullptr) {
	            cchar	*c = COPYRIGHT ;
	            if ((rs = locinfo_copyright(lip,c,-1)) >= 0) {
	                if (lip->webmaster == nullptr) {
	                    lip->webmaster = WEBMASTER ;
	                }
	            }
	        }
	        if (rs < 0) {
	            config_finish(csp) ;
	        }
	    } /* end if (config_start) */
	    if (rs < 0) {
	        lm_free(p) ;
	        pip->config = nullptr ;
	    }
	} /* end if (memory-allocation) */

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("b_homepage/procourconf_begin: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (procourconf_begin) */

local int procourconf_end(PI *pip) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	if (pip->config) {
	    CONFIG	*csp = pip->config ;
	    rs1 = config_finish(csp) ;
	    if (rs >= 0) rs = rs1 ;
	    rs1 = lm_free(pip->config) ;
	    if (rs >= 0) rs = rs1 ;
	    pip->config = nullptr ;
	}
	return rs ;
} /* end subroutine (procourconf_end) */

local int process(PI *pip,cchar *ofn,cchar *afn) noex {
	LI		*lip = pip->lip ;
	int		rs ;
	int		rs1 ;
	int		c = 0 ;

	if ((rs = ids_load(&pip->id)) >= 0) {
	    if ((rs = locinfo_tmpourdname(lip)) >= 0) {
	        if (pip->fl.background || pip->fl.daemon) {
	            if ((rs = procbackdefs(pip)) >= 0) {
	                if ((rs = procpidfname(pip)) >= 0) {
	                    if ((rs = procbackinfo(pip)) >= 0) {
	                        if (pip->fl.background) {
	                            rs = procback(pip) ;
	                        } else if (pip->fl.daemon) {
	                            rs = procdaemon(pip) ;
	                            c = rs ;
	                        }
	                    } /* end if (procbackinfo) */
	                } /* end if (procpidfname) */
	            } /* end if (procbackdefs) */
	        } else {
	            rs = procregular(pip,ofn,afn) ;
	            c = rs ;
	        }
	    } /* end if (locinfo_tmpourdname) */
	    rs1 = ids_release(&pip->id) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (ids) */

#ifdef	COMMENT
	if ((rs >= 0) && (pip->debuglevel > 0)) {
	    shio_printf(pip->efp, "%s: updates=%u\n",pip->progname,c) ;
	} /* end if */
#endif /* COMMENT */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("msumain/process: ret rs=%d c=%u\n",rs,c) ;
#endif

	return (rs >= 0) ? c : rs ;
} /* end subroutine (process) */

local int procbackdefs(PI *pip) noex {
	LI		*lip = pip->lip ;
	int		rs = SR_OK ;
	if (rs >= 0){
	    rs = locinfo_defpfname(lip) ;
	}
	return rs ;
} /* end subroutine (procbackdefs) */

local int procdaemondefs(PI *pip) noex {
	LI		*lip = pip->lip ;
	int		rs = SR_OK ;
	if (rs >= 0) {
	    rs = locinfo_defpfname(lip) ;
	}
	if (lip->reqfname == nullptr) {
	    cchar	**envv = pip->envv ;
	    cchar	*cp ;
	    if ((cp = getourenv(envv,VARREQFNAME)) != nullptr) {
	        lip->finval.reqfname = true ;
	        lip->have.reqfname = true ;
	        lip->reqfname = cp ;
	    }
	}

	if (pip->intidle == 0) pip->intidle = TO_IDLE ;

	if (pip->intpoll == 0) pip->intpoll = TO_POLL ;

	if (pip->intmark == 0) pip->intmark = TO_MARK ;

	if (pip->intlock == 0) pip->intlock = TO_LOCK ;

#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("msumain: daemon=%u logging=%u\n",
	        pip->fl.daemon,pip->have.logprog) ;
#endif
#if	CF_DEBUG
	debugprintf("msumain: daemon=%u logging=%u\n",
	    pip->fl.daemon,pip->have.logprog) ;
#endif

	if (pip->debuglevel > 0) {
	    cchar	*pn = pip->progname ;
	    cchar	*fmt ;
	    if (pip->intpoll >= 0) {
	        fmt = "%s: mspoll=%u\n" ;
	    } else {
	        fmt = "%s: mspoll=Inf\n" ;
	    }
	    shio_printf(pip->efp,fmt,pn,pip->intpoll) ;
	    shio_printf(pip->efp,"%s: intspeed=%u\n",pn,lip->intspeed) ;
	}

	return rs ;
}
/* end subroutine (procdaemondefs) */

local int procpidfname(PI *pip) noex {
	int		rs = SR_OK ;
	int		pfl = -1 ;
	int		f_changed = false ;
	cchar		*sn = pip->searchname ;
	cchar		*pfp = pip->pidfname ;
	char		rundname[MAXPATHLEN+1] ;
	char		cname[MAXNAMELEN + 1] ;
	char		tmpfname[MAXPATHLEN + 1] ;

	if ((pfp == nullptr) || (pfp[0] == '+')) {

	    f_changed = true ;
	    if ((rs = mkpath2(rundname,pip->pr,RUNDNAME)) >= 0) {
	        if (ustat sb ; (rs = uc_stat(rundname,&sb)) >= 0) {
	            if (! S_ISDIR(sb.st_mode)) {
	                rs = SR_NOTDIR ;
	            }
	        } else if (rs == SR_NOENT) {
	            rs = mkdirs(rundname,0777) ;
	        }
	    }

	    if (rs >= 0) {
	        if ((rs = snsds(cname,MAXNAMELEN,pip->nodename,sn)) >= 0) {
	            pfp = tmpfname ;
	            rs = mkpath2(tmpfname,rundname,cname) ;
	            pfl = rs ;
	        }
	    }

	} /* end if (creating a default PID file-name) */

	if ((rs >= 0) && (pfp != nullptr) && (pfp[0] == '-')) {
	    pfp = nullptr ;
	    pip->have.pidfname = false ;
	    pip->fl.pidfname = false ;
	    f_changed = false ;
	}

	if ((rs >= 0) && (pfp != nullptr) && f_changed) {
	    cchar	**vpp = &pip->pidfname ;
	    rs = proginfo_setentry(pip,vpp,pfp,pfl) ;
	}

	return (rs >= 0) ? f_changed : rs ;
} /* end subroutine (procpidfname) */

local int procbackinfo(PI *pip) noex {
	LI		*lip = pip->lip ;
	int		rs = SR_OK ;

	if ((rs >= 0) && (pip->debuglevel > 0)) {
	    cchar	*pn = pip->progname ;
	    cchar	*mntfname = lip->mntfname ;

	    if (pip->pidfname != nullptr) {
	        shio_printf(pip->efp,"%s: pidfile=%s\n",pn,pip->pidfname) ;
	    }

	    if (mntfname != nullptr) {
	        shio_printf(pip->efp,"%s: mntfile=%s\n",pn,mntfname) ;
	    }

	    shio_flush(pip->efp) ;
	} /* end if (debugging information) */

	return rs ;
} /* end subroutine (procbackinfo) */

local int procdaemoninfo(PI *pip) noex {
	LI		*lip = pip->lip ;
	int		rs = SR_OK ;

	if ((rs >= 0) && (pip->debuglevel > 0)) {
	    cchar	*pn = pip->progname ;
	    if (pip->fl.daemon) {
	        shio_printf(pip->efp,"%s: pidfile=%u\n",pn,pip->pidfname) ;
	        shio_printf(pip->efp,"%s: intrun=%u\n",pn,pip->intrun) ;
	        shio_printf(pip->efp,"%s: intidle=%u\n",pn,pip->intidle) ;
	        shio_printf(pip->efp,"%s: intpoll=%u\n",pn,pip->intpoll) ;
	        shio_printf(pip->efp,"%s: intmark=%u\n",pn,pip->intmark) ;
	        shio_printf(pip->efp,"%s: intwait=%u\n",pn,lip->intwait) ;
	    }
	    shio_printf(pip->efp,"%s: intcache=%u\n",pn,lip->intcache) ;
	    shio_flush(pip->efp) ;
	} /* end if (debugging information) */

	if (pip->fl.daemon) {
	    proglog_printf(pip,"pidfile=%d",pip->pidfname) ;
	    proglog_printf(pip,"intrun=%d",pip->intrun) ;
	    proglog_printf(pip,"intidle=%d",pip->intidle) ;
	    proglog_printf(pip,"intpoll=%d",pip->intpoll) ;
	    proglog_printf(pip,"intlock=%d",pip->intlock) ;
	    proglog_printf(pip,"intwait=%d",lip->intwait) ;
	}
	proglog_printf(pip,"intcache=%d",lip->intcache) ;
	proglog_flush(pip) ;

	return rs ;
} /* end subroutine (procdaemoninfo) */

#if	CF_PROCARGS
local int procargs(PI *pip,ARGINFO *aip,bits *bop,
		cc *ofn,cc *ifn,cc *afn) noex {
	SHIO		ofile, *ofp = &ofile ;
	cint	to_open = pip->to_open ;
	int		rs ;
	int		wlen = 0 ;
	cchar		*pn = pip->progname ;
	cchar		*fmt ;

	if ((ofn == nullptr) || (ofn[0] == '\0') || (ofn[0] == '-'))
	    ofn = STDFNOUT ;

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/procargs: ofn=%s\n",ofn) ;
#endif

	if ((rs = shio_opene(ofp,ofn,"wc",0666,to_open)) >= 0) {
	    LI	*lip = pip->lip ;
	    int		pan = 0 ;
	    int		cl ;
	    cchar	*cp ;

	    if (lip->fl.trunc) lip->start = 0L ;

	    if ((rs = shio_isseekable(ofp)) > 0) {
	        if (lip->start >= 0) {
	            off_t	o = lip->start ;
	            rs = shio_seek(ofp,o,SEEK_SET) ;
	        } else {
	            rs = shio_seek(ofp,0L,SEEK_END) ;
	        }
	    } /* end if (stat) */

#ifdef	COMMENT
	    if (pip->fl.bufnone)
	        shio_control(ofp,SHIO_CSETBUFNONE,true) ;

	    if (pip->have.bufline)
	        shio_control(ofp,SHIO_CSETBUFLINE,pip->fl.bufline) ;

	    if (pip->have.bufwhole)
	        shio_control(ofp,SHIO_CSETBUFWHOLE,pip->fl.bufwhole) ;
#endif /* COMMENT */

	    if (rs >= 0) {
	        int	ai ;
	        int	f ;
	        cchar	**argv = aip->argv ;
	        for (ai = aip->ai_continue ; ai < aip->argc ; ai += 1) {

	            f = (ai <= aip->ai_max) && (bits_test(bop,ai) > 0) ;
	            f = f || ((ai > aip->ai_pos) && (argv[ai] != nullptr)) ;
	            if (f) {
	                cp = aip->argv[ai] ;
	                if (cp[0] != '\0') {
	                    pan += 1 ;
	                    rs = procfile(pip,ofp,cp) ;
	                    wlen += rs ;
	                }
	            }

	            if (rs >= 0) rs = lib_sigterm() ;
	            if (rs >= 0) rs = lib_sigintr() ;
	            if (rs < 0) break ;
	        } /* end for */
	    } /* end if (ok) */

	    if ((rs >= 0) && (afn != nullptr) && (afn[0] != '\0')) {
	        SHIO	afile, *afp = &afile ;

	        if (strcmp(afn,"-") == 0) afn = STDFNIN ;

	        if ((rs = shio_open(afp,afn,"r",0666)) >= 0) {
	            cint	llen = LINEBUFLEN ;
	            char	lbuf[LINEBUFLEN + 1] ;

	            while ((rs = shio_readline(afp,lbuf,llen)) > 0) {
	                int	len = rs ;

	                if (lbuf[len - 1] == '\n') len -= 1 ;
	                lbuf[len] = '\0' ;

	                cp = lbuf ;
	                cl = len ;

	                if (cl > 0) {
	                    pan += 1 ;
	                    rs = procfile(pip,ofp,cp) ;
	                    wlen += rs ;
	                }

	                if (rs >= 0) rs = lib_sigterm() ;
	                if (rs >= 0) rs = lib_sigintr() ;
	                if (rs < 0) break ;
	            } /* end while (reading lines) */

	            shio_close(afp) ;
	        } else {
	            fmt = "%s: inaccessible argument-list (%d)\n" ;
	            shio_printf(pip->efp,fmt,pn,rs) ;
	            shio_printf(pip->efp,"%s: afile=%s\n",pn,afn) ;
	        } /* end if */

	    } /* end if (procesing file argument file list) */

	    if ((rs >= 0) && (pan == 0)) {

	        cp = (ifn != nullptr) ? ifn : "-" ;
	        pan += 1 ;
	        rs = procfile(pip,ofp,cp) ;
	        wlen += rs ;

	    } /* end if (standard-input) */

	    lip->nproc = pan ;
	    shio_close(ofp) ;
	} else {
	    fmt = "%s: inaccessible output (%d)\n" ;
	    shio_printf(pip->efp,fmt,pn,rs) ;
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/procargs: ret rs=%d wlen=%u\n",rs,wlen) ;
#endif

	return (rs >= 0) ? wlen : rs ;
} /* end subroutine (procargs) */
#endif /* CF_PROCARGS */

local int procback(PI *pip) noex {
	int		rs ;
	if (pip->open.logprog) {
	    proglog_printf(pip,"mode=background") ;
	    proglog_flush(pip) ;
	}
	if (pip->debuglevel > 0) {
	    shio_printf(pip->efp,"%s: mode=background\n",pip->progname) ;
	    shio_flush(pip->efp) ;
	}
	if ((rs = procbackcheck(pip)) >= 0) {
	    if ((rs = procbackmaint(pip)) >= 0) {
	        rs = procbacks(pip) ;
	    }
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_homepage/procback: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (procback) */

local int procbackmaint(PI *pip) noex {
	LI		*lip = pip->lip ;
	cint	to = TO_TMPFILES ;
	int		rs = SR_OK ;
	if (lip->fl.maint || (lip->intmaint > 0)) {
	    cchar	*dir = lip->tmpourdname ;
	    cchar	*prefix = "homepage" ;
	    if ((rs = rmdirfiles(dir,prefix,to)) > 0) {
	        if (pip->debuglevel > 0) {
	            cchar	*pn = pip->progname ;
	            cchar	*fmt ;
	            fmt = "%s: files-cleaned=%u\n" ;
	            shio_printf(pip->efp,fmt,pn,rs) ;
	        }
	        proglog_printf(pip,"files-cleaned=%u",rs) ;
	    }
	} /* end if (maintenance) */
	return rs ;
} /* end subroutine (procbackmaint) */

local int procbackcheck(PI *pip) noex {
	LI		*lip = pip->lip ;
	int		rs ;
	int		rs1 ;
	cchar		*pn = pip->progname ;
	cchar		*fmt ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_homepage/procbackcheck: ent\n") ;
#endif

	if ((rs = locinfo_lockbegin(lip)) >= 0) {
	    {
	        rs = procmntcheck(pip) ;
	    }
	    rs1 = locinfo_lockend(lip) ;
	    if (rs >= 0) rs = rs1 ;
	} else {
	    cint	f = ((pip->debuglevel > 0) || (! lip->fl.quietlock)) ;
	    if ((! pip->fl.quiet) && f) {
	        cchar	*lfn = pip->pidfname ;
	        fmt = "%s: could not acquire PID lock (%d)\n" ;
	        shio_printf(pip->efp,fmt,pn,rs) ;
	        fmt = "%s: lock=%s\n" ;
	        shio_printf(pip->efp,fmt,pn,lfn) ;
	    }
	    proglog_printf(pip,"could not capture PID lock (%d)",rs) ;
	}

	return rs ;
} /* end subroutine (procbackcheck) */

local int procmntcheck(PI *pip) noex {
	LI		*lip = pip->lip ;
	int		rs = SR_OK ;
	if (lip == nullptr) return SR_FAULT ;
#if	CF_MNTCHECK
	if (lip->mntfname != nullptr) {
	    ustat	usb ;
	    cchar	*pn = pip->progname ;
	    cchar	*fmt ;
	    if ((rs = u_stat(lip->mntfname,&usb)) >= 0) {
	        if (S_ISREG(usb.st_mode)) {
	            rs = permids(&pip->id,&usb,W_OK) ;
	        } else {
	            rs = SR_BUSY ;
	        }
	        if (rs < 0) {
	            if (! pip->fl.quiet) {
	                fmt = "%s: inaccessible mount point (%d)\n" ;
	                shio_printf(pip->efp,fmt,pn,rs) ;
	            }
	        }
	    }
	} /* end if (mntfname) */
#endif /* CF_MNTCHECK */
	return rs ;
} /* end subroutine (procmntcheck) */

local int procbacks(PI *pip) noex {
	cint		elen = MAXPATHLEN ;
	int		rs ;
	cchar		*pn = pip->progname ;
	cchar		*fmt ;
	char		ebuf[MAXPATHLEN+1] ;

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("b_homepage/procbacks: ent\n") ;
#endif

	if ((rs = procexecname(pip,ebuf,elen)) >= 0) {
	    int		el = rs ;
	    cchar	*pf = ebuf ;
	    cchar	*tp ;
	    char	pbuf[MAXPATHLEN+1] ;

	    if (pip->debuglevel > 0) {
	        fmt = "%s: execname=%r\n" ;
	        shio_printf(pip->efp,fmt,pn,ebuf,el) ;
	    }

	    if ((tp = strnrbrk(ebuf,el,"/.")) != nullptr) {
	        if (tp[0] == '.') {
	            el = (tp-ebuf) ;
	            ebuf[el] = '\0' ;
	        }
	    }

#if	CF_DEBUG
	    if (DEBUGLEVEL(3))
	        debugprintf("b_homepage/procbacks: ebuf=%r\n",ebuf,el) ;
#endif

	    if ((rs = prgetprogpath(pip->pr,pbuf,ebuf,el)) > 0) {
	        pf = pbuf ;
	    }

#if	CF_DEBUG
	    if (DEBUGLEVEL(3)) {
	        debugprintf("b_homepage/procbacks: mid rs=%d\n",rs) ;
	        debugprintf("b_homepage/procbacks: mid pf=%s\n",pf) ;
	    }
#endif

	    if (rs >= 0) {
	        int	i = 0 ;
	        cchar	*av[5] ;
	        char	dbuf[10+1] ;
	        if (pip->debuglevel > 0) {
	            fmt = "%s: pf=%s\n" ;
	            shio_printf(pip->efp,fmt,pn,pf) ;
	        }
	        av[i++] = pip->progname ;
	        av[i++] = "-daemon" ;
	        if (pip->debuglevel > 0) {
	            bufprintf(dbuf,10,"-D=%u",pip->debuglevel) ;
	            av[i++] = dbuf ;
	        }
	        av[i++] = nullptr ;
	        rs = procbacker(pip,pf,av) ;
#if	CF_DEBUG
	        if (DEBUGLEVEL(3))
	            debugprintf("b_homepage/procbacks: procbacker() rs=%d\n",rs) ;
#endif
	    } /* end if (ok) */
	} /* end if (procexecname) */

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("b_homepage/procbacks: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (procbacks) */

local int procbacker(PI *pip,cchar *pf,cchar **av) noex {
	int		rs ;
	int		rs1 ;
	int		pid = 0 ;
	mainv		ev = pip->envv ;
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/procbacker: ent\n") ;
#endif
	if (spawner s ; (rs = spawner_start(&s,pf,av,ev)) >= 0) {
	    if ((rs = procbackenv(pip,&s)) >= 0) {
	        int	i ;
	        for (i = 0 ; sigignores[i] > 0 ; i += 1) {
	            spawner_sigignore(&s,sigignores[i]) ;
	        }
	        spawner_setsid(&s) ;
	        if (pip->uid != pip->euid) {
	            spawner_seteuid(&s,pip->uid) ;
	        }
	        if (pip->gid != pip->egid) {
	            spawner_setegid(&s,pip->gid) ;
	        }
	        for (i = 0 ; i < 2 ; i += 1) {
	            spawner_fdclose(&s,i) ;
	        }
	        if ((rs = spawner_run(&s)) >= 0) {
	            cchar	*fmt ;
	            pid = rs ;
	            if (pip->open.logprog) {
	                fmt = "backgrounded (%u)" ;
	                proglog_printf(pip,fmt,pid) ;
	            }
	        }
	    } /* end if (procbackenv) */
	    rs1 = spawner_finish(&s) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (spawner) */
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/procbacker: ret rs=%d pid=%u\n",rs,pid) ;
#endif
	return (rs >= 0) ? pid : rs ;
} /* end subroutine (procbacker) */

local int procbackenv(PI *pip,spanwer *srp) noex {
	LI		*lip = pip->lip ;
	int		rs ;
	int		rs1 ;
	cchar		*varopts = VAROPTS ;
	if (bufferb ; (rs = buffer_start(&b,ENVBUFLEN)) >= 0) {
	    cchar	*namp ;
	    int		v ;
	    int		i ;
	    int		c = 0 ;

	    for (i = 0 ; i < 9 ; i += 1) {
	        namp = nullptr ;
	        v = 0 ;
	        switch (i) {
	        case 0:
	            v = pip->intrun ;
	            if (v > 0) namp = "intrun" ;
	            break ;
	        case 1:
	            v = pip->intidle ;
	            if (v > 0) namp = "intidle" ;
	            break ;
	        case 2:
	            v = pip->intpoll ;
	            if (v > 0) namp = "intpoll" ;
	            break ;
	        case 3:
	            v = lip->intconf ;
	            if (v > 0) namp = "intconf" ;
	            break ;
	        case 4:
	            v = lip->intsvcs ;
	            if (v > 0) namp = "intsvcs" ;
	            break ;
	        case 5:
	            v = lip->intspeed ;
	            if (v > 0) namp = "intspeed" ;
	            break ;
	        case 6:
	            v = (pip->fl.reuseaddr&1) ;
	            if (v > 0) namp = "resueaddr" ;
	            break ;
	        case 7:
	            v = (pip->fl.quiet&1) ;
	            if (v > 0) namp = "quiet" ;
	            break ;
	        case 8:
	            v = (lip->fl.quietlock&1) ;
	            if (v > 0) namp = "quietlock" ;
	            break ;
	        } /* end switch */
	        if (namp && (v > 0)) {
	            if (c++ > 0) {
	                buffer_chr(&b,CH_COMMA) ;
	            }
	            rs = buffer_printf(&b,"%s=%d",namp,v) ;
	        }
	        if (rs < 0) break ;
	    } /* end for */

	    if (rs >= 0) {
	        cchar	*vp ;
	        for (i = 0 ; i < 3 ; i += 1) {
	            namp = nullptr ;
	            switch (i) {
	            case 0:
	                if (pip->pidfname != nullptr) {
	                    namp = "pidfile" ;
	                    vp = pip->pidfname ;
	                }
	                break ;
	            case 1:
	                if (lip->mntfname != nullptr) {
	                    namp = "mntfile" ;
	                    vp = lip->mntfname ;
	                }
	                break ;
	            case 2:
	                if (lip->msfname != nullptr) {
	                    namp = "msfile" ;
	                    vp = lip->msfname ;
	                }
	                break ;
	            } /* end switch */
	            if (namp) {
	                if (c++ > 0) {
	                    buffer_chr(&b,CH_COMMA) ;
	                }
	                rs = buffer_printf(&b,"%s=%s",namp,vp) ;
	            } /* end if (non-null) */
	            if (rs < 0) break ;
	        } /* end for */
	    } /* end if (ok) */

	    if ((rs >= 0) && (c > 0)) {
	        if ((rs = buffer_get(&b,&namp)) >= 0) {
	            rs = spawner_envset(srp,varopts,namp,rs) ;
	        }
	    }

	    rs1 = buffer_finish(&b) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (buffer) */

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/procbackenv: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (procbackenv) */

local int procdaemon(PI *pip) noex {
	int		rs ;
	int		rs1 ;
	int		c = 0 ;

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/procdaemon: ent\n") ;
#endif

	if (pip->open.logprog) {
	    proglog_printf(pip,"mode=daemon pid=%d",pip->pid) ;
	    proglog_flush(pip) ;
	}
	if (pip->debuglevel > 0) {
	    pid_t	pid = pip->pid ;
	    cchar	*pn = pip->progname ;
	    shio_printf(pip->efp,"%s: mode=daemon pid=%d\n",pn,pid) ;
	}

	if ((rs = procdaemondefs(pip)) >= 0) {
	    if ((rs = procdaemoncheck(pip)) >= 0) {
	        LI	*lip = pip->lip ;
	        if ((rs = locinfo_defs(lip)) >= 0) {
	            if ((rs = locinfo_defdaemon(lip)) >= 0) {
	                if ((rs = procdaemoninfo(pip)) >= 0) {
	                    if ((rs = locinfo_lockbegin(lip)) >= 0) {
	                        if ((rs = locinfo_svcsbegin(lip)) >= 0) {
	                            {
	                                rs = procdaemons(pip) ;
	                                c = rs ;
	                            }
	                            rs1 = locinfo_svcsend(lip) ;
	                            if (rs >= 0) rs = rs1 ;
	                        } /* end if (locinfo-svcs) */
	                        rs1 = locinfo_lockend(lip) ;
	                        if (rs >= 0) rs = rs1 ;
	                    } /* end if (lock) */
	                } /* end if (procdaemoninfo) */
	            } /* end if (locinfo_defdaemon) */
	        } /* end if (locinfo_defs) */
	    } /* end if (procdaemoncheck) */
	} /* end if (procdaemondefs) */

	if ((pip->debuglevel > 0) && (pip->efp != nullptr)) {
	    cchar	*pn = pip->progname ;
	    cchar	*fmt ;
	    if (rs == SR_TIMEDOUT) {
	        fmt = "%s: daemon run-time expired (%d)\n" ;
	    } else if (rs == SR_EXIT) {
	        fmt = "%s: daemon terminated (%d)\n" ;
	    } else {
	        fmt = "%s: daemon exiting (%d)\n" ;
	    }
	    shio_printf(pip->efp,fmt,pn,rs) ;
	}
	if (pip->open.logprog) {
	    char	tbuf[TIMEBUFLEN+1] ;
	    timestr_logz(pip->daytime,tbuf) ;
	    proglog_printf(pip,"%s exiting (%d)",tbuf,rs) ;
	}

	return (rs >= 0) ? c : rs ;
} /* end subroutine (procdaemon) */

local int procdaemons(PI *pip) noex {
	LI		*lip = pip->lip ;
	custime		ti_start = pip->daytime ;
	time_t		ti_last = pip->daytime ;
	time_t		ti_wait = pip->daytime ;
	time_t		ti_pid = pip->daytime ;
	time_t		ti_logflush = pip->daytime ;
	time_t		ti_config = pip->daytime ;
	time_t		ti_svcs = pip->daytime ;
	int		rs = SR_OK ;
	int		rs1 ;
	int		c = 0 ;
	int		u = 0 ;
	int		nhandle = 0 ;

	if (lip == nullptr) return SR_FAULT ;

	while (rs >= 0) {
	    int	f ;
	    if (c++ > 0) {
	        rs = uc_safesleep(pip->intidle) ;
	        pip->daytime = time(nullptr) ;
	    }
	    if ((rs >= 0) && ((pip->daytime-ti_last) >= pip->intpoll)) {
	        ti_last = pip->daytime ;
	        u += 1 ;
	        nhandle += 1 ;
	        rs = procdaemoner(pip) ;
	    }
	    if (rs >= 0) rs = lib_sigterm() ;
	    if (rs >= 0) rs = lib_sigintr() ;

	    f = ((pip->daytime - ti_wait) > lip->intwait) ;
	    if ((rs >= 0) && (nhandle || f)) {
	        ti_wait = pip->daytime ;
	        rs1 = SR_OK ;
	        while ((nhandle > 0) || f) {
	            rs1 = u_waitpid(-1,nullptr,WNOHANG) ;
	            if (rs1 <= 0) break ;
	            if (nhandle > 0) nhandle -= 1 ;
	        }
	        if ((pip->daytime & 3) == 0) {
	            if ((rs1 == SR_CHILD) && (nhandle > 0)) {
	                nhandle -= 1 ;
	            }
	        }
	    }

	    if ((rs >= 0) && (pip->config != nullptr)) {
	        f = ((pip->daytime - ti_config) >= lip->intconf) ;
	        if (f) {
	            CONFIG	*csp = (CONFIG *) pip->config ;
	            ti_config = pip->daytime ;
	            rs = config_check(csp) ;
	        }
	    } /* end if (have) */

	    if (rs >= 0) {
	        f = ((pip->daytime - ti_svcs) >= lip->intsvcs) ;
	        if (f) {
	            ti_svcs = pip->daytime ;
	            rs = locinfo_svcscheck(lip,pip->daytime) ;
	        }
	    } /* end if (have) */

	    f = ((pip->daytime - ti_pid) >= pip->intlock) ;
	    if ((rs >= 0) && f && lip->open.pidlock) {
	        ti_pid = pip->daytime ;
	        rs = proclockcheck(pip) ;
	    }

	    f = ((pip->daytime - ti_logflush) >= TO_LOGFLUSH) ;
	    if ((rs >= 0) && f) {
	        ti_logflush = pip->daytime ;
	        rs = proglog_flush(pip) ;
	    }

	    if (pip->intrun > 0) {
	        if ((pip->daytime-ti_start) >= pip->intrun) break ;
	    }
	} /* end while */

	return (rs >= 0) ? u : rs ;
} /* end subroutine (procdaemons) */

local int procdaemoner(PI *pip) noex {
	LI		*lip = pip->lip ;
	int		rs ;
	int		clen = 0 ;
	cchar		*dname ;
	cchar		*dcn = "default.htm" ;
	char		dbuf[MAXPATHLEN+1] ;
	dname = lip->tmpourdname ;
	if (pip->debuglevel > 0) {
	    cchar	*pn = pip->progname ;
	    cchar	*fmt = "%s: making\n" ;
	    shio_printf(pip->efp,fmt,pn) ;
	}
	{
	    char	tbuf[TIMEBUFLEN+1] ;
	    timestr_logz(pip->daytime,tbuf) ;
	    proglog_printf(pip,"%s making",tbuf) ;
	}
	if ((rs = mkpath2(dbuf,dname,dcn)) >= 0) {
	    char	tbuf[MAXPATHLEN+1] ;
	    if ((rs = procpage_make(pip,tbuf)) >= 0) {
	        clen = rs ;
	        rs = u_rename(tbuf,dbuf) ;
	        if (rs < 0) {
	            uc_unlink(tbuf) ;
	        }
	    } /* end if (procpage_make) */
	} /* end if (mkpath) */
	return (rs >= 0) ? clen : rs ;
} /* end subroutine (procdaemoner) */

local int procdaemoncheck(PI *pip) noex {
	int		rs ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("msumain/procdaemoncheck: ent\n") ;
#endif

	rs = procmntcheck(pip) ;

	return rs ;
} /* end subroutine (procdaemoncheck) */

local int procregular(PI *pip,cchar *ofn,cchar *afn) noex {
	LI		*lip = pip->lip ;
	int		rs ;
	int		c = 1 ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("msumain/procregular: ent\n") ;
#endif

	if ((rs = locinfo_defs(lip)) >= 0) {
	    if ((rs = locinfo_defreg(lip)) >= 0) {
	        if ((rs = procbackmaint(pip)) >= 0) {
	            rs = procresp(pip,ofn,afn) ;
	        }
	    } /* end if (locinfo_defreg) */
	} /* end if (locinfo_defs) */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("msumain/procregular: ret rs=%d\n",rs) ;
#endif

	return (rs >= 0) ? c : rs ;
} /* end subroutine (procregular) */

local int procresp(PI *pip,cchar *ofn,cchar *afn) noex {
	LI		*lip = pip->lip ;
	int		rs ;
	int		rs1 ;
	int		wlen = 0 ;

	if ((ofn == nullptr) || (ofn[0] == '\0') || (ofn[0] == '-'))
	    ofn = STDFNOUT ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("main/procresp: ent ofn=%s\n",ofn) ;
#endif

	if ((rs = locinfo_svcsbegin(lip)) >= 0) {
	    SHIO		ofile, *ofp = &ofile ;
	    const mode_t	om = 0666 ;
	    if ((rs = shio_open(ofp,ofn,"wct",om)) >= 0) {
	        char	tbuf[MAXPATHLEN+1] ;
	        if ((rs = procpage_begin(pip,tbuf)) >= 0) {
	            if (pip->verboselevel > 0) {
	                CGI		hdrs, *hp = &hdrs ;
	                cint	clen = rs ;
	                if ((rs = cgi_start(hp,ofp)) >= 0) {
	                    if ((rs = prochdrs(pip,hp,clen)) >= 0) {
	                        wlen += rs ;
	                        rs = shio_writefile(ofp,tbuf) ;
	                        wlen += rs ;
	                    } /* end if (prochdrs) */
	                    rs1 = cgi_finish(hp) ;
	                    if (rs >= 0) rs = rs1 ;
	                } /* end if (cgi) */
	            } /* end if (verbose) */
	            rs1 = procpage_end(pip,tbuf) ;
	            if (rs >= 0) rs = rs1 ;
	        } /* end if (mkpath-template) */
	        rs1 = shio_close(ofp) ;
	        if (rs >= 0) rs = rs1 ;
	    } else {
	        cchar	*pn = pip->progname ;
	        cchar	*fmt = "%s: inaccessible output (%d)\n" ;
	        shio_printf(pip->efp,fmt,pn,rs) ;
	    } /* end if (file-output) */
	    rs1 = locinfo_svcsend(lip) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (locinfo-svcs) */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("main/procresp: ret rs=%d wlen=%u\n",rs,wlen) ;
#endif

	return (rs >= 0) ? wlen : rs ;
} /* end subroutine (procresp) */

local int procexecname(PI *pip,char *rbuf,int rlen) noex {
	int		rs ;
	if ((rs = proginfo_progdname(pip)) >= 0) {
	    cchar	*dn = pip->progdname ;
	    cchar	*pn = pip->progname ;
	    rs = mknpath2(rbuf,rlen,dn,pn) ;
	}
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("main/procexecname: ret rs=%d\n",rs) ;
#endif
	return rs ;
} /* end subroutine (procexecname) */

local int procpage_begin(PI *pip,char *dbuf) noex {
	LI		*lip = pip->lip ;
	int		rs ;
	int		clen = 0 ;
	cchar		*dcn = DEFPAGE ;
#if	CF_DEBUG
	if (DEBUGLEVEL(4)) {
	    debugprintf("procpage_begin: ent\n") ;
	    debugprintf("procpage_begin: f_havesvcs=%u\n",lip->have.svcs) ;
	    debugprintf("procpage_begin: svcs{%p}\n",lip->svcs) ;
	}
#endif
	if ((rs = locinfo_tmpourdname(lip)) >= 0) {
	    cchar	*dname = lip->tmpourdname ;
	    if ((rs = locinfo_svclistfinal(lip)) == 0) {
	        if ((rs = mkpath2(dbuf,dname,dcn)) >= 0) {
	            ustat		sb ;
	            const time_t	dt = pip->daytime ;
	            cint		nrs = SR_NOENT ;
	            cint		am = R_OK ;
	            cint		f_force = lip->fl.force ;
	            char		tbuf[MAXPATHLEN+1] ;

#if	CF_DEBUG
	            if (DEBUGLEVEL(4))
	                debugprintf("procpage_begin: d=%s\n",dbuf) ;
#endif
	            if ((rs = u_stat(dbuf,&sb)) == nrs) {
#if	CF_DEBUG
	                if (DEBUGLEVEL(4))
	                    debugprintf("procpage_begin: NOENT\n") ;
#endif
	                if ((rs = procpage_make(pip,tbuf)) >= 0) {
	                    clen = rs ;
#if	CF_DEBUG
	                    if (DEBUGLEVEL(4)) {
	                        debugprintf("procpage_begin: "
	                            "procpage_make() rs=%d\n", rs) ;
	                        debugprintf("procpage_begin: o=%s\n",tbuf) ;
	                        debugprintf("procpage_begin: n=%s\n",dbuf) ;
	                    }
#endif
	                    lip->fl.defpage = true ;
	                    rs = u_rename(tbuf,dbuf) ;
#if	CF_DEBUG
	                    if (DEBUGLEVEL(4))
	                        debugprintf("procpage_begin: "
	                            "u_rename() rs=%d\n",rs) ;
#endif
	                }
	            } else if ((rs = permids(&pip->id,&sb,am)) == SR_ACCESS) {
#if	CF_DEBUG
	                if (DEBUGLEVEL(4))
	                    debugprintf("procpage_begin: ACCESS\n") ;
#endif
	                if ((rs = procpage_make(pip,tbuf)) >= 0) {
	                    clen = rs ;
	                    rs = mkpath1(dbuf,tbuf) ;
	                }
	            } else if (((dt-sb.st_mtime) >= lip->intcache) || f_force) {
#if	CF_DEBUG
	                if (DEBUGLEVEL(4))
	                    debugprintf("procpage_begin: TIMEOUT\n") ;
#endif
	                if ((rs = procpage_make(pip,tbuf)) >= 0) {
	                    clen = rs ;
	                    lip->fl.defpage = true ;
	                    rs = u_rename(tbuf,dbuf) ;
	                }
	            } else {
	                clen = sb.st_size ;
	                lip->fl.defpage = true ;
	            } /* end if (stat) */
	        } /* end if (mkpath) */
	    } else {
	        rs = procpage_make(pip,dbuf) ;
	        clen = rs ;
	    }
	} /* end if (locinfo_tmpourdname) */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("procpage_begin: ret rs=%d clen=%u\n",rs,clen) ;
#endif
	return (rs >= 0) ? clen : rs ;
} /* end subroutine (procpage_begin) */

local int procpage_end(PI *pip,char *tbuf) noex {
	LI		*lip = pip->lip ;
	int		rs = SR_OK ;
	if ((! lip->fl.defpage) && (tbuf[0] != '\0')) {
	    uc_unlink(tbuf) ;
	    tbuf[0] = '\0' ;
	}
	return rs ;
} /* end subroutine (procpage_end) */

local int procpage_make(PI *pip,char *tbuf) noex {
	LI		*lip = pip->lip ;
	int		rs ;
	int		rs1 ;
	int		clen = 0 ;
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("procpage_make: ent\n") ;
#endif
	tbuf[0] = '\0' ;
	if ((rs = locinfo_gatherbegin(lip)) >= 0) {
	    cchar	*tcn = "homepageXXXXXX" ;
	    cchar	*dname = lip->tmpourdname ;
	    char	template[MAXPATHLEN+1] ;
	    if ((rs = mkpath2(template,dname,tcn)) >= 0) {
	        const mode_t	om = 0664 ;
	        if ((rs = mktmpfile(tbuf,om,template)) >= 0) {
	            rs = procdoc(pip,tbuf) ;
	            clen = rs ;
	            if (rs < 0) {
	                uc_unlink(tbuf) ;
	                tbuf[0] = '\0' ;
	            }
	        } /* end if (mktmpfile) */
	    } /* end if (mkpath) */
	    rs1 = locinfo_gatherend(lip) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (locinfo-gather) */
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("procpage_make: ret rs=%d clen=%u\n",rs,clen) ;
#endif
	return (rs >= 0) ? clen : rs ;
} /* end subroutine (procpage_make) */

local int procdoc(PI *pip,cchar *ofn) noex {
	LI		*lip = pip->lip ;
	SHIO		ofile, *ofp = &ofile ;
	HTM		h ;
	const mode_t	om = 0666 ;
	int		rs ;
	int		rs1 ;
	int		wlen = 0 ;
	cchar		*lang = "en" ;

	if (ofn == nullptr) return SR_FAULT ;
	if (ofn[0] == '\0') return SR_INVALID ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("main/procdoc: ent ofn=%s\n",ofn) ;
#endif
	if ((rs = shio_open(ofp,ofn,"wct",om)) >= 0) {
#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("main/procdoc: yes?\n") ;
#endif
	    if ((rs = htm_start(&h,ofp,lang)) >= 0) {
	        cchar	*hfname = lip->hfname ;
#if	CF_DEBUG
	        if (DEBUGLEVEL(4)) {
	            debugprintf("main/procdoc: opened() rs=%d\n",rs) ;
	            debugprintf("main/procdoc: hfname=%s\n",lip->hfname) ;
	        }
#endif
	        if ((rs = htm_headbegin(&h,hfname)) >= 0) {
	            rs1 = htm_headend(&h) ;
	            if (rs >= 0) rs = rs1 ;
	        } /* end if (htm-head) */
	        if (rs >= 0) {
	            if ((rs = htm_bodybegin(&h,nullptr)) >= 0) {
	                {
	                    rs = procdocbody(pip,&h) ;
	                }
	                rs1 = htm_bodyend(&h) ;
	                if (rs >= 0) rs = rs1 ;
	            } /* end if (htm-body) */
	        } /* end if (ok) */
	        wlen = htm_finish(&h) ;
	        if (rs >= 0) rs = wlen ;
	    } /* end if (htm) */
#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("main/procdoc: shio_open out rs=%d\n",rs) ;
#endif
	    rs1 = shio_close(ofp) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (file-output) */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("main/procdoc: ret rs=%d\n",rs) ;
#endif
	return (rs >= 0) ? wlen : rs ;
} /* end subroutine (procdoc) */

local int procdocbody(PI *pip,HTM *hdp) noex {
	int		rs ;
	int		rs1 ;

	if ((rs = htm_tagbegin(hdp,"div","particular",nullptr,nullptr)) >= 0) {
	    if (rs >= 0) {
	        rs = procdocbodyhdr(pip,hdp) ;
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("main/procdocbody: _bodyheader() rs=%d\n",rs) ;
#endif
	    } /* end if (ok) */
	    if (rs >= 0) {
	        rs = procdocbodymain(pip,hdp) ;
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("main/procdocbody: _bodymain() rs=%d\n",rs) ;
#endif
	    } /* end if (ok) */
	    if (rs >= 0) {
	        rs = procdocbodyfooter(pip,hdp) ;
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("main/procdocbody: _bodyfooter() rs=%d\n",rs) ;
#endif
	    } /* end if (ok) */
	    rs1 = htm_tagend(hdp,"div") ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (htm-div) */

	return rs ;
} /* end subroutine (procdocbody) */

local int procdocbodyhdr(PI *pip,HTM *hdp) noex {
	int		rs ;
	int		rs1 ;

	if ((rs = htm_tagbegin(hdp,"header",nullptr,nullptr,nullptr)) >= 0) {
	    LI		*lip = pip->lip ;
	    if ((rs = htm_tagbegin(hdp,"h1",nullptr,nullptr,nullptr)) >= 0) {
	        cint	hlen = HBUFLEN ;
	        cchar		*fn = lip->wfname ;
	        char		hbuf[HBUFLEN+1] ;
	        if ((rs = procdocbodyhdrfile(pip,hbuf,hlen,fn)) >= 0) {
	            rs = htm_printline(hdp,hbuf,rs) ;
	        } else if (isNotPresent(rs)) {
	            cchar	*hp = pip->org ;
	            if (hp == nullptr) hp = "home page" ;
	            rs = htm_printline(hdp,hp,-1) ;
	        }
	        rs1 = htm_tagend(hdp,"h1") ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (htm-h1) */
	    rs1 = htm_tagend(hdp,"header") ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (htm-header) */

	return rs ;
} /* end subroutine (procdocbodyhdr) */

local int procdocbodyhdrfile(PI *pip,char *hbuf,int hlen,cchar *fn) noex {
	bfile		hfile, *hfp = &hfile ;
	int		rs ;
	int		rs1 ;
	int		len = 0 ;
	if (pip == nullptr) return SR_FAULT ;
	if ((fn == nullptr) || (fn[0] == '\0')) return SR_NOENT ; /* special */
	if ((rs = bopen(hfp,fn,"r",0666)) >= 0) {
	    int		cl ;
	    cchar	*cp ;
	    while ((rs = breadln(hfp,hbuf,hlen)) > 0) {
	        len = rs ;
	        if (hbuf[len-1] == '\n') len -= 1 ;
	        if ((cl = sfshrink(hbuf,len,&cp)) > 0) {
	            if (cp != hbuf) {
	                memmove(hbuf,cp,cl) ;
	                hbuf[cl] = '\0' ;
	            }
	            len = cl ;
	        } else {
	            len = 0 ;
	        }
	        if (len > 0) break ;
	    } /* end while */
	    rs1 = bclose(hfp) ;
	    if (rs >= 0) rs = rs1 ;
	} /* enbd if (bfile) */
	return (rs >= 0) ? len : rs ;
} /* end subroutine (procdocbodyhdrfile) */

local int procdocbodymain(PI *pip,HTM *hdp) noex {
	LI		*lip = pip->lip ;
	int		rs = SR_OK ;
	int		rs1 ;

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("b_homepage/procdocbodyb_homepage: ent\n") ;
#endif

	if (lip->sfname != nullptr) {
	    cchar	*tag = "main" ;
#if	CF_DEBUG
	    if (DEBUGLEVEL(5)) {
	        debugprintf("b_homepage/procdocbodyb_homepage: svcs¬\n") ;
	        if ((rs = locinfo_svclistcount(lip)) > 0) {
	            vecpstr	*slp = lip->svcs ;
	            int		i ;
	            cchar	*cp ;
	            for (i = 0 ; vecpstr_get(slp,i,&cp) >= 0 ; i += 1) {
	                if (cp == nullptr) continue ;
	                debugprintf("b_homepage/procdocbodyb_homepage: "
	                    "svc=%s\n",cp) ;
	            }
	        }
	    }
#endif /* CF_DEBUG */
	    if ((rs = htm_tagbegin(hdp,tag,nullptr,nullptr,nullptr)) >= 0) {
	        {
	            rs = procdocbodymain_svcs(pip,hdp) ;
	        }
	        rs1 = htm_tagend(hdp,tag) ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (htm-main) */
	} /* end if (svc-fname) */

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("b_homepage/procdocbodyb_homepage: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (procdocbodymain) */

local int procdocbodyfooter(PI *pip,HTM *hdp) noex {
	int		rs ;
	int		rs1 ;
	cchar		*tag = "footer" ;

	if ((rs = htm_tagbegin(hdp,tag,nullptr,nullptr,nullptr)) >= 0) {
	    if (rs >= 0) {
	        rs = htm_hr(hdp,nullptr,nullptr) ;
	    } /* end if (ok) */
	    if (rs >= 0) {
	        rs = procdocbodyfooterleft(pip,hdp) ;
	    } /* end if (ok) */
	    if (rs >= 0) {
	        rs = procdocbodyfooterext(pip,hdp) ;
	    } /* end if (ok) */
	    rs1 = htm_tagend(hdp,tag) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (htm-footer) */

	return rs ;
} /* end subroutine (procdocbodyfooter) */

local int procdocbodyfooterleft(PI *pip,HTM *hdp) noex {
    	cnullptr	np{} ;
	int		rs ;
	int		rs1 ;
	if (pip == nullptr) return SR_FAULT ;
	if ((rs = htm_tagbegin(hdp,"div","left",nullptr,nullptr)) >= 0) {
	    LI	*lip = pip->lip ;
	    if ((rs = htm_tagbegin(hdp,"p","center",nullptr,nullptr)) >= 0) {
	        cint	hlen = (xstrlen(lip->webmaster)+10) ;
	        cchar		*n = nullptr ;
	        cchar		*class = "center" ;
	        cchar		*title = lip->webmaster ;
	        char		*hbuf ;
	        if ((rs = lm_mall((hlen+1),&hbuf)) >= 0) {
	            cchar	*href = hbuf ;
	            strdcpy2(hbuf,hlen,"mailto:",title) ;
	            if ((rs = htm_abegin(hdp,class,n,href,title)) >= 0) {
	                {
	                    rs = htm_printline(hdp,"webmaster",-1) ;
	                }
	                rs1 = htm_aend(hdp) ;
	                if (rs >= 0) rs = rs1 ;
	            } /* end if (htm-a) */
	            lm_free(hbuf) ;
	        } /* end if (m-a-f) */
	        rs1 = htm_tagend(hdp,"p") ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (htm-p) */
	    if (rs >= 0) {
	        if ((rs = htm_tagbegin(hdp,"p","center",np,np)) >= 0) {
	            LI	*lip = pip->lip ;
	            {
	                cchar	*c = lip->copyright ;
	                rs = htm_printline(hdp,c,-1) ;
	            }
	            rs1 = htm_tagend(hdp,"p") ;
	            if (rs >= 0) rs = rs1 ;
	        } /* end if (htm-tag) */
	    } /* end if (ok) */
	    rs1 = htm_tagend(hdp,"div") ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (htm-tag) */
	return rs ;
} /* end subroutine (procdocbodyfooterleft) */

local int procdocbodyfooterext(PI *pip,HTM *hdp) noex {
	int		rs ;
	int		rs1 ;
	cchar *src = "http://rightcore.com/CGI/wc?db=rightcore&c=homepage" ;
	if (pip == nullptr) return SR_FAULT ;
	if ((rs = htm_tagbegin(hdp,"div","exticons",np,np)) >= 0) {
	    {
	        cint	w = 20 ;
	        cint	h = 22 ;
	        cchar		*class = nullptr ;
	        cchar		*id = nullptr ;
	        cchar		*title = "web counter" ;
	        cchar		*alt = "web counter" ;
	        rs = htm_img(hdp,class,id,src,title,alt,w,h) ;
	    }
	    rs1 = htm_tagend(hdp,"div") ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (htm-div) */
	return rs ;
} /* end subroutine (procdocbodyfooterext) */

local int procdocbodymain_svcs(PI *pip,HTM *hdp) noex {
	int		rs ;
	{
	rs = procdocbodymain_svcprint(pip,hdp) ;
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("b_homepage/procdocbodymain_svcs: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (procdocbodymain_svcs) */

local int procdocbodymain_svcprint(PI *pip,HTM *hdp) noex {
	LI		*lip = pip->lip ;
	int		rs = SR_OK ;

	{
	    SVCFILE	*slp = &lip->s ;
	    GATHER	*glp = &lip->g ;
	    SVCFILE_ENT	se ;
	    int		i ;
	    cchar	*snp ;
	    for (i = 0 ; locinfo_svclistget(lip,i,&snp) >= 0 ; i += 1) {
	        if (snp != nullptr) {
	            cint	el = SVCENTLEN ;
	            char	eb[SVCENTLEN+1] ;
	            void	*n = nullptr ;
	            if ((rs = svcfile_fetch(slp,snp,n,&se,eb,el)) > 0) {
	                rs = procdocbodymain_svcer(pip,hdp,glp,&se) ;
	            }
	        } /* end if (non-null) */
	        if (rs < 0) break ;
	    } /* end for */
	} /* end block */

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("b_homepage/procdocbodymain_svcprint: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (procdocbodymain_svcprint) */

local int procdocbodymain_svcer(PI *pip,HTM *hdp,GATHER *glp,
		SVCFILE_ENT *sep) noex {
	int		rs ;
	cchar		*hp ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4)) {
	    cint	n = sep->nkeys ;
	    int		hl ;
	    debugprintf("procdocbodymain_svcer: svc=%s\n",sep->svc) ;
	    if ((hl = svckv_val(sep->keyvals,n,"h",&hp)) > 0) {
	        debugprintf("procdocbodymain_svcer: h=>%r<\n",hp,hl) ;
	    }
	}
#endif /* CF_DEBUG */

	if ((rs = svcent_getval(sep,"h",&hp)) > 0) {
	    cint	n = sep->nkeys ;
	    cchar	*(*kv)[2] = sep->keyvals ;
	    cchar	*k = "include" ;
	    cchar	*vp ;
	    if ((rs = svcent_getval(sep,k,&vp)) > 0) {
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("procdocbodymain_svcer: include\n") ;
#endif
	        rs = procdocbodymain_svcerinc(pip,hdp,sep) ;
	    } else if ((rs = svckv_isfile(kv,n,&vp)) > 0) {
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("procdocbodymain_svcer: isfile\n") ;
#endif
	        rs = procdocbodymain_svcerfile(pip,hdp,glp,sep) ;
	    } else if ((rs = svckv_isprog(kv,n,&vp)) > 0) {
	        rs = 2 ;
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("procdocbodymain_svcer: isexec\n") ;
#endif
	    } /* end if (alternatives) */
	} /* end if (svckv) */

	return rs ;
} /* end subroutine (procdocbodymain_svcer) */

local int procdocbodymain_svcerhdr(PI *pip,HTM *hdp,
		SVCFILE_ENT *sep) noex {
	int		rs ;
	int		rs1 ;
	int		vl ;
	cchar		*vp ;
	if (pip == nullptr) return SR_FAULT ;
	if ((rs = svcent_getdeval(sep,"h",&vp)) > 0) {
	    vl = rs ;
	    if ((rs = htm_tagbegin(hdp,"h3",nullptr,nullptr,nullptr)) >= 0) {
	        {
	            rs = htm_printline(hdp,vp,vl) ;
	        }
	        rs1 = htm_tagend(hdp,"h3") ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (htm-h3) */
	} /* end if (svcent_getdeval) */
	return rs ;
} /* end subroutine (procdocbodymain_svcerhdr) */

local int procdocbodymain_svcerinc(PI *pip,HTM *hdp,
		SVCFILE_ENT *sep) noex {
	int		rs ;
	cchar		*k = "include" ;
	cchar		*vp ;
	if ((rs = svcent_getval(sep,k,&vp)) > 0) {
	    char	ibuf[MAXPATHLEN+1] ;
	    if ((rs = mkincfname(pip,ibuf,vp,rs)) >= 0) {
	        rs = procdocbodymain_svcerincer(pip,hdp,sep,ibuf) ;
	    } /* end if (mkincfname) */
	} /* end if (svcent_getval) */
	return rs ;
} /* end subroutine (procdocbodymain_svcerinc) */

local int procdocbodymain_svcerincer(PI *pip,HTM *hdp,
		SVCFILE_ENT *sep,cchar *ibuf) noex {
	bfile		ifile, *ifp = &ifile ;
	int		rs ;
	int		rs1 ;
	if ((rs = bopen(ifp,ibuf,"r",0666)) >= 0) {
	    if ((rs = bsize(ifp)) > 0) {
	        if ((rs = procdocbodymain_svcerhdr(pip,hdp,sep)) >= 0) {
	            cint	llen = LINEBUFLEN ;
	            char		lbuf[LINEBUFLEN+1] ;
	            while ((rs = breadln(ifp,lbuf,llen)) > 0) {
	                rs = htm_write(hdp,lbuf,rs) ;
	                if (rs < 0) break ;
	            } /* end while */
	        } /* end if (procdocbodymain_svcerhdr) */
	    } /* end if (bsize) */
	    rs1 = bclose(ifp) ;
	    if (rs >= 0) rs = rs1 ;
	} else if (isNotPresent(rs)) {
	    rs = SR_OK ;
	}
	return rs ;
} /* end subroutine (procdocbodymain_svcerincer) */

local int procdocbodymain_svcerfile(PI *pip,HTM *hdp,GATHER *glp,
		SVCFILE_ENT *sep) noex {
	int		rs ;
	cchar		*svc = sep->svc ;

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("procdocbodymain_svcerfile: ent svc=%s\n",svc) ;
#endif

	if ((rs = gather_getlines(glp,svc)) > 0) {
	    cint	nl = rs ;
#if	CF_DEBUG
	    if (DEBUGLEVEL(5))
	        debugprintf("procdocbodymain_svcerfile: nl=%u\n",nl) ;
#endif
	    if ((rs = procdocbodymain_svcerhdr(pip,hdp,sep)) >= 0) {
	        rs = procdocbodymain_svcerfiler(pip,hdp,glp,sep,nl) ;
	    }
#if	CF_DEBUG
	    if (DEBUGLEVEL(5))
	        debugprintf("procdocbodymain_svcerfile: "
	            "dequote-out rs=%d\n",rs) ;
#endif
	} /* end if (gather_getlines) */

	if (rs < 0) {
	    cchar	*fmt = "FAILED svc=%s (%d)" ;
	    proglog_printf(pip,fmt,svc,rs) ;
	}


#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("procdocbodymain_svcerfile: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (procdocbodymain_svcerfile) */

local int procdocbodymain_svcerfiler(PI *pip,HTM *hdp,GATHER *glp,
		SVCFILE_ENT *sep,int nl) noex {
	cint		n = sep->nkeys ;
	int		rs = SR_OK ;
	int		rs1 ;
	int		vl ;
	cchar		*vp ;
	if (pip == nullptr) return SR_FAULT ;
	if ((vl = svckv_dequote(sep->keyvals,n,"h",&vp)) > 0) {
	    NULSTR	ts ;
	    cchar	*t ;
#if	CF_DEBUG
	    if (DEBUGLEVEL(5))
	        debugprintf("procdocbodymain_svcerfile: t=>%r<\n",vp,vl) ;
#endif
	    if ((rs = nulstr_start(&ts,vp,vl,&t)) >= 0) {
	        cchar	*svc = sep->svc ;
	        cchar	*dp ;
	        if ((rs = gather_getbuf(glp,svc,&dp)) >= 0) {
	            cint	dl = rs ;
	            cint	r = (nl+1) ;
	            cint	c = COLUMNS ;
	            int		i = 0 ;
	            cchar		*class = nullptr ;
	            cchar		*id = nullptr ;
	            cchar		*kv[2][2] ;
	            kv[i][0] = "readonly" ;
	            kv[i][1] = nullptr ;
	            i += 1 ;
	            kv[i][0] = nullptr ;
	            kv[i][1] = nullptr ;
	            if ((rs = htm_textbegin(hdp,class,id,t,r,c,kv)) >= 0) {

#if	CF_DEBUG
	                if (DEBUGLEVEL(5))
	                    debugprintf("procdocbodymain_svcerfile: "
	                        "htm_write()\n") ;
#endif

	                if (dl > 0) {
	                    if ((rs = htm_write(hdp,dp,dl)) >= 0) {
	                        if (dp[dl-1] != '\n') {
	                            rs = htm_putc(hdp,CH_NL) ;
	                        }
	                    }
	                } /* end if (positive) */

#if	CF_DEBUG
	                if (DEBUGLEVEL(5))
	                    debugprintf("procdocbodymain_svcerfile: "
	                        "htm_write() rs=%d\n",rs) ;
#endif
	                rs1 = htm_textend(hdp) ;
	                if (rs >= 0) rs = rs1 ;
	            } /* end if (htm-text) */
#if	CF_DEBUG
	            if (DEBUGLEVEL(5))
	                debugprintf("procdocbodymain_svcerfile: "
	                    "htm-out rs=%d\n",rs) ;
#endif
	        } /* end if (gather_getbuf) */
#if	CF_DEBUG
	        if (DEBUGLEVEL(5))
	            debugprintf("procdocbodymain_svcerfile: "
	                "gather rs=%d\n",rs) ;
#endif
	        rs1 = nulstr_finish(&ts) ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (nulstr) */
#if	CF_DEBUG
	    if (DEBUGLEVEL(5))
	        debugprintf("procdocbodymain_svcerfile: "
	            "nulstr-out rs=%d\n",rs) ;
#endif
	} /* end if (svckv) */
	return rs ;
} /* end subroutine (procdocbodymain_svcerfiler) */

local int prochdrs(PI *pip,CGI *hp,int clen) noex {
	cint		hlen = HBUFLEN ;
	int		rs ;
	int		wlen = 0 ;
	char		hbuf[HBUFLEN+1] ;
	if ((rs = ctdeci(hbuf,hlen,clen)) >= 0) {
	    rs = cgi_hdr(hp,"content-length",hbuf,rs) ;
	    wlen += rs ;
	}
	if (rs >= 0) {
	    time_t	t = pip->daytime ;
	    rs = cgi_hdrdate(hp,t) ;
	    wlen += rs ;
	}
	if (rs >= 0) {
	    cchar	*k = "content-type" ;
	    cchar	*v = "text/html; charset=ISO-8859-1" ;
	    rs = cgi_hdr(hp,k,v,-1) ;
	    wlen += rs ;
	}
	if (rs >= 0) {
	    rs = cgi_eoh(hp) ;
	    wlen += rs ;
	}
	return (rs >= 0) ? wlen : rs ;
} /* end subroutine (prochdrs) */

local int proclockcheck(PI *pip) noex {
	LI		*lip = pip->lip ;
	lfm_check	lc ;
	int		rs ;
	if ((rs = locinfo_lockcheck(lip,&lc)) < 0) {
	    if ((rs == SR_LOCKFAIL) || (rs == SR_AGAIN)) {
	        proclockprint(pip,pip->pidfname,&lc) ;
	    }
	}

	return rs ;
} /* end subroutine (proclockcheck) */

/* print out lock-check information */
local int proclockprint(PI *pip,cchar *lfname,lfm_check *lcp) noex {
	LI		*lip = pip->lip ;
	int		rs = SR_OK ;
	cchar		*namp ;

	switch (lcp->stat) {
	case SR_AGAIN:
	    namp = "busy" ;
	    break ;
	case SR_LOCKFAIL:
	    namp = "lost" ;
	    break ;
	default:
	    namp = "unknown" ;
	    break ;
	} /* end switch */

	if (pip->open.logprog) {
	    if (! lip->fl.quietlock) {
	        proclocklog(pip,lfname,lcp,namp) ;
	    } else {
	        proglog_printf(pip,"lock=%s",lfname) ;
	        proglog_printf(pip,"pid=%s",namp) ;
	    }
	}

	if (! lip->fl.quietlock) {
	    if ((pip->debuglevel > 0) && (pip->efp != nullptr)) {
	        cchar	*pn = pip->progname ;
	        char	tbuf[TIMEBUFLEN + 1] ;

	        timestr_logz(pip->daytime,tbuf) ;
	        shio_printf(pip->efp,"%s: %s lock %s\n",pn,tbuf,namp) ;

	        shio_printf(pip->efp,"%s: other_pid=%d\n",pn,lcp->pid) ;

	        if (lcp->nodename != nullptr) {
	            cchar	*nn = lcp->nodename ;
	            shio_printf(pip->efp,"%s: other_node=%s\n",pn,nn) ;
	        }

	        if (lcp->username != nullptr) {
	            cchar	*un = lcp->username ;
	            rs = shio_printf(pip->efp,"%s: other_user=%s\n",pn,un) ;
	        }

	        if (lcp->banner != nullptr) {
	            cchar	*bn = lcp->banner ;
	            shio_printf(pip->efp,"%s: other_banner=>%s<\n",pn,bn) ;
	        }

	    } /* end if (standard-error) */
	} /* end if (quiet-lock) */

	return rs ;
}
/* end subroutine (proclockprint) */

local int proclocklog(PI *pip,cchar *lfname,lfm_check *lcp,cchar *namp) noex {
	int		rs = SR_OK ;
	char		tbuf[TIMEBUFLEN + 1] ;

	timestr_logz(pip->daytime,tbuf) ;
	proglog_printf(pip, "%s lock %s\n",tbuf,namp) ;

	proglog_printf(pip, "pid=%s",lfname) ;

	proglog_printf(pip, "other_pid=%d\n", lcp->pid) ;

	if (lcp->nodename != nullptr) {
	    proglog_printf(pip, "other_node=%s\n", lcp->nodename) ;
	}

	if (lcp->username != nullptr) {
	    proglog_printf(pip, "other_user=%s\n", lcp->username) ;
	}

	if (lcp->banner != nullptr) {
	    proglog_printf(pip, "other_banner=>%s<\n", lcp->banner) ;
	}

	return rs ;
} /* end subroutine (proclocklog) */

local int gather_start(GATHER *glp,PI *pip,cchar *termtype,int cols) noex {
	int		rs ;
	cchar		*cp ;
	if (glp == nullptr) return SR_FAULT ;
	if (termtype == nullptr) return SR_FAULT ;
	if (cols <= 0) cols = COLUMNS ;
	memclear(glp) ; /* dangerous */
	glp->pip = pip ;
	if ((rs = lm_strw(termtype,-1,&cp)) >= 0) {
	    ptm *mxp = &glp->mx ;
	    glp->termtype = cp ;
	    glp->cols = cols ;
	    if ((rs = mxp->create)) >= 0) {
		ptc *cnp = &glp->cn ;
	        if ((rs = cnp->create) >= 0) {
	            rs = vechand_start(&glp->ents,6,0) ;
	            if (rs < 0) {
	                cnp->destroy() ;
		    }
	        }
	        if (rs < 0) {
	            mxp->destroy() ;
		}
	    } /* end if (ptm_create) */
	    if (rs < 0) {
	        lm_free(glp->termtype) ;
	        glp->termtype = nullptr ;
	    }
	} /* end if (m-a) */
	return rs ;
} /* end subroutine (gather_start) */

local int gather_finish(GATHER *glp) noex {
	int		rs = SR_FAULT ;
	int		rs1 ;
	int		used = 0 ;
	if (glp) {
	    rs = SR_OK ;
	    {
	        rs1 = gather_entfins(glp) ;
	        if (rs >= 0) rs = rs1 ;
	        used = rs1 ;
	    }
	    {
	        rs1 = vechand_finish(&glp->ents) ;
	        if (rs >= 0) rs = rs1 ;
	    }
	    {
	        ptc *cnp = &glp->cn ;
	        rs1 = cnp->destroy ;
	        if (rs >= 0) rs = rs1 ;
	    }
	    {
	        ptm *mxp = &glp->mx ;
	        rs1 = mxp->destroy() ;
	        if (rs >= 0) rs = rs1 ;
	    }
	    if (glp->termtype != nullptr) {
	        rs1 = lm_free(glp->termtype) ;
	        if (rs >= 0) rs = rs1 ;
	        glp->termtype = nullptr ;
	    }
	} /* end if (non-null) */
	return (rs >= 0) ? used : rs ;
} /* end subroutine (gather_finish) */

local int gather_file(GATHER *glp,cchar *svc,int f_to,cchar *fp,int fl) noex {
	cint	fsize = szof(FILEDAT) ;
	int		rs ;
	void		*p ;

#if	CF_DEBUG
	debugprintf("b_homepage/gather_file: ent\n") ;
	debugprintf("b_homepage/gather_file: svc=%s\n",svc) ;
	debugprintf("b_homepage/gather_file: fn=%r\n",fp,fl) ;
#endif

	if (glp == nullptr) return SR_FAULT ;
	if (svc == nullptr) return SR_FAULT ;
	if (fp == nullptr) return SR_FAULT ;
	if (svc[0] == '\0') return SR_INVALID ;
	if (fp[0] == '\0') return SR_INVALID ;

	if ((rs = lm_mall(fsize,&p)) >= 0) {
	    FILEDAT	*fep = p ;
	    cint	c = glp->cols ;
	    cchar	*tt = (f_to) ? glp->termtype : nullptr ;
	    if ((rs = filedat_start(fep,glp->pip,tt,c,svc,fp,fl)) >= 0) {
	        if ((rs = vechand_add(&glp->ents,fep)) >= 0) {
	            glp->nout += 1 ;
	        }
	        if (rs < 0) {
	            filedat_finish(fep) ;
		}
	    } /* end if (filedat_start) */
	    if (rs < 0) {
	        lm_free(p) ;
	    }
	} /* end if (m-a) */

#if	CF_DEBUG
	debugprintf("b_homepage/gather_file: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (gather_file) */

local int gather_entfins(GATHER *glp) noex {
	FILEDAT		*fep ;
	vechand		*elp = &glp->ents ;
	int		rs = SR_OK ;
	int		rs1 ;
	int		i ;
	int		used = 0 ;

	if (glp == nullptr) return SR_FAULT ;

	for (i = 0 ; vechand_get(elp,i,&fep) >= 0 ; i += 1) {
	    if (fep != nullptr) {
	        rs1 = filedat_finish(fep) ;
	        if (rs >= 0) rs = rs1 ;
	        if ((rs1 >= 0) && (rs1 > used)) used = rs1 ;
	        rs1 = lm_free(fep) ;
	        if (rs >= 0) rs = rs1 ;
	    }
	} /* end for */

#if	CF_DEBUG
	debugprintf("b_homepage/gather_entfins: ret rs=%d u=%d\n",rs,used) ;
#endif

	return (rs >= 0) ? used : rs ;
} /* end subroutine (gather_entfins) */

local int gather_getlines(GATHER *glp,cchar *svc) noex {
	FILEDAT		*fep = nullptr ;
	int		rs = SR_OK ;
	int		lines = 0 ;

#if	CF_DEBUG
	debugprintf("b_homepage/gather_getlines: ent svc=%s\n",svc) ;
#endif

	if (glp == nullptr) return SR_FAULT ;
	if (svc == nullptr) return SR_FAULT ;

	if (svc[0] == '\0') return SR_INVALID ;

	{
	    VECHAND	*elp = &glp->ents ;
	    int		i ;
	    for (i = 0 ; (rs = vechand_get(elp,i,&fep)) >= 0 ; i += 1) {
	        if (fep != nullptr) {
	            if ((rs = filedat_havesvc(fep,svc)) > 0) break ;
	        }
	        if (rs < 0) break ;
	    } /* end for */
	} /* end block */

	if ((rs >= 0) && (fep != nullptr)) {
	    rs = filedat_getlines(fep) ;
	    lines = rs ;
	}

#if	CF_DEBUG
	debugprintf("b_homepage/gather_getlines: ret rs=%d lns=%u\n",
	    rs,lines) ;
#endif

	return (rs >= 0) ? lines : rs ;
} /* end subroutine (gather_getlines) */

local int gather_getbuf(GATHER *glp,cchar *svc,cchar **rpp) noex {
	FILEDAT		*fep = nullptr ;
	int		rs = SR_OK ;
	int		rl = 0 ;

	if (glp == nullptr) return SR_FAULT ;
	if (rpp != nullptr) *rpp = nullptr ;

	{
	    VECHAND	*elp = &glp->ents ;
	    int		i ;
	    for (i = 0 ; (rs = vechand_get(elp,i,&fep)) >= 0 ; i += 1) {
	        if (fep != nullptr) {
	            if ((rs = filedat_havesvc(fep,svc)) > 0) break ;
	        }
	    } /* end for */
	} /* end block */

	if ((rs >= 0) && (fep != nullptr)) {
	    rs = filedat_getbuf(fep,rpp) ;
	    rl = rs ;
	}

	return (rs >= 0) ? rl : rs ;
} /* end subroutine (gather_getbuf) */

local int filedat_start(FILEDAT *fep,PI *pip,cchar *tt,int cols,
		cchar *svc,cchar *fp,int fl) noex {
	cint		to = 30 ;
	int		rs ;
	int		rs1 ;
	int		size = 0 ;
	char		*bp ;

#if	CF_DEBUG
	debugprintf("b_homepage/filedat_start: ent svc=%s\n",svc) ;
	debugprintf("b_homepage/filedat_start: fn=%r\n",fp,fl) ;
#endif

	memclear(fep) ;
	fep->pip = pip ;
	fep->cols = cols ;
	fep->to = to ;
	fep->f_stackcheck = CF_STACKCHECK ;

	size += (xstrlen(svc)+1) ;
	size += (xstrnlen(fp,fl)+1) ;
	if (tt != nullptr) {
	    size += (xstrlen(tt)+1) ;
	}
	if ((rs = lm_mall(size,&bp)) >= 0) {
	    cint	dsize = FILEDAT_DSIZE ;
	    fep->a = bp ;
	    fep->svc = bp ;
	    bp = (strwcpy(bp,svc,-1)+1) ;
	    fep->fname = bp ;
	    bp = (strwcpy(bp,fp,fl)+1) ;
	    if (tt != nullptr) {
	        fep->termtype = bp ;
	        bp = (strwcpy(bp,tt,-1)+1) ;
	    }
	    if ((rs = lm_mall(dsize,&bp)) >= 0) {
	        fep->dbuf = bp ;
	        fep->dsize = dsize ;
	        bp[0] = '\0' ;
	        if ((rs = filedat_stackbegin(fep)) >= 0) {
	            PTA		ta ;
	            if ((rs = pta_create(&ta)) >= 0) {
	                const caddr_t	saddr = fep->saddr ;
	                int		ssize = fep->ssize ;
	                if ((rs = pta_setstack(&ta,saddr,ssize)) >= 0) {
	                    tworker	tw = (tworker) filedat_worker ;
	                    pthread_t	tid ;
	                    if ((rs = uptcreate(&tid,&ta,tw,fep)) >= 0) {
	                        fep->tid = tid ;
	                        fep->f_running = true ;
	                    }
	                } /* end if (pta_setstack) */
	                rs1 = pta_destroy(&ta) ;
	                if (rs >= 0) rs = rs1 ;
	            } /* end if (pta) */
	            if (rs < 0)
	                filedat_stackend(fep) ;
	        } /* end if (filedat_stackbegin) */
	        if (rs < 0) {
	            lm_free(fep->dbuf) ;
	            fep->dbuf = nullptr ;
	        }
	    } /* end if (m-a data) */
	    if (rs < 0) {
	        lm_free(fep->a) ;
	        fep->a = nullptr ;
	    }
	} /* end if (m-a) */

#if	CF_DEBUG
	debugprintf("b_homepage/filedat_start: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (filedat_start) */

local int filedat_stackbegin(FILEDAT *fep) noex {
    	cnullptr	np{} ;
	size_t		ms ;
	cint	ps = getpagesize() ;
	cint	ssize = MAX(STACKSIZE,PTHREAD_STACK_MIN) ;
	int		rs ;
	int		mp = (PROT_READ|PROT_WRITE) ;
	int		mf = (MAP_PRIVATE|MAP_NORESERVE|MAP_ANON) ;
	int		ss ;
	int		fd = -1 ;
	void		*md ;
	ss = iceil(ssize,ps) ;
	ms = (ss+(2*ps)) ;
	if ((rs = u_mmap(np,ms,mp,mf,fd,0z,&md)) >= 0) {
	    caddr_t	maddr = md ;
	    fep->maddr = md ;
	    fep->msize = ms ;
	    fep->saddr = (maddr + ps) ;
	    fep->ssize = ss ;
	    if ((rs = u_mprotect((maddr-0),ps,PROT_NONE)) >= 0) {
	        if ((rs = u_mprotect((maddr+ms-ps),ps,PROT_NONE)) >= 0) {
	            if (fep->f_stackcheck) {
	                rs = filedat_stackfill(fep) ;
	            }
	        } /* end if (u_mprotect) */
	    } /* end if (u_mprotect) */
	} /* end if (u_mmap) */
	return rs ;
} /* end subroutine (filedat_stackbegin) */

local int filedat_stackend(FILEDAT *fep) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	int		used = 0 ;
	if (fep->saddr != nullptr) {
	    if (fep->f_stackcheck) {
	        used = filedat_stackused(fep) ;
	        if (rs >= 0) rs = used ;
	    }
	    rs1 = u_munmap(fep->maddr,fep->msize) ;
	    if (rs >= 0) rs = rs1 ;
	    fep->maddr = nullptr ;
	    fep->saddr = nullptr ;
	    fep->msize = 0 ;
	    fep->ssize = 0 ;
	}
	return (rs >= 0) ? used : rs ;
} /* end subroutine (filedat_stackend) */

local int filedat_stackfill(FILEDAT *fep) noex {
	int		rs = SR_OK ;
	if ((fep->saddr != nullptr) && fep->f_stackcheck) {
	    cint	shift = ffbsi(szof(int)) ;
	    cint	pattern = MEMPATTERN ;
	    int		*ip = (int *) fep->saddr ;
	    int		il ;
	    int		i ;
	    il = (fep->ssize >> shift) ;
	    for (i = 0 ; i < il ; i += 1) {
	        *ip++ = pattern ;
	    }
	} /* end if (stackcheck) */
	return rs ;
} /* end subroutine (filedat_stackfill) */

local int filedat_stackused(FILEDAT *fep) noex {
	int		rs = SR_OK ;
	int		used = 0 ;
	if (fep->saddr != nullptr) {
	    cint	shift = ffbsi(szof(int)) ;
	    cint	pattern = MEMPATTERN ;
	    int		*ip = (int *) fep->saddr ;
	    int		il ;
	    int		i ;
	    il = (fep->ssize >> shift) ;
	    for (i = 0 ; i < il ; i += 1) {
	        if (*ip++ != pattern) break ;
	    }
	    used = ((il-i) << shift) ;
	}
	return (rs >= 0) ? used : rs ;
} /* end subroutine (filedat_stackused) */

local int filedat_finish(FILEDAT *fep) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	int		used = 0 ;

#if	CF_DEBUG
	debugprintf("b_homepage/filedat_finish: ent svc=%s\n",fep->svc) ;
#endif

	rs1 = filedat_getlines(fep) ;
	if (rs >= 0) rs = rs1 ;

	rs1 = filedat_stackend(fep) ;
	if (rs >= 0) rs = rs1 ;
	if (rs >= 0) used = rs ;

#if	CF_DEBUG
	debugprintf("b_homepage/filedat_finish: svc=%s stack used=%u\n",
	    fep->svc,used) ;
#endif

	if (fep->dbuf != nullptr) {
	    rs1 = lm_free(fep->dbuf) ;
	    if (rs >= 0) rs = rs1 ;
	    fep->dbuf = nullptr ;
	}

	if (fep->a != nullptr) {
	    rs1 = lm_free(fep->a) ;
	    if (rs >= 0) rs = rs1 ;
	    fep->a = nullptr ;
	}

	fep->fname = nullptr ;
	fep->svc = nullptr ;

#if	CF_DEBUG
	debugprintf("b_homepage/filedat_finish: ret rs=%d\n",rs) ;
#endif

	return (rs >= 0) ? used : rs ;
} /* end subroutine (filedat_finish) */

/* this is an independent thread */
local int filedat_worker(FILEDAT *fep) noex {
	int		rs ;
	int		rl = 0 ;

#if	CF_DEBUG
	debugprintf("filedat_worker: ent svc=%s\n",fep->svc) ;
	debugprintf("filedat_worker: fn=%s\n",fep->fname) ;
	debugprintf("filedat_worker: uc_open() svc=%s\n",fep->svc) ;
#endif

	if ((rs = uc_open(fep->fname,O_RDONLY,0666)) >= 0) {
	    cint	fd = rs ;
	    if ((rs = filedat_workread(fep,fd)) > 0) {
	        int	lines = 0 ;
	        int	sl = rs ;
	        cchar	*sp = fep->dbuf ;
	        cchar	*tp ;
	        rl = rs ;
	        while ((tp = strnchr(sp,sl,'\n')) != nullptr) {
	            lines += 1 ;
	            sl -= ((tp+1)-sp) ;
	            sp = (tp+1) ;
	        } /* end while */
	        fep->lines = lines ;
	    } /* end if (filedat_workread) */
	    u_close(fd) ;
	} /* end if (file) */

#if	CF_DEBUG
	debugprintf("filedat_worker: ret rs=%d svc=%s rl=%u lines=%u\n",
	    rs,fep->svc,rl,fep->lines) ;
#endif

	return (rs >= 0) ? rl : rs ;
} /* end subroutine (filedat_worker) */

local int filedat_workread(FILEDAT *fep,int fd) noex {
	int		rs ;
#if	CF_DEBUG
	debugprintf("filedat_workread: ent svc=%s tt=%s\n",
	    fep->svc,fep->termtype) ;
#endif
	if (fep->termtype != nullptr) {
	    rs = filedat_workreadterm(fep,fd) ;
	} else {
	    rs = filedat_workreadreg(fep,fd) ;
	}
#if	CF_DEBUG
	debugprintf("filedat_workread: ret rs=%d svc=%s\n",rs,fep->svc) ;
#endif
	return rs ;
} /* end subroutine (filedat_workread) */

local int filedat_workreadreg(FILEDAT *fep,int fd) noex {
	cint	rlen = (fep->dsize-1) ;
	cint	to = fep->to ;
	int		rs = SR_OK ;
	int		ml ;
	int		len ;
	int		rl = 0 ;
	char		*rp = fep->dbuf ;
#if	CF_DEBUG
	debugprintf("filedat_workreadreg: ent svc=%s\n",fep->svc) ;
#endif
	while ((rs >= 0) && (rl < rlen)) {
	    ml = (rlen-rl) ;
#if	CF_DEBUG
	    debugprintf("filedat_workreadreg: svc=%s uc_reade() ml=%d\n",
	        fep->svc,ml) ;
#endif
#if	CF_UCREADE
	    rs = uc_reade(fd,rp,ml,to,0) ;
#else
	    rs = u_read(fd,rp,ml) ;
#endif
#if	CF_DEBUG
	    debugprintf("filedat_workreadreg: svc=%s uc_reade() rs=%d\n",
	        fep->svc,rs) ;
#endif
	    if (rs <= 0) break ;
	    len = rs ;
	    rp += len ;
	    rl += len ;
	} /* end while */
#if	CF_DEBUG
	debugprintf("filedat_workreadreg: ret rs=%d svc=%s rl=%u\n",
	    rs,fep->svc,rl) ;
#endif
	return (rs >= 0) ? rl : rs ;
} /* end subroutine (filedat_workreadreg) */

local int filedat_workreadterm(FILEDAT *fep,int fd) noex {
	TERMOUT		out ;
	cint		cols = fep->cols ;
	int		rs ;
	int		rs1 ;
	int		rl = 0 ;
	cchar		*tt = fep->termtype ;
	if ((rs = termout_start(&out,tt,-1,cols)) >= 0) {
	    cint	llen = LINEBUFLEN ;
	    char	*lbuf ;
	    if ((rs = lm_mall((llen+1),&lbuf)) >= 0) {
	        if (filer b ; (rs = filer_start(&b,fd,0z,512,0)) >= 0) {
	            cint	to = fep->to ;
	            void	*n = nullptr ;
	            char	*dp = fep->dbuf ;
	            while ((rs = filer_readlns(&b,lbuf,llen,to,n)) > 0) {
	                cint	len = rs ;
	                rs = filedat_workreadtermline(fep,&out,dp,lbuf,len) ;
	                if (rs > 0) {
	                    rl += rs ;
	                    dp += rs ;
	                }
	                if (rs <= 0) break ;
	            } /* end while (reading lines) */
	            rs1 = filer_finish(&b) ;
	            if (rs >= 0) rs = rs1 ;
	        } /* end if (filedat) */
	        rs1 = lm_free(lbuf) ;
		if (rs >= 0) rs = rs1 ;
	    } /* end if (m-a-f) */
	    rs1 = termout_finish(&out) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (termout) */
	return (rs >= 0) ? rl : rs ;
} /* end subroutine (filedat_workreadterm) */

local int filedat_workreadtermline(FILEDAT *fep,TERMOUT *top,char *rp,
		cchar *lbuf,int len) noex {
	int		rlen = (fep->dsize-1) ;
	int		rs ;
	int		rl = 0 ;
#if	CF_DEBUG
	debugprintf("filedat_workreadtermline: ent l=>%r<\n",
	    lbuf,strlinelen(lbuf,len,40)) ;
#endif
	if ((rs = termout_load(top,lbuf,len)) >= 0) {
	    cint	ln = rs ;
	    int		lenrem = (rlen-(rp-fep->dbuf)) ;
	    int		i ;
	    int		ll ;
	    int		ml ;
	    cchar	*lp ;
	    for (i = 0 ; i < ln ; i += 1) {
	        ll = termout_getline(top,i,&lp) ;
	        if (ll < 0) break ;
#if	CF_DEBUG
	        debugprintf("filedat_workreadtermline: l=>%r<\n",
	            lp,strlinelen(lp,ll,30)) ;
#endif
	        if (lenrem > 0) {
	            ml = MIN(ll,(lenrem-1)) ;
#if	CF_DEBUG
	            debugprintf("filedat_workreadtermline: ml=%u\n",ml) ;
#endif
	            rp = strwcpy(rp,lp,ml) ;
	            *rp++ = CH_NL ;
	            rl += (ml+1) ;
	            lenrem -= (ml+1) ;
	        } /* end if (positive) */
	        if (lenrem == 0) break ;
	    } /* end for */
	} /* end if (termout_load) */
#if	CF_DEBUG
	debugprintf("filedat_workreadtermline: ret rs=%d rl=%u\n",rs,rl) ;
#endif
	return (rs >= 0) ? rl : rs ;
} /* end subroutine (filedat_workreadtermline) */

local int filedat_getlines(FILEDAT *fep) noex {
	proginfo	*pip = fep->pip ;
	int		rs ;
#if	CF_DEBUG
	debugprintf("b_homepage/filedat_getlines: ent svc=%s f_run=%u\n",
	    fep->svc,fep->f_running) ;
#endif
	if (fep->f_running) {
	    cchar	*pn = pip->progname ;
	    cchar	*fmt ;
	    int		trs ;
	    fep->f_running = false ;
	    if ((rs = uptjoin(fep->tid,&trs)) >= 0) {
#if	CF_DEBUG
	        debugprintf("b_homepage/filedat_getlines: uptjoin() trs=%d\n",
	            trs) ;
#endif
	        if (trs >= 0) {
	            fep->dl = trs ;
	            rs = fep->lines ;
	        } else {
	            rs = trs ;
	            fmt = "%s: worker failure svc=%s (%d)\n" ;
	            shio_printf(pip->efp,fmt,pn,fep->svc,rs) ;
	        }
	    } else {
	        fmt = "%s: join failure svc=%s (%d)\n" ;
	        shio_printf(pip->efp,fmt,pn,fep->svc,rs) ;
	    } /* end if (pthead-join) */
#if	CF_DEBUG
	    debugprintf("b_homepage/filedat_getlines: uptjoin-out rs=%d\n",rs) ;
#endif
	} else {
	    rs = fep->lines ;
	}
#if	CF_DEBUG
	debugprintf("b_homepage/filedat_getlines: ret rs=%d svc=%s lns=%u\n",
	    rs,fep->svc,fep->lines) ;
#endif
	return rs ;
} /* end subroutine (filedat_getlines) */

local int filedat_getbuf(FILEDAT *fep,cchar **rpp) noex {
	int		rs ;
	int		rl = 0 ;
	if (rpp != nullptr) *rpp = nullptr ;
	if ((rs = filedat_getlines(fep)) >= 0) {
	    if (rpp != nullptr) *rpp = fep->dbuf ;
	    rl = fep->dl ;
	}
	return (rs >= 0) ? rl : rs ;
} /* end subroutine (filedat_getbuf) */

local int filedat_havesvc(FILEDAT *fep,cchar *svc) noex {
	int		f = true ;
	if (fep == nullptr) return SR_FAULT ;
	if (svc == nullptr) return SR_FAULT ;
	f = f && (svc[0] == fep->svc[0]) ;
	f = f && (strcmp(svc,fep->svc) == 0) ;
	return f ;
} /* end subroutine (filedat_havesvc) */

local int locinfo_start(LI *lip,PI *pip) noex {
	int		rs = SR_OK ;
	if (lip == nullptr) return SR_FAULT ;
	memclear(lip) ;
	lip->pip = pip ;
	lip->to_cache = -1 ;
	lip->to_lock = -1 ;
	return rs ;
} /* end subroutine (locinfo_start) */

local int locinfo_finish(LI *lip) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	if (lip == nullptr) return SR_FAULT ;

	rs1 = locinfo_svclistend(lip) ;
	if (rs >= 0) rs = rs1 ;

	if (lip->open.cooks) {
	    lip->open.cooks = false ;
	    rs1 = locinfo_cookend(lip) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if */

	if (lip->open.stores) {
	    lip->open.stores = false ;
	    rs1 = vecstr_finish(&lip->stores) ;
	    if (rs >= 0) rs = rs1 ;
	}

	return rs ;
} /* end subroutine (locinfo_finish) */

local int locinfo_setentry(LI *lip,cchar **epp,cchar *vp,int vl) noex {
	vecstr		*slp ;
	int		rs = SR_OK ;
	int		len = 0 ;

	if (lip == nullptr) return SR_FAULT ;
	if (epp == nullptr) return SR_FAULT ;

	slp = &lip->stores ;
	if (! lip->open.stores) {
	    rs = vecstr_start(slp,4,0) ;
	    lip->open.stores = (rs >= 0) ;
	}

	if (rs >= 0) {
	    int	oi = -1 ;
	    if (*epp != nullptr) {
	        oi = vecstr_findaddr(slp,*epp) ;
	    }
	    if (vp != nullptr) {
	        len = xstrnlen(vp,vl) ;
	        rs = vecstr_store(slp,vp,len,epp) ;
	    } else {
	        *epp = nullptr ;
	    }
	    if ((rs >= 0) && (oi >= 0)) {
	        vecstr_del(slp,oi) ;
	    }
	} /* end if (ok) */

	return (rs >= 0) ? len : rs ;
} /* end subroutine (locinfo_setentry) */

local int locinfo_dbinfo(LI *lip,cchar *basedname,cchar *dbfname) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;

	if (pip == nullptr) return SR_FAULT ;

#if	CF_DEBUG
	if (DEBUGLEVEL(3)) {
	    debugprintf("main/locinfo_dbinfo: basedb=%s\n",basedname) ;
	    debugprintf("main/locinfo_dbinfo: dbfname=%s\n",dbfname) ;
	}
#endif

	if ((rs >= 0) && (lip->basedname == nullptr)) {
	    if (basedname != nullptr) {
	        rs = locinfo_basedir(lip,basedname,-1) ;
	    }
	}

	if ((rs >= 0) && (lip->dbfname == nullptr)) {
	    if (dbfname != nullptr) {
	        cchar	**vpp = &lip->dbfname ;
	        lip->finval.dbfname = true ;
	        rs = locinfo_setentry(lip,vpp,dbfname,-1) ;
	    }
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(3)) {
	    debugprintf("main/locinfo_dbinfo: ret basedb=%s\n",lip->basedname) ;
	    debugprintf("main/locinfo_dbinfo: ret dbfname=%s\n",lip->dbfname) ;
	    debugprintf("main/locinfo_dbinfo: ret rs=%d\n",rs) ;
	}
#endif

	return rs ;
} /* end subroutine (locinfo_dbinfo) */

local int locinfo_basedir(LI *lip,cchar *vp,int vl) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;

	if (lip->basedname == nullptr) {
	    cchar	*inter = ETCCNAME ;
	    char	tbuf[MAXPATHLEN+1] ;
	    if ((rs = mkourname(pip,tbuf,inter,vp,vl)) >= 0) {
	        cchar	**vpp = &lip->basedname ;
	        rs = locinfo_setentry(lip,vpp,tbuf,rs) ;
	    }
	}

	return rs ;
} /* end subroutine (locinfo_basedir) */

local int locinfo_wfname(LI *lip,cchar *wfname) noex {
	int		rs = SR_OK ;
	if ((lip->wfname == nullptr) && (wfname != nullptr)) {
	    cchar	**vpp = &lip->wfname ;
	    rs = locinfo_setentry(lip,vpp,wfname,-1) ;
	}
	return rs ;
} /* end subroutine (locinfo_wfname) */

local int locinfo_sethead(LI *lip,cchar *vp,int vl) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/locinfo_sethead: v=%r\n",vp,vl) ;
#endif

	if (lip->hfname == nullptr) {
	    cchar	*inter = ETCCNAME ;
	    char	tbuf[MAXPATHLEN+1] ;
	    if ((rs = mkourname(pip,tbuf,inter,vp,vl)) >= 0) {
	        cchar	**vpp = &lip->hfname ;
#if	CF_DEBUG
	        if (DEBUGLEVEL(3))
	            debugprintf("main/locinfo_sethead: tbuf=%r\n",tbuf,rs) ;
#endif
	        rs = locinfo_setentry(lip,vpp,tbuf,rs) ;
	    }
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/locinfo_sethead: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (locinfo_sethead) */

local int locinfo_svclistbegin(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;
	int		f = false ;
	if (pip == nullptr) return SR_FAULT ;
	if (lip->svcs == nullptr) {
	    cint	size = szof(vecpstr) ;
	    f = true ;
	    if (void *p ; (rs = lm_mall(size,&p)) >= 0) {
	        vecpstr		*slp = p ;
	        lip->svcs = p ;
	        rs = vecpstr_start(slp,5,0,0) ;
	        if (rs < 0) {
	            lm_free(lip->svcs) ;
	            lip->svcs = nullptr ;
	        }
	    } /* end if (m-a) */
	} /* end if (svcs) */
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/locinfo_svclistbegin: ret rs=%d f=%u\n",rs,f) ;
#endif
	return (rs >= 0) ? f : rs ;
} /* end subroutine (locinfo_svclistbegin) */

local int locinfo_svclistend(LI *lip) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	if (lip->svcs != nullptr) {
	    vecpstr	*slp = lip->svcs ;
	    rs1 = vecpstr_finish(slp) ;
	    if (rs >= 0) rs = rs1 ;
	    rs1 = lm_free(lip->svcs) ;
	    if (rs >= 0) rs = rs1 ;
	    lip->svcs = nullptr ;
	}
	return rs ;
} /* end subroutine (locinfo_svclistend) */

local int locinfo_svclistadds(LI *lip,cchar *sp,int sl) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;
	int		cl ;
	int		c = 0 ;
	cchar		*tp ;
	cchar		*cp ;
	if (pip == nullptr) return SR_FAULT ;
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("b_homepage/locinfo_svclistadds: s=>%r<\n",sp,sl) ;
#endif
	while ((tp = strnchr(sp,sl,',')) != nullptr) {
#if	CF_DEBUG
	    if (DEBUGLEVEL(3))
	        debugprintf("b_homepage/locinfo_svclistadds: svc=>%r<\n",
	            sp,(tp-sp)) ;
#endif
	    if ((cl = sfshrink(sp,(tp-sp),&cp)) > 0) {
	        rs = locinfo_svclistadd(lip,cp,cl) ;
	        c += rs ;
	    }
	    sl -= ((tp+1)-sp) ;
	    sp = (tp+1) ;
	    if (rs < 0) break ;
	} /* end while */
	if ((rs >= 0) && sl) {
	    if ((cl = sfshrink(sp,sl,&cp)) > 0) {
	        rs = locinfo_svclistadd(lip,cp,cl) ;
	        c += rs ;
	    }
	}
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("b_homepage/locinfo_svclistadds: ret rs=%d c=%u\n",rs,c) ;
#endif
	return (rs >= 0) ? c : rs ;
} /* end subroutine (locinfo_svclistadds) */

local int locinfo_svclistadd(LI *lip,cchar *vp,int vl) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;
	int		c = 0 ;

	if (pip == nullptr) return SR_FAULT ;

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/locinfo_svclistadd: v=%r\n",vp,vl) ;
#endif

	if (lip->svcs == nullptr) {
	    rs = locinfo_svclistbegin(lip) ;
	} /* end if (svcs) */

	if (rs >= 0) {
	    vecpstr	*slp = lip->svcs ;
	    int		sl ;
	    cchar	st[] = { 
	        CH_FS, 0 		} ;
	    cchar	*sp ;
	    if (vl < 0) vl = xstrlen(vp) ;
	    while (vl > 0) {
	        if ((sl = sfnextbrk(vp,vl,st,&sp)) < 0) break ;
#if	CF_DEBUG
	        if (DEBUGLEVEL(3))
	            debugprintf("main/locinfo_svclistadd: s=>%r<\n",sp,sl) ;
#endif
	        if (sl > 0) {
	            lip->have.svcs = true ;
	            rs = vecpstr_adduniq(slp,sp,sl) ;
	            if (rs < INT_MAX) c += 1 ;
	        }
	        vl -= ((sp+sl+1)-vp) ;
	        vp = (sp+sl+1) ;
	        if (rs < 0) break ;
	    } /* end while */
	} /* end if (ok) */

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("main/locinfo_svclistadd: ret rs=%d c=%u\n",rs,c) ;
#endif

	return (rs >= 0) ? c : rs ;
} /* end subroutine (locinfo_svclistadd) */

local int locinfo_svclistget(LI *lip,int i,cchar **rpp) noex {
	int		rs = SR_NOTFOUND ;
	if (lip->svcs != nullptr) {
	    vecpstr	*slp = lip->svcs ;
	    rs = vecpstr_get(slp,i,rpp) ;
	}
	return rs ;
} /* end subroutine (locinfo_svclistget) */

local int locinfo_svclistdel(LI *lip,int i) noex {
	int		rs = SR_OK ;
	if (lip->svcs != nullptr) {
	    vecpstr	*slp = lip->svcs ;
	    rs = vecpstr_del(slp,i) ;
	}
	return rs ;
} /* end subroutine (locinfo_svclistdel) */

local int locinfo_svclistdelall(LI *lip) noex {
	int		rs = SR_OK ;
	if ((lip->svcs != nullptr) && (! lip->finval.svcs)) {
	    vecpstr	*slp = lip->svcs ;
	    rs = vecpstr_delall(slp) ;
	}
	return rs ;
} /* end subroutine (locinfo_svclistdelall) */

local int locinfo_svclistcount(LI *lip) noex {
	int		rs = SR_OK ;
	if (lip->svcs != nullptr) {
	    vecpstr	*slp = lip->svcs ;
	    rs = vecpstr_count(slp) ;
	}
	return rs ;
} /* end subroutine (locinfo_svclistcount) */

local int locinfo_svclistfinal(LI *lip) noex {
	cint	f = lip->finval.svcs ;
	return f ;
} /* end subroutine (locinfo_svclistfinal) */

local int locinfo_copyright(LI *lip,cchar *vp,int vl) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;

	if (pip == nullptr) return SR_FAULT ;

	if (lip->copyright == nullptr) {
	    cint	elen = (vl+MAXNAMELEN) ;
	    char	*ebuf ;
	    if ((rs = lm_mall((elen+1),&ebuf)) >= 0) {
	        cint	w = false ;
	        if ((rs = locinfo_cookexp(lip,w,ebuf,elen,vp,vl)) >= 0) {
	            cchar	**vpp = &lip->copyright ;
	            rs = locinfo_setentry(lip,vpp,ebuf,rs) ;
	        }
	        lm_free(ebuf) ;
	    } /* end if (m-a-f) */
	} /* end if (needed) */

	return rs ;
} /* end subroutine (locinfo_copyright) */

local int locinfo_defs(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;

	if ((rs >= 0) && (lip->hfname == nullptr)) {
	    cchar	*inter = ETCCNAME ;
	    cchar	*fn = HEADFNAME ;
	    char	tbuf[MAXPATHLEN+1] ;
	    if ((rs = mkourname(pip,tbuf,inter,fn,-1)) >= 0) {
	        cint	tl = rs ;
	        if ((rs = perm(tbuf,-1,-1,nullptr,R_OK)) >= 0) {
	            cchar	**vpp = &lip->hfname ;
	            rs = locinfo_setentry(lip,vpp,tbuf,tl) ;
	        } else if (isNotPresent(rs)) {
	            rs = SR_OK ;
	        }
	    } /* end if (mkourname) */
	}

	if (rs >= 0) {
	    rs = locinfo_defsvc(lip) ;
	}

	if (lip->termtype == nullptr) lip->termtype = DEFTERMTYPE ;

	if ((rs >= 0) && (lip->cols == 0)) {
	    cchar	*vp = getourenv(pip->envv,VARCOLUMNS) ;
	    if ((vp != nullptr) && (vp[0] != '\0')) {
	        rs = optvalue(vp,-1) ;
	        lip->cols = rs ;
	    }
	    if (lip->cols == 0) lip->cols = COLUMNS ;
	} /* end if (columns) */

	if (lip->intcache == 0) lip->intcache = TO_CACHE ;

	if (lip->intwait == 0) lip->intwait = TO_WAIT ;

	if (lip->intconf == 0) lip->intconf = TO_CONFIG ;

	if (lip->intsvcs == 0) lip->intsvcs = TO_SVCS ;

	return rs ;
} /* end subroutine (locinfo_defs) */

local int locinfo_defsvc(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;

	if ((rs >= 0) && (lip->sfname == nullptr)) {
	    char	tbuf[MAXPATHLEN+1] ;
	    cchar	*inter = ETCCNAME ;
	    cchar	*fn = SVCFNAME ;
	    if ((rs = mkourname(pip,tbuf,inter,fn,-1)) >= 0) {
	        cint	tl = rs ;
	        if ((rs = perm(tbuf,-1,-1,nullptr,R_OK)) >= 0) {
	            cchar	**vpp = &lip->sfname ;
	            rs = locinfo_setentry(lip,vpp,tbuf,tl) ;
	        } else if (isNotAccess(rs)) {
	            rs = SR_OK ;
	        }
	    }
	}

	return rs ;
} /* end subroutine (locinfo_defsvc) */

local int locinfo_svcsbegin(LI *lip) noex {
	SVCFILE		*sfp = &lip->s ;
	int		rs ;
	if ((rs = svcfile_open(sfp,lip->sfname)) >= 0) {
	    lip->open.s = true ;
	}
	return rs ;
} /* end subroutine (locinfo_svcsbegin) */

local int locinfo_svcsend(LI *lip) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	if (lip->open.s) {
	    rs1 = svcfile_close(&lip->s) ;
	    if (rs >= 0) rs = rs1 ;
	}
	return rs ;
} /* end subroutine (locinfo_svcsend) */

local int locinfo_svcscheck(LI *lip,time_t dt) noex {
	int		rs = SR_OK ;
	if (lip->open.s) {
	    if (dt < 0) dt = time(nullptr) ;
	    rs = svcfile_check(&lip->s,dt) ;
	}
	return rs ;
} /* end subroutine (locinfo_svcsend) */

local int locinfo_gatherbegin(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	GATHER		*glp = &lip->g ;
	int		rs ;
	if (pip == nullptr) return SR_FAULT ;
	if ((rs = gather_start(glp,pip,lip->termtype,lip->cols)) >= 0) {
	    if ((rs = locinfo_svclistcount(lip)) > 0) {
	        rs = locinfo_gatherbeginsome(lip) ;
	    } else {
	        rs = locinfo_gatherbeginall(lip) ;
	    }
	    if (rs < 0) {
	        gather_finish(&lip->g) ;
	    }
	} /* end if (gather_start) */
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("b_homepage/locinfo_gatherbegin: ret rs=%d\n",rs) ;
#endif
	return rs ;
} /* end subroutine (locinfo_gatherbegin) */

local int locinfo_gatherbeginall(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	SVCFILE		*sfp = &lip->s ;
	SVCFILE_CUR	c ;
	SVCFILE_ENT	se ;
	int		rs ;
	int		rs1 ;

	if (pip == nullptr) return SR_FAULT ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("locinfo_gatherbeginall: ent\n") ;
#endif

	lip->fl.defsvcs = true ;
	if ((rs = svcfile_curbegin(sfp,&c)) >= 0) {
	    cint	slen = SVCLEN ;
	    cint	el = SVCENTLEN ;
	    char	sbuf[SVCLEN+1] ;
	    char	eb[SVCENTLEN+1] ;
	    void	*n = nullptr ;
	    while ((rs1 = svcfile_curenumsvc(sfp,&c,sbuf,slen)) > 0) {
	        if ((rs = locinfo_svclistadd(lip,sbuf,rs1)) >= 0) {
	            if ((rs = svcfile_fetch(sfp,sbuf,n,&se,eb,el)) > 0) {
	                rs = locinfo_gatherbeginer(lip,&se) ;
	            }
	        } /* end if (locinfo_svclistadd) */
	        if (rs < 0) break ;
	    } /* end while */
#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("locinfo_gatherbeginall: while-out "
	            "rs=%d rs1=%d\n",rs,rs1) ;
#endif
	    if ((rs >= 0) && (rs1 != SR_NOTFOUND)) rs = rs1 ;
	    rs1 = svcfile_curend(sfp,&c) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (svcfile-cur) */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("locinfo_gatherbeginall: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (locinfo_gatherbeginall) */

local int locinfo_gatherbeginsome(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	SVCFILE		*sfp = &lip->s ;
	cint	nrs = SR_NOTFOUND ;
	int		rs = SR_OK ;
	int		i ;
	cchar		*snp ;

	if (pip == nullptr) return SR_FAULT ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("locinfo_gatherbeginsome: ent\n") ;
#endif

	for (i = 0 ; locinfo_svclistget(lip,i,&snp) >= 0 ; i += 1) {
	    if (snp != nullptr) {
	        SVCFILE_ENT	se ;
	        cint	el = SVCENTLEN ;
	        void		*n = nullptr ;
	        char		eb[SVCENTLEN+1] ;
	        if ((rs = svcfile_fetch(sfp,snp,n,&se,eb,el)) > 0) {
	            rs = locinfo_gatherbeginer(lip,&se) ;
	        }
	        if (rs == nrs) {
	            rs = locinfo_svclistdel(lip,i) ;
	        }
	    } /* end if (non-null) */
	    if (rs < 0) break ;
	} /* end for */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("locinfo_gatherbeginsome: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (locinfo_gatherbeginsome) */

local int locinfo_gatherend(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;
	int		rs1 ;
	int		used = 0 ;
	cchar		*pn ;

	if (pip == nullptr) return SR_FAULT ;

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("b_homepage/locinfo_gatherend: ent\n") ;
#endif

	pn = pip->progname ;
	rs1 = gather_finish(&lip->g) ;
	if (rs >= 0) rs = rs1 ;
	used = rs1 ;

#if	CF_STACKCHECK
	if ((rs >= 0) && (used >= (STACKSIZE-STACKGUARD))) {
	    cchar	*fmt = "%s: stack-guard violated used=%d\n" ;
	    shio_printf(pip->efp,fmt,pn,used) ;
	}
#endif /* CF_STACKCHECK */

	if (pip->debuglevel > 0) {
	    shio_printf(pip->efp,"%s: max stack used=%u\n",pn,used) ;
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("b_homepage/locinfo_gatherend: ret rs=%d\n",rs) ;
#endif

	return (rs >= 0) ? used : rs ;
} /* end subroutine (locinfo_gatherend) */

local int locinfo_gatherbeginer(LI *lip,SVCFILE_ENT *sep) noex {
	proginfo	*pip = lip->pip ;
	GATHER		*glp = &lip->g ;
	cint	n = sep->nkeys ;
	int		rs ;
	cchar		*(*kv)[2] = sep->keyvals ;
	cchar		*vp ;

	if (pip == nullptr) return SR_FAULT ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("locinfo_gatherbeginer: ent\n") ;
#endif

	if ((rs = svckv_dequote(kv,n,"h",&vp)) > 0) {
	    cchar	*fp ;
	    if ((rs = svckv_isfile(kv,n,&fp)) > 0) {
	        int	fl = rs ;

#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("locinfo_gatherbeginer: "
	                "svckv_svcopts()\n") ;
#endif

	        if ((rs = svckv_svcopts(kv,n)) >= 0) {
	            cint	f_to = bwtst(rs,svcopt_termout) ;
	            cchar	*svc = sep->svc ;
#if	CF_DEBUG
	            if (DEBUGLEVEL(4))
	                debugprintf("locinfo_gatherbeginer: rs=\\b%04ß\n",rs) ;
#endif
	            rs = gather_file(glp,svc,f_to,fp,fl) ;
	        }

#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("locinfo_gatherbeginer: "
	                "gather_file() rs=%d\n",rs) ;
#endif

	    } /* end if (svckv_isfile) */
#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("locinfo_gatherbeginer: "
	            "svckv-isfile-out rs=%d\n",rs) ;
#endif
	} /* end if (svckv) */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("locinfo_gatherbeginer: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (locinfo_gatherbeginer) */

int locinfo_defreg(LI *lip) noex {
	if (lip->intspeed == 0) lip->intspeed = TO_SPEED ;
	return SR_OK ;
/* end subroutine (locinfo_defreg) */

int locinfo_defdaemon(LI *lip) noex {
	proginfo	*pip = lip->pip ;

	if (lip->intspeed == 0) lip->intspeed = TO_SPEED ;

	if (pip->uid != pip->euid) {
	    u_setreuid(pip->euid,-1) ;
	}
	if (pip->gid != pip->egid) {
	    u_setregid(pip->egid,-1) ;
	}

	return SR_OK ;
} /* end subroutine (locinfo_defdaemon) */

int locinfo_lockbegin(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	LFM		*lfp = &lip->pidlock ;
	int		rs ;
	cchar		*lockfname = pip->pidfname ;
	if ((rs = locinfo_lockbeginone(lip,lfp,lockfname)) >= 0) {
	    lip->open.pidlock = (rs > 0) ;
	}
	return rs ;
} /* end subroutine (locinfo_lockbegin) */

int locinfo_lockcheck(LI *lip,lfm_check *lcp) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;
	if (lip->open.pidlock) {
	    rs = lfm_check(&lip->pidlock,lcp,pip->daytime) ;
	} /* end if (pidlock) */
	return rs ;
} /* end subroutine (locinfo_lockcheck) */

int locinfo_lockend(LI *lip) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	if (lip->open.pidlock) {
	    lip->open.pidlock = false ;
	    rs1 = lfm_finish(&lip->pidlock) ;
	    if (rs >= 0) rs = rs1 ;
	}
	return rs ;
} /* end subroutine (locinfo_lockend) */

local int locinfo_lockbeginone(LI *lip,LFM *lfp,cchar *lfn) noex {
	proginfo	*pip = lip->pip ;
	cmode		dmode = 0777 ;
	int		rs = SR_OK ;
	int		f_opened = false ;

	if ((rs >= 0) && lfn && (lfn[0] != '\0') && (lfn[0] != '-')) {
	    int		cl ;
	    cchar	*cp ;
	    char	tmpfname[MAXPATHLEN+1] ;

	    cl = sfdirname(lfn,-1,&cp) ;
	    if ((rs = mkpath1w(tmpfname,cp,cl)) >= 0) {
	        if (ustat usb ; (rs = u_stat(tmpfname,&usb)) >= 0) {
	            if (! S_ISDIR(usb.st_mode)) {
	                rs = SR_NOTDIR ;
	            }
	        } else if (rs == SR_NOENT) {
	            rs = mkdirs(tmpfname,dmode) ;
	        }
	    }

	    if (rs >= 0) {
	        lfm_check	lc ;
	        cint	ltype = LFM_TRECORD ;
	        cint	to_lock = lip->to_lock ;
	        cchar		*nn = pip->nodename ;
	        cchar		*un = pip->username ;
	        cchar		*bn = pip->banner ;
	        rs = lfm_start(lfp,lfn,ltype,to_lock,&lc,nn,un,bn) ;
	        f_opened = (rs >= 0) ;

#if	CF_DEBUG
	        if (DEBUGLEVEL(4)) {
	            debugprintf("locinfo_lockbeginone: lfm_start() rs=%d\n",
	                rs) ;
	        }
#endif

#ifdef	COMMENT
	        if ((pip->debuglevel > 0) && (rs < 0))
	            shio_printf(pip->efp,
	                "%s: inaccessible PID lock (%d)\n",
	                pip->progname,rs) ;
#endif /* COMMENT */

	        if ((rs == SR_LOCKFAIL) || (rs == SR_AGAIN)) {
	            proclockprint(pip,ccp,&lc) ;
	        }
	    } /* end if */
	} /* end if (establish lock) */
	return (rs >= 0) ? f_opened : rs ;
} /* end subroutine (locinfo_lockbeginone) */

int locinfo_tmpourdname(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;
	int		rs1 ;
	int		pl = 0 ;

	if (lip->tmpourdname == nullptr) {
	    if ((rs = proginfo_rootname(pip)) >= 0) {
	        mode_t	dm = 0777 ;
	        int	f_needmode = false ;
	        int	f_created = false ;
	        int	f_runasprn ;
	        cchar	*tn = pip->tmpdname ;
	        cchar	*sn = pip->searchname ;
	        cchar	*un = pip->username ;
	        cchar	*rn = pip->rootname ;
	        char	tmpourdname[MAXPATHLEN + 1] = { 
	            0 			} ;
	        f_runasprn = (strcmp(un,rn) == 0) ;
	        if (! f_runasprn) dm = 0777 ;
	        rs1 = SR_OK ;
	        if ((rs = mkpath3(tmpourdname,tn,rn,sn)) >= 0) {
	            ustat	usb ;
	            pl = rs ;
	            if ((rs1 = u_stat(tmpourdname,&usb)) >= 0) {
	                if (S_ISDIR(usb.st_mode)) {
	                    cint	am = (R_OK|W_OK|X_OK) ;
	                    f_needmode = ((usb.st_mode & dm) != dm) ;
	                    rs = u_access(tmpourdname,am) ;
	                } else {
	                    rs = SR_NOTDIR ;
	                }
	            }
	            if (rs >= 0) {
	                if (rs1 == SR_NOENT) {
	                    f_needmode = true ;
	                    f_created = true ;
	                    rs = mkdirs(tmpourdname,dm) ;
	                }
	                if ((rs >= 0) && f_needmode) {
	                    rs = uc_minmod(tmpourdname,dm) ;
	                }

#if	CF_TMPGROUP
	                if ((rs >= 0) && f_created) {
	                    rs = locinfo_tmpgroup(lip,tmpourdname) ;
	                }
#endif /* CF_TMPGROUP */

	                if (rs >= 0) {
	                    cchar	**vpp = &lip->tmpourdname ;
	                    rs = locinfo_setentry(lip,vpp,tmpourdname,pl) ;
	                }

	            } /* end if (ok) */
	        } /* end if (mkpath) */
	    } else {
	        cchar	*pn = pip->progname ;
	        cchar	*fmt ;
	        fmt = "%s: TMPDIR access problem (%d)\n" ;
	        shio_printf(pip->efp,fmt,pn,rs) ;
	        fmt = "%s: tmpdir=%s\n" ;
	        shio_printf(pip->efp,fmt,pn,lip->tmpourdname) ;
	        if (pip->open.logprog) {
	            proglog_printf(pip,"TMPDIR access problem (%d)\n",rs) ;
	            proglog_printf(pip,"tmpdir=%s",lip->tmpourdname) ;
	        }
	    } /* end if */
	} else {
	    pl = xstrlen(lip->tmpourdname) ;
	}

	return (rs >= 0) ? pl : rs ;
} /* end subroutine (locinfo_tmpourdname) */

#if	CF_TMPGROUP
local int locinfo_tmpgroup(LI *lip,cchar *tmpourdname) noex {
	proginfo	*pip = lip->pip ;
	int		rs ;
	int		rs1 ;
	if ((rs = bufsizeget(bufsize_pw)) >= 0) {
	    PASSWD	pw ;
	    cint	pwlen = rs ;
	    if (char *pwbuf ; (rs = lm_mall((pwlen+1),&pwbuf)) >= 0) {
	        cchar	*rn = pip->rootname ;
	        cchar	*un = pip->username ;
	        if ((rs = locinfo_gidrootname(lip,&pw,pwbuf,pwlen)) >= 0) {
	            const gid_t	gid_prn = lip->gid_rootname ;
	            cint	f_runasprn = (strcmp(un,rn) == 0) ;
	            if (! f_runasprn) {
	                uid_t	uid_prn = pip->euid ;
	                if ((rs = GETPW_NAME(&pw,pwbuf,pwlen,rn)) >= 0) {
	                    uid_prn = pw.pw_uid ;
	                } else if (isNotPresent(rs)) {
	                    rs = SR_OK ;
	                }
	                if (rs >= 0) {
	                    rs = u_chown(tmpourdname,uid_prn,gid_prn) ;
	                }
	            } else {
	                rs = u_chown(tmpourdname,-1,gid_prn) ;
	            }
	        } /* end if (locinfo_gidrootname) */
	        rs1 = lm_free(pwbuf) ;
		if (rs >= 0) rs = rs1 ;
	    } /* end if (m-a-f) */
	} /* end if (bufsizeget) */
	return rs ;
} /* end subroutine (locinfo_tmpgroup) */
#endif /* CF_TMPGROUP */

#ifdef	COMMENT
int locinfo_ipcpid(LI *lip,int f) noex {
	proginfo	*pip = lip->pip ;
	cint	oflags = (O_CREAT | O_WRONLY | O_TRUNC) ;
	int		rs = SR_OK ;
	cchar		*pidcname = PIDCNAME ;
	cchar		*pf ;
	char		pidfname[MAXPATHLEN + 1] ;

	if (pip->pidfname == nullptr) {
	    int		pl ;

	    rs = locinfo_tmpourdname(lip) ;

#if	CF_DEBUG
	    if (DEBUGLEVEL(5))
	        debugprintf("msulocinfo/_ipcpid: _tmpourdname() rs=%d\n",rs) ;
#endif

	    if (rs >= 0) {
	        rs = mkpath2(pidfname,lip->tmpourdname,pidcname) ;
	        pl = rs ;
	    }

	    if (rs >= 0) {
	        cchar	**vpp = &pip->pidfname ;
	        rs = proginfo_setentry(pip,vpp,pidfname,pl) ;
	    }

	    if ((rs < 0) && (pip->efp != nullptr)) {
	        shio_printf(pip->efp,"%s: TMPDIR access problem (%d)\n",
	            pip->progname,rs) ;
	    }

	} /* end if (creating PID file) */

	if (rs >= 0) {
	    pf = pip->pidfname ;
	    if (f) { /* activate */

	        pip->fl.pidfname = false ;
	        if ((rs = u_open(pf,oflags,0664)) >= 0) {
	            int 	fd = rs ;
	            int		wl ;
	            char	*pidbuf = pidfname ;
	            if ((rs = ctdeci(pidbuf,MAXPATHLEN,pip->pid)) >= 0) {
	                wl = rs ;
	                pidbuf[wl++] = '\n' ;
	                rs = u_write(fd,pidbuf,wl) ;
	                pip->fl.pidfname = (rs >= 0) ;
	            }
	            u_close(fd) ;
	        } /* end if (file) */

#if	CF_DEBUG
	        if (DEBUGLEVEL(5))
	            debugprintf("msulocinfo/_ipcpid: activate rs=%d\n",rs) ;
#endif

	    } else { /* de-activate */

	        if ((pf != nullptr) && pip->fl.pidfname) {
	            pip->fl.pidfname = false ;
	            if (pf[0] != '\0') u_unlink(pf) ;
	        }

	    } /* end if (invocation mode) */
	} /* end if (ok) */

	if ((rs < 0) && (pip->efp != nullptr)) {
	    shio_printf(pip->efp,"%s: PID access problem (%d)\n",
	        pip->progname,rs) ;
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("msulocinfo/_ipcpid: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (locinfo_ipcpid) */
#endif /* COMMENT */

int locinfo_gidrootname(LI *lip,PASSWD *pwp,char *pwbuf,int pwlen) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;
	int		rs1 ;

	if (lip->gid_rootname == 0) {
	    if ((rs = proginfo_rootname(pip)) >= 0) {
	        rs1 = GETPW_NAME(pwp,pwbuf,pwlen,pip->rootname) ;
	        if (rs1 >= 0) lip->gid_rootname = pwp->pw_gid ;
	        if (lip->gid_rootname <= 0) {
	            cchar	*tmpdname = pip->tmpdname ;
	            cchar	*rn = pip->rootname ;
	            char	dname[MAXPATHLEN+1] ;
	            if ((rs = mkpath2(dname,tmpdname,rn)) >= 0) {
	                ustat	sb ;
	                if ((rs = u_stat(dname,&sb)) >= 0) {
	                    if (S_ISDIR(sb.st_mode)) {
	                        lip->gid_rootname = sb.st_gid ;
	                    } else 
	                        rs = SR_NOTDIR ;
	                }
	            } /* end if (mkpath) */
	        } /* end if (needed) */
	    } /* end if (rootname) */
	} /* end if (needed) */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("msulocinfo_gidrootname: ret rs=%d gid=%d\n",
	        rs,lip->gid_rootname) ;
#endif

	return rs ;
} /* end subroutine (locinfo_gidrootname) */

local int locinfo_defpfname(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	int		rs = SR_OK ;
	if (pip->pidfname == nullptr) {
	    cchar	**envv = pip->envv ;
	    cchar	*cp ;
	    if ((cp = getourenv(envv,VARPIDFNAME)) != nullptr) {
	        pip->finval.pidfname = true ;
	        pip->have.pidfname = true ;
	        pip->pidfname = cp ;
	    }
	}
	return rs ;
} /* end subroutine (locinfo_defpfname) */

local int locinfo_qs(LI *lip,cchar *qs) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	int		c = 0 ;
	if ((! lip->finval.svcs) && (qs != nullptr)) {
	    querystr		ps ;
	    querystr_cur	cur ;
	    if ((rs = querystr_start(&ps,qs,-1)) >= 0) {
	        if ((rs = querystr_curbegin(&ps,&cur)) >= 0) {
	            cchar	*kp, *vp ;
	            while ((rs1 = querystr_curenum(&ps,&cur,&kp,&vp)) >= 0) {
	                if (vp == nullptr) break ; /* lint */
	                rs = locinfo_svclistadd(lip,kp,-1) ;
	                c += rs ;
	                if (rs < 0) break ;
	            } /* end while */
	            if ((rs >= 0) && (rs1 != SR_NOTFOUND)) rs = rs1 ;
	        } /* end if (querystring-cur) */
	        rs1 = querystr_finish(&ps) ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (querystring) */
	    lip->finval.svcs = (c > 0) ;
	} /* end if (allowed) */
	return (rs >= 0) ? c : rs ;
} /* end subroutine (locinfo_qs) */

/* finalize argument objects */
local int locinfo_finalize(LI *lip) noex {
	int		rs ;
	if ((rs = locinfo_svclistcount(lip)) > 0) {
	    lip->finval.svcs = true ;
	}
	return rs ;
} /* end subroutine (finalize) */

local int locinfo_cookbegin(LI *lip) noex {
	proginfo	*pip = lip->pip ;
	expcook		*clp = &lip->cooks ;
	int		rs ;

	if ((rs = expcook_start(clp)) >= 0) {
	    cint	hlen = MAXHOSTNAMELEN ;
	    int		i ;
	    int		kch ;
	    int		vl ;
	    cchar	*ks = "PSNDHRUY" ;
	    cchar	*vp ;
	    char	hbuf[MAXHOSTNAMELEN+1] ;
	    char	kbuf[2] ;

	    kbuf[1] = '\0' ;
	    for (i = 0 ; (rs >= 0) && (ks[i] != '\0') ; i += 1) {
	        kch = MKCHAR(ks[i]) ;
	        vp = nullptr ;
	        vl = -1 ;
	        switch (kch) {
	        case 'P':
	            vp = pip->progname ;
	            break ;
	        case 'S':
	            vp = pip->searchname ;
	            break ;
	        case 'N':
	            vp = pip->nodename ;
	            break ;
	        case 'D':
	            vp = pip->domainname ;
	            break ;
	        case 'H':
	            {
	                cchar	*nn = pip->nodename ;
	                cchar	*dn = pip->domainname ;
	                rs = snsds(hbuf,hlen,nn,dn) ;
	                vl = rs ;
	                vp = hbuf ;
	            }
	            break ;
	        case 'R':
	            vp = pip->pr ;
	            break ;
	        case 'U':
	            vp = pip->username ;
	            break ;
	        case 'Y':
	            {
	                cint	ylen = USERNAMELEN ;
	                char		ybuf[USERNAMELEN+1] ;
	                rs = ctdeci(ybuf,ylen,lip->year) ;
	                vl = rs ;
	                vp = ybuf ;
	            }
	            break ;
	        } /* end switch */
	        if ((rs >= 0) && (vp != nullptr)) {
	            kbuf[0] = kch ;
	            rs = expcook_add(clp,kbuf,vp,vl) ;
	        }
	    } /* end for */

	    if (rs >= 0) {
	        lip->open.cooks = true ;
	    } else {
	        expcook_finish(clp) ;
	    }
	} /* end if (expcook_start) */

	return rs ;
} /* end subroutine (locinfo_cookbegin) */

local int locinfo_cookend(LI *lip) noex {
	expcook		*clp = &lip->cooks ;
	int		rs = SR_OK ;
	int		rs1 ;
	if (lip->open.cooks) {
	    lip->open.cooks = false ;
	    rs1 = expcook_finish(clp) ;
	    if (rs >= 0) rs = rs1 ;
	}
	return rs ;
} /* end subroutine (locinfo_cookend) */

local int locinfo_cookexp(LI *lip,int w,char *ebuf,int elen,
		cchar *vbuf,int vl) noex {
	expcook		*clp = &lip->cooks ;
	int		rs ;
	{
	    rs = expcook_exp(clp,w,ebuf,elen,vbuf,vl) ;
	}
	return rs ;
} /* end subroutine (locinfo_cookexp) */

/* configuration */
local int config_start(CONFIG *csp,PI *pip,paramopt *app,cchar *cfname) noex {
	cint	esz = szof(CONFIG_FILE) ;
	int		rs ;

	if (csp == nullptr) return SR_FAULT ;
	if (app == nullptr) return SR_FAULT ;

	memclear(csp) ; /* dangerous */
	csp->pip = pip ;
	csp->app = app ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("config_start: cfname=%s\n",cfname) ;
#endif

#ifdef	COMMENT
	if ((pip->debuglevel > 0) && (cfname != nullptr)) {
	    shio_printf(pip->efp,"%s: cf=%s\n",
	        pip->progname,cfname) ;
	}
#endif /* COMMENT */

	if ((rs = vecobj_start(&csp->files,esz,2,0)) >= 0) {
	    if ((cfname != nullptr) && (cfname[0] != '\0')) {
	        if ((rs = config_addfile(csp,cfname)) >= 0) {
	            rs = config_read(csp,cfname) ;
	        }
	    } else {
	        cfname = CONFIGFNAME ;
	        rs = config_load(csp,cfname) ;
	    }
#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("config_start: mid rs=%d\n",rs) ;
#endif
	    if (rs >= 0) {
	        csp->f_p = true ;
	        csp->magic = CONFIG_MAGIC ;
	    }
	    if (rs < 0)
	        vecobj_finish(&csp->files) ;
	} /* end if (vecobj_start) */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("config_start: ret rs=%d f=%u\n",rs,csp->f_p) ;
#endif

	return rs ;
} /* end subroutine (config_start) */

local int config_finish(CONFIG *csp) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	if (csp == nullptr) return SR_FAULT ;
	if (csp->magic != CONFIG_MAGIC) return SR_NOTOPEN ;
	{
	    rs1 = config_addfins(csp) ;
	    if (rs >= 0) rs = rs1 ;
	}
	{
	    rs1 = vecobj_finish(&csp->files) ;
	    if (rs >= 0) rs = rs1 ;
	}
	return rs ;
} /* end subroutine (config_finish) */

local int config_addfile(CONFIG *csp,cchar *cfn) noex {
	int		rs ;
	if (ustat sb ; (rs = u_stat(cfn,&sb)) >= 0) {
	    if (cchar *cp ; (rs = lm_strw(cfn,-1,&cp)) >= 0) {
	        CONFIG_FILE	f ;
	        f.fname = cp ;
	        f.mtime = sb.st_mtime ;
	        rs = vecobj_add(&csp->files,&f) ;
	        if (rs < 0) {
	            lm_free(cp) ;
		}
	    } /* end if (m-a) */
	} /* end if (stat) */
	return rs ;
} /* end subroutine (config_addfile) */

local int config_addfins(CONFIG *csp) noex {
	vecobj		*flp = &csp->files ;
	CONFIG_FILE	*ep ;
	int		rs = SR_OK ;
	int		rs1 ;
	int		i ;
	for (i = 0 ; (rs1 = vecobj_get(flp,i,&ep)) >= 0 ; i += 1) {
	    if (ep != nullptr) {
	        rs1 = lm_free(ep->fname) ;
	        if (rs >= 0) rs = rs1 ;
	    }
	} /* end for */
	if ((rs >= 0) && (rs1 != SR_NOTFOUND)) rs = rs1 ;
	return rs ;
} /* end subroutine (config_addfins) */

local int config_check(CONFIG *csp) noex {
	proginfo	*pip ;
	LI		*lip ;
	int		rs = SR_OK ;

	if (csp == nullptr) return SR_FAULT ;

	if (csp->magic != CONFIG_MAGIC) return SR_NOTOPEN ;

	pip = csp->pip ;
	lip = pip->lip ;
	if (lip == nullptr) return SR_FAULT ;

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("config_check: ent intconf=%u\n",lip->intconf) ;
#endif

	if (csp->f_p) {
	    ustat	sb ;
	    vecobj	*flp = &csp->files ;
	    CONFIG_FILE	*ep ;
	    int		f = false ;
	    for (int i = 0 ; vecobj_get(flp,i,&ep) >= 0 ; i += 1) {
	        if (ep != nullptr) {
	            if ((rs = u_stat(ep->fname,&sb)) >= 0) {
	                if (sb.st_mtime > ep->mtime) {
	                    ep->mtime = sb.st_mtime ;
	                    f = true ;
	                }
	            } else if (isNotPresent(rs)) {
	                rs = SR_OK ;
	            }
	        } /* end if */
	        if (f) break ;
	        if (rs < 0) break ;
	    } /* end for */
	    if ((rs >= 0) && f) {
	        if ((rs = locinfo_svclistdelall(lip)) >= 0) {
	            cchar	*fn = ep->fname ;
	            for (i = 0 ; vecobj_get(flp,i,&ep) >= 0 ; i += 1) {
	                if ((rs = u_stat(fn,&sb)) >= 0) {
	                    rs = config_read(csp,fn) ;
	                } else if (isNotPresent(rs)) {
	                    rs = SR_OK ;
	                }
	                if (rs < 0) break ;
	            } /* end for */
	        } /* end if (locinfo_svclistdelall) */
	    } /* end if */
	} /* end if (active) */

#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("config_check: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (config_check) */

local int config_load(CONFIG *csp,cchar *cfn) noex {
	proginfo	*pip = csp->pip ;
	vecstr		sv ;
	int		rs ;
	int		rs1 ;

	if ((rs = vecstr_start(&sv,3,0)) >= 0) {
	    int		i ;
	    cchar	*pn = pip->progname ;
	    cchar	*sn = pip->searchname ;
	    cchar	*pr ;
	    char	cbuf[MAXPATHLEN+1] ;
	    if ((rs = vecstr_envset(&sv,"n",sn,-1)) >= 0) {
	        for (i = 0 ; i < 2 ; i += 1) {
	            switch (i) {
	            case 0:
	                pr = pip->username ;
	                break ;
	            case 1:
	                pr = pip->pr ;
	                break ;
	            } /* end switch */
	            if ((rs = config_havefile(csp,&sv,cbuf,pr,cfn)) > 0) {
#if	CF_DEBUG
	                if (DEBUGLEVEL(4))
	                    debugprintf("config_load: cfn=%s\n",cbuf) ;
#endif
	                if (pip->debuglevel > 0) {
	                    shio_printf(pip->efp,"%s: cf=%s\n",pn,cbuf) ;
	                }

	                if ((rs = config_addfile(csp,cbuf)) >= 0) {
	                    rs = config_read(csp,cbuf) ;
#if	CF_DEBUG
	                    if (DEBUGLEVEL(4))
	                        debugprintf("config_load: _read() rs=%d\n",rs) ;
#endif
	                }
	            }
	            if (rs < 0) break ;
	        } /* end for */
	    } /* end if (vecstr_envset) */
	    rs1 = vecstr_finish(&sv) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (finding file) */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("config_load: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (config_load) */

local int config_havefile(CONFIG *csp,vecstr *svp,char *cbuf,
		cc *pr,cc* cfn) noex {
	cint		clen = MAXPATHLEN ;
	int		rs ;
	int		pl = 0 ;

	if (csp == nullptr) return SR_FAULT ;

	if ((rs = vecstr_envset(svp,"p",pr,-1)) >= 0) {
	    if ((rs = permsched(csched,svp,cbuf,clen,cfn,R_OK)) >= 0) {
	        pl = rs ;
	    } else if (isNotPresent(rs)) {
	        rs = SR_OK ;
	        cbuf[0] = '\0' ;
	    }
	} /* end if (str-add) */

	return (rs >= 0) ? pl : rs ;
} /* end subroutine (config_havefile) */

local int config_read(CONFIG *csp,cchar *cfname) noex {
	proginfo	*pip = csp->pip ;
	paramfile	p ;
	int		rs ;
	int		rs1 ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("config_read: ent cfn=%s\n",cfname) ;
#endif

#ifdef	COMMENT
	if (pip->debuglevel > 0)
	    shio_printf(pip->efp,"%s: cf=%s\n",pip->progname,cfname) ;
#endif

	if ((rs = paramfile_open(&p,pip->envv,cfname)) >= 0) {

	    rs = config_reader(csp,&p) ;

#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("config_read: _reader() rs=%d\n",rs) ;
#endif

	    rs1 = paramfile_close(&p) ;
	    if (rs >= 0) rs = rs1 ;
	} else if (isNotPresent(rs)) {
	    rs = SR_OK ;
	}

	return rs ;
} /* end subroutine (config_read) */

local int config_reader(CONFIG *csp,paramfile *pfp) noex {
	proginfo	*pip = csp->pip ;
	LI		*lip ;
	cint	vlen = VBUFLEN ;
	cint	elen = EBUFLEN ;
	int		rs = SR_OK ;
	int		rs1 ;
	int		i ;
	int		ml, vl, el ;
	int		v ;
	cchar		*pr = pip->pr ;
	char		vbuf[VBUFLEN + 1] ;
	char		ebuf[EBUFLEN + 1] ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("config_reader: ent f_p=%u\n",csp->f_p) ;
#endif

	lip = pip->lip ;
	if (lip == nullptr) return SR_FAULT ;

	for (i = 0 ; cparams[i] != nullptr ; i += 1) {

#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("config_reader: cp=%s\n",cparams[i]) ;
#endif

	    paramfile_cur	cur ;
	    if ((rs = paramfile_curbegin(pfp,&cur)) >= 0) {

	        while (rs >= 0) {
	            vl = paramfile_fetch(pfp,cparams[i],&cur,vbuf,vlen) ;
	            if (vl == SR_NOTFOUND) break ;
	            rs = vl ;
	            if (rs < 0) break ;

#if	CF_DEBUG
	            if (DEBUGLEVEL(4))
	                debugprintf("config_reader: vbuf=>%r<\n",vbuf,vl) ;
#endif

	            ebuf[0] = '\0' ;
	            el = 0 ;
	            if (vl > 0) {
	                rs = locinfo_cookexp(lip,0,ebuf,elen,vbuf,vl) ;
	                el = rs ;
	            }

#if	CF_DEBUG
	            if (DEBUGLEVEL(4))
	                debugprintf("config_reader: ebuf=>%r<\n",ebuf,el) ;
#endif

	            if ((rs >= 0) && (el > 0)) {
	                cchar	*sn = pip->searchname ;
	                char	tbuf[MAXPATHLEN + 1] ;
	                switch (i) {
	                case cparam_logsize:
	                    if (! pip->finval.logsize) {
	                        if ((rs = cfdecmfi(ebuf,el,&v)) >= 0) {
	                            if (v >= 0) {
	                                switch (i) {
	                                case cparam_logsize:
	                                    pip->logsize = v ;
	                                    break ;
	                                } /* end switch */
	                            }
	                        } /* end if (valid number) */
	                    }
	                    break ;
	                case cparam_basedir:
	                case cparam_basedb:
	                    if (! lip->finval.basedname) {
	                        if (el > 0) {
	                            lip->finval.basedname = true ;
	                            rs = locinfo_basedir(lip,ebuf,el) ;
	                        }
	                    }
	                    break ;
	                case cparam_logfile:
	                    if (! pip->finval.lfname) {
	                        cchar	*lc = LOGCNAME ;
	                        cchar	*lfn = pip->lfname ;
	                        cchar	*tfn = tbuf ;
	                        pip->finval.lfname = true ;
	                        pip->have.lfname = true ;
	                        ml = prmkfname(pr,tbuf,ebuf,el,true,lc,sn,"") ;
	                        if ((lfn == nullptr) || 
	                            (strcmp(lfn,tfn) != 0)) {
	                            cchar	**vpp = &pip->lfname ;
	                            pip->changed.lfname = true ;
	                            rs = proginfo_setentry(pip,vpp,tbuf,ml) ;
	                        }
	                    }
	                    break ;
	                case cparam_head:
	                    if (! lip->finval.hfname) {
	                        if (el > 0) {
	                            lip->finval.hfname = true ;
	                            rs = locinfo_sethead(lip,ebuf,el) ;
	                        }
	                    }
	                    break ;
	                case cparam_svcs:
	                    if (! lip->finval.svcs) {
	                        if (el > 0) {
	                            lip->have.svcs = true ;
	                            rs = locinfo_svclistadd(lip,ebuf,el) ;
	                        }
	                    }
	                    break ;
	                case cparam_copyright:
	                    if (lip->copyright == nullptr) {
	                        if (el > 0) {
	                            rs = locinfo_copyright(lip,ebuf,el) ;
	                        }
	                    }
	                    break ;
	                case cparam_webmaster:
	                    if (lip->webmaster == nullptr) {
	                        if (el > 0) {
	                            cchar	**vpp = &lip->webmaster ;
	                            rs = locinfo_setentry(lip,vpp,ebuf,el) ;
	                        }
	                    }
	                    break ;
	                case cparam_intrun:
	                    if (! pip->finval.intrun) {
	                        if (el > 0) {
	                            rs = cfdecti(ebuf,el,&v) ;
	                            pip->intrun = v ;
	                        }
	                    }
	                    break ;
	                case cparam_intpoll:
	                    if (! pip->finval.intpoll) {
	                        if (el > 0) {
	                            rs = cfdecti(ebuf,el,&v) ;
	                            pip->intpoll = v ;
	                        }
	                    }
	                    break ;
	                case cparam_intconf:
	                    if (! lip->finval.intconf) {
	                        if (el > 0) {
	                            rs = cfdecti(ebuf,el,&v) ;
	                            lip->intconf = v ;
	                        }
	                    }
	                    break ;
	                case cparam_intidle:
	                    if (! pip->finval.intidle) {
	                        if (el > 0) {
	                            rs = cfdecti(ebuf,el,&v) ;
	                            pip->intidle = v ;
	                        }
	                    }
	                    break ;
	                case cparam_intlock:
	                    if (! pip->finval.intlock) {
	                        if (el > 0) {
	                            rs = cfdecti(ebuf,el,&v) ;
	                            pip->intlock = v ;
	                        }
	                    }
	                    break ;
	                case cparam_intmark:
	                    if (! pip->finval.intmark) {
	                        if (el > 0) {
	                            rs = cfdecti(ebuf,el,&v) ;
	                            pip->intmark = v ;
	                        }
	                    }
	                    break ;
	                case cparam_intcache:
	                    if (! lip->finval.intcache) {
	                        if (el > 0) {
	                            rs = cfdecti(ebuf,el,&v) ;
	                            lip->intcache = v ;
	                        }
	                    }
	                    break ;
	                case cparam_intwait:
	                    if (! lip->finval.intwait) {
	                        if (el > 0) {
	                            rs = cfdecti(ebuf,el,&v) ;
	                            lip->intwait = v ;
	                        }
	                    }
	                    break ;
	                case cparam_intmaint:
	                    if (! lip->finval.intmaint) {
	                        if (el > 0) {
	                            rs = cfdecti(ebuf,el,&v) ;
	                            lip->intmaint = v ;
	                        }
	                    }
	                    break ;
	                } /* end switch */
	            } /* end if (got one) */

	        } /* end while (fetching) */

	        rs1 = paramfile_curend(pfp,&cur) ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (paramfile-cur) */

#if	CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("config_reader: bot rs=%d\n",rs) ;
#endif

	    if (rs < 0) break ;
	} /* end for (parameters) */

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("config_reader: ret rs=%d\n",rs) ;
#endif

	return rs ;
} /* end subroutine (config_reader) */

local int mkourname(PI *pip,char *rbuf,cchar *inter,cchar *sp,int sl) noex {
	int		rs = SR_OK ;
	cchar		*pr = pip->pr ;
	cchar		*sn = pip->searchname ;

#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_homepage/mkourname: ent int=%s s=%r\n",
	        inter,sp,sl) ;
#endif

	if (strnchr(sp,sl,'/') != nullptr) {
	    if (sp[0] != '/') {
	        rs = mkpath2w(rbuf,pr,sp,sl) ;
	    } else {
	        rs = mkpath1w(rbuf,sp,sl) ;
	    }
	} else {
	    rs = mkpath4w(rbuf,pr,inter,sn,sp,sl) ;
	}

#if	CF_DEBUG
	if (DEBUGLEVEL(4)) {
	    debugprintf("b_homepage/mkourname: ret rs=%d\n",rs) ;
	    debugprintf("b_homepage/mkourname: ret rbuf=%s\n",rbuf) ;
	}
#endif

	return rs ;
} /* end subroutine (mkourname) */

local int mkincfname(PI *pip,char *ibuf,cchar *sp,int sl) noex {
	int		rs ;
	if (sp[0] != '/') {
	    cchar	*pr = pip->pr ;
	    cchar	*sn = pip->searchname ;
	    cchar	*inter = ETCCNAME ;
	    rs = mkpath4w(ibuf,pr,inter,sn,sp,sl) ;
	} else {
	    rs = mkpath1w(ibuf,sp,sl) ;
	}
	return rs ;
} /* end subroutine (mkincfname) */


