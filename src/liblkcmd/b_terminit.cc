...
int b_terminit(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
/* end subroutine (b_terminit) */


int p_terminit(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	return mainsub(argc,argv,envv,contextp) ;
}
/* end subroutine (p_terminit) */
...
static int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_terminit: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_terminit: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
/* statvfs initialization */
	...
	if ((statfname == NULL) || (statfname[0] == '\0')) {
	    ex = EX_NOINPUT ;
	...
	rs = statvfsdir(statfname,&lip->fss) ;
	if (rs < 0) {
	    ex = EX_NOINPUT ;
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_terminit: final mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
static int procspec(PROGINFO *pip,void *ofp,cchar req[])
{
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_terminit/procspec: req=>%s<\n",req) ;
#endif
	...
	switch (ri) {
		    ...
		    case qopt_fsutil:
			{
			    ...
			    if (fssp->f_blocks != 0) {
			        int per = +(f_bused * 100) / fssp->f_blocks ;
			...
			break ;
		    case qopt_fstype:
			rs = snwcpy(cvtbuf,CVTBUFLEN,fssp->f_basetype,FSTYPSZ) ;
			break ;
		    case qopt_fsstr:
			rs = snwcpy(cvtbuf,CVTBUFLEN,fssp->f_fstr,32) ;
			break ;
		    case qopt_fsid:
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_terminit/procspec: ret rs=%d wlen=%u\n",rs,wlen) ;
#endif
