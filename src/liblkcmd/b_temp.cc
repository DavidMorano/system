...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_temp: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_temp: final mallout=%u\n",mo-mo_start) ;
	    uc_mallset(0) ;
	}
#endif
...
local int procquery(PROGINFO *pip,void *ofp,cchar qp[],int ql)
{
	...
	if ((cl = sfshrink(qp,ql,&cp)) > 0) {
	    ...
	    if ((rs >= 0) && ((rs = cfdecd(cp,cl,&v)) >= 0)) {
	        ...
	        vc = cvt_celsius(vk) ;
	        rs = shio_printf(ofp,"%6.1f°F %6.1f°C\n",vf,vc) ;
	        wlen += rs ;
	    } /* end if (value) */
	} /* end if (sfshrink) */
