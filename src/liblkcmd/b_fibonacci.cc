/* b_fibonacci SUPPORT (KSH builtin) /
/* charset=ISO8859-1 */
...
int b_fibonacci(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
/* end subroutine (b_fibonacci) */


int p_fibonacci(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
...
static int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_fibonacci: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_fibonacci: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_fibonacci/procspec: name=%r\n",np,nl) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_fibonacci/procspec: ret rs=%d wlen=%u\n",rs,wlen) ;
#endif
