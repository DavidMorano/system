/* b_factorial SUPPORT (KSH builtin) */
/* charset=ISO8859-1 */
...
local int	procspec(PROGINFO *,void *,const char *,int) ;
...
int b_factorial(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
/* end subroutine (b_factorial) */


int p_factorial(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
...
local int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_factorial: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_factorial: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
local int procspec(PROGINFO *pip,void *ofp,cchar *np,int nl)
{
	int		n ;
	int		rs ;
	int		wlen = 0 ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("b_factorial/procspec: name=%r\n",np,nl) ;
#endif
