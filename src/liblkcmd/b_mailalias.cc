...
int b_mailalias(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
int p_mailalias(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
...
static int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	const char	*madomain = NULL ;
	const char	*maprofile = NULL ;
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_mailalias: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_mailalias: debuglevel=%u\n",pip->debuglevel) ;
#endif

	if (pip->debuglevel > 0) {
	    shio_printf(pip->efp,"%s: verboselevel=%u\n",
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_mailalias: done w/ proc options\n") ;
#endif

	if (afname == NULL) afname = getourenv(envv,VARAFNAME) ;

	if ((pip->debuglevel > 0) && (madomain != NULL)) {
	    shio_printf(pip->efp,"%s: madomain=%s\n",
	        pip->progname,madomain) ;
	}

/* open the database */

	if ((maprofile == NULL) || (maprofile[0] == '-'))
	    maprofile = DEFMAFNAME ;

	if ((rs >= 0) && (pip->debuglevel > 0)) {
	    shio_printf(pip->efp,"%s: maprofile=%s\n",
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_mailalias: final mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
static int madump(PROGINFO *pip,cchar *dumpfname)
{
	LOCINFO		*lip = pip->lip ;
	...
	if ((rs = shio_open(dfp,dumpfname,"rwct",0666)) >= 0) {
	    MAILALIAS		*map = &lip->madb ;
	    MAILALIAS_CUR	cur ;
	    ...
	    if ((rs = mailalias_curbegin(map,&cur)) >= 0) {
	        ...
	        while (rs >= 0) {
	            ...
	            if (rs >= 0) {
	                c += 1 ;
	                rs = madprint(pip,dfp,kbuf,vbuf) ;
	            }

	        } /* end while */

	        rs1 = mailalias_curend(&lip->madb,&cur) ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (cursor) */
