...
int b_fsinfo(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
/* end subroutine (b_fsinfo) */


int p_fsinfo(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	return mainsub(argc,argv,envv,contextp) ;
}
/* end subroutine (p_fsinfo) */
...
static int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_fsinfo: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_fsinfo: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
static int usage(PROGINFO *pip)
{
	...
	if (rs >= 0) {
	    fmt = "%s: USAGE> %s <file> [<spec(s)> ...] [-af <afile>]\n" ;
...
static int procspec(PROGINFO *pip,void *ofp,cchar *rp,int rl)
{
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_fsinfo/procspec: matostr() rc=%d\n",ri) ;
#endif
...
#if	CF_DEBUG
	    if (DEBUGLEVEL(2))
	        debugprintf("b_fsinfo/procspec: matextra() rc=%d\n",ri) ;
#endif
	...
	case qopt_fsutil:
	    ...
	    break ;
	case qopt_fstype:
	    rs = snwcpy(cvtbuf,cvtlen,fssp->f_basetype,FSTYPSZ) ;
	    break ;
	case qopt_fsstr:
	    rs = snwcpy(cvtbuf,cvtlen,fssp->f_fstr,32) ;
	    break ;
	case qopt_fsid:
#if	CF_HEXID
	    rs = cthexul(cvtbuf,cvtlen,fssp->f_fsid) ;
#else
	    rs = ctdecul(cvtbuf,cvtlen,fssp->f_fsid) ;
#endif /* CF_HEXID */
	    break ;
	case qopt_fsflags:
#if	CF_DEBUG
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_fsinfo/procspec: ret rs=%d wlen=%u\n",rs,wlen) ;
#endif
...
static int locinfo_finish(LOCINFO *lip)
{
	...
	if (lip->homedname != NULL) {
	    rs1 = uc_free(lip->homedname) ;
	    if (rs >= 0) rs = rs1 ;
	    lip->homedname = NULL ;
	}
...
static int locinfo_sethome(LOCINFO *lip,cchar *hbuf,int hlen)
{
	...
	if (lip->homedname == NULL) {
	    cchar	*ccp ;
	    rs = uc_mallocstrw(hbuf,hlen,&ccp) ;
	    if (rs >= 0) lip->homedname = ccp ;
	}
...
/* end subroutine (locinfo_setpath) */
