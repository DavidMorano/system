...
int b_shells(int argc,cchar *argv[],void *contextp)
{
	int		rs ;
	int		rs1 ;
	int		ex = EX_OK ;
...
/* end subroutine (b_shells) */


int p_shells(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	return mainsub(argc,argv,envv,contextp) ;
}
/* end subroutine (p_shells) */
...
static int mainsub(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
	...
	if ((cp = getourenv(envv,VARDEBUGFNAME)) != NULL) {
	    rs = debugopen(cp) ;
	    debugprintf("b_shells: starting DFD=%d\n",rs) ;
	}
#endif /* CF_DEBUGS */
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_shells: debuglevel=%u\n",pip->debuglevel) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	debugprintf("b_shells: exiting ex=%u (%d)\n",ex,rs) ;
#endif
...
badprogstart:
...
#if	(CF_DEBUGS || CF_DEBUG) && CF_DEBUGMALL
	{
	    uint	mo ;
	    uc_mallout(&mo) ;
	    debugprintf("b_shells: final mallout=%u\n",(mo-mo_start)) ;
	    uc_mallset(0) ;
	}
#endif /* CF_DEBUGMALL */
...
static int process(PROGINFO *pip,const char *ofn)
{
	...
	if ((rs = shio_open(ofp,ofn,"wct",0666)) >= 0) {
	    ...
	    if ((rs = sysusershells_open(&us,NULL)) >= 0) {
		const int	rlen = MAXPATHLEN ;
		char		rbuf[MAXPATHLEN+1] ;
		while ((rs = sysusershells_readent(&us,rbuf,rlen)) > 0) {
		    rs = shio_print(ofp,rbuf,rs) ;
		    if (rs < 0) break ;
		} /* end while */
		rs1 = sysusershells_close(&us) ;
		if (rs >= 0) rs = rs1 ;
	    } /* end if (getus) */
