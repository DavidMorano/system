...
#if	CF_DEBUG
	    if (pip->debuglevel > 1)
	        eprintf("checkname: ctime was zero\n") ;
#endif
...
#if	CF_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("checkname: name was a dotter\n") ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    eprintf("checkname: symbolic link ?\n") ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    eprintf("checkname: directory or dngling symbolic link ?\n") ;
#endif
...
#if	CF_DEBUG
	    if (pip->debuglevel > 1)
	        eprintf("checkname: suffix requested\n") ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    eprintf("checkname: addsize() size=%ld\n",sbp->st_size) ;
#endif
