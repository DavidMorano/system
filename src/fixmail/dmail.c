...
/* write out the envvment to the log file */

/*
	Since we are using the 'execvp(2)' call later, let's use this
	opportunity to ensure that the we have a good PATH envvment
	variable.
*/

	bprintf(ofp,"- envvment\n") ;

	for (i = 0 ; envv[i] != ((char *) 0) ; i += 1) {

	    bprintf(ofp,"	%s\n",envv[i]) ;

/* if an envvment variable is messaed up, remove and insert a substitue */
...
/* add a path to the envvment if there was not one */
