...
extern int	bopen(), bclose(), bseek() ;
extern int	bgetline(), bcopyblock() ;
...
/* initialize some basic stuff */
	...
	if ((rs = bopen(ifp,infname,"r",0666)) < 0) goto badopenin ;
...
/* copy the input file to a temporary file if necessary (if not seekable) */
	...
	if ((rs = bseek(ifp,0L,SEEK_CUR)) < 0) {
		...
		sprintf(buf,"%s/easXXXXXXXXXXX",g.tmpdir) ;

	    if ((rs = mktmpfile(buf,0644,tmpfname)) < 0)
	        goto badtmpmake ;
...
/* are we in "separate" mode or not ? */
	...
	if (! gdp->f.separate) {
	    ...
	    if (outfname == NULL) outfname = BIO_STDOUT ;

	    if ((rs = bopen(gdp->ofp,outfname,"wct",0666)) < 0)
	        goto badopenout ;
...
/* try to get a subcricuit "name" */
	...
	if (gdp->debuglevel > 1) {
	    ...
	    eprintf("gotcircuit: cp=%w\n",cp,10) ;

		l = strlen(cp) ;

		eprintf("gotcircuit: len=%d\n",l) ;
...
struct circuit	*cirp ;
{
	...
	if (BTST(cirtype,cirp->type)) return TRUE ;
