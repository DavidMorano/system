/* hostaddrinfo_main SUPPORT (HOSTADDRINFO) */
/* charset=ISO8859-1 */
/* lang=C++20 */
...
local int	procinfo(PI *,bfile *,char *,cchar *,int) ;
local int	procprint4(PI *,bfile *,cchar *,cuchar *,int) ;
local int	procprint6(PI *,bfile *,cchar *,cuchar *,int) ;

local int	prepname_start(PREPNAME *,cchar *,int) ;
local int	prepname_finish(PREPNAME *) ;
...
local int	morder_ruint(cvoid *,uint *) ;
...
local int procname(PI *pip,bfile *ofp,cchar *np)
{
	NULSTR		n ;
	int		rs ;
	int		rs1 ;
...
local int procspecial(PI *pip,bfile *ofp,cchar *np,int nl)
{
	LOCINFO		*lip = pip->lip ;
	int		rs = SR_OK ;
	int		f = true ;
...
local int procinfo(PI *pip,bfile *ofp,char *cname,cchar *np,int nl)
{
	LOCINFO		*lip = pip->lip ;
	PREPNAME	pn ;
	int		rs ;
	...
	if ((rs = prepname_start(&pn,np,nl)) >= 0) {
	    HOSTINFO		hi ;
	    HOSTINFO_CUR	cur ;
	    cint		f_all = (! lip->fl.ao) ;
	    cchar		*hn = pn.hostname ;
...
local int procprint4(PI *pip,bfile *ofp,cchar *indent,
		const uchar *ap,int al)
{
	...
	inetaddr ia ;
	if ((rs = inetaddr_start(&ia,at,ap,-1)) >= 0) {
	    cint	aslen = INET4_ADDRSTRLEN ;
...
local int procprint6(PI *pip,bfile *ofp,cchar *indent,
		const uchar *ap,int al) noex {
	LOCINFO		*lip = pip->lip ;
...
local int prepname_start(PREPNAME *ep,cchar *namp,int naml) noex {
	int		rs = SR_FAULT ;
	int		f = false ;
	if (ep && namp) {
	    memclear(ep) ;
...
/* machine-order */
local int morder_ruint(cvoid *buf,uint *vp) noex {
	void		*vbuf = (void *) buf ;
