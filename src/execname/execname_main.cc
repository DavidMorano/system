...
local int	procexecbegin(PROGINFO *,ARGINFO *,cchar *) ;
local int	procexecend(PROGINFO *) ;
local int	procexecbeginer(PROGINFO *,ARGINFO *,cchar *) ;
local int	procexecargz(PROGINFO *,ARGINFO *,cchar *) ;
local int	procexecname(PROGINFO *,ARGINFO *,cchar *) ;
local int	procexecgo(PROGINFO *,ARGINFO *) ;

local int	procexecvarspecials(PROGINFO *) ;
local int	procexecvarclean(PROGINFO *) ;
local int	procexecvarcleaner(PROGINFO *,cchar *) ;
local int	procexecvarcleanmk(PROGINFO *,cchar *,int,cchar *,int) ;
local int	procexecaddenv(PROGINFO *,cchar *,int,cchar *,int) ;

local int	findfilex(IDS *,char *,cchar *) ;

#if	CF_ISBADENV
local int	isbadenv(cchar *) ;
local int	hasourbad(cchar *,int) ;
#endif

#if	CF_DEBUGS && CF_SOFAR
local int	sofar(cchar *,int) ;
#endif
...
local int procexecbegin(PROGINFO *pip,ARGINFO *aip,cchar *program)
{
	cint	size = sizeof(PROGEXEC) ;
	int		rs ;
	void		*p ;
...
local int procexecend(PROGINFO *pip)
{
	PROGEXEC	*pep ;
	int		rs = SR_OK ;
	int		rs1 ;
...
local int procexecargz(PROGINFO *pip,ARGINFO *aip,cchar *program)
{
...
local int procexecbeginer(PROGINFO *pip,ARGINFO *aip,cchar *program)
{
	PROGEXEC	*pep = pip->progexec ;
	cint	ssize = 1024 ;
	cint	vn = 150 ;
...
local int procexecname(PROGINFO *pip,ARGINFO *aip,cchar *program)
{
	PROGEXEC	*pep = pip->progexec ;
	IDS		id ;
	int		rs ;
...
local int procexecgo(PROGINFO *pip,ARGINFO *aip)
{
	PROGEXEC	*pep = pip->progexec ;
	int		rs ;
	cint	n = (aip->argr+1) ;
...
local int procexecvarspecials(PROGINFO *pip)
{
	PROGEXEC	*pep = pip->progexec ;
...
local int procexecvarclean(PROGINFO *pip)
{
	PROGEXEC	*pep = pip->progexec ;
	vechand 	*elp ;
	int		rs = SR_OK ;
...
local int procexecvarcleaner(PROGINFO *pip,cchar *ep)
{
	cint	el = strlen(ep) ;
	int		rs = SR_OK ;
	int		vl = 0 ;
...
local int procexecaddenv(PROGINFO *pip,cchar *kp,int kl,cchar *vp,int vl)
{
	PROGEXEC	*pep = pip->progexec ;
	strpack		*spp ;
	int		rs ;
...
local int procexecvarcleanmk(PROGINFO *pip,cchar *kp,int kl,cchar *vp,int vl)
{
	DIRLIST		db ;
	int		rs ;
	int		rs1 ;
...
local int isbadenv(cchar *envp) noex {
	uint		a ;
	cint		ps = getpagesize() ;
	int		sl ;
...
local int hasourbad(cchar *sp,int sl)
{
	uint		sch ;
	register int	f = TRUE ;
...
#if	CF_DEBUGS && CF_SOFAR
local int sofar(cchar *bp,int bl) {
	int		cl ;
	cchar	*tp ;
