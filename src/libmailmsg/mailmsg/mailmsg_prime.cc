...
static int mailmsg_ctor(mailmsg *op,Args ... args) noex {
	...
	if (op && (args && ...)) ylikely {
	    rs = SR_NOMEM ;
	    memclear(hop) ;
	    if ((op->slp = new(nothrow) strpack) != np) ylikely {
	        if ((op->elp = new(nothrow) vecobj) != np) ylikely {
	            if ((op->hlp = new(nothrow) vecobj) != np) ylikely {
			rs = SR_OK ;
		    } /* end if (new-vecobj) */
		    if (rs < 0) {
		        delete op->elp ;
...
int mailmsg_start(mailmsg *op) noex {
	int		rs ;
	if ((rs = mailmsg_ctor(op)) >= 0) ylikely {
...
int mailmsg_finish(mailmsg *op) noex {
	int		rs ;
	int		rs1 ;
	if ((rs = mailmsg_magic(op)) >= 0) ylikely {
	    {
...
int mailmsg_loadline(mailmsg *op,cchar *lp,int ll) noex {
	int		rs ;
	if ((rs = mailmsg_magic(op,lp)) >= 0) ylikely {
...
int mailmsg_envaddress(mailmsg *op,int i,cchar **rpp) noex {
	...
	if ((rs = mailmsg_magic(op)) >= 0) ylikely {
	    cchar	*rp = nullptr ;
	    if (void *vp ; (rs = vecobj_get(op->elp,i,&vp)) >= 0) ylikely {
	        if (vp) {
...
int mailmsg_hdrcount(mailmsg *op,cchar *name) noex {
	int		rs ;
	int		c = 0 ;
	if ((rs = mailmsg_magic(op,name)) >= 0) ylikely {
	    MMHNAME	*hnp{} ;
...
static int msghdrname_start(MMHNAME *hnp,cc *hp,int hl,cc *valp,int vall) noex {
	...
	hnp->namelen = 0 ;
	hnp->lastinst = -1 ;
	if ((rs = vecobj_start(ilp,isz,2,0)) >= 0) ylikely {
	    if (cchar *cp ; (rs = libmem.strw(hp,hl,&cp)) >= 0) ylikely {
	        hnp->name = cp ;
	        hnp->namelen = (rs-1) ;
...
static int msghdrname_addnew(MMHNAME *hnp,cchar *vp,int vl) noex {
	...
	int		rs ;
	int		f = false ;
	if (void *p ; (rs = vecobj_addnew(ilp,&p)) >= 0) ylikely {
...
static int msghdrname_iline(MMHNAME *hnp,int hi,int li,cchar **rpp) noex {
	...
	int		rs ;
	int		len = 0 ;
	if (void *vp ; (rs = ilp->get(hi,&vp)) >= 0) ylikely {
	    MMHINST	*hip = (MMHINST *) vp ;
	    if (vp) {
...
static int msghdrinst_start(MMHINST *hip,cchar *vp,int vl) noex {
	...
	hip->vp = nullptr ;
	hip->vl = 0 ;
	if ((rs = vecobj_start(&hip->vals,vsz,4,0)) >= 0) ylikely {
	    if ((vl > 0) && vp) {
...
static int msghdrinst_add(MMHINST *hip,cchar *vp,int vl) noex {
	...
	if (vl > 0) {
	    vecobj	*vlp = &hip->vals ;
	    if (void *p ; (rs = vecobj_addnew(vlp,&p)) >= 0) ylikely {
...
static int msghdrinst_ival(MMHINST *hip,int li,cchar **rpp) noex {
	...
	int		vl = 0 ;
	cchar		*rp = nullptr ;
	if (void *vp ; (rs = vecobj_get(&hip->vals,li,&vp)) >= 0) ylikely {
