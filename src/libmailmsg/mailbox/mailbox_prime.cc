...
#include	<filer.h>
#include	<filer_mailsup.h>	/* MAILMSG */
#include	<fbliner.h>
#include	<ascii.h>
#include	<vecobj.h>
#include	<vecstr.h>
...
#include	<char.h>
#include	<hasx.h>		/* |hasEOH(3uc)| */
#include	<ischarx.h>
#include	<localmisc.h>
#include	<mailmsghdrval.h>	/* MAILMSG */
#include	<mailmsghdrfold.h>	/* MAILMSG */
...
#define	MB_READ			mailbox_read
#define	MB_READ			mailbox_read
#define	MB_READSIZE		MAILBOX_READSIZE
...
local int mailbox_dtor(MB *op) noex {
	int		rs = SR_FAULT ;
	if (op) ylikely {
	    rs = SR_OK ;
	    if (op->mlp) ylikely {
...
local inline bool hasfmat(cc *lp,int ll) noex {
    	return ((ll > 5) && (lp[0] == 'F')) ;
}
...
constexpr cpcchar	msghdrkeys[] = {
	[headkey_clen]		= "content-length",
	[headkey_clines]	= "content-lines",
	[headkey_lines]		= "lines",
	[headkey_xlines]	= "x-lines",
	[headkey_overlast]	= nullptr
} ; /* end array */
...
int mailbox_open(MB *op,cchar *mbfname,int mflags) noex {
	int		rs ;
	int		nmsgs = 0 ;
	if ((rs = mailbox_ctor(op,mbfname)) >= 0) ylikely {
	    rs = SR_INVALID ;
...
int mailbox_close(MB *op) noex {
	int		rs ;
	int		rs1 ;
	int		mblen = 0 ;
	if ((rs = mailbox_magic(op)) >= 0) ylikely {
	    bool	f = true ;
	    f = f && (! op->fl.readonly) ;
...
int mailbox_mbfile(MB *op,char *mbuf,int mlen) noex {
	int		rs ;
	if ((rs = mailbox_magic(op,mbuf)) >= 0) ylikely {
...
int mailbox_getinfo(MB *op,mailbox_info *mip) noex {
	int		rs ;
	int		total = 0 ;
	if ((rs = mailbox_magic(op,mip)) >= 0) ylikely {
	    memclear(mip) ;
...
int mailbox_check(MB *op,time_t dt) noex {
	int		rs ;
	int		f = false ;
	if ((rs = mailbox_magic(op)) >= 0) ylikely {
...
int mailbox_msgdel(MB *op,int mi,int f) noex {
	int		rs ;
	int		f_prev = false ;
	if ((rs = mailbox_magic(op)) >= 0) ylikely {
	    rs = SR_INVALID ;
...
int mailbox_countdel(MB *op) noex {
	int		rs ;
	int		count = 0 ;
	if ((rs = mailbox_magic(op)) >= 0) ylikely {
	    count = op->msgs_del ;
...
int mailbox_msgoff(MB *op,int mi,off_t *offp) noex {
	int		rs ;
	int		mlen = 0 ;
	if ((rs = mailbox_magic(op,offp)) >= 0) ylikely {
	    rs = SR_INVALID ;
	    if (mi >= 0) {
		rs = SR_NOTFOUND ;
	        if (mi < op->msgs_total) {
...
int mailbox_msgret(MB *op,int mi,MB_MI **rpp) noex {
	int		rs ;
	int		mlen = 0 ;
	if ((rs = mailbox_magic(op,rpp)) >= 0) ylikely {
	    rs = SR_INVALID ;
	    if (mi >= 0) {
	        rs = SR_NOTFOUND ;
	        if (mi < op->msgs_total) {
...
int mailbox_msghdradd(MB *op,int mi,cchar *k,cchar *sp,int sl) noex {
	int		rs ;
	int		rv = 0 ;
	if ((rs = mailbox_magic(op,k)) >= 0) ylikely {
	    rs = SR_INVALID ;
	    if (k[0]) {
    		vecobj	*mlp = op->mlp ;
	        if (void *vp{} ; (rs = mlp->get(mi,&vp)) >= 0) {
...
int mailbox_readbegin(MB *op,MB_READ *curp,off_t foff,int rsz) noex {
	int		rs ;
	if ((rs = mailbox_magic(op,curp)) >= 0) ylikely {
	    memclear(curp) ;
	    if (rsz <= 0) rsz = MB_READSIZE ;
...
int mailbox_readend(MB *op,MB_READ *curp) noex {
	int		rs ;
	int		rs1 ;
	if ((rs = mailbox_magic(op,curp)) >= 0) ylikely {
	    rs = SR_BADFD ;
...
int mailbox_readln(MB *op,MB_READ *curp,char *lbuf,int llen) noex {
	int		rs ;
	int		tlen = 0 ;
	if ((rs = mailbox_magic(op,curp,lbuf)) >= 0) ylikely {
	    cint	mfd = op->mfd ;
	    int		i{} ;
	    int		mlen ;
	    char	*lbp = lbuf ;
...
local int mailbox_opener(MB *op,cc *mbfname,int of) noex {
	int		rs ;
	int		nmsgs = 0 ;
	cmode		om = 0666 ;
...
local int mailbox_parse(MB *op) noex {
	coff		soff = 0z ;
	...
	if (filer fb ; (rs = fb.start(op->mfd,soff,bsz,0)) >= 0) ylikely {
	    fbliner	ls, *lsp = &ls ;
...
local int mailbox_parsemsg(MB *op,fbliner *lsp,int mi) noex {
	int		rs ;
	int		rs1 ;
	int		ll = 0 ;
	if (MAILBOXPI pi ; (rs = mailboxpi_start(&pi,lsp,mi)) >= 0) ylikely {
	    mmenvdat	me ;
	    int		vi = 0 ;
	    /* find message start */
...
local int mailbox_parsemsger(MB *op,mmenvdat *mep,MAILBOXPI *pip) noex {
	fbliner		*lsp = pip->lsp ;
	cint		mi = pip->mi ;
	int		rs ;
	int		rs1 ;
	int		ll = 0 ;
	MB_MI		msg, *msgp = &msg ;
	if ((rs = msginfo_start(msgp,lsp->poff,mi)) >= 0) ylikely {
	    mailmsghdrval	mhv ;
	    int		hi = 0 ;
	    int		kl = 0 ;
	    int		vl = 0 ;
...
local int mailbox_loadmsghead(MB *op,MB_MI *msgp,
...
local int mailbox_msgfins(MB *op) noex {
    	vecobj		*mlp = op->mlp ;
	int		rs = SR_OK ;
	int		rs1 ;
...
local int mailbox_rewrite(MB *op) noex {
	int		rs ;
	int		rs1 ;
	int		rv = 0 ;
	if (char *dbuf ; (rs = lm_mp(&dbuf)) >= 0) ylikely {
	    cint	dlen = rs ;
	    if ((rs = tmpmailboxes(dbuf,dlen)) >= 0) ylikely {
	        cchar	*mb = "mbXXXXXXXXXXXX" ;
	        if (char *tpat ; (rs = lm_mp(&tpat)) >= 0) {
...
local int mailbox_rewrites(MB *op,cchar *tpat) noex {
	int		rs ;
	int		rs1 ;
	int		rv = 0 ;
	if (sigblocker sb ; (rs = sb.start(sigblockers)) >= 0) ylikely {
...
local int mailbox_rewriter(MB *op,int tfd) noex {
	cint		ps = op->pagesize ;
	int		rs ;
	int		rs1 ;
	int		bl = op->pagesize ;
	int		wlen = 0 ;
	if (char *bp ; (rs = libmem.vall(bl,&bp)) >= 0) ylikely {
	    filer	fb, *fbp = &fb ;
	    MSGCOPY	mc{} ;
	    off_t	mbchange = -1 ;
	    cint	mfd = op->mfd ;
...
local int mailbox_msgcopy(MB *op,MSGCOPY *mcp,MB_MI *mip) noex {
	int		rs = SR_OK ;
	int		mfd = op->mfd ;
	int		wlen = 0 ;
...
local int mailbox_msgcopyadd(MB *op,MSGCOPY *mcp,MB_MI *mip) noex {
	cint		mfd = op->mfd ;
...
local int msginfo_start(MB_MI *mp,off_t off,int mi) noex {
	int		rs = SR_OK ;	
	if (mp) ylikely {
	    rs = memclear(mp) ;
	    mp->msgi = mi ;
...
local int msginfo_finish(MB_MI *mp) noex {
	int		rs = SR_FAULT ;
	int		rs1 ;
	if (mp) ylikely {
	    rs = SR_OK ;
	    if (mp->fl.hdradds) ylikely {
	        mp->fl.hdradds = false ;
...
local int msginfo_setenv(MB_MI *msgp,mmenvdat *mep) noex {
	int		rs = SR_FAULT ;
	if (msgp && mep) ylikely {
	    rs = SR_OK ;
	    msgp->fl.env = true ;
...
local int mailboxpi_start(MAILBOXPI *pip,fbliner *lsp,int mi) noex {
	int		rs = SR_FAULT ;
	if (pip && lsp) ylikely {
...
local int mailboxpi_finish(MAILBOXPI *pip) noex {
	int		rs = SR_FAULT ;
	if (pip) ylikely {
	    rs = SR_OK ;
	} /* end if (non-null) */
...
local int mailboxpi_havemsg(MAILBOXPI *pip) noex {
	return (pip->fl.fenv || pip->fl.fhdr || pip->fl.feoh) ;
}
/* end subroutine (mailboxpi_havemsg) */

local int writeblanks(int mfd,int len) noex {
	int		rs ;
	int		rs1 ;
