...
local int	procargs(PI *,ARGINFO *,bits *,vecstr *,cchar *) ;
local int	procout(PI *,vecstr *,cchar *) ;
local int	procorg(PI *) ;
local int	client(PI *,bfile *,int,int,int,cchar *,cchar *,cchar *) ;
local int	server(PI *) ;
...
local int	locinfo_defs(LOCINFO *) ;
local int	locinfo_dialer(LOCINFO *,cchar *) ;

#if	CF_LOCSETENT
local int	locinfo_setentry(LOCINFO *,cchar **,cchar *,int) ;
#endif

local int	anyformat(bfile *,int,int) noex ;
...
int main(int argc,con mainv argv,con mainv envv) {
	...
	cchar	*dialerspec = "TCP" ;
	cchar	*addrspec = nullptr ;
	cchar	*wlspec = nullptr ;
	cchar	*cp ;
...
/* check arguments */
	...
	if (wlspec == nullptr) wlspec = argval ;

	if ((rs >= 0) && (lip->wl < 0) && (wlspec != nullptr)) {
	...
	if (rs >= 0) {
	    if ((dialerspec == nullptr) || (dialerspec[0] == '\0')) {
	        rs = SR_INVALID ;
	    }
	}
...
local int process(PI *pip,ARGINFO *aip,bits *bop,cc *afn,cc *ofn) noex {
	vecstr		names ;
	cint	vo = 0 ;
	int		rs ;
	int		rs1 ;
...
local int procargs(PI *pip,ARGINFO *aip,bits *bop,vecstr *nlp,cc *afn)
{
	int		rs = SR_OK ;
	int		rs1 ;
	int		pan = 0 ;
...
local int procout(PI *pip,vecstr *nlp,cchar *ofn) noex {
	LOCINFO		*lip = pip->lip ;
	...
	if ((rs = bopen(ofp,ofn,"wct",0666)) >= 0) {
	    ...
	    cint	af = lip->af ;
	    int		i ;
	    cchar	*pspec = lip->pspec ;
	    cchar	*sspec = lip->sspec ;
	    cchar	*cp ;
...
local int client(PI *pip,bfile *ofp,int wl,int af,int to,
		cc *name,cc *pspec,cc *svc) noex {
	LOCINFO		*lip = pip->lip ;
	cint	opts = 0 ;
	cint	dlen = DBUFLEN ;
	int		rs ;
	...
	if ((rs = opendial(di,af,name,pspec,svc,nullptr,nullptr,to,opts)) >= 0) {
	    cint	s = rs ;
...
/* handle case of data-gram transports */
	    ...
	    if (isasocket(fd_portal)) {
	        ...
	        if ((rs = u_getsockopt(fd_portal,sol,cmd,&optv,&optl)) >= 0) {
	            ...
	            SOCKADDRESS		from ;
	            cint 		size = NIOVECS * sizeof(struct iovec) ;
...
local int locinfo_defs(LOCINFO *lip) noex {
	if (lip->af < 0) lip->af = AF_UNSPEC ;
	if ((lip->sspec == nullptr) || (lip->sspec[0] == '\0')) {
	    lip->sspec = INETSVC_DAYTIME ;
	}
	return SR_OK ;
} /* end subroutine (locinfo_defs) */

local int locinfo_dialer(LOCINFO *lip,cchar *dialerspec) noex {
	cint	dlen = DBUFLEN ;
	int		rs ;
	cchar		*tp ;
	char		dbuf[DBUFLEN+1] ;
	if ((tp = strchr(dialerspec,':')) != nullptr) {
	    lip->pspec = (tp + 1) ;
...
local int anyformat(bfile *ofp,int s,int to) noex {
	cint	llen = LINEBUFLEN ;
	int		rs ;
	int		len ;
	int		wlen = 0 ;
	...
	while ((rs = uc_readlnto(s,lbuf,llen,to)) > 0) {
	    len = rs ;
