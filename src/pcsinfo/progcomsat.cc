...
#if	CF_DEBUG
	                    if (DEBUGLEVEL(4))
	                        debugprintf("progcomsat/recips: cm=>%r<\n",
	                            cbuf,cl) ;
#endif
...
static int mkcsmsg(char *mbuf,int mlen,cchar *up,int ul,uint val) noex {
	int		rs ;
	int		rs1 ;
	if (sbuf b ; (rs = sbuf_start(&b,mbuf,mlen)) >= 0) {
	    {
	        sbuf_strw(&b,up,ul) ;
	        sbuf_chr(&b,'@') ;
	        sbuf_deci(&b,val) ;
