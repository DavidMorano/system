...
static int bvi_checkupdate(BVI *op,time_t dt)
{
	...
	if ((rs1 = u_stat(op->fname,&sb)) >= 0) {
	    if ((sb.st_mtime > mip->ti_mod) || (sb.st_mtime > mip->ti_map)) {
	        f = TRUE ;
	        bvi_loadend(op) ;
...
#if	CF_DEBUGS
	debugprintf("bvi_loadbve: loff[%u]=%u\n",i,lt[li+i][0]) ;
	debugprintf("bvi_loadbve: llen[%u]=%u\n",i,lt[li+i][1]) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("bvi_loadbve: ret rs=%d rlen=%u\n",rs,rlen) ;
#endif
