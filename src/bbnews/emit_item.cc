...
/* go through the header loops */
	...
	f_board = FALSE ;
	f_newsgroups = FALSE ;
	while ((len = breadln(afp,linebuf,LINEBUFLEN)) > 0) {
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("emit_article: before 'breadln()'\n") ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("emit_article: past 'breadln()'\n") ;
#endif
...
/* help */
	case '?':
		...
		pip->helpfname) ;
	        ...
	        while ((sl = breadln(hfp,linebuf,LINEBUFLEN)) > 0) {
...
char		afname[] ;
{
	...
	if ((rs = bopen(hfp,bbhostsfname,"r",0666)) >= 0) {
	    ...
	    while ((len = breadln(hfp,linebuf,LINEBUFLEN)) > 0) {
...
char	hostbuf[] ;
{
	...
	while ((len = breadln(nfp,linebuf,LINEBUFLEN)) > 0) {
