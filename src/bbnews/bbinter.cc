...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("bbinter_loadcmdkey: f=>%r<\n",fp,fl) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("bbinter_loadcmdkeyone: termtype=%r\n",cp,cl) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("bbinter_loadcmdkeyone: keyname=%r\n",
	        fs[2].fp,fs[2].fl) ;
#endif
...
/* is it there? and acccessable? */
	...
	if (rs >= 0) {
	    rs = display_input(&iap->di,"mb=%r\v", mbname,mblen) ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4)) {
	    debugprintf("bbinter_mailend: sfshrink() cl=%d c=>%r<\n",
		cl,cp,cl) ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	debugprintf("bbinter_cmdpipe: cmd=»%r«\n",cp,cl) ;
#endif
...
#if	CF_DEBUG
		if (DEBUGLEVEL(5))
		debugprintf("bbinter_cmdmove: response=>%r<\n",
	...
	for (i = 0 ; (i < n) && (si < iap->nmsgs) ; i += 1) {
	    ...
	    if ((rs >= 0) && (! f)) {
			...
			cchar	*cp ;
	                ...
	                debugprintf("bbinter_scancheck: vs[subject]=>%r<\n",
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	    debugprintf("bbinter_change: mb=>%r<\n",cp,cl) ;
#endif
...
BBINTER		*iap ;
{
	...
	if (DEBUGLEVEL(3)) {
	    ...
	    debugprintf("bbinter_cmdpathprefix: r=>%r<\n",rbuf,rl) ;
	}
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("bbinter_msgviewopen: rs1=%d mid=>%r<\n",
		rs1,midp,rs1) ;
#endif
