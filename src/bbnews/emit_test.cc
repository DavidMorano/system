...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("emit_test: ng=\"%s\" af=\"%s\"\n",
	        ngdir,af) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	        debugprintf("emit_test: mailbox a=\"%s\"\n",afname) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("emit_test: regular stuff, a=\"%s\"\n",afname) ;
#endif
...
/* go through the header loops */
	...
	if (DEBUGLEVEL(5)) {
	        debugprintf(
	            "emit_test: top while, f_newsgroups=%d\n",
	            f_newsgroups) ;
	        debugprintf("emit_test: >%s<\n",lbuf) ;
	    }
#endif /* CF_DEBUG */
	...
	if (DEBUGLEVEL(5))
		debugprintf(
	            "emit_test: special header, f_continue=%d F_IW=%d\n",
	            f_continue,CHAR_ISWHITE(lbuf[0])) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
			debugprintf("emit_test: s=\"%s\" sl=%d\n",
			hv_subject,sl) ;
#endif
...
/* this is for backward compatibility with old software */
...
#if	CF_DEBUG
		if (DEBUGLEVEL(5))
		    debugprintf("emit_test: compatibility code\n") ;
#endif
	...
	if (DEBUGLEVEL(5)) {
			debugprintf(
			    "emit_test: case 1, f_newsgroups=%d\n",
	                f_newsgroups) ;
			debugprintf("emit_test: cp=%s\n",cp) ;
		}
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5)) {
			debugprintf("emit_test: fmt=?\n") ;
			debugprintf("emit_test: fmt=>%s<\n",fmt) ;
			}
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
			debugprintf("emit_test: header leader\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
			debugprintf("emit_test: is/was a header\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
		    debugprintf("emit_test: content-type=%s\n",
	            cp) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
			debugprintf("emit_test: got some non-text\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
		    debugprintf("emit_test: f_newsgroups=%d\n",
	            f_newsgroups) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
		debugprintf("emit_test: bottom while, f_newsgroups=%d\n",
	        f_newsgroups) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
		debugprintf("emit_test: done w/ headers, f_shown=%d\n",
	    f_shown) ;
#endif
...
/* print out a "from" if not present */
	...
	if (DEBUGLEVEL(5))
		debugprintf(
	            "emit_test: optional FROM/DATE processing\n") ;
#endif
...
/* do we need a "from" header */
	...
	if (DEBUGLEVEL(5))
		    debugprintf(
	                "emit_test: got a PASSWD entry on the file\n") ;
#endif
	...
	if (DEBUGLEVEL(5))
			debugprintf(
	                    "emit_test: printing the FROM line\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
		debugprintf("emit_test: about to look at the DATE\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
			debugprintf("emit_test: getting file time \n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
		    debugprintf("emit_test: about to print the DATE\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
		    debugprintf("emit_test: printed the DATE\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
		debugprintf("emit_test: bottom of optional stuff\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("emit_test: past optional processing\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
		debugprintf("emit_test: optional article ID processing\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("emit_test: pop screen?\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
		debugprintf("emit_test: running METAMAIL\n") ;
#endif
	    ...
	    breadln(pip->ifp,response,ANSLEN) ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("emit_test: past possible METAMAIL\n") ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("emit_test: processing rest of article \n") ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("emit_test: popscreen prompt\n") ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("emit_test: popscreen prompt fmt=>%s<\n",
		fmt) ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
		debugprintf("emit_test: funny change thing\n") ;
#endif

	                    badreads = 0 ;
			    while (rs >= 0) {

	                    	sl = breadln(pip->ifp,response,ANSLEN) ;
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
		debugprintf("emit_test: past funny change thing, sl=%d\n",
		sl) ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("emit_test: done processing rest of article \n") ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("emit_test: before prompt\n") ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("emit_test: before 'breadln()'\n") ;
#endif

	badreads = 0 ;
	while ((sl = breadln(pip->ifp,response,ANSLEN)) < 0) {

#if	CF_DEBUG
	    if (pip->debuglevel > 1)
	        debugprintf("emit_test: top loop\n") ;
#endif
...
#if	CF_DEBUG
	    if (pip->debuglevel > 1)
	        debugprintf("emit_test: bottom loop\n") ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("emit_test: past 'breadln()'\n") ;
#endif
...
#if	CF_DEBUG
	            if (pip->debuglevel > 1)
	                debugprintf("emit_test: opened file \n") ;
#endif
...
#if	CF_DEBUG
	        if (pip->debuglevel > 1)
	            debugprintf("emit_test: helpfname file=%s\n",
		pip->helpfname) ;
#endif

	        while ((sl = breadln(hfp,lbuf,llen)) > 0) {
...
#if	CF_DEBUG
	        if (pip->debuglevel > 1)
	            debugprintf("emit_test: default help\n") ;
#endif
...
#if	CF_DEBUG
	    if (pip->debuglevel > 1) {
	        debugprintf("emit_test: possible delete \n") ;
	        debugprintf("emit_test: cmd=%s\n",cp) ;
	        debugprintf("emit_test: user=%s\n",un) ;
	    }
#endif /* CF_DEBUG */
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("emit_test/deluser: ent\n") ;
#endif
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("emit_test/deluser: no match\n") ;
#endif
...
char		afname[] ;
{
	...
	if ((rs = bopen(hfp,bbhostsfname,"r",0666)) >= 0) {
	    ...
	    while ((len = breadln(hfp,lbuf,LINEBUFLEN)) > 0) {
