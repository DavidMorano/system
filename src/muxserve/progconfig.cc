...
int		progconfigread(PI *) noex ;

local int	progconfigreader(PI *,vecobj *,char *,int) noex ;

local int	proclistenadd(PI *,vecobj *,cchar *,int) noex ;
local int	proclistenmerge(PI *,vecobj *) noex ;
local int	proclistenpresent(PI *,vecobj *,LISTENSPEC *) noex ;
local int	proclistentmpdel(PI *,vecobj *,int) noex ;
local int	proclistenfins(PI *,vecobj *) noex ;
...
int progconfigstart(PI *pip,mainv sched,cchar *cfname) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	int		f_secreq ;
	cchar	**schedp ;
...
/* look for configuration file */
	...
	schedp = (sched != nullptr) ? sched : schedpconf ;

#if	CF_DEBUG
	if (DEBUGLEVEL(3)) {
	    int	i ;
	    debugprintf("progconfigstart: search-schedule:\n") ;
	    for (i = 0 ; schedp[i] != nullptr ; i += 1)
...
local int progconfigreader(PI *pip,vecobj *tlp,char *pbuf,int plen)
{
		...
		kl = paramfile_curenum(pfp,&cur,&pe,pbuf,plen) ;
...
#if	CF_DEBUG
	            if (DEBUGLEVEL(4))
	                debugprintf("progconfigread: enum k=%r\n",kp,kl) ;
#endif
...
#if	CF_DEBUG
	            if (DEBUGLEVEL(4)) {
	                debugprintf("progconfigread: ebuf=>%r<\n",ebuf,el) ;
	            ...
	            case param_listen:
	                ...
	                if (DEBUGLEVEL(4)) {
	                    ...
	                    debugprintf("progconfigread: listen ebuf=>%r<\n",
	                        ebuf,el) ;
	                }
#endif
#if	CF_LISTEN
...
local int proclistenadd(PI *pip,vecobj *tlp,cchar *ebuf,int elen)
{
	LISTENSPEC	ls ;
	VECSTR		al, *alp = &al ;
	int		rs = SR_OK ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("progconfig/proclistenadd: ebuf=>%r<\n",
	        ebuf,elen) ;
#endif
	...
	if ((rs = vecstr_start(alp,5,0)) >= 0) {
	    ...
	    while ((tp = strnchr(sp,sl,CH_FS)) != nullptr) {
	        c += 1 ;
...
local int proclistenmerge(PI *pip,vecobj *tlp)
{
	LISTENSPEC	*lsp ;
	LISTENSPEC	*tlsp ;
	int		rs = SR_OK ;
...
local int proclistenpresent(PI *pip,vecobj *tlp,LISTENSPEC *lsp)
{
	LISTENSPEC	*tlsp ;
	int		rs ;
	int		rs1 ;
...
local int proclistentmpdel(PI *pip,vecobj *tlp,int ei)
{
	LISTENSPEC	*tlsp = nullptr ;
	int		rs ;
	int		rs1 ;
...
local int proclistenfins(PI *pip,vecobj *tlp) noex {
	LISTENSPEC	*lsp ;
	int		rs = SR_OK ;
	int		rs1 ;
	for (int i = 0 ; vecobj_get(tlp,i,&lsp) >= 0 ; i += 1) {
	    if (lsp != nullptr) {
	        rs1 = listenspec_finish(lsp) ;
	        if (rs >= 0) rs = rs1 ;
	    }
