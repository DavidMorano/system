...
/* populate the name-address translation table for PCS */


#define	CF_DEBUGS	0	/* debugging */
...
/* determine table type, is this a user table or a system table ? */
	    ...
	    if (substring(file,strlen(file),PCS) >= 0) {
	        ...
	        type = TT_SYSTEM ;
#if	DEBUG
	        logfile_printf(&g.eh,"got a system translation\n") ;
#endif
...
/* if name database overflows then drop out */
	        ...
	        if ((tname = on_nmalloc()) == NULL) break ;
...
/*jm*/
	...
	if (tname == NULL) {
	    ...
	    fprintf(errlog,"%s	%s	%s\n",
	        getpwuid(getuid())->pw_name, g.envdate,
