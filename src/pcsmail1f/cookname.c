...
/* assume it is a login name */
	        ...
	        strcpy(tname->realname,g.mailname) ;

	        strcpy(tname->mailaddress,g.username) ;
...
/* Make sure that aliases are handled right PAS-JM 2/14/85 */
	        ...
	        if (g.debuglevel > 0)
	            logfile_printf(&g.eh,
			"cookname: found translated ema=\"%s\"\n",
	                aname->mailaddress) ;
#endif
...
/* first check if we are in the AT&T domain */
	        ...
	        if (strcmp(hp,g.companydomain) == 0) {
...
/* put any SubDomain length in variable 'sdlen' */
	            ...
	            sdlen = 0 ;
	            if ((dlen > 0) &&
	                ((i = substring(dp,dlen,g.companydomain)) >= 0) &&
	                (dlen == (i + strlen(g.companydomain)))) {

#if	CF_DEBUG
	                if (g.debuglevel > 0)
	                    logfile_printf(&g.eh,
				"putinmbag: got an AT&T string\n") ;
#endif
...
/* remember that there may not be ANY host at this point !!! */
	        ...
	        if (g.debuglevel > 0) {
	            ...
	            logfile_printf(&g.eh,"putinmbag: lb=%d\n",
	                ((dp != NULL) && (strcmp(dp,g.localdomain) == 0))) ;
	        ...
	        if (g.debuglevel > 0)
	            logfile_printf(&g.eh,
				"putinmbag: sdlen=%d f_cd=%d f_ld=%d\n",
	                sdlen,f_companydomain,f_localdomain) ;
#endif
...
/* look for the local mailhost (IF WE EVEN HAVE A HOST in 'hp' variable) */
	        ...
	        if ((hp != NULL) &&
			((strcmp(hp,g.nodename) == 0) ||
	            ((strcmp(hp,g.mailnode) == 0) && 
			(strcmp(g.localdomain,g.maildomain) == 0)) ||
	            (strcmp(hp,g.mailhost) == 0))) {
