...
local int	procschedbegin(proginfo *) ;
local int	procschedend(proginfo *) ;

local int	loadschedvars(proginfo *) ;
local int	loadgroupname(proginfo *) ;
local int	loadplatform(proginfo *) ;
local int	loadarchitecture(proginfo *) ;
local int	loadhz(proginfo *) ;
local int	loadncpu(proginfo *) ;
local int	loadorg(proginfo *) ;
local int	loadprovider(proginfo *) ;
local int	loadcooks(proginfo *) ;

local int	loaddefs(proginfo *,cchar *,cchar **,cchar **) ;
local int	loaddefsfile(proginfo *,cchar *) ;
local int	loaddefsfind(proginfo *,cchar **) ;
...
local int	loadsysinfo(proginfo *) ;
...
local int	pvarsbegin(proginfo *,cchar **,cchar *) ;
local int	pvarsend(proginfo *) ;

local int	isenvok(cchar **,cchar *,int) ;

#if	CF_DEBUG && CF_DEBUGCOOKS
local int	debugcooks(proginfo *,cchar *) ;
#endif
...
local int procschedbegin(proginfo *pip)
{
	int		rs ;
	int		c = 0 ;
...
local int procschedend(proginfo *pip)
{
	int		rs = SR_OK ;
	int		rs1 ;
...
local int loadplatform(proginfo *pip)
{
	int		rs = SR_NOSYS ;
	int		cl ;
...
local int loadorg(proginfo *pip)
{
...
local int loadprovider(proginfo *pip)
{
	int		rs = SR_OK ;
	int		len = 0 ;
...
local int loaddefs(proginfo *pip,cchar *dfname,cchar **s1,cchar **s2)
{
	int		rs = SR_OK ;
...
local int loaddefsfind(proginfo *pip,cchar *sched[])
{
...
local int pvarsbegin(proginfo *pip,cchar **pathvars,cchar *fname)
{
...
local int pvarsend(proginfo *pip)
{
	vecstr		*pvp ;
	int		rs = SR_OK ;
	int		rs1 ;
...
local int loadsysinfo(proginfo *pip)
{
	int		rs ;
...
local int isenvok(cchar *envbads[],cchar *kp,int kl)
{
	int		f = TRUE ;
...
local int debugcooks(proginfo *pip,cchar s[]) noex {
	expcook_cur	cur ;
	int		rs1 ;
	char		buf[BUFLEN + 1] ;
	char		*cp ;
	...
	while (expcook_curenum(&pip->cooks,&cur,buf,BUFLEN) >= 0) {
