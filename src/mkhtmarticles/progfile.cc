/* mkhtmarticles_bprogfile SUPPORT */
/* charset=ISO8859-1 */
...
/* revision history:
	...
	keep the old change notes?  Also, what was the previous
...
extern int	strdcpy1w(char *,int,cchar *,int) ;
...
static int	sihyphen(cchar *,int) ;
static int	artdate(int,cchar *,int,time_t *) ;
static int	mkrealname(char *,int,cchar *,int) ;


/* local variables */

static cchar	*fulldays[] = {
	"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday",
	"Friday", "Saturday", NULL
} ;

static cchar	*fullmonths[] = {
...
cchar	fname[] ;
{
	...
	memset(pfp,0,sizeof(struct artinfo)) ;
...
/* FALLTHROUGH */
	    case state_title:
	        ...
	        if (cl > 0) {
	            ...
	            strdcpy1w(pfp->title,ARTINFO_LITEM,buf,si) ;
	...
	if (DEBUGLEVEL(3))
		    ...
		    char	namebuf[REALNAMELEN + 1] ;
	            ...
	            if (! pfp->fl.noauthor) {
			if ((rs1 = mkrealname(namebuf,REALNAMELEN,cp,cl)) >= 0) {
			    cp = namebuf ;
			    cl = rs1 ;
			}
		    }

		    strdcpy1w(pfp->author,ARTINFO_LITEM,cp,cl) ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	            debugprintf("progfile: artdate() rs=%d\n",rs) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	        debugprintf("progfile: publisher s=%u ps=%u\n",
	            state,pstate) ;
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	        debugprintf("progfile: publisher cl=%d\n",cl) ;
#endif
		    ...
		    strdcpy1w(pfp->publisher,ARTINFO_LITEM,buf,bl) ;

#if	CF_DEBUG
	if (DEBUGLEVEL(3))
	            debugprintf("progfile: publisher=%s\n",
	                pfp->publisher) ;
#endif
