...
/*******************************************************************************
	...
	int opensvc_mailmsgs(pr,prn,of,om,argv,envv,to)
	const char	*pr ;
	const char	*prn ;
	int		of ;
	mode_t		om ;
...
#define	NDEBFNAME	"opensvcmailmsgs.deb"
...
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"opensvc_mailmsgs: ent\n") ;
#endif
...
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"opensvc_mailmsgs: 0 un=%s\n",un) ;
#endif
...
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"opensvc_mailmsgs: 1 rs=%d un=%s\n",rs,un) ;
#endif
...
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"opensvc_mailmsgs: 2 rs=%d un=%s\n",rs,un) ;
#endif
...
#if	CF_DEBUGN
	nprintf(NDEBFNAME,"opensvc_mailmsgs: pcsmailcheck() rs=%d\n",rs) ;
#endif
...
/* end subroutine (opensvc_mailmsgs) */
...
#if	CF_SETENTRY
int subinfo_setentry(SUBINFO *sip,cchar **epp,char *vp,int vl)
{
	int		rs = SR_OK ;
	int		vnlen = 0 ;
