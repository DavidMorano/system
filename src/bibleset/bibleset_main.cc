...
/* revision history:
	...
	This subroutine was adapted from someting else to be a
	simple front-end for this program.
...
extern int	progoutbegin(PI *,bfile *) ;
...
/* some initialization */
	...
	if ((rs >= 0) && (pip->linewidth == 0) && (argval != nullptr)) {
	...
	if (ofname == nullptr) ofname = getenv(VAROFNAME) ;
...
/* more initialization */
	...
	if (pip->pub == nullptr) pip->pub = PUBLISHER ;
...
/* if no font-family was specified on invocation -> assume "Times" */
	...
	if ((rs >= 0) && ((pip->ff == nullptr) || (pip->ff[0] == '\0'))) {
	    cchar	**vpp = &pip->ff ;
...
static int procopts(PI *pip,keyopt *kop) noex {
	...
	if (rs >= 0) {
	    if (keyopt_cur kcur ; (rs = keyopt_curbegin(kop,&kcur)) >= 0) {
	        int	oi ;
	        int	kl, vl ;
	        cchar	*kp, *vp ;
...
static int procpagetitle(PI *pip)
{
	...
	if (pip->pagetitle == nullptr) pip->pagetitle = DEFTITLE ;
	return rs ;
}
