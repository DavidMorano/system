...
int p_strerror(int argc,cchar *argv[],cchar *envv[],void *contextp)
{
...
static int procspecs(PROGINFO *pip,void *ofp,cchar *lbuf,int llen) noex {
	int		rs ;
	int		wlen = 0 ;
...
#if	CF_DEBUG
	if (DEBUGLEVEL(2))
	    debugprintf("b_strerror/procspec: np=%t\n",np,nl) ;
#endif
