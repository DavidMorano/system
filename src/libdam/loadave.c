...
/*******************************************************************************
	...
	usually newer SVR4 systems.  This object is *not*
	multi-thread-safe because according to the lastest I knew,
	the KSTAT system API was not multi-thread-safe!
...
int loadave_readvalues(LOADAVE *eop,LOADAVE_VALUES *vp) noex {
	time_t		daytime = time(NULL) ;
	int		rs = SR_OK ;
