...
int lookword_open(LW *op,cchar *dfname,int opts) noex {
	...
	if (dfname == nullptr) return SR_FAULT ;
...
int lookword_lookup(LW *op,LW_CUR *curp,cchar *string) noex {
	...
	if (op == nullptr) return SR_FAULT ;
	if (curp == nullptr) return SR_FAULT ;
	if (string == nullptr) return SR_FAULT ;
...
static int lookword_proc(LW *op,LW_CUR *curp,
		cc *front,cc *back,cc *string) noex {
	int		rs = SR_OK ;
	int		c = 0 ;
...
static int lookword_record(LW *op,LW_CUR *curp,
	...
	if ((rs = vecobj_start(&ans,esize,1,0)) >= 0) {
	    ...
	    while ((rs >= 0) && (front < back) && 
	        (compare(op,front,back,string,nullptr) == 0)) {
...
static cchar *binary_search(LW *op,cc *front,cc *back,cc *string) noex {
	...
	while ((p < back) && (back > front)) {
	    ...
	    if (compare(op,p,back,string,nullptr) > 0) {
	        front = p ;
	    } else {
	        back = p ;
	    }
...
static cchar *linear_search(LW *op,cc *front,cc *back,cc *string) noex {
	int		rc = 0 ;
	while (front < back) {
	    rc = compare(op,front,back,string,nullptr) ;
	    if (rc <= 0) break ;
	    SKNL(front, back) ;
	} /* end while */
...
static int compare(LW *op,cchar *s2,cchar *back,cchar *s1,int *np) noex {
	int		ch1, ch2 ;
	int		fch1, fch2 ;
	int		i = 0 ;
	int		j = 0 ;
