...
/*******************************************************************************
	...
	This module contains a family of subroutines. These subroutines
	form a compatible replacement for:

		gethostbyname
		gethostbyaddr

	when the canonical name is supposed to be given in the first
	returned name slot for 'gethostbyname' and when a canonical
	name is passed to 'gethostbyaddr' that is not in the database.
...
HOSTENT *getchostbyname(cchar *name) noex {
	UTSNAME		uts ;
	HOSTENT		*hep = &he ;
	unsigned long	addr ;
	int	rs ;
