...
#ifndef	BIBDBBIBENTRY_INCLUDE
#define	BIBDBBIBENTRY_INCLUDE
#ifdef	__cplusplus
...
enum bibentrymems {
    	...
    	bibentrymem_end,
    	bibentrymem_finish,
    	bibentrymem_overlast
} ; /* end enm (bibentrymems) */
struct bibentry ;
struct bibentry_co {
	bibentry	*op = nullptr ;
	int		w = -1 ;
	void operator () (bibentry *p,int m) noex {
	    op = p ;
	    w = m ;
	} ;
	operator int () noex ;
...
struct bibentry {
	...
	int		fi ;
	bibentry() noex {
	    count	(this,bibentrymem_count) ;
	    end		(this,bibentrymem_end) ;
	    finish	(this,bibentrymem_finish) ;
	    kvlp	= nullptr ;
	    vbp		= nullptr ;
	    kp		= nullptr ;
	} ; /* end ctor */
	int getkv	(int,keyval **) noex ;
	void dtor() noex ;
	destruct bibentry() {
	    if (kvlp) dtor() ;
	} ; /* end destruct */
...
namespace bibdbx {
    extern int bibentry_start	(bibentry *,int) noex ;
    extern int bibentry_begin	(bibentry *,cchar *,int,cchar *,int) noex ;
    extern int bibentry_cont	(bibentry *,cchar *,int) noex ;
    extern int bibentry_count	(bibentry *) noex ;
    extern int bibentry_end	(bibentry *) noex ;
    extern int bibentry_finish	(bibentry *) noex ;
} /* end namespace (bibdbx) */


#endif /* __cplusplus */
#endif /* BIBDBBIBENTRY_INCLUDE */
