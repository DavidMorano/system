...
local inline int varsub_ctor(VS *op,Args ... args) noex {
	...
	if (op && (args && ...)) ylikely {
	    rs = SR_NOMEM ;
	    op->magic = 0 ;
	    if ((op->slp = new(nothrow) vechand) != np) ylikely {
		rs = SR_OK ;
	    } /* end if (new-vechand) */
	} /* end if (non-null) */
	return rs ;
...
int varsub_start(VS *op,int aopts) noex {
	int		rs ;
	if ((rs = varsub_ctor(op)) >= 0) ylikely {
	    op->i = 0 ;
	    op->n = VS_DEFENT ;
...
int varsub_exp(VS *op,char *dbuf,int dlen,cchar *sbuf,int slen) noex {
	...
	int		rs1 ;
	int		bl = 0 ;
	if ((rs = varsub_magic(op,dbuf,sbuf)) >= 0) ylikely {
...
int varsub_expbuf(VS *op,buffer *bufp,cchar *sbuf,int slen) noex {
	int		rs ;
	if ((rs = varsub_magic(op,bufp,sbuf)) >= 0) ylikely {
	    if (slen < 0) slen = lenstr(sbuf) ;
	    if ((rs = varsub_sort(op)) >= 0) ylikely {
...
local int varsub_iadd(VS *op,cchar *k,int klen,cchar *v,int vlen) noex {
	...
	if (ent tmp ; (rs = entry_tmp(&tmp,k,klen,v,vlen)) >= 0) ylikely {
	    vechand	*elp = op->slp ;
	    int		rs1 ;
	    void	*vp{} ; /* used-multiple */
	    if ((rs1 = vechand_search(elp,&tmp,vcmpent,&vp)) >= 0) ylikely {
	        ent	*dep = entp(vp) ;
...
local int varsub_iaddq(VS *op,cchar *k,int klen,cchar *v,int vlen) noex {
	...
	if (klen > 0) {
	    vechand	*elp = op->slp ;
	    cint	msize = szof(ent) ;
	    if (void *vp ; (rs = lm_mall(msize,&vp)) >= 0) ylikely {
	        ent	*ep = entp(vp) ;
	        if ((rs = entry_start(ep,k,klen,v,vlen)) >= 0) ylikely {
	            op->fl.sorted = false ;
	            rs = vechand_add(elp,ep) ;
	            if (rs < 0) {
	                entry_finish(ep) ;
