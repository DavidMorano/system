...
#define	MSF		msfile
#define	MSF_ENT		msfilee
#define	MSF_MAG		MSFILE_MAGIC
#define	MSF_IDOFF	0
#define	MSF_HEADTABOFF	(MSF_IDOFF + (16 + szof(uint)))
#define	MSF_TABOFF	(MSF_HEADTABOFF + (3 * szof(uint)))

#define	MSF_ENTSZ	MSFILEE_SZ
#define	MSF_MAXFILESIZE	(4 * 1024 * 1024)
#define	MSF_NWAYS	4
#define	MSF_NEPW	(((8*1024) / MSF_ENTSZ) - 1)
#define	MSF_NIDXENT	100
...
local inline int msfile_magic(msfile *op,Args ... args) noex {
	int		rs = SR_FAULT ;
	if (op && (args && ...)) {
	    rs = (op->magval == MSF_MAG) ? SR_OK : SR_NOTOPEN ;
	}
	return rs ;
} /* end subroutine (msfile_magic) */

local int	msfile_fileopen(MSF *,time_t) noex ;
local int	msfile_fileclose(MSF *) noex ;
local int	msfile_filesetinfo(MSF *) noex ;
local int	msfile_lockget(MSF *,time_t,int) noex ;
local int	msfile_lockrelease(MSF *) noex ;
local int	msfile_filebegin(MSF *,time_t) noex ;
local int	msfile_acquire(MSF *,time_t,int) noex ;
local int	msfile_filecheck(MSF *) noex ;
local int	msfile_entbufstart(MSF *) noex ;
local int	msfile_entbuffinish(MSF *) noex ;

local int	msfile_filetopwrite(MSF *) noex ;
local int	msfile_filetopread(MSF *) noex ;
local int	msfile_fileverify(MSF *) noex ;
local int	msfile_headtab(MSF *,int) noex ;

local int	msfile_findname(MSF *,cchar *,int,char **) noex ;
local int	msfile_search(MSF *,cchar *,int,char **) noex ;
local int	msfile_readent(MSF *,int,char **) noex ;

#if	CF_NISEARCH
local int	msfile_index(MSF *,cchar *,int,int) noex ;
#endif

local int	msfile_headwrite(MSF *) noex ;
...
int msfile_open(MSF *op,cchar *fname,int oflags,mode_t operm) noex {
	...
	DEBUGPRINTF("ret rs=%d f_created=%u\n",rs,f_created) ;
	return (rs >= 0) ? f_created : rs ;
} /* end subroutine (msfile_open) */

int msfile_close(MSF *op) noex {
	int		rs ;
	int		rs1 ;
	if ((rs = msfile_magic(op)) >= 0) {
	    {
	        DEBUGPRINTF("mapstrint_finish() \n") ;
...
int msfile_count(MSF *op) noex {
	int		rs ;
	int		c = 0 ;
	if ((rs = msfile_magic(op,curp)) >= 0) {
	    c = ((op->filesize - MSF_TABOFF) / MSF_ENTSZ) ;
	} /* end if (msfile_magic) */
	return (rs >= 0) ? c : rs ;
} /* end subroutine (msfile_count) */

int msfile_curbegin(MSF *op,MSF_CUR *curp) noex {
    	int		rs ;
	if ((rs = msfile_magic(op,curp)) >= 0) {
	    op->ncursors += 1 ;
...
int msfile_curend(MSF *op,MSF_CUR *curp) noex {
	int		rs ;
	int		rs1 ;
	if ((rs = msfile_magic(op,curp)) >= 0) {
...
int msfile_curenum(MSF *op,MSF_CUR *curp,MSF_ENT *ep) noex {
	int		rs = SR_OK ;
	int		ei = 0 ;
	DEBUGPRINTF("ent\n") ;
	if ((rs = msfile_magic(op,curp)) >= 0) {
	    ...
	    if ((rs = msfile_acquire(op,dt,1)) >= 0) {
	        ...
	        if ((rs = msfile_readent(op,ei,&bp)) >= 0) {
	            if ((ep != nullptr) && (bp != nullptr)) {
	                rs = msfilee_proc(ep,1,bp,ebs) ;
	            }
	            if (rs >= 0) {
	                curp->i = ei ;
	...
	DEBUGPRINTF("ret rs=%d ei=%u\n",rs,ei) ;
	return (rs >= 0) ? ei : rs ;
} /* end subroutine (msfile_curenum) */

int msfile_match(MSF *op,time_t dt,cchar *nnp,int nnl,MSF_ENT *ep) noex {
	cint		ebs = MSF_ENTSZ ;
	int		rs = SR_OK ;
	int		i = MSF_NODENAMELEN ;
	int		ei = 0 ;
	char		*bp ;
	...
	if (op->magval != MSF_MAGIC) return SR_NOTOPEN ;
#endif /* CF_SAFE */

	if (nnp == nullptr) return SR_FAULT ;

	DEBUGPRINTF("ent nnl=%d nnp=%r\n",
	    nnl,nnp,lenstr(nnp,nnl)) ;

	if (nnl >= 0) {
	    i = MIN(nnl,MSF_NODENAMELEN) ;
	}

	nnl = lenstr(nnp,i) ;

	DEBUGPRINTF("nnl=%d nnp=%r\n",nnl,nnp,nnl) ;
	...
	if ((rs = msfile_acquire(op,dt,1)) >= 0) {
	    ...
	    if ((rs >= 0) && (ep != nullptr)) {
	        DEBUGPRINTF("found it rs=%d\n",rs) ;
	        rs = msfilee_proc(ep,1,bp,ebs) ;
	        DEBUGPRINTF("rs1=%d ep->nodename=%s\n",
	            rs,ep->nodename) ;
	    } /* end if */
...
int msfile_write(MSF *op,time_t dt,cchar *nnp,int nnl,MSF_ENT *ep) noex {
	cint		ebs = MSF_ENTSZ ;
	int		rs = SR_OK ;
	int		i = MSF_NODENAMELEN ;
	int		ei ;
...
int msfile_update(MSF *op,time_t dt,MSF_ENT *ep) noex {
	int		rs = SR_OK ;
	int		ei = 0 ;
	DEBUGPRINTF("ent\n") ;
	...
	if ((rs = msfile_acquire(op,dt,1)) >= 0) {
	    ...
	    cint	ebs = MSF_ENTSZ ;
	    char	entbuf[MSF_ENTSZ + 2] ;
	    char	*bp ;

/* do the search */

	    nnl = lenstr(nnp,MSF_NODENAMELEN) ;
	    if (nnl > 0) {
...
/* found existing entry */
	            ...
	            DEBUGPRINTF("1 entbuf_pwrite() rs=%d\n", rs) ;

	        } /* end if (entry update) */
		/* update the file header */
	        DEBUGPRINTF("header rs=%d\n",rs) ;
	        if (rs >= 0) {
	            if (dt == 0) dt = time(nullptr) ;
	            op->h.wcount += 1 ;
	            op->h.wtime = dt ;
...
int msfile_check(MSF *op,time_t dt) noex {
	int		rs = SR_OK ;
	int		f = false ;
...
local int msfile_findname(MSF *op,cchar *nnp,int nnl,char **rpp) noex {
	int		rs ;
	int		ei = 0 ;
	char		*bp = nullptr ;
	char		*nap ;
	...
	if ((rs = mapstrint_fetch(&op->ni,nnp,nnl,nullptr,&ei)) >= 0) {
	    ...
	    if ((rs = entbuf_read(&op->ebm,ei,&bp)) > 0) {
	        ...
	        DEBUGPRINTF("entbuf_read() rs=%d\n",rs) ;

	        nap = bp + MSFILEE_ONODENAME ;
	        if (! namematch(nap,nnp,nnl)) {
	            rs = SR_NOTFOUND ;
...
local int msfile_search(MSF *op,cchar *nnp,int nnl,char **rpp) noex {
	cint		ebs = MSF_ENTSZ ;
	int		rs = SR_OK ;
	int		i ;
	int		ne ;
	...
	DEBUGPRINTF("ent nnl=%u nodename=%r\n",
	    nnl,nnp,lenstr(nnp,nnl)) ;

	if (nnl < 0)
	    nnl = lenstr(nnp) ;

	DEBUGPRINTF("nnl=%u pagesize=%u ebs=%u\n",
	    nnl,op->pagesize,ebs) ;
...
/* is this a match for what we want? */
	        ...
	        DEBUGPRINTF("i=%u db_node=%r\n",i,
	            nap,lenstr(nap,MSFILEE_LNODENAME)) ;
	    ...
	    DEBUGPRINTF("for-out i=%d f_found=%u\n", i,f_found) ;
	    ei += i ;
	    DEBUGPRINTF("ei=%d f_found=%u\n",ei,f_found) ;
	    if (f_found) break ;
	} /* end while */

	DEBUGPRINTF("fin rs=%d f_found=%u\n",rs,f_found) ;
	DEBUGPRINTF("ne=%d \n",ne) ;
...
local int msfile_acquire(MSF *op,time_t dt,int f_read) noex {
	cint		ebs = MSF_ENTSZ ;
	int		rs = SR_OK ;
	int		f_changed = false ;
	int		f ;
	...
	if ((rs >= 0) && (! f)) {
	    if ((rs = msfile_lockget(op,dt,f_read)) >= 0) {
	        ...
	        if ((rs = msfile_filecheck(op)) >= 0) {
	            ...
	            if (f_changed) {
	                int n = ((op->filesize - MSF_TOPLEN) / ebs) ;
...
local int msfile_filebegin(MSF *op,time_t dt) noex {
	int		rs = SR_OK ;
	int		f_locked = false ;
	DEBUGPRINTF("ent fd=%d filesize=%u\n",
	    op->fd,op->filesize) ;
...
local int msfile_filecheck(MSF *op) noex {
	int		rs = SR_OK ;
	int		f_changed = false ;
...
local int msfile_filetopwrite(MSF *op) noex {
	off_t	poff = 0L ;
	int		ml ;
	int		rs = SR_OK ;
	int		bl ;
	...
	ml = mkmagic(bp,MSF_FILEMAGICSIZE,MSF_FILEMAGIC) ;
	bp += ml ;
	...
	*bp++ = 0 ;		/* unused */
	...
	bl = bp - op->topbuf ;
	op->fileversion = MSF_FILEVERSION ;
	op->filetype = 0 ;
	op->h = {} ;
...
local int msfile_filetopread(MSF *op) noex {
	off_t	poff = 0L ;
	int		rs ;

	rs = u_readp(op->fd,op->topbuf,MSF_TOPLEN,poff) ;
	op->topsize = rs ;

	return rs ;
} /* end subroutine (msfile_filetopread) */

local int msfile_fileverify(MSF *op) noex {
	int		rs = SR_OK ;

	DEBUGPRINTF("ent\n") ;
	DEBUGPRINTF("filemagic=%s\n", MSF_FILEMAGIC) ;
	DEBUGPRINTF("filemagicsize=%u\n", MSF_FILEMAGICSIZE) ;

	if (op->topsize >= MSF_TOPLEN) {
	    cint	msize = MSF_FILEMAGICSIZE ;
	    cchar	*cp = op->topbuf ;
	    DEBUGPRINTF("ms=%r\n",cp,strlinelen(cp,-1,40)) ;
	    if (hasValidMagic(cp,msize,MSF_FILEMAGIC)) {
	        cp += msize ;
	        if (cp[0] <= MSF_FILEVERSION) {
	            op->fileversion = cp[0] ;
	            if (cp[1] == MSF_ENDIAN) {
	                op->filetype = cp[2] ;
	            } else {
...
local int msfile_headtab(MSF *op,int f_read) noex {
	int		rs = SR_OK ;
	int		f_changed = false ;
	char		*bp = (op->topbuf + MSF_HEADTABOFF) ;
...
local int msfile_lockget(MSF *op,time_t dt,int f_read) noex {
	USTAT		sb ;
	int		rs = SR_OK ;
	int		lockcmd ;
	int		f_already = false ;
...
#if	CF_DEBUG && 0
	        DEBUGPRINTF("LOCK lockfile() rs=%d\n",rs) ;
#endif
...
local int msfile_lockrelease(MSF *op) noex {
	int		rs = SR_OK ;
...
#if	CF_DEBUG && 0
	        DEBUGPRINTF("lockfile() rs=%d\n",rs) ;
#endif

#if	CF_DEBUG
	        DEBUGPRINTF("UNLOCK lockfile() rs=%d\n",
	            rs) ;
#endif
...
local int msfile_fileopen(MSF *op,time_t dt) noex {
	int		rs = SR_OK ;
	int		f_created = false ;
...
#if	CF_DEBUG
	DEBUGPRINTF("ret rs=%d f_create=%u\n",rs,f_created) ;
#endif
...
int msfile_fileclose(MSF *op) noex {
	int		rs = SR_OK ;
	int		rs1 ;
...
local int msfile_filesetinfo(MSF *op) noex {
	USTAT		sb ;
	int		rs ;
	int		amode ;
...
local int msfile_entbuffinish(MSF *op) noex {
	int		rs = SR_OK ;
	int		rs1 ;
...
local int msfile_readent(MSF *op,int ei,char **rpp) noex {
	int		rs ;
	char		*bp ;
...
/* end subroutine (msfile_readent) */


#if	CF_NISEARCH

local int msfile_index(MSF *op,cchar *nap,int nl,int ei) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	int		ei2 ;
	...
	rs1 = mapstrint_fetch(&op->ni,nap,nl,nullptr,&ei2) ;

	if ((rs1 >= 0) && (ei != ei2)) {
	    rs1 = SR_NOTFOUND ;
	    mapstrint_delkey(&op->ni,nap,nl) ;
	}

	if (rs1 == SR_NOTFOUND) {
	    int	nl2 = lenstr(nap,MSFILEE_LNODENAME) ;
	    mapstrint_add(&op->ni,nap,nl2,ei) ;
	} /* end if (not found) */
...
local int msfile_headwrite(MSF *op) noex {
	int		rs ;

	if ((rs = msfile_headtab(op,0)) >= 0) {
	    off_t	poff ;
	    cint	toff = MSF_TABOFF ;
	    cint	htoff = MSF_HEADTABOFF ;
	    int		bl ;
	    cchar	*bp ;
	    bp = (op->topbuf + htoff) ;
	    bl = (toff - htoff) ;
