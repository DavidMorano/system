...
static int	msgid_buf(msgid *,int,int,char **) noex ;
...
int msgid_enum(msgid *op,MSGID_CUR *curp,MSGID_ENT *ep) noex {
	const time_t	dt = time(nullptr) ;
	int		rs ;
	int		eoff ;
	int		ei = 0 ;
...
static int msgid_buf(msgid *op,int roff,int rlen,char **rpp) noex {
	off_t		poff ;
	uint		bend, fext ;
	uint		foff ;
