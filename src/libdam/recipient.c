...
int recipient_start(RECIPIENT *op,int n) noex {
	int		rs ;
	int		opts ;
...
int recipient_add(RECIPIENT *op,cc *host,cc *local,int type) noex {
	HDB_DATUM	key, val ;
...
int recipient_already(RECIPIENT *op,cchar *host,cchar *local,int type) noex {
	HDB_DATUM	key, val ;
	HDB_CUR		cur ;
	RECIPIENT_VAL	*vp ;
	int		rs ;
...
int recipient_finish(RECIPIENT *op) noex {
	HDB_CUR		keycursor ;
	HDB_DATUM	key, val ;
	int		rs ;
	int		rs1 ;
...
int recipient_counthosts(RECIPIENT *op) noex {
	int		rs ;
	if (op == NULL) return SR_FAULT ;
...
int recipient_count(RECIPIENT *op) noex {
	int		rs ;
	if (op == NULL) return SR_FAULT ;
...
int recipient_sort(RECIPIENT *vhp) noex {
...
int recipient_enumhost(RECIPIENT *op,RECIPIENT_HCUR *hcp,cchar **hnpp) noex {
{
	int		rs ;
	int		i ;
	const char	*dump ;
...
int recipient_fetchvalue(RECIPIENT *op,cc *host,RECIPIENT_VCUR *vcp,
		RECIPIENT_VAL **vepp) noex {
	HDB_DATUM	key ;
	HDB_DATUM	val ;
	int		rs ;
...
int recipient_hcurbegin(RECIPIENT *op,RECIPIENT_HCUR *hcp) noex {
	if (op == NULL) return SR_FAULT ;
	if (hcp == NULL) return SR_FAULT ;
	*hcp = -1 ;
	return SR_OK ;
...
int recipient_hcurend(RECIPIENT *op,RECIPIENT_HCUR *hcp) noex {
	if (op == NULL) return SR_FAULT ;
	if (hcp == NULL) return SR_FAULT ;
	*hcp = -1 ;
	return SR_OK ;
...
int recipient_vcurbegin(RECIPIENT *op,RECIPIENT_VCUR *vcp) noex {
	int		rs ;
	if (op == NULL) return SR_FAULT ;
...
int recipient_vcurend(RECIPIENT *op,RECIPIENT_VCUR *vcp) noex {
	int		rs ;
	if (op == NULL) return SR_FAULT ;
	if (vcp == NULL) return SR_FAULT ;
