...
/*******************************************************************************
	...
	Description:
	This subroutine is used to dial out to a program that resides
	on the local cluster.  The program will be executed on
	whatever node in the cluster is specified.  This subroutine
	has the typical "dial" type sematics in that a single
	bi-directional pipe is returned with the input and output
	from the program on it.  An optional additional pipe with
	the standard error from the program is possible also.

	Also, note that Solaris 7 appears to be broken, along with
	maybe Solaris 8 and all that follow.  The stupid rules for
	how stupid sockets work keeps changing.  There are several
	semantics for stupid sockets on Solaris now and even within
	one of those, its stupid semantics seems to subtly change
	from time to time.
...
int dialcprog(cchar *pr,cchar *node,cchar *fname,mainv av,mainv ev,
...
/* but let DISPLAY through, if it gets fixed properly */
	                ...
	                if (ev[i][8] == ':') {
	                    cint	elen = ENVBUFLEN ;
	                    cchar	*nn = dip->nodename ;
	                    cchar	*var = VARDISPLAY ;
	                    cchar	*rp = (cp+8) ;
	                    char	*bp = envbuf ;
