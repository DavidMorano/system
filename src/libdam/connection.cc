...
local int connection_addname(CON *,vecstr *,cc *) noex ;
local int connection_ip4lookup(CON *,char *,int) noex ;
local int connection_ip6lookup(CON *,char *,int) noex ;
...
int connection_start(CON *op,cchar *inetdomain) noex {
	...
	if (op && inetdomain) ylikely {
	    ...
	    if (static cint rsv = var ; (rs = rsv) >= 0) ylikely {
	        ...
	        op->fl.inet = false ;
	        op->sock = -1 ;
	        if (void *vp ; (rs = lm_mall(ssz,&vp)) >= 0) ylikely {
		    op->sap = sockaddressp(vp) ;
	        } /* end if (memory0allocation) */
	    } /* end if (vars) */
	} /* end if (non-null) */
	return rs ;
...
int connection_socklocname(CON *op,char *dp,int dl,int s) noex {
	...
	if (op && dp) ylikely {
	    rs = SR_BADF ;
	    if (s >= 0) ylikely {
		if (ustat sb ; (rs = uc_fstat(s,&sb)) >= 0) ylikely {
...
int connection_peername(CON *op,SA *sap,int sal,char *dp,int dl) noex {
	...
	if (op && sap) ylikely {
	    dp[0] = '\0' ;
	    if ((rs = sockaddress_getaf(sap)) >= 0) ylikely {
	        cint	af = rs ;
	        op->fl.addr = false ;
	        op->fl.sa = true ;
		memcopy(op->sap,sap,szof(SA)) ; /* <- special-issue */
	        op->sal = sal ;
	        switch (af) {
	        case AF_UNIX:
		    if (dp) {
...
sub_mknames::operator int () noex {
	...
	int		n = 0 ;
	if (op->fl.sa) ylikely {
	    if ((rs = sockaddress_getaf(op->sap)) >= 0) ylikely {
	        cint	af = rs ;
	        switch (af) {
	        case AF_UNIX:
...
int sub_mknames::addstuff(int af) noex {
	...
	int		rs1 ;
	int		n = 0 ;
	if (hostinfo hi ; (rs = hostinfo_start(&hi,af,op->name)) >= 0) ylikely {
	    if ((rs = addnames(&hi)) >= 0) {
		n += rs ;
		rs = addresses(&hi) ;
		n += rs ;
...
int sub_mknames::addnames(hostinfo *hip) noex {
	...
	int		n = 0 ;
	cchar		*hp ;
	if ((rs = hostinfo_geteffective(hip,&hp)) >= 0) ylikely {
	    if ((rs = connection_addname(op,nlp,hp)) >= 0) ylikely {
	        n += rs ;
	        if ((rs = hostinfo_getcanonical(hip,&hp)) >= 0) ylikely {
	            if ((rs = connection_addname(op,nlp,hp)) >= 0) ylikely {
		        hostinfo_cur	hc ;
	                n += rs ;
	                if ((rs = hostinfo_curbegin(hip,&hc)) >= 0) ylikely {
	                    cauto cn = hostinfo_curenumname ;
	                    while ((rs = cn(hip,&hc,&hp)) > 0) {
...
int sub_mknames::addresses(hostinfo *hip) noex {
	...
	if (char *nbuf ; (rs = lm_hostname(&nbuf)) >= 0) ylikely {
	    ...
	    if (hostinfo_cur hc ; (rs = hostinfo_curbegin(hip,&hc)) >= 0) {
		    ...
		    if (inetaddr ia ; (rs = ia.start(at,ap)) >= 0) {
...
int sub_mknames::adddots(int af) noex {
	...
	if ((af == AF_INET4) && op->fl.inet && op->fl.addr) {
		...
		inetaddrs	at = inetaddr_bin ;
	        cint		nlen = rs ;
		const INADDR	*addrp = &op->netipaddr ;
	        if (inetaddr ia ; (rs = ia.start(at,addrp)) >= 0) {
	            if ((rs = ia.getdotaddr(nbuf,nlen)) >= 0) {
...
local int connection_addname(CON *op,vecstr *nlp,cc *name) noex {
	int		sl = lenstr(name) ;
	int		rs ;
	int		n = 0 ;
	if ((rs = nlp->adduniq(name,sl)) >= 0) ylikely {
...
local int connection_ip4lookup(CON *op,char *dp,int dl) noex {
	sockaddress	*sap = op->sap ;
	cint		alen = in4addrlen ;
	int		rs ;
	int		rs1 ;
	int		len = 0 ;
	if ((rs = sockaddress_getaddr(sap,&op->netipaddr,alen)) >= 0) ylikely {
...
local int connection_ip6lookup(CON *op,char *dp,int dl) noex {
	int		rs ;
	int		rs1 ;
	int		len = 0 ;
		...
		cint		al = in6addrlen ;
	        if ((rs = sockaddress_getaddr(sap,in6addr,al)) >= 0) {
	            if (isin4mapped(in6addr)) {
	                op->fl.addr = true ;
	                memcopy(&op->netipaddr,(in6addr + 6)) ;
	                rs = connection_ip4lookup(op,dp,dl) ;
	                len = rs ;
