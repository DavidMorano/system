...
static int lineindex_idxopener(LI *op,time_t dt) noex {
	...
	if ((rs = u_open(op->ifn,of,om)) >= 0) {
	    ...
	    if (fc && (op->tfn != nullptr)) {
	        if ((rs = lineindex_ns::lineindex_mkindex(op)) >= 0) {
...
static int lineindex_idxopen(LI *op,time_t dt) noex {
	int		rs ;
	    if ((rs = lineindex_idxopener(op,dt)) >= 0) {
...
static int lineindex_idxverify(LI *op) noex {
	...
	if (op->mapdata) {
	    ...
	    if (lineindexhdr hdr{} ; (rs = lineindexhdr_wr(&hdr,mp,ml)) >= 0) {
