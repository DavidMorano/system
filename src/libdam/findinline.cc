...
local int	getdash(findinline *,cchar *,int) noex ;
local int	getpair(findinline *,cchar *,int) noex ;
...
local int getdash(findinline *fip,cchar *sp,int sl) noex {
...
local int getpair(findinline *fip,cchar *sp,int sl) noex {
    	cnullptr	np{} ;
	int		skiplen = 0 ; /* return-value */
	cchar		*start = (sp - 1) ;
	fip->kp = sp ;
