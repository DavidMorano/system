...
constexpr cpcchar	envdialers[] = {
	"SYSDIALER_ROOT",
	"SYSDIALER_HOST",
	"SYSDIALER_SVC",
	"SYSDIALER_SVCARGS",
	nullptr
} ; /* end array (envdialers) */
...
int prog_open(PROG *op,SD_ARGS *ap,cc *hostname,cc *svcname,mainv av) noex {
	SUBINFO		si, *sip = &si ;
	int		rs = SR_OK ;
	int		rs1 ;
	int		opts = 0 ;
...
static int loaddefsfile(SUBINFO *sip,cchar *dfname)
{
	int		rs = SR_OK ;
	int		rs1 ;
	mainv		envv = mainv(sip->envv) ;

	if (ustat sb ; (rs1 = u_stat(dfname,&sb)) >= 0) {
	    rs1 = SR_NOENT ;
