...
int commandments_open(CMS *op,cchar *pr,cchar *dbn) noex {
	...
	if ((rs = commandments_ctor(op,pr)) >= 0) {
	    rs = SR_INVALID ;
	    if (pr[0]) {
	        op->uid = -1 ;
	        op->uid_pr = -1 ;
	        op->gid_pr = -1 ;
	        if ((rs = commandments_argsbegin(op,pr,dbn)) >= 0) {
	            if ((rs = commandments_findbegin(op,pr,dbn)) >= 0) {
	                if ((rs = commandments_fileloadbegin(op,dbn)) >= 0) {
		            c = op->nents ;
		            op->magval = CMS_MAGIC ;
...
int commandments_read(CMS *op,char *vbuf,int vlen,uint cn) noex {
	...
	if ((rs = commandments_magic(op,vbuf)) >= 0) {
	    ...
	    if (rs >= 0) {
	        ...
	        char	*lb = charp(lines) ;
	        if ((rs = cmi_read(op->idxp,&viv,lb,lsz,cn)) >= 0) {
	            if (vbuf) {
...
int commandments_curbegin(CMS *op,CMS_CUR *curp) noex {
	int		rs ;
	if ((rs = commandments_magic(op,curp)) >= 0) {
	    cint csz = szof(cmi_cur) ;
	    if (void *vp ; (rs = mem.mall(csz,&vp)) >= 0) {
		if (curp->vicurp = new(vp) cmi_cur ; curp->vicurp) {
...
int commandments_curenum(CMS *op,CMS_CUR *curp,CMS_ENT *ep,
	...
	int		rs ;
	int		len = 0 ;
	if ((rs = commandments_magic(op,curp,ep,vbuf)) >= 0) {
	    time_t	dt = 0 ;
	    /* check for update */
	    if (op->ncursors == 0) {
	        rs = commandments_checkupdate(op,dt) ;
...
local int commandments_tmpcheck(CMS *op,char *tbuf,ustat *sbp,
		cchar *dbname) noex {
	...
	if (char *bp ; (rs = mem.mall(sz,&bp)) >= 0) {
	    ...
	    if ((rs = prmktmpdir(op->pr,dbuf,vtmp,sn,dm)) >= 0) {
		    ...
		    if (ustat tsb ; (rs = uc_stat(abuf,&tsb)) >= 0) {
...
local int commandments_tmpcopy(CMS *op,char *tbuf,
	...
	int		rs ;
	int		rs1 ;
	if (char *xbuf ; (rs = mem.mall((xlen+1),&xbuf)) >= 0) {
...
local int commandments_idxmk(CMS *op,cchar *tbuf) noex {
	...
	if ((rs = cmimk_open(&mk,tbuf,of,om)) >= 0) {
	    if ((rs = cmimk_setdb(&mk,op->db_size,op->ti_db)) >= 0) {
...
local int mkent_start(mkent *ep,int cn,uint eoff,uint elen) noex {
	...
	if (ep) {
	    ...
	    ep->eoff = eoff ;
	    ep->elen = elen ;
	    if (void *vp ; (rs = mem.call(ne,lsz,&vp)) >= 0) {
