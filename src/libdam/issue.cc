...
int issue_process(ISSUE *op,cchar *groupname,cchar **adms,int fd) noex {
	int		rs ;
	int		n ;
	int		size ;
	int		wlen = 0 ;
...
static int issue_mapfind(ISSUE *op,time_t dt) noex {
	int		rs ;
...
static int issue_maplose(ISSUE *op) noex {
	int		rs = SR_OK ;
	int		rs1 ;
...
static int issue_mapfname(ISSUE *op,char fbuf[]) noex {
	vecstr		scheds ;
	int		rs = SR_OK ;
	int		rs1 ;
	int		c = 0 ;
...
static int issue_schedload(ISSUE *op,vecstr *slp) noex {
	int		rs = SR_OK ;
	int		i ;
	cchar		*keys = "pen" ;
...
static int issue_checker(ISSUE *op,time_t dt) noex {
	int		rs = SR_OK ;
	int		nchanged = 0 ;
	if (op->nmaps > 0) {
...
static int issue_envbegin(ISSUE *op) noex {
	cint	es = envpre[0] ;
...
static int issue_envend(ISSUE *op) noex {
	int		rs = SR_OK ;
	int		rs1 ;
...
static int issue_envadds(ISSUE *op,STRPACK *spp,cchar **ev,
		cchar *keyname) noex {
...
static int issue_processor(ISSUE *op,cchar **ev,cchar **adms,cchar *gn,
		int fd) noex {
	int		rs ;
	int		wlen = 0 ;
...
#if	CF_ISSUEAUDIT
static int issue_audit(ISSUE *op) noex {
	int		rs ;
...
static int mapper_start(ISSUE_MAPPER *mmp,time_t dt,cchar fname[]) noex {
	int		rs ;
...
static int mapper_finish(ISSUE_MAPPER *mmp) noex {
	int		rs = SR_OK ;
	int		rs1 ;
...
static int mapper_check(ISSUE_MAPPER *mmp,time_t dt) noex {
...
static int mapper_mapadd(ISSUE_MAPPER *mmp,cchar *kp,int kl,
		cchar *vp,int vl) noex {
	ISSUE_MAPDIR	*ep ;
	cint	size = szof(ISSUE_MAPDIR) ;
	int		rs ;
...
static int mapper_mapfins(ISSUE_MAPPER *mmp) noex {
...
#if	CF_MAPPERAUDIT
static int mapper_audit(ISSUE_MAPPER *mmp) noex {
	vechand		*mlp = &mmp->mapdirs ;
	int		rs ;
...
static int mapdir_start(ISSUE_MAPDIR *ep,cchar *kp,int kl,
		cchar *vp,int vl) noex {
	int		rs ;
...
static int mapdir_finish(ISSUE_MAPDIR *ep) noex {
	int		rs = SR_OK ;
	int		rs1 ;
...
static int mapdir_expand(ISSUE_MAPDIR *ep) noex {
	cint	to_lock = TO_LOCK ;
	int		rs ;
...
static int mapdir_expander(ISSUE_MAPDIR *ep) noex {
	int		rs = SR_OK ;
	int		rs1 = SR_OK ;
	int		fl = 0 ;
...
static int mapdir_processor(ISSUE_MAPDIR *ep,cchar **ev,
		cchar *gn,int fd) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	int		n ;
...
static int mapdir_procouter(ISSUE_MAPDIR *ep,cchar **ev,cchar *fname,
		int ofd) noex {
...
static int loadstrs(cchar **strs,cchar *gn,cchar *def,cchar *all,
		cchar	*n) noex {
	int		i = 0 ;
