...
local int	ipasswd_hdrload(ipasswd *) noex ;
local int	ipasswd_hdrloader(ipasswd *) noex ;
local int	ipasswd_enterbegin(ipasswd *,time_t) noex ;
local int	ipasswd_enterend(ipasswd *,time_t) noex ;

local int	ipasswd_fileopen(ipasswd *,time_t) noex ;
local int	ipasswd_fileclose(ipasswd *) noex ;
local int	ipasswd_mapbegin(ipasswd *,time_t) noex ;
local int	ipasswd_mapend(ipasswd *) noex ;
local int	ipasswd_remotefs(ipasswd *) noex ;
local int	ipasswd_keymatchfl3(ipasswd *,int,int,realname *) noex ;
local int	ipasswd_keymatchl3(ipasswd *,int,int,realname *) noex ;
local int	ipasswd_keymatchl1(ipasswd *,int,int,realname *) noex ;
local int	ipasswd_keymatchf(ipasswd *,int,int,realname *) noex ;
local int	ipasswd_keymatchall(ipasswd *,int,int,realname *) noex ;

local int	ipaswd_mapcheck(ipasswd *,time_t) noex ;

#ifdef	COMMENT
local int	ipasswd_keymatchlast(ipasswd *,int,int,char *,int) noex ;
#endif
	...
	if (op && dbname) ylikely {
	    ...
	    if ((rs = rsv) >= 0) ylikely {
	        if ((rs = pagesize) >= 0) ylikely {
	            ...
	            op->oflags = O_RDONLY ;
	            op->operm = 0666 ;
	            if (char *dbfname ; (rs = lm_mp(&dbfname)) >= 0) {
	                if ((rs = mkourfname(dbfname,dbname)) >= 0) {
	                    cchar	*cp ;
	                    if ((rs = lm_strw(dbfname,-1,&cp)) >= 0) {
	                        op->fname = cp ;
...
int ipasswd_close(ipasswd *op) noex {
	int		rs ;
	int		rs1 ;
	if ((rs = ipasswd_magic(op)) >= 0) ylikely {
...
local int ipasswd_hdrload(ipasswd *op) noex {
	int		rs = SR_OK ;
...
local int ipasswd_keymatchall(ipasswd *op,int opts,int ri,realname *rp) noex {
	int		si ;
	int		f = true ;
	cchar		*s = op->stab ;
	cchar		*sp ;
