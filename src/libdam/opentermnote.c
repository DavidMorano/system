...
int opentermnote(cchar *pr,cchar **recips,int max,int opts) noex {
	int		rs ;
	int		pipes[2] ;
	int		fd = -1 ;
...
static int targs_finish(TARGS *tap) noex {
	int		rs = SR_OK ;
	int		rs1 ;
...
static int termnoter(TARGS *tap) noex {
	TERMNOTE	tn ;
	int		rs ;
