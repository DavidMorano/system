...
static int	procmsg(proginfo *,procdata *, char *,int) noex ;
static int	procmsgenv(proginfo *,msginfo *, MSG *,char *,int) noex ;
static int	ext_id(struct proginfo *,char *,char *) noex ;
static int	ph_write() noex ;
static int	didheader(vecstr *,struct msg_header *) noex ;
static int	sfcomment(cchar *,int,char **) noex ;
static int	cheapspamcheck(struct proginfo *,cchar *,int) noex ;

static inline bool hasfmat(cchar *lp,int ll) noex {
    	return ((ll > 5) && (lp[0] == 'F')) ;
}
...
constexpr char		atypes[] = "LUIR" ;	/* address types */
...
int process(proginfo *pip,bfile *ifp,bfile *tfp,vecobj *fip) noex {
	pcsconf		*pp = pip->pp ;
	procdata	pd{} ;
	LOGZONES	lz ;
	cnullptr	np{} ;
...
/* match on either an envelope or a header! */
	    ...
	    while (hasfmat(lbuf,len)) {
		rs = mailmsgmatenv(&pd.ei,lbuf,len) ;
		if (rs <= 0) break ;
	        if ((rs = mailmsgmathdr(lbuf,len,np)) != 0) break ;
	        if ((len <= 1) && (lbuf[0] == '\n')) break ;
		pd.off_start = pd.offset ;
	        rs = breadln(ifp,lbuf,MAXLINELEN) ;
		len = rs ;
	        pd.offset += len ;
		if (rs < 0) break ;
	    } /* end while */
...
static int cheapspamcheck(proginfo *pip,cchar *buf,int buflen) noex {
	int	sl, cl ;
	char	*sp, *cp ;
	sp = (char *) buf ;
