...
constexpr char		appleterminal	[] = "Apple_Terminal" ;
...
int proginfo::getpn(mainv names) noex {
	...
	if (argv) ylikely {
	    rs = SR_INVALID ;
	    if ((argc > 0) && (argv[0] != nullptr)) ylikely {
	        cchar	*bp{} ;
	        if (int bl ; (bl = sfbasename(argv[0],-1,&bp)) > 0) ylikely {
		    int		pl = rmchr(bp,bl,'.') ;
		    cchar	*pp = bp ;
		    if (pl > 0) ylikely {
	                if ((pm = matstr(names,pp,pl)) >= 0) ylikely {
			    pn = names[pm] ;
		            rs = pm ;
	                }
...
int proginfo::termout() noex {
	...
	int		rs = SR_OK ;
	if (cchar *tn ; (tn = getourenv(envv,vn)) != nullptr) ylikely {
