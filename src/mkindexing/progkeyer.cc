...
#if	CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("progkeyer: line> %r", lbuf,len) ;
#endif
...
#if	CF_DEBUG
	                    if (DEBUGLEVEL(4))
	                        debugprintf("progkeyer: lowercase word=%r\n",
	                            sp,sl) ;
#endif
...
#if	CF_DEBUG
	                    if (DEBUGLEVEL(4))
	                        debugprintf("progkeyer: wl=%u wp=>%r<\n",
	                            cl,cp,cl) ;
#endif
	...
	if ((pip->maxkeys == 0) || (n < pip->maxkeys)) {
	    if ((wl > 0) && (wl <= NATURALWORDLEN)) {
	        if (wl >= pip->minwordlen) {
	            ...
	            if (DEBUGLEVEL(4)) {
	                if (wl > 0) {
	                    debugprintf("progkeyer/procword: key=%r\n",wp,wl) ;
	                } else {
