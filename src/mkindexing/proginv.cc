...
static int procdnamecheck(PROGINFO *pip,cchar *dname,mode_t m)
{
	...
	if ((rs = u_stat(dname,&sb)) >= 0) {
		...
		const int	am = (X_OK|W_OK) ;
	        if ((rs = progids_permids(pip,&sb,am)) == nrs) {
	            rs = mkdirs(dname,m) ;
	        }
	    } else {
	        rs = SR_NOTDIR ;
...
/* go to it, read the file line by line */
	    ...
	    while (rs >= 0) {
	        ...
	        if (DEBUGLEVEL(4)) {
	            ...
	            debugprintf("proginv/procfile: line> %r\n",
	                ...
	                if (sp[0] == '-') {
	                    ...
	                    if ((rs >= 0) && (si > 0)) {
	                        ...
	                        if (DEBUGLEVEL(4)) {
	                            debugprintf("proginv/procfile: "
	                                "ktag_start()\n") ;
	                            debugprintf("proginv/procfile: >%r<\n",
	                                sip,
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("proginv/proccmd: cmd=>%r<\n",
...
#if	CF_DEBUGS
	debugprintf("ktag_start: fname=>%r<\n",fname,tip->fnamelen) ;
#endif
