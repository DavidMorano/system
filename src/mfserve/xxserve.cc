...
#if	CF_DEBUG
	if (DEBUGLEVEL(4)) {
	    debugprintf("procserver/procserverlib: shlib=%r\n",pnp,pnl) ;
	    debugprintf("procserver/procserverlib: entry=%r\n",enp,enl) ;
	}
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(4)) {
	    debugprintf("procserver/procserverlib: 2 entry=%r\n",enp,enl) ;
	}
#endif
...
/* can we execute this service daemon? */
	...
	if (DEBUGLEVEL(4)) {
	    ...
	    debugprintf("procserver: shlib=%r\n",pnp,pnl) ;
	}
#endif
...
/* can we execute this service daemon? */
	...
	if (DEBUGLEVEL(4)) {
	    debugprintf("procserver: past X check\n") ;
	    debugprintf("procserver: program=%r\n",pnp,pnl) ;
	}
#endif
...
#if	CF_DEBUG
	if (DEBUGLEVEL(5))
	    debugprintf("progserve/procfindprog: pn=%r\n",pnp,pnl) ;
#endif
