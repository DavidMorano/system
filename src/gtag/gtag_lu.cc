...
char		keys[] ;
{
	...
	bdbgetkey(bdbp,nullptr,&database) ;
...
void bufdesc_init(struct bufdesc *bdp) noex {
	bdp->buf[0] = '\0' ;
	bdp->len = 0 ;
...
void br_init(struct br *brp) noex {
	bufdesc_init(&brp->a) ;
	bufdesc_init(&brp->t) ;
	bufdesc_init(&brp->d) ;
	bufdesc_init(&brp->j) ;
