...
#if	CF_DEBUG
	    if (DEBUGLEVEL(2)) {
	        debugprintf("main: VS key=%r\n",kp,strnlen(kp,kl)) ;
	        if (vp != NULL)
	            debugprintf("main: VS val=>%r<\n",vp,strnlen(vp,vl)) ;
	    }
#endif
...
#if	CF_DEBUGS
	debugprintf("vecstr_addpair: key=%r\n",kp,strnlen(kp,kl)) ;
	if (vp != NULL)
	    debugprintf("vecstr_addpair: val=>%r<\n",vp,strnlen(vp,vl)) ;
#endif
...
/* process the filter file first */
	...
	fbp = fbuf ;
	line = 0 ;
	while ((rs = breadln(ffp,lbuf,LINEBUFLEN)) > 0) {
