...
/*******************************************************************************
	...
	Description:
	Compare TIMEVAL objects.

	Synopses:
	int cmptermeval(CTIMEVAL *e1p,CTIMEVAL *e2p) noex
...
int cmptimeval(CTIMEVAL *e1p,CTIMEVAL *e2p) noex {
	int		rc = 0 ;
	if (e1p || e2p) ylikely {
	    rc = +1 ;
	    if (e1p) {
		...
		if (e2p) {
		    if ((rc = intcmp(e1p->tv_sec,e2p->tv_sec)) == 0) {
	    	        rc = intcmp(e1p->tv_usec,e2p->tv_usec) ;
		    }
		} /* end if */
	    } /* end if */
	} /* end if */
	return rc ;
} /* end subroutine (cmptimeval) */
