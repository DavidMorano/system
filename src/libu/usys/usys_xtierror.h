...
#if	defined(SYSHAS_XTIERROR) && (SYSHAS_XTIERROR > 0)

/* congratulations: you have it! */

#else /* defined(SYSHAS_XTIERROR) && (SYSHAS_XTIERROR > 0) */
...
/*
 * Following new error codes added to namespace with introduction of XTIERROR
 */
 ...
 * used with TCP level options by XTIERROR for historical XTIERROR specification
 * reasons.
...
/*
 * XTIERROR defined value reserved for stating 'unspecified' value used
 * in some option namespaces.
 */
#define	T_UNSPEC	(~0-2)	/* applicable to ulong_t, long, char */

/*
 * XTIERROR inspired option management defined an option name T_ALLOPT
