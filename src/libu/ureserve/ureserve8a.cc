/* ureserve8a MODULE (vecstr - implementation{) */
/* charset=ISO8859-1 */
/* lang=C++20 */
...
/* if found returns the value length, else SR_NOTFOUND */
int vecstr::findkey(cchar *sp,int sl,cchar **rpp) noex {
    	int		rs = SR_FAULT ;
	int		rs1 ;
	int		rl = -1 ;
	if (sp) {
	    if (vecstr_cur cur ; (rs = curbegin(&cur)) >= 0) {
		for (cc *ep ; (rs = curenum(&cur,&ep)) >= 0 ; ) {
		    if (strnkeycmp(ep,sp,sl) == 0) {
			if (cc *tp = strochr(ep,'=') ; tp) {
			    rl = lenstr(tp + 1) ;
			    if (rpp) {
			       *rpp = (tp + 1) ;
			    }
