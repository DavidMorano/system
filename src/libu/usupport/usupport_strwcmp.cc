...
using libu::strnbasecmp ;		/* subroutine */
using libu::strncasecmp ;		/* subroutine */
using libu::strnfoldcmp ;		/* subroutine */
...
int cmpx::x(cchar *s1,cchar *s2,int µs2len) const noex {
	...
	if (s1 && s2) {
	    rc = tox(s1[0]) ;
	    if (cint s2len = getlenstr(s2,µs2len) ; s2len > 0) {
	        cint	ch1 = tox(*s1) ;
	        cint	ch2 = tox(*s2) ;
	        if ((rc = (ch1 - ch2)) == 0) {
