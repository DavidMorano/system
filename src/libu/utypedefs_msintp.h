/* utypedefs_msintp HEADER (Integer-Signed-Mutable) */
/* charset=ISO8859-1 */
/* lang=C20 */

/* virtual-system definitions */
/* version %I% last-modified %G% */


/* revision history:

	= 1998-03-21, David A­D­ Morano
	This module was originally written.

	= 2017-08-01, David A­D­ Morano
	Updated for lack of interfaces in MacOS Darwin

*/

/* Copyright © 1998,2017 David A­D­ Morano.  All rights reserved. */

#ifndef	UTYPEDEFSMSINTP_INCLUDE
#define	UTYPEDEFSMSINTP_INCLUDE


#include	<envstandards.h>	/* MUST be first to configure */
#include	<usysnative.h>		/* useful native system headers */
#include	<clanguage.h>		/* <- necessary inclusion */
#include	<stdintx.h>		/* extended integer types */
#include	<syspredefined.h>	/* system pre-defined types */


#ifndef	TYPEDEF_SCHARP
#define	TYPEDEF_SCHARP
typedef signed char *		scharp ;
#endif /* TYPEDEF_SCHARP */

#ifndef	TYPEDEF_SSHORTP
#define	TYPEDEF_SSHORTP
typedef signed short *		sshortp ;
#endif /* TYPEDEF_SSHORTP */

#ifndef	TYPEDEF_SINTP
#define	TYPEDEF_SINTP
typedef signed int *		sintp ;
#endif /* TYPEDEF_SINTP */

#ifndef	TYPEDEF_SLONGP
#define	TYPEDEF_SLONGP
typedef signed long *		slongp ;
#endif /* TYPEDEF_SLONGP */


#endif /* UTYPEDEFSMSINTP_INCLUDE */


