...
#include	<numeric>		/* STD-C++ |saturate_cast(3c++)| */
...
namespace {
	...
	uopen		*op = nullptr ;
	int		w = -1 ;
	void operator () (uopen *p,int m) noex {
	    op = p ;
	    w = m ;
	} ;
	operator int () noex ;
	...
	uopen_fl	fl ;
	uopen() noex {
	    lockbegin(this,uopenmem_lockbegin) ;
	    lockend(this,uopenmem_lockend) ;
	} ;
	int init() noex ;
	int fini() noex ;
...
namespace libu {
	...
	return uopen_data.lockbegin ;
    }
    sysret_t uopen_lockend() noex {
	return uopen_data.lockend ;
    }
} /* end namespace (libu) */
...
int uopen::init() noex {
	...
	if (! fvoid) {
	    ...
	    if (! finit.testandset) {
	        if ((rs = mx.create) >= 0) {
	            void_f	b = uopen_atforkbefore ;
	            void_f	a = uopen_atforkafter ;
