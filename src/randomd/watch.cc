...
extern int	handle(struct proginfo *,
			int,int,varsub *,vecstr *,SRVTAB *,ACCTAB *,BUILTIN *) ;
...
int watch(gp,s,ssp,elp,sfp,atp,bip)
struct proginfo	*gp ;
int		s ;
varsub		*ssp ;
vecstr		*elp ;
...
/* we are now the CHILD !! */
			...
			gp->logid = jep->jobid ;
	                ...
	                rs = handle(gp,ns,ns,ssp,elp,sfp,atp,bip) ;
...
/* process this guy's termination */
	        ...
	        if ((efd = u_open(jep->efname,O_RDONLY,0666)) >= 0) {
	            ...
	            logfile_printf(&gp->lh, "%s server exit, es=%d\n",
...
/* maintenance the lock file */
	...
	if ((gp->lockfp != NULL) && 
	    ...
	    if ((rs = checklockfile(gp->lockfp,gp->lockfname,daytime)) != 0) {
...
int watchone(gp,ns,ssp,elp,sfp,atp,bip)
struct proginfo	*gp ;
int		ns ;
varsub		*ssp ;
vecstr		*elp ;
...
/* call the handler */
	        ...
	        rs = handle(gp,ns,FD_STDOUT,ssp,elp,sfp,atp,bip) ;
