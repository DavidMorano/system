...
extern int	cfdecti(char *,int,int *) ;
...
static unsigned char 	oterms[32] = {
	0x00, 0x0B, 0x00, 0x00,
	0x09, 0x10, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00,
...
/* loop through the DEFINEd variables in the configuration file */
	    ...
	    for (i = 0 ; vecstr_get(&cf.defines,i,&cp) >= 0 ; i += 1) {
	        ...
	        if ((sl > 0) &&
	            ...
	            varsub_merge(&vsh_d,&defines,buf2,sl2) ;
...
/* all of the rest of the configuration file stuff */
	    ...
	    if ((cf.port != NULL) && (portspec == NULL)) {
	        ...
	        if (((sl = varsub_subbuf(&vsh_d,&vsh_e,cf.port,
	            -1,buf,BUFLEN)) > 0) &&
	            ((sl2 = expander(&g,buf,sl,buf2,BUFLEN)) > 0)) {
	    ...
	    if ((cf.userpass != NULL) && (pip->userpass == NULL)) {
	        ...
	        if (((sl = varsub_subbuf(&vsh_d,&vsh_e,cf.userpass,
	            -1,buf,BUFLEN)) > 0) &&
	            ((sl2 = expander(&g,buf,sl,buf2,BUFLEN)) > 0)) {
	    ...
	    if ((cf.machpass != NULL) && (pip->machpass == NULL)) {
	        ...
	        if (((sl = varsub_subbuf(&vsh_d,&vsh_e,cf.machpass,
	            -1,buf,BUFLEN)) > 0) &&
	            ((sl2 = expander(&g,buf,sl,buf2,BUFLEN)) > 0)) {
...
/* loop through the EXPORTed variables in the configuration file */
	    ...
	    for (i = 0 ; vecstr_get(&cf.exports,i,&cp) >= 0 ; i += 1) {
	        ...
	        if ((sl > 0) &&
	            ...
	            varsub_merge(NULL,&exports,buf2,sl2) ;
...
/* get a serial number for logging purposes */
	...
	bufprintf(buf,MAXPATHLEN + 1,"%s/spool/%s",
		pip->programroot,"serial") ;

	rs = getserial(buf) ;

	if (rs < 0)
		rs = getserial(SERIALFNAME) ;
