...
#include	<getlogx.h>		/* LIBUC |getloghost(3dam)| */
#include	<realname.h>
#include	<pwfile.h>
#include	<user.h>		/* |userdomain(3uc)| */
	        ...
	        if ((ci = matostr(qopts,2,argv[i],-1)) >= 0) {
	            ...
	            case qopt_udomain:
	                if (f_entok) {
	                    ...
	                    cp = entry.username ;
	                    rs = userdomain(np,udomainname,-1,cp) ;
	                    if (rs <= 0) {
