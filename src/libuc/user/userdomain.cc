...
/*******************************************************************************
	...
	and are administered through the file 'etc/userdomain' located
	...
	Synopsis:
	int userdomain(cchar *pr,char *dbuf,int dlen,cchar *username) noex
	...
	Format of file entries:
		username	domainname
	Note that this strategy of reading the "userdomain" file is obsoleted by
...
#ifndef	UDOMASTDFNIN
#define	UDOMASTDFNIN	"/etc/userdomain"
#endif
...
extern "C" {
    int userdomain(cchar *,char *,int,cchar *) noex ;
} /* end extern (C) */
...
namespace {
	...
	int userdomainer(cchar *) noex ;
	int userdomainerm(cchar *) noex ;
	int userdomainerf(cchar *) noex ;
	    ...
	    if (username[0]) ylikely {
	        ...
	        if (pr && (pr[0] != '\0') && (strcmp(pr,"/") != 0)) {
		     if (char *fbuf ; (rs = lm_mp(&fbuf)) >= 0) {
	             ...
	             rs = a.userdomainer(fname) ;
		     len = rs ;
	         }
	    } /* end if (valid) */
	} /* end if (non-null) */
	return (rs >= 0) ? len : rs ;
} /* end subroutine (userdomain) */


/* local subroutines */

int uargs::userdomainer(cchar *fname) noex {
	int		rs ;
	int		len = 0 ;
	if (ustat sb ; (rs = u_stat(fname,&sb)) >= 0) {
	    if (S_ISREG(sb.st_mode) && (sb.st_size < maxfilesz)) {
		rs = userdomainerm(fname) ;
		len = rs ;
	    } else {
		rs = userdomainerf(fname) ;
		len = rs ;
	    } /* end if (type of file) */
	} /* end if (stat) */
	return (rs >= 0) ? len : rs ;
} /* end subroutine (uargs::userdomainer) */

int uargs::userdomainerm(cchar *fname) noex {
	csize		mfsize = size_t(maxfilesz) ;
	int		rs ;
	int		rs1 ;
	int		len = 0 ;
	...
	return (rs >= 0) ? len : rs ;
} /* end method (uargs::userdomainerm) */

int uargs::userdomainerf(cchar *fname) noex {
	int		rs ;
	int		rs1 ;
	int		len = 0 ;
	...
	return (rs >= 0) ? len : rs ;
} /* end method (uargs::userdomainerf) */
