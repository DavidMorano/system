...
local inline int hdb_ctor(hdb *op,Args ... args) noex {
	int		rs = SR_FAULT ;
	if (op && (args && ...)) ylikely {
	    rs = SR_NOMEM ;
	    op->magic = 0 ;
	    ...
	    op->count = 0 ;
	    op->at = 0 ;
	    if ((op->esp = new(nothrow) lookaside) != nullptr) ylikely {
		rs = SR_OK ;
	    }
	} /* end if (non-null) */
	return rs ;
...
local inline int hdb_dtor(hdb *op) noex {
	int		rs = SR_FAULT ;
	if (op) ylikely {
	    rs = SR_OK ;
	    if (op->esp) ylikely {
...
local int	hdb_entnew(hdb *,ENT **) noex ;
local int	hdb_entdel(hdb *,ENT *) noex ;

local int	hdb_findkeye(hdb *,FETCUR *,HDB_D *,ENT **) noex ;
local int	hdb_get(hdb *,int,DAT,CUR *,HDB_D *,DAT *) noex ;

local int	hdb_ext(hdb *) noex ;
local int	hdb_extinsert(hdb *,ENT **,int,ENT *) noex ;
local int	hdb_extkeyfree(hdb *,ENT *) noex ;
local int	hdb_getentry(hdb *,ENTRYINFO *,CUR *) noex ;

local int	entry_load(ENT *,uint,HDB_D *,DAT *) noex ;
local int	entry_match(ENT *,hdbcmp_f,uint,HDB_D *) noex ;

local int	cursor_stabilize(CUR *) noex ;
local int	cursor_inc(CUR *) noex ;

local int	fetchcur_load(FETCUR *,fc_f,int,CUR *,HDB_D *) noex ;
local int	fetchcur_adv(FETCUR *) noex ;

local ENT	**getpoint(hdb *,uint,HDB_D *) noex ;
...
int hdb_start(hdb *op,int n,int at,hdbhash_f h,hdbcmp_f c) noex {
	...
	if ((rs = hdb_ctor(op)) >= 0) ylikely {
	    ...
	    if (rs >= 0) ylikely {
		cint	tsize = (n * esz) ;
	        if (void *vp ; (rs = libmem.mall(tsize,&vp)) >= 0) ylikely {
	            ENT		**hepp = (ENT **) vp ;
	            memclear(hepp,tsize) ;
	            op->htlen = n ;
...
int hdb_store(hdb *op,HDB_D key,DAT val) noex {
	...
	if ((rs = hdb_magic(op,key.buf)) >= 0) ylikely {
	    ...
	    if (rs >= 0) ylikely {
	        if (ENT *ep{} ; (rs = hdb_entnew(op,&ep)) >= 0) ylikely {
	            ENT		*ohp ;
...
int hdb_curbegin(hdb *op,CUR *curp) noex {
	int		rs ;
	if ((rs = hdb_magic(op,curp)) >= 0) ylikely {
	    *curp = icur ;
	} /* end if (magic) */
	return rs ;
}
...
int hdb_curcopy(hdb *op,CUR *curp,CUR *othp) noex {
	int		rs ;
	if ((rs = hdb_magic(op,curp,othp)) >= 0) ylikely {
	    *othp = *curp ;
	} /* end if (magic) */
	return rs ;
}
...
int hdb_fetchrec(hdb *op,DAT key,CUR *curp,DAT *keyp,DAT *valp) noex {
	...
	if ((rs = hdb_magic(op,curp,key.buf)) >= 0) ylikely {
            ...
            if ((rs = fetchcur_load(&fc,hf,htlen,&ncur,&key)) >= 0) ylikely {
                ENT         *ep{} ;
...
int hdb_hashtablen(hdb *op,uint *rp) noex {
	int		rs ;
	if ((rs = hdb_magic(op,rp)) >= 0) ylikely {
	    *rp = op->htlen ;
	} /* end if (magic) */
	return rs ;
}
...
local int hdb_entnew(hdb *op,ENT **epp) noex {
	int		rs ;
	if (op->at == 0) {
	    cint	esz = szof(ENT) ;
...
local int hdb_extinsert(hdb *op,ENT **htaddr,int htlen,ENT *hep) noex {
	ENT		*nhep = nullptr ;
	int		rs ;
	if ((rs = hdb_entnew(op,&nhep)) >= 0) ylikely {
	    hdbhv	hv = hep->hv ;
	    int		hi ;
