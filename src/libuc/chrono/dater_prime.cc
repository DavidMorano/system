...
/*******************************************************************************
	...
	are due to timezone differences and leap-years and leap-seconds.
	The fact that timezone data is not a part of many date-related
	functions only complicates matters worse as we then have
	to try and figure out a reasonable timezone when they are
	not supplied.

	Comment parsing for RFC-822 dates:
	Note that dates given to us for mail-message (MSG) (RFC-822)
	processing, might have comments in them.  These comments
	are those specified in RFC-822.  Note that the tmz object
	does some comment processing on MSG dates but only in the
	same way that NetNews does ; namely, only for a comment at
	the end of the string and which contains a time-zone-name.
...
local int	dater_initcur	(dater *) noex ;
local int	dater_ldname	(dater *,cchar *,int) noex ;
local int	dater_pname	(dater *) noex ;
local int	dater_pnum	(dater *) noex ;
local int	dater_findzname	(dater *) noex ;
local int	dater_findzoff	(dater *,TM *) noex ;
local int	dater_mkptime	(dater *,TM *,int) noex ;
local int	dater_mktime	(dater *,TM *) noex ;
local int	dater_mkpzoff	(dater *,TM *,int) noex ;

local int	dater_initbase	(dater *) noex ;
local int	dater_ldtmz	(dater *,tmz *) noex ;
local int	dater_ldcomzone	(dater *,comparse *) noex ;
local int	dater_ldzname	(dater *,cchar *,int) noex ;
local int	dater_defs	(dater *,tmz *) noex ;

local inline int znwcpy(char *zb,int zl,cc *zsp,int zsl = -1) noex {
	cint rs = intconv(strnwcpylc(zb,zl,zsp,zsl) - zb) ; /* <- lower-case */
	zb[zl] = '\0' ;
	return rs ;
} /* end subroutine (znwcpy) */


/* local variables */

static bufsizevar	znlen			(bufsize_zn) ;
cint			SecondsPerMinute	= 60 ;
...
int dater_startcopy(dater *op,dater *d2p) noex {
	...
	int		rs = SR_FAULT ;
	if (op && d2p) ylikely {
	    if ((rs = dater_start(op,np,np,0)) >= 0) ylikely {
	        cint	znl = op->znamelen ;
	        strncpy(op->cname,d2p->cname,(znl + 1)) ;
	        strncpy(op->zname,d2p->zname,(znl + 1)) ;
	        op->cb		= d2p->cb ;
	        op->b		= d2p->b ;
	        op->fl		= d2p->fl ;
	        op->cyear	= d2p->cyear ;
...
int dater_finish(dater *op) noex {
	int		rs ;
	int		rs1 ;
	if ((rs = dater_magic(op)) >= 0) ylikely {
	    if (op->cname) {
		rs1 = lm_free(op->cname) ;
		if (rs >= 0) rs = rs1 ;
		op->cname = nullptr ;
		op->zname = nullptr ;
	    } /* end if (cname) */
...
int dater_setcopy(dater *op,dater *d2p) noex {
	int		rs ;
	if ((rs = dater_magic(op,d2p)) >= 0) ylikely {
	    cint	znl = op->znamelen ;
	    strncpy(op->cname,d2p->cname,(znl + 1)) ;
	    strncpy(op->zname,d2p->zname,(znl + 1)) ;
	    op->cb	= d2p->cb ;
...
int dater_setstd(dater *op,cchar *sp,int sl) noex {
	...
	if ((rs = dater_magic(op,sp)) >= 0) ylikely {
	    if (sl < 0) sl = lenstr(sp) ;
	    if (tmz stz ; (rs = tmz_xstd(&stz,sp,sl)) >= 0) ylikely {
	        TM	dst = stz.st ;
	        dater_ldtmz(op,&stz) ;
...
int dater_setmsg(dater *op,cchar *sp,int sl) noex {
	...
	if ((rs = dater_magic(op,sp)) >= 0) ylikely {
	    if (sl < 0) sl = lenstr(sp) ;
	    if (comparse vc ; (rs = comparse_start(&vc,sp,sl)) >= 0) ylikely {
	        cchar	*vp{} ;
	        if (int vl ; (rs = comparse_getval(&vc,&vp)) >= 0) ylikely {
	            vl = rs ;
	            if (tmz stz ; (rs = tmz_xmsg(&stz,vp,vl)) >= 0) ylikely {
	                TM	dst = stz.st ;
	                dater_ldtmz(op,&stz) ;
...
int dater_setstrdig(dater *op,cchar *sp,int sl) noex {
	...
	if ((rs = dater_magic(op,sp)) >= 0) ylikely {
	    if (sl < 0) sl = lenstr(sp) ;
	    if (tmz stz ; (rs = tmz_xstrdig(&stz,sp,sl)) >= 0) ylikely {
	        TM	dst = stz.st ;
	        dater_ldtmz(op,&stz) ;
	        if ((rs = dater_defs(op,&stz)) >= 0) ylikely {
	            rs = dater_mktime(op,&dst) ;
		    rv = rs ;
	        } /* end if (dater_defs) */
	    } /* end if tmz_xstrdig) */
...
int dater_setlogz(dater *op,cchar *sp,int sl) noex {
	...
	if ((rs = dater_magic(op,sp)) >= 0) ylikely {
	    if (sl < 0) sl = lenstr(sp) ;
	    if (tmz stz ; (rs = tmz_xlogz(&stz,sp,sl)) >= 0) ylikely {
	        TM	dst = stz.st ;
	        dater_ldtmz(op,&stz) ;
	        if ((rs = dater_defs(op,&stz)) >= 0) ylikely {
	            rs = dater_mktime(op,&dst) ;
...
int dater_settouch(dater *op,cchar *sp,int sl) noex {
	...
	if ((rs = dater_magic(op,sp)) >= 0) ylikely {
	    if (sl < 0) sl = lenstr(sp) ;
	    if (tmz stz ; (rs = tmz_xtouch(&stz,sp,sl)) >= 0) ylikely {
	        TM	dst = stz.st ;
	        dater_ldtmz(op,&stz) ;
	        if ((rs = dater_defs(op,&stz)) >= 0) ylikely {
	            rs = dater_mktime(op,&dst) ;
...
int dater_settoucht(dater *op,cchar *sp,int sl) noex {
	...
	if ((rs = dater_magic(op,sp)) >= 0) ylikely {
	    if (sl < 0) sl = lenstr(sp) ;
	    if (tmz stz ; (rs = tmz_xtoucht(&stz,sp,sl)) >= 0) ylikely {
	        TM	dst = stz.st ;
	        dater_ldtmz(op,&stz) ;
	        if ((rs = dater_defs(op,&stz)) >= 0) {
	            rs = dater_mktime(op,&dst) ;
...
int dater_settmzon(dater *op,TM *stp,int zoff,cchar *zstr,int zlen) noex {
	int		rs ;
	int		rv = 0 ; /* return-value */
	if ((rs = dater_magic(op,stp)) >= 0) ylikely {
	    rs = SR_INVALID ;
		...
		/* lookup the zone-name */
			...
			cint	znl = rs ;
	                op->fl.zname = true ;
	                znwcpy(op->zname,znl,zstr,zlen) ;
	                if (zdb zr ; (rs = zr.nameoff(zstr,zlen,zoff)) >= 0) {
	                    if (stp->tm_isdst < 0) stp->tm_isdst = zr.isdst ;
	                    if (zoff == TZO_EMPTY) zoff = zr.woff ;
...
int dater_settmzn(dater *op,TM *stp,cchar *zstr,int zlen) noex {
	...
	if ((rs = dater_magic(op,stp)) >= 0) ylikely {
	    int		zoff = TZO_EMPTY ;
	    /* initialize */
	    if ((rs = dater_initbase(op)) >= 0) ylikely {
	        /* lookup the zone-name */
	        if (zstr && (zstr[0] != '\0')) {
		    if ((rs = znlen) >= 0) {
			cint	znl = rs ;
	                op->fl.zname = true ;
	                znwcpy(op->zname,znl,zstr,zlen) ;
	                if (zdb zr ; (rs = zr.setname(zstr,zlen)) >= 0) {
	                    if (stp->tm_isdst < 0) {
				stp->tm_isdst = zr.isdst ;
			    }
	                    if (zoff == TZO_EMPTY) {
				zoff = zr.woff ;
			    }
	...
	if ((rs = dater_magic(op)) >= 0) ylikely {
	    if ((rs = dater_initbase(op)) >= 0) ylikely {
	        ...
	        if ((zname != nullptr) && (zname[0] != '\0')) {
		        ...
		        if ((rs = znlen) >= 0) {
			    cint	znl = rs ;
	                    znwcpy(op->zname,znl,znp) ;
		        } /* end if (znlen) */
	                if (isdst < 0) {
			    isdst = tmt.isdst ;
		        }
	                if (isdst >= 0) {
			    int mins = short(tmt.gmtoff / SecondsPerMinute) ;
	                    op->b.timezone = short(mins) ;
	                } /* end if (have TZ) */
	            } /* end if */
	            op->b.dstflag = shortconv(isdst) ;
	            op->fl.zoff = true ;
		...
		cint	znl = rs ;
	        ...
	        if ((zname != nullptr) && (zname[0] != '\0')) {
			    ...
			    isdst = tmt.isdst ;
	                ...
	                if (isdst >= 0) {
	                    cint mins = (tmt.gmtoff / SecondsPerMinute) ;
	                    op->b.timezone = shortconv(mins) ;
	                } /* end if (have TZ) */
	            } /* end if (tmtime) */
	            op->b.dstflag = shortconv(isdst) ;
	            op->fl.zoff = true ;
...
int dater_mkdatestr(dater *op,datertypes dt,char *dbuf,int dlen) noex {
	int		rs ;
	int		sl = 0 ;
	if ((rs = dater_magic(op,dbuf)) >= 0) ylikely {
	    rs = SR_INVALID ;
	    if ((dt >= 0) && (dt < datertype_overlast)) ylikely {
	        time_t	t	= op->b.time ;
	        int	zoff	= op->b.timezone ;
	        cchar	*znp	= op->zname ;
		...
		/* conversion */
	        t -= (zoff * SecondsPerMinute) ;
	        if (tmtime tmt ; (rs = tmt.timegm(t)) >= 0) ylikely {
	            tmt.gmtoff = (zoff * SecondsPerMinute) ;
		    if ((rs = znlen) >= 0) ylikely {
			cint	znl = rs ;
	        	cchar	*fmt = nullptr ;
	                strwcpyuc(tmt.znbuf,znp,znl) ;
	                switch (dt) {
		        /* UNIX® envelope */
	                case datertype_env:
	                    fmt = "%a %b %d %T %Z %Y %O" ;
...
int dater_mkstd(dater *op,char *dbuf,int dlen) noex {
	return dater_mkdatestr(op,datertype_std,dbuf,dlen) ;
...
int dater_mkmsg(dater *op,char *dbuf,int dlen) noex {
	return dater_mkdatestr(op,datertype_msg,dbuf,dlen) ;
...
int dater_getzoneoff(dater *op,int *zp) noex {
	int		rs ;
	if ((rs = dater_magic(op,zp)) >= 0) ylikely {
	    if (! op->fl.zoff) {
	        rs = SR_NOENT ;
	    }
	    if (zp) {
...
int dater_getzonename(dater *op,char *rbuf,int rlen) noex {
	int		rs ;
	int		zl = 0 ;
	if ((rs = dater_magic(op,rbuf)) >= 0) ylikely {
	    rbuf[0] = '\0' ;
	    if (op->fl.zname) {
...
int dater_zinfoget(dater *op,dater_zi *zip,char *zbuf,int zlen) noex {
	int		rs ;
	int		zl = 0 ;
	if ((rs = dater_magic(op,zip,zbuf)) >= 0) ylikely {
	    zip->zoff = op->b.timezone ;
	    zip->isdst = op->b.dstflag ;
	    if ((rs = sncpy(zbuf,zlen,op->zname)) >= 0) ylikely {
		zl = rs ;
		if (! op->fl.zoff) {
	            rs = SR_NOTFOUND ;
		}
...
int dater_zi(dater *op,dater_zi *zip,int ei) noex {
	...
	int		rs ;
	int		zl = 0 ;
	if ((rs = dater_magic(op,zip)) >= 0) ylikely {
	    memclear(zip) ;
	    rs = SR_NOTFOUND ;
		...
		if ((rs = znlen) >= 0) ylikely {
		    cint	znl = rs ;
	            ...
	            zip->zoff = zones[ei].woff ;
	            zl = strwcpy(zbuf,zones[ei].zname,znl) - zbuf ;
		} /* end if (znlen) */
	    } /* end if (valid) */
	} /* end if (magic) */
	return (rs >= 0) ? zl : rs ;
...
local int dater_initcur(dater *op) noex {
	int		rs = SR_OK ;
	if (! op->fl.cyear) {
	    if (! op->fl.cb) {
	    ...
	    op->fl.tzset = true ;
	    if (tmtime tmt ; (rs = tmt.timelocal(op->cb.time)) >= 0) ylikely {
		if ((rs = znlen) >= 0) {
		    cint	znl = rs ;
	            cint	zo = tmt.gmtoff ; /* seconds west of GMT */
	            op->cyear		= shortconv(tmt.year) ;
	            op->cb.timezone	= shortconv(zo / SecondsPerMinute) ;
	            op->cb.dstflag	= shortconv(tmt.isdst) ;
	            znwcpy(op->cname,znl,tmt.znbuf) ;
	            op->fl.cb		= true ;
	            op->fl.czn		= true ;
	            op->fl.cyear	= true ;
		} /* end if (znlen) */
...
local int dater_mkpzoff(dater *op,TM *stp,int zoff) noex {
	int		rs ;
	op->zname[0] = '\0' ;
	if (zdb zr ; (rs = zr.offisdst(zoff,stp->tm_isdst)) >= 0) ylikely {
	    if ((rs = znlen) >= 0)  ylikely {
		cint	znl = rs ;
...
local int dater_ldtmz(dater *op,tmz *tp) noex {
    	int		rs ;
	if ((rs = tp->getdst) >= 0) {
	    op->b.dstflag = shortconv(rs) ;
	    if (short zoff ; (rs = tp->getzoff(&zoff)) >= 0) {
	        op->fl.zoff = (rs > 0) ;
	        op->b.timezone = zoff ;
	        if (tp->zname[0] != '\0') ylikely {
	            if ((rs = znlen) >= 0) {
		        cint	znl = rs ;
...
local int dater_ldzname(dater *op,cchar *sp,int sl) noex {
    	int		rs = SR_OK ;
	int		len = 0 ;
	if (cchar *tp = strnbrk(sp,sl," \t") ; tp) {
	    sl = intconv(tp - sp) ;
	}
	if ((rs = znlen) >= 0) ylikely {
	    cint	znl = rs ;
	    len = znwcpy(op->zname,znl,sp,sl) ;
	} /* end if (znlen) */
	return (rs >= 0) ? len : rs ;
} /* end subroutine (dater_ldzname) */

local int dater_ldcomzone(dater *op,comparse *cpp) noex {
	int		rs = SR_OK ;
	if (op->zname[0] == '\0') {
	    cchar	*cp{} ;
	    if (int cl ; (cl = comparse_getcom(cpp,&cp)) > 0) {
	        dater_ldzname(op,cp,cl) ;
	    } /* end if (comparse_getcom) */
	} /* end if (zname) */
	return rs ;
} /* end subroutine (dater_ldcomzone) */

local int dater_defs(dater *op,tmz *tp) noex {
...
local int dater_ldname(dater *op,cchar *zstrp,int zstrl) noex {
	int		rs ;
	if ((rs = znlen) >= 0) ylikely {
	    cint	znl = rs ;
	    char	*dnp = op->zname ;
...
local int dater_pname(dater *op) noex {
	...
	if (zdb zr ; (rs = zr.setname(zp,zl)) >= 0) ylikely {
	    if (! op->fl.zoff) {
	        op->b.timezone = zr.woff ;
	    }
	    if (op->b.dstflag < 0) {
...
local int dater_pnum(dater *op) noex {
	int		rs = SR_OK ;
	if (!op->fl.zoff) {
	    if (int zo{} ; (rs = zos_get(op->zname,-1,&zo)) >= 0) ylikely {
...
local int dater_findzname(dater *op) noex {
	int		rs = SR_OK ;
	if (op->zname[0] == '\0') ylikely {
	    if (! op->fl.cyear) {
		rs = dater_initcur(op) ;
	    } /* end if */
	    if (rs >= 0) ylikely {
		if (op->fl.zoff && (op->b.timezone == op->cb.timezone)) {
			...
			cint	znl = rs ;
	                op->b.dstflag = op->cb.dstflag ;
	                op->fl.zname = true ;
	                rs = znwcpy(op->zname,znl,op->cname) ;
		    } /* end if (znlen) */
		} /* end if */
	    } /* end if (ok) */
	} /* end if */
...
local int dater_findzoff(dater *op,TM *stp) noex {
	int		rs = SR_FAULT ;
	int		zoff = 0 ;
	if (stp) ylikely {
	    rs = SR_OK ;
	    if (! op->fl.zoff) {
			...
			cint	znl = rs ;
			cchar	*zn = op->zname ;
	                if (zdb zr ; (rs = zr.setname(zn,znl)) >= 0) ylikely {
	                    op->b.timezone = zr.woff ;
	                    op->b.dstflag = zr.isdst ;
	                    op->fl.zoff = true ;
	                } else if (rs == SR_NOTFOUND) {
...
local int dater_mkptime(dater *op,TM *stp,int zoff) noex {
	int		rs = SR_OK ;
	...
	if (rs >= 0) ylikely {
	    if (tmtime tmt ; (rs = tmt.insert(stp)) >= 0) ylikely {
	        tmt.gmtoff = (zoff * SecondsPerMinute) ;
	        op->fl.tzset = true ;		/* mktime(3c) calls it! */
...
/* try to make a time for the given date */
local int dater_mktime(dater *op,TM *stp) noex {
	time_t		t{} ;
	int		rs = SR_OK ;
	int		rs1 ;
	if (op->fl.zoff) {
	    if (tmtime tmt ; (rs = tmt.insert(stp)) >= 0) ylikely {
	        tmt.gmtoff = (op->b.timezone * SecondsPerMinute) ;
	    ...
	    op->fl.tzset = true ;		/* mktime() calls it! */
	    if ((rs = uc_mktime(stp,&t)) >= 0) ylikely {
	        op->b.time = t ;
		if (char *znb ; (rs = lm_zn(&znb)) >= 0) ylikely {
		    cint	znl = rs ;
		    cint	isd = stp->tm_isdst ;
	            if (defzdata zi ; (rs = zi.get(znb,znl,isd)) >= 0) ylikely {
	                znwcpy(op->zname,znl,znb,rs) ;
	                op->b.timezone	= shortconv(zi.zoff) ;
	                op->b.dstflag	= shortconv(stp->tm_isdst) ;
	                op->fl.zoff	= true ;
	                op->fl.zname	= true ;
	            } /* end if (getdefzdata) */
		    rs1 = lm_free(znb) ;
...
local int dater_initbase(dater *op) noex {
	int		rs = SR_FAULT ;
	if (op) ylikely {
	    rs = SR_OK ;
	    op->zname[0]	= '\0' ;
