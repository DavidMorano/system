...
int dater_mkdatestr(dater *op,int type,char *dbuf,int dlen) noex {
	...
	if ((rs = dater_magic(op,dbuf)) >= 0) {
		...
		/* conversion */
	        t -= (zoff * 60) ;
	        if (tmtime tmt ; (rs = tmt.gmtime(t)) >= 0) {
	            tmt.gmtoff = (zoff * 60) ;
		    if ((rs = znlen) >= 0) {
			cint	znl = rs ;
...
static int dater_initcur(dater *op) noex {
	...
	if (! op->fl.cyear) {
	    ...
	    op->fl.tzset = true ;
	    if (tmtime tmt ; (rs = tmt.localtime(op->cb.time)) >= 0) {
		if ((rs = znlen) >= 0) {
		    cint	znl = rs ;
...
static int dater_mkpzoff(dater *op,TM *stp,int zoff) noex {
	int		rs ;
	op->zname[0] = '\0' ;
	if (zdb zr ; (rs = zr.offisdst(zoff,stp->tm_isdst)) >= 0) {
	    if ((rs = znlen) >= 0) {
		cint	znl = rs ;
...
static int dater_ldname(dater *op,cchar *zstrp,int zstrl) noex {
	int		rs ;
	if ((rs = znlen) >= 0) {
	    cint	znl = rs ;
	    char	*dnp = op->zname ;
...
static int dater_pname(dater *op) noex {
	...
	if (zdb zr ; (rs = zr.setname(zp,zl)) >= 0) {
	    if (! op->fl.zoff) {
...
static int dater_pnum(dater *op) noex {
	int		rs = SR_OK ;
	if (!op->fl.zoff) {
	    if (int zo{} ; (rs = zos_get(op->zname,-1,&zo)) >= 0) {
...
static int dater_findzoff(dater *op,TM *stp) noex {
	...
	if (stp) {
	    ...
	    if (! op->fl.zoff) {
			...
			cint	znl = rs ;
			cchar	*zn = op->zname ;
	                if (zdb zr ; (rs = zr.setname(zn,znl)) >= 0) {
	                    op->b.timezone = zr.woff ;
	                    op->b.dstflag = zr.isdst ;
	                    op->fl.zoff = true ;
	                } else if (rs == SR_NOTFOUND) {
...
static int dater_mktime(dater *op,TM *stp) noex {
	...
	if (op->fl.zoff) {
	    ...
	    if ((rs = uc_mktime(stp,&t)) >= 0) {
	        op->b.time = t ;
		if (char *znb ; (rs = malloc_zn(&znb)) >= 0) {
		    cint	znl = rs ;
		    cint	isd = stp->tm_isdst ;
	            if (defzdata zi ; (rs = zi.get(znb,znl,isd)) >= 0) {
