...
int timemgr::cmdtrans(uctimxent *tep) noex {
    	int		rs ;
	DEBPRINTF("ent\n") ;
...
int timemgr::sigerdump() noex {
	...
	DEBPRINTF("ent\n") ;
	    ...
	    if (uctimxent *uep = resumelife<uctimxent>(tep)) {
	        destroy_at(uep) ;
	    }
	} /* end for */
        if ((rs >= 0) && (rs1 != SR_NOTFOUND)) rs = rs1 ;
