...
int realname_start(realname *op,cchar *sp,int sl) noex {
	int		rs ;
	if ((rs = realname_ctor(op)) >= 0) ylikely {
	    if (sp) {
...
int realname_finish(realname *op) noex {
	int		rs ;
	int		rs1 ;
	if ((rs = realname_magic(op)) >= 0) ylikely {
	    {
...
int realname_loadparts(realname *op,dstr *stp) noex {
	int		rs ;
	int		rs1 ;
	if ((rs = realname_magic(op,stp)) >= 0) ylikely {
	    if ((rs = realname_avail(op)) >= 0) ylikely {
	        cint	slen = REALNAME_STORELEN ;
	        if (storeitem s ; (rs = s.start(op->stbuf,slen)) >= 0) ylikely {
	            int		cl ;
	            cchar	*cp ;
		    /* first */
...
int realname_loadpieces(realname *op,cchar **sa,int sn) noex {
	int		rs ;
	if ((rs = realname_magic(op,sa)) >= 0) ylikely {
	    if ((rs = realname_avail(op)) >= 0) ylikely {
	        int		c = 0 ;
...
int realname_fullname(realname *op,char *rbuf,int rlen) noex {
	...
	if (op) ylikely {
	    if (sbuf s ; (rs = s.start(rbuf,rlen)) >= 0) ylikely {
	        ...
	        if ((op->first != nullptr) && (op->first[0] != '\0')) {
	            f = true ;
	            s.strw(op->first,op->len.first) ;
	            if (op->abv.first) s.chr('.') ;
	        }
	        if ((op->m1 != nullptr) && (op->m1[0] != '\0')) {
	            if (f) s.chr(' ') ;
	            s.strw(op->m1,op->len.m1) ;
	            if (op->abv.m1) s.chr('.') ;
	        }
	        if ((op->m2 != nullptr) && (op->m2[0] != '\0')) {
	            if (f) s.chr(' ') ;
	            s.strw(op->m2,op->len.m2) ;
	            if (op->abv.m2) s.chr('.') ;
	        }
	        if ((op->m3 != nullptr) && (op->m3[0] != '\0')) {
	            if (f) s.chr(' ') ;
	            s.strw(op->m3,op->len.m3) ;
	            if (op->abv.m3) s.chr('.') ;
	        }
	        if ((op->last != nullptr) && (op->last[0] != '\0')) {
	            if (f) s.chr(' ') ;
	            s.strw(op->last,op->len.last) ;
	            if (op->abv.last) s.chr('.') ;
	        }
	        len = s.finish ;
	        if (rs >= 0) rs = len ;
	    } /* end if (buffer) */
...
static int realname_parse(realname *op,cchar *sbuf,int slen) noex {
	...
	if (op && sbuf) ylikely {
	    memclear(op) ;
	    if (names n ; (rs = names_start(&n,op->stbuf)) >= 0) ylikely {
	        int	fabv{} ;
	        int	flast{} ;
	        if (namestr ns ; (rs = ns.start(sbuf,slen)) >= 0) ylikely {
		    int		sl ;
	            cchar	*sp{} ;
	        ...
	        if (rs >= 0) ylikely {
		    int		i ; /* used-multiple */
		    /* load up the information on the name parts */
	            if (n.li < 0) {
	                if (n.count >= nnames) {
	                    i = n.i ;
	                } else {
...
static int names_add(names *op,cchar *nbuf,int nlen,int f_abv,int f_last) noex {
	...
	if ((nlen > 0) && ((op->li < 0) || (op->i != op->li))) {
	    ...
	    if (cchar *cp{} ; (rs = op->s.strw(nbuf,nlen,&cp)) >= 0) ylikely {
	        cint	cl = rs ;
	        op->a[op->i] = cp ;
	        op->l[op->i] = uchar(cl) ;
