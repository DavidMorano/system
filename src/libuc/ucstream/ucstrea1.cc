...
typedef ucstream::stype::iterator	setiter ;
...
int ucstream::istart(int n) noex {
	cnullptr	np{} ;
	int		rs = SR_INVALID ;
	if (n >= 0) ylikely {
	...
	return rs ;
} /* end method (ucstream::istart) */

int ucstream::ifinish() noex {
	int		rs = SR_NOTOPEN ;
	if (setp) ylikely {
	    delete setp ;
	    setp = nullptr ;
	...
	return rs ;
} /* end method (ucstream::ifinish) */

int ucstream::checkin(CUSTAT *sbp) noex {
	int		rs = SR_FAULT ;
	int		f = false ;
	if (sbp) ylikely {
	    rs = SR_BUGCHECK ;
	    if (setp) ylikely {
	        ucstream_devino	k(sbp->st_dev,sbp->st_ino) ;
		try {
	...
	return (rs >= 0) ? f : rs ;
} /* end method (ucstream::checkin) */

int ucstream::icount() noex {
	int		rs = SR_BUGCHECK ;
	if (setp) ylikely {
	    csize cnt = setp->size() ;
	    rs = intconv(cnt) ;
...
int ucstream_co::operator () (int a) noex {
	int		rs = SR_BUGCHECK ;
	if (op) ylikely {
	    switch (w) {
	    case ucstreammem_start:
	        rs = op->istart(a) ;
	        break ;
	    case ucstreammem_finish:
	        rs = op->ifinish() ;
	        break ;
	    case ucstreammem_count:
	        rs = op->icount() ;
	        break ;
	    } /* end switch */
	} /* end if (non-null) */
