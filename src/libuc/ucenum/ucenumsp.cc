/* ucenumsp SUPPORT */
/* charset=ISO8859-1 */
/* lang=C++20 */

/* enumerate system SYSDB-XX entries */
/* version %I% last-modified %G% */


/* revision history:

	= 1998-03-24, David A­D­ Morano
	This object module was morphed from some previous one.  I
	do not remember what the previous one was.

*/

/* Copyright © 1998 David A­D­ Morano.  All rights reserved. */

/*******************************************************************************

  	Group:
	ucenumsp

	Description:
	We enumerate (reentrantly and thread safely) SYSDB entries
	from the system SYSDB-XX database.

*******************************************************************************/

#include	<envstandards.h>	/* ordered first to configure */
#include	<cstddef>		/* CSTD */
#include	<cstdlib>		/* CSTD */
#include	<clanguage.h>		/* LIBU */
#include	<usysbase.h>		/* LIBU */
#include	<usyscalls.h>		/* LIBU |ulogerror(3u)| */
#include	<sysdbfn.h>		/* LIBU */
#include	<localmisc.h>		/* LIBU */

#include	"ucenumxx.h"
#include	"ucenumsp.h"


/* local defines */


/* imported namespaces */


/* local typedefs */


/* external subroutines */


/* local structures */


/* forward references */


/* local variables */

constexpr uint		mxx = UCENUMSP_MAGIC ;
constexpr sysdbfiles	w = sysdbfile_sp ;


/* local subroutines */


/* exported variables */


/* exported subroutines */

int ucenumsp_open(ucenumsp *op,cchar *efname) noex {
	ucenumxxco<ucenumsp_ent>	ueo(op,mxx) ;
	int		rs ;
	if (cchar *rp ; (rs = sysdbfn_get(w,efname,&rp)) >= 0) {
	    rs = ueo.open(rp) ;
	} /* end if (sysdbfn_get) */
	return rs ;
} /* end if (ucenumsp_open) */

int ucenumsp_close(ucenumsp *op) noex {
	ucenumxxco<ucenumsp_ent>	ueo(op,mxx) ;
	return ueo.close() ;
} /* end subroutine (ucenumsp_close) */

int ucenumsp_readent(ucenumsp *op,ucenumsp_ent *ep,char *bp,int bl) noex {
	ucenumxxco<ucenumsp_ent>	ueo(op,mxx) ;
	return ueo.readent(ep,bp,bl) ;
} /* end subroutine (ucenumsp_readent) */

int ucenumsp_reset(ucenumsp *op) noex {
	ucenumxxco<ucenumsp_ent>	ueo(op,mxx) ;
	return ueo.reset() ;
} /* end subroutine (ucenumsp_reset) */


/* local subroutines */

int ucenumsp::readent(ucenumsp_ent *uap,char *uabuf,int ualen) noex {
	return ucenumsp_readent(this,uap,uabuf,ualen) ;
} /* end method (ucenumsp::readent) */

void ucenumsp::dtor() noex {
	if (cint rs = close ; rs < 0) {
	    ulogerror("ucenumsp",rs,"fini-close") ;
	}
} /* end method (ucenumsp::dtor) */

int ucenumsp_op::operator () (cchar *fn) noex {
	int		rs = SR_BUGCHECK ;
	if (op) {
	    switch (w) {
	    case ucenumspmem_open:
	        rs = ucenumsp_open(op,fn) ;
	        break ;
	    } /* end switch */
	} /* end if (non-null) */
	return rs ;
} /* end method (ucenumsp_op::operator) */

ucenumsp_co::operator int () noex {
	int		rs = SR_BUGCHECK ;
	if (op) {
	    switch (w) {
	    case ucenumspmem_reset:
	        rs = ucenumsp_reset(op) ;
	        break ;
	    case ucenumspmem_close:
	        rs = ucenumsp_close(op) ;
	        break ;
	    } /* end switch */
	} /* end if (non-null) */
	return rs ;
} /* end method (ucenumsp_co::operator) */


