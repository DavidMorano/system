...
int		psem_create(PSEM *,int,uint) noex ;
...
int psem_create(PSEM *psp,int pshared,uint count) noex {
...
int psem_getvalue(PSEM *psp,int *rp) noex {
	int		rs ;
	int		c = 0 ;
	if (psp == NULL) return SR_FAULT ;
	if (rp == NULL) rp = &c ;
