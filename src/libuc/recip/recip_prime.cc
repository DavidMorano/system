...
local inline int recip_ctor(recip *op,Args ... args) noex {
	cnullptr	np{} ;
	int		rs = SR_FAULT ;
	if (op && (args && ...)) ylikely {
	    rs = SR_NOMEM ;
...
local inline int recip_dtor(recip *op) noex {
	int		rs = SR_FAULT ;
	if (op) ylikely {
	    rs = SR_OK ;
	    if (op->mdp) ylikely {
...
int recip_start(recip *op,cchar *sp,int sl) noex {
	int		rs ;
	if ((rs = recip_ctor(op,sp)) >= 0) ylikely {
	    op->uid = -1 ;
...
int recip_setname(recip *op,cchar *sp,int sl) noex {
	int		rs ;
	if ((rs = recip_magic(op,sp)) >= 0) ylikely {
	    rs = SR_INVALID ;
	    if (op->name != nullptr) {
		void *vp = voidp(op->name) ;
	        libmem.free(vp) ;
