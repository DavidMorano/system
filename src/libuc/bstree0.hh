...
public:
	bstree_iter() noex { } ;
	bstree_iter(bstree_node<T,Comp> *an) : n(an) noex { } ;
	...
	bstree_iter<T,Comp> &operator = (const bstree_node<T,Comp> *nn) noex {
	...
	void setnode(bstree_node<T,Comp> *nn) noex {
	    n = nn ; /* possible nullptr */
	} ;
...
class bstree {
	...
	int			c = 0 ;
	typedef			bstree_node<T,Comp> nodetype ;
        bstree_iter<T,Comp> FindNodeByVal(nodetype *n,const T &v) const noex {
	...
	void ReplaceUsInParent(nodetype *np,nodetype *c) noex {
	...
	nodetype *GetChild(nodetype *np) const noex {
	    return (np->left != nullptr) ? np->left : np->right ;
	} ;
	nodetype *FindMinNode(nodetype *np) const noex {
	    while (np->left != nullptr) {
		np = np->left ;
	    }
	    return np ;
	} ;
	int delnodes(bstree_node<T,Comp> *n) noex {
	    int		i = 0 ;
	    if (n != nullptr) {
		i += 1 ;
	        c += delnodes(n->left) ;
	...
	int insert(bstree_node<T,Comp> *n,bstree_node<T,Comp> *nn) noex {
	    int		d = 0 ;
	...
	int walk(std::vector<T> &vl,bstree_node<T,Comp> *n) const noex {
	    int		i = 0 ;
	    if (n != nullptr) {
		if (n->left) {
		    i += walk(vl,n->left) ;
