...
int vecobj_start(vecobj *op,int osize,int n,int opts) noex {
	int		rs ;
	if ((rs = vecobj_ctor(op)) >= 0) ylikely {
	    rs = SR_INVALID ;
	    if (osize > 0) {
	        if (n <= 0) n = defents ;
	        op->va = nullptr ;
	        op->esz = osize ;
	        if ((rs = vecobj_setopts(op,opts)) >= 0) ylikely {
	            cint	size = (n + 1) * szof(char **) ;
	            op->i = 0 ;
	            op->c = 0 ;
	            op->fi = 0 ;
	            if (void *vp ; (rs = libmem.mall(size,&vp)) >= 0) ylikely {
	                op->va = voidpp(vp) ;
	                op->va[0] = nullptr ;
	                op->n = n ;
...
int vecobj_add(vecobj *op,cvoid *s) noex {
	int		rs ;
	if (void *ep ; (rs = vecobj_addnew(op,&ep)) >= 0) ylikely {
	    memcpy(ep,s,op->esz) ;
	}
	return rs ;
}
...
int vecobj_addnew(vecobj *op,void **epp) noex {
	...
	if (op) ylikely {
	    ...
	    if (op->va) ylikely {
	        ...
	        if ((rs = lookaside_get(op->lap,&sp)) >= 0) ylikely {
	            bool	f = true ;
	            bool	f_done = false ;
...
int vecobj_store(vecobj *op,cvoid *s,void **rpp) noex {
	int		rs ;
	int		i = 0 ;
	if ((rs = vecobj_add(op,s)) >= 0) ylikely {
	    i = rs ;
	    if (rpp) {
...
int sub_fetch::first(cur *curp) noex {
	...
	int		i = 0 ;
	void		*rep{} ;
	if ((rs = vecobj_search(op,ep,vcf,&rep)) >= 0) ylikely {
	    i = rs ;
	    if (op->fl.osorted) {
		vg_f	v = vecobj_iget ;
