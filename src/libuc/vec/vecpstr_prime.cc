...
local int	vecpstr_insertsp(vecpstr *,int,cchar *) noex ;
local int	vecpstr_finchunks(vecpstr *) noex ;
local int	vecpstr_extstr(vecpstr *,int) noex ;
local int	vecpstr_newchunk(vecpstr *,int) noex ;
local int	vecpstr_extvec(vecpstr *,int = 0) noex ;
local int	vecpstr_record(vecpstr *,cchar *) noex ;
...
int vecpstr_start(vecpstr *op,int vn,int vsz,int vo) noex {
	int		rs ;
	if ((rs = vecpstr_ctor(op)) >= 0) ylikely {
...
int vecpstr_adduniq(vecpstr *op,cchar *sp,int µsl) noex {
	int		rs = SR_FAULT ;
	if (op) ylikely {
...
int vecpstr_addkeyval(vecpstr *op,cchar *kp,int kl,cchar *vp,int vl) noex {
	int		rs ;
	int		i = 0 ;
	if ((rs = vecpstr_magic(op,kp)) >= 0) ylikely {
            int     amount = 0 ;
            ...
            if ((rs = vecpstr_extstr(op,amount)) >= 0) ylikely {
                cchar       *cp{} ;
...
int vecpstr_insert(vecpstr *op,int ii,cchar *sp,int µsl) noex {
	int		rs ;
	int		i = 0 ;
...
int vecpstr_store(vecpstr *op,cchar *sp,int µsl,cchar **rpp) noex {
	int		rs ;
	int		i = 0 ;
...
int vecpstr_get(vecpstr *op,int i,cchar **rpp) noex {
	int		rs ;
	if ((rs = vecpstr_validx(op,i)) >= 0) ylikely {
	    if (rpp) {
...
int vecpstr_findaddr(vecpstr *op,cchar *addr) noex {
	int		rs ;
	if ((rs = vecpstr_magic(op,addr)) >= 0) ylikely {
            rs = SR_NOTFOUND ;
            if (op->va) ylikely {
                for (int i = 0 ; i < op->idx ; i += 1) {
                    cchar   *ep = op->va[i] ;
...
int vecpstr_recmk(vecpstr *op,int *rec,int recsize) noex {
	int		rs ;
	int		c = 0 ;
	if ((rs = vecpstr_magic(op,rec)) >= 0) ylikely {
...
int vecpstr_getvec(vecpstr *op,mainv *rppp) noex {
	int		rs ;
	int		i = 0 ;
	if ((rs = vecpstr_magic(op,rppp)) >= 0) ylikely {
	    if ((rs = vecpstr_extvec(op,1)) >= 0) ylikely {
	        *rppp = mainv(op->va) ;
		i = op->idx ;
	    }
	} /* end if (magic) */
...
local int vecpstr_ctor(vecpstr *op) noex {
	cnullptr	np{} ;
	int		rs = SR_FAULT ;
	if (op) ylikely {
	    rs = SR_NOMEM ;
