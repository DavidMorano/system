...
local inline int ema_dtor(ema *op) noex {
	int		rs = SR_FAULT ;
	if (op) ylikely {
	    rs = SR_OK ;
	    if (op->elp) ylikely {
...
int ema_parse(ema *op,cchar *sp,int µsl) noex {
	int		rs ;
	int		rs1 ;
	if ((rs = ema_magic(op,sp)) >= 0) ylikely {
	    if (int sl ; (sl = getlenstr(sp,µsl)) >= 0) ylikely {
	        if (asstr desc ; (rs = desc.start(sp,sl)) >= 0) ylikely {
	            if ((rs = ema_ns::ema_parseit(op,&desc)) > 0) ylikely {
	                op->n += rs ;
	            }
		    rs1 = desc.finish ;
		    if (rs >= 0) rs = rs1 ;
