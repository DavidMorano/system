...
namespace {
    ...
    struct codemgr {
	ucha		idx[ne] ;
	consteval void tabload() noex {
	    for (int i = 0 ; i < ne ; i += 1) {
		idx[i] = i ;
	    } ; /* end for */
	} ; /* end method (load) */
	consteval void tabsort() noex {
	    sort(idx,(idx + ne),cmpf) ;
	} ;
