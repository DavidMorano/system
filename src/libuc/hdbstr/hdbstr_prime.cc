...
local int hdbstr_ctor(hdbstr *op,Args ... args) noex {
    	HDBSTR		*hop = op ;
	int		rs = SR_FAULT ;
	if (op && (args && ...)) ylikely {
	    rs = memclear(hop) ;
...
local int hdbstr_dtor(hdbstr *op) noex {
	int		rs = SR_FAULT ;
	if (op) ylikely {
	    rs = SR_OK ;
	} /* end if (non-null) */
...
template<typename ... Args>
local inline int hdbstr_magic(hdbstr *op,Args ... args) noex {
	int		rs = SR_FAULT ;
	if (op && (args && ...)) ylikely {
	    rs = SR_OK ;
	}
...
local int	hdbstr_finents(hdbstr *) noex ;
...
int hdbstr_start(hdbstr *op,int ne) noex {
	int		rs ;
	if ((rs = hdbstr_ctor(op)) >= 0) ylikely {
...
int hdbstr_finish(hdbstr *op) noex {
	...
	int		rs1 ;
	int		n = 0 ;
	if ((rs = hdbstr_magic(op)) >= 0) ylikely {
	    {
...
local int hdbstr_finents(hdbstr *op) noex {
	cint		rsn = SR_NOTFOUND ;
	int		rs ;
	int		rs1 ;
	int		n = 0 ;
	if (hdb_cur cur ; (rs = hdb_curbegin(op,&cur)) >= 0) ylikely {
            hdb_dat	key{} ;
            hdb_dat	val{} ;
...
int hdbstr_add(hdbstr *op,cchar *kstr,int klen,cchar *vstr,int vlen) noex {
	int		rs ;
	if ((rs = hdbstr_magic(op,kstr)) >= 0) ylikely {
	    int		sz ;
...
int hdbstr_curenum(hdbstr *op,hc *curp,cc **kpp,cc **vpp,int *vlp) noex {
	int		rs ;
	int		kl = 0 ;
	if ((rs = hdbstr_magic(op,curp)) >= 0) ylikely {
	    hdb_dat	key ;
	    hdb_dat	val{} ;
	    if (kpp) *kpp = nullptr ;
	    if ((rs = hdb_curenum(op,curp,&key,&val)) >= 0) ylikely {
	        kl = key.len ;
	        if (kpp) {
	            *kpp = charp(key.buf) ;
	        }
...
int hdbstr_fetchrec(hdbstr *op,cc *kstr,int klen,hc *curp,cc **kpp,cc **vpp,
	...
	int		rs ;
	int		len = 0 ;
	if ((rs = hdbstr_magic(op,kstr,curp)) >= 0) ylikely {
	    hdb_dat	key ;
	    hdb_dat	val{} ;
	    hdb_dat	rkey{} ;
	    key.buf = kstr ;
	    key.len = (klen < 0) ? lenstr(kstr) : klen ;
	    if (kpp) *kpp = nullptr ;
	    if ((rs = hdb_fetchrec(op,key,curp,&rkey,&val)) >= 0) ylikely {
	        if (kpp) {
	            *kpp = charp(rkey.buf) ;
	        }
	        if (vpp) {
...
int hdbstr_curget(hdbstr *op,hc *curp,cc **kpp,cc **vpp,int *vlp) noex {
	...
	if ((rs = hdbstr_magic(op,curp)) >= 0) ylikely {
	    ...
	    hdb_dat	val{} ;
	    if (kpp) *kpp = nullptr ;
	    if ((rs = hdb_curget(op,curp,&key,&val)) >= 0) ylikely {
	        if (kpp) {
	            *kpp = charp(key.buf) ;
	        }
	        if (vpp) {
...
int hdbstr_curdel(hdbstr *op,hc *curp,int f_adv) noex {
	...
	if ((rs = hdbstr_magic(op,curp)) >= 0) ylikely {
	    ...
	    if ((rs = hdb_curget(op,curp,&key,&val)) >= 0) ylikely {
	        ...
	        if ((rs = hdb_curdel(op,curp,f_adv)) >= 0) ylikely {
	            if (kp) {
			void *vp = voidp(kp) ;
			libmem.free(vp) ;
		    }
...
int hdbstr_curcopy(hdbstr *op,hc *curp,hc *othp) noex {
	int		rs ;
	if ((rs = hdbstr_magic(op,curp,othp)) >= 0) ylikely {
