...
local int	setstr_finents(setstr *) noex ;
...
int setstr_already(setstr *op,cchar *sp,int µsl) noex {
	...
	if (op && sp) ylikely {
	    if (int sl ; (sl = getlenstr(sp,µsl)) >= 0) ylikely {
	       ...
	       val.buf = sp ;
	       val.len = sl ;
	       if ((rs = hdb_fetch(op,key,np,&val)) == rsn) ylikely {
	           f = false ;
	           rs = SR_OK ;
	       }
	    } /* end if (getlenstr) */
...
local int setstr_finents(setstr *op) noex {
    	int		rs = SR_OK ;
	int		rs1 ;
	int		rs2 ;
	int		c = 0 ; /* return-value */
	if (hdb_cur cur ; (rs1 = hdb_curbegin(op,&cur)) >= 0) ylikely {
	    hdb_dat	key{} ;
	    hdb_dat	val{} ;
