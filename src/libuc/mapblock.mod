...
/* revision history:
	...
	= 2025-01-27, David A­D­ Morano
	I am trying to turn this code into a C++ module.  Ya, we
	will see how far that goes.  Turning code into a module is
	pretty easy and straight forward (literally add a couple
	of keywords).  Integrating modules into my build system is
	the real problem.

*/

/* Copyright © 2011,2025 David A­D­ Morano.  All rights reserved. */
...
export module mapblock ;

export {
    template<typename K,typename B> class mapblock ;
    template <typename K,typename B> class mapblock_iter {
...
} ; /* end enum (mapblockmems) */

export {
    template<typename K,typename B> struct mapblock_co {
	typedef mapblock<K,B>	mb_t ;
	mb_t		*op = nullptr ;
	int		w = -1 ;
    ...
    template<typename K,typename B> int mapblock<K,B>::istart(int n) noex {
    ...
    template<typename K,typename B> int mapblock<K,B>::ifinish() noex {
	int		rs = SR_BUGCHECK ;
	if (mp) {
	    delete mp ;
	    mp = nullptr ;
    ...
    template<typename K,typename B> int mapblock<K,B>::rem(K k) noex {
    ...
    template<typename K,typename B> int mapblock<K,B>::present(K &k) noex {
	typedef typename maptype::iterator	mit_t ;
	int		rs = SR_BUGCHECK ;
	if (mp) {
    ...
    template<typename K,typename B> int mapblock<K,B>::get(K k,B *vp) noex {
	typedef typename maptype::iterator	mit_t ;
	int		rs = SR_FAULT ;
	if (vp) {
	    rs = SR_BUGCHECK ;
    ...
    template<typename K,typename B> int mapblock<K,B>::icount() noex {
	int		rs = SR_BUGCHECK ;
	if (mp) {
	    rs = mp->size() ;
	} /* end if (non-null) */
