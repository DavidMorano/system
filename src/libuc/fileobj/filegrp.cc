...
struct filegrp_rec {
	...
	char		*gn ;			/* <- allocated */
	gid_t		gid ;
} ; /* end struct (filegrp_rec) */


/* forward references */

template<typename ... Args>
local inline int filegrp_ctor(filegrp *op,Args ... args) noex {
	int		rs = SR_FAULT ;
	if (op && (args && ...)) ylikely {
	    rs = SR_NOMEM ;
	    op->ti_check = 0 ;
	    ...
	    op->nmax = 0 ;
	    op->s = {} ;
 	    if ((op->flp = new(nothrow) cq) != nullptr) ylikely {
 	        if ((op->alp = new(nothrow) vechand) != nullptr) ylikely {
		    rs = SR_OK ;
	        } /* end if (new-vechand) */
		if (rs < 0) {
		    delete op->flp ;
	...
	return rs ;
} /* end subroutine (filegrp_ctor) */

local inline int filegrp_dtor(filegrp *op) noex {
	int		rs = SR_FAULT ;
	if (op) ylikely {
	    rs = SR_OK ;
	    if (op->alp) ylikely {
...
static bufsizevar	gnl(bufsize_gn) ;
...
int filegrp_finish(FG *op) noex {
	...
	if ((rs = filegrp_magic(op)) >= 0) ylikely {
            if (op->alp) {
                vechand     *rlp = op->alp ;
                void        *vp{} ;
...
int filegrp_stats(FG *op,FG_ST *sp) noex {
	int		rs ;
	if ((rs = filegrp_magic(op,sp)) >= 0) ylikely {
	    if ((rs = vechand_count(op->alp)) >= 0) ylikely {
	        *sp = op->s ;
	        sp->nentries = rs ;
	    }
	} /* end if (magic) */
...
local int filegrp_newrec(FG *op,time_t dt,FG_REC **rpp,gid_t gid,cc *gn) noex {
	FG_REC		*rp{} ; /* used-afterwards */
	int		rs ;
	int		gl = 0 ;
	if ((rs = filegrp_allocrec(op,&rp)) >= 0) ylikely {
...
local int record_start(FG_REC *rp,time_t dt,gid_t gid,cc *gn) noex {
	...
	if (rp && gn) ylikely {
	    ...
	    if (dt == 0) dt = time(nullptr) ;
	    if (gn[0]) ylikely {
		if (char *gp ; (rs = lm_gn(&gp)) >= 0) ylikely {
		    rp->gn = gp ;
	            rp->gid = gid ;
	            rp->ti_create = dt ;
	            rp->ti_access = dt ;
...
local int record_refresh(FG_REC *rp,time_t dt) noex {
	...
	int		gl = 0 ;
	char		gn[gnl + 1] ;
	if ((rs = getgroupname(gn,gnl,rp->gid)) >= 0) ylikely {
	    gl = rs ;
	    rs = record_update(rp,dt,gn) ;
	}
	return (rs >= 0) ? gl : rs ;
