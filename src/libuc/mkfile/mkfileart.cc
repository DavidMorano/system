...
/*******************************************************************************
	...
	Synopsis:
	int mkfileart(char *rbuf,cc *dir,cc *pre,int serial,mode_t om) noex
...
int mkfileart(char *rbuf,cc *dname,cc *prefix,int serial,mode_t om) noex {
	int		rs = SR_FAULT ;
	if (rbuf && dname && prefix) {
	    rs = SR_INVALID ;
	    if (dname[0] && prefix[0] && (serial >= 0)) ylikely {
...
trier::operator int () noex {
	...
	if ((rs = maxpathlen) >= 0) ylikely {
	    cint	maxpath = rs ;
	    if ((rs = mkpath(rbuf,dn,"/")) >= 0) ylikely {
	        rl = rs ;
		nbuf = (rbuf + rs) ;
		nlen = (maxpath - rs) ;
...
int trier::mktry(uint ts,int ss,int es) noex {
	...
	int		rs ;
	int		len = 0 ; /* return-value */
        if ((rs = mkoutname(nbuf,nlen,pn,ts,ss,es)) >= 0) ylikely {
            len = (rl + rs) ;
