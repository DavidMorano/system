...
static inline int filer_ctor(filer *op,Args ... args) noex {
	int		rs = SR_FAULT ;
	if (op && (args && ...)) {
	    rs = SR_OK ;
	    op->off = 0 ;
...
int filer_readlns(filer *fp,char *lbuf,int llen,int to,int *lcp) noex {
	int		rs = SR_FAULT ;
	int		i = 0 ;
	if (fp && lbuf) {
	    int		lines = 0 ;
	    ...
	    while ((lines == 0) || (f_cont = ISCONT(lbuf,i))) {
	        if (f_cont) i -= 2 ;
	        rs = filer_readln(fp,(lbuf + i),(llen - i),to) ;
	        if (rs <= 0) break ;
	        i += rs ;
	        lines += 1 ;
	    } /* end while */
