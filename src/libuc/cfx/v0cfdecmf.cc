...
using std::in_range ;			/* subroutine -- unused */

template<typename T>
using cfdec_f = int (*)(cchar *,int,T *) ;
...
template<typename T>
local int cfdecmfx(cfdec_f<T> cfdecx,cc *sp,int µsl,T *rp) noex {
	int		rs = SR_FAULT ;
	if (sp && rp) ylikely {
	    rs = SR_INVALID ;
	    if (int sl ; (sl = getlenstr(sp,µsl)) > 0) {
	        if (int64_t mf{} ; (rs = getmf(sp,sl,&mf)) > 0) ylikely {
	            ...
	            if ((rs = inrange<T>(mf)) >= 0) ylikely {
	                if (T v ; (rs = cfdecx(sp,ml,&v)) >= 0) ylikely {
		            const T	mfv = (T) mf ;
...
local int getmf(cchar *sbuf,int slen,int64_t *rp) noex {
	int64_t		mf = 1 ; /* return-value */
	int		rs = SR_INVALID ;
	int		sl = slen ;
	while ((sl > 0) && ourwht(sbuf[sl - 1])) {
	    sl -= 1 ;
	}
	if (sl > 0) ylikely {
	    rs = SR_OK ;
