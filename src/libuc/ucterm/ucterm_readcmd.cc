...
/*******************************************************************************
	...
	Synopsis:
	int ucterm_readcmd(ucterm *utp,termcmd *ckp,int to,int ich) noex
...
#include	<localmisc.h>		/* LIBU |DIGBUFLEN| + |UC()| */
...
local int sub_start		(sub *,ucterm *,termcmd *,int,int) noex ;
...
int ucterm_readcmd(ucterm *utp,termcmd *ckp,int to,int ich) noex {
	...
	return (rs >= 0) ? rv : rs ;
} /* end subroutine (ucterm_readcmd) */


/* local subroutines */

local int sub_start(sub *sip,ucterm *utp,termcmd *ckp,int to,int ich) noex {
	int		rs = SR_FAULT ;
	if (sip) ylikely {
	    rs = memclear(sip) ;
	    sip->maxdig = maxdig ;
...
local int sub_readch(sub *sip) noex {
	...
	if (sip->ich != 0) {
	    ...
	    cint	to = sip->to ;
	    char	rbuf[2] ;
	    rs = ucterm_reade(utp,rbuf,1,to,ropts,NULL,NULL) ;
	    rch = (rbuf[0] & 0xff) ;
	    if (rs > 0) sip->rlen += 1 ;
	}
	return (rs >= 0) ? rch : rs ;
