...
#include	<cstring>		/* CSTD |strchr(3c)| + |strcspn(3n)| */
...
int siobrk(cchar *sp,int µsl,con chrset *setp) noex {
...
int sirbrk(cchar *sp,int µsl,con chrset *setp) noex {
	int		i = 0 ; /* return-value */
	bool		f = false ;
	if (int sl ; (sl = getlenstr(sp,µsl)) >= 0) {
	    for (i = (sl - 1) ; i >= 0 ; i -= 1) {
...
int sirbrk(cchar *sp,int µsl,cchar *ss) noex {
	int		i = 0 ; /* return-value */
	bool		f = false ;
