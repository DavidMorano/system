/* getehostname SUPPORT (Get-Effective-Hostname) */
/* charset=ISO8859-1 */
/* lang=C++20 */
...
/*******************************************************************************
	...
	new) name that is guaranteed to be able to translate to an
	INET address.  If no suitable substitute name is found that
	can be translated into an INET addres, a BAD return value
	is returned.  If a suitable name can be found that can be
	translated into a valid INET address, then that name or
	another valid name is returned in the supplied buffer and
	an OK value is returned.
