...
local int strpack_dtor(strpack *op) noex {
	int		rs = SR_OK ;
	if (op->clp) ylikely {
	    delete op->clp ;
	    op->clp = nullptr ;
...
int strpack_finish(strpack *op) noex {
	int		rs ;
	int		rs1 ;
	if ((rs = strpack_magic(op)) >= 0) ylikely {
	    {
...
int strpack_store(strpack *op,cchar *sp,int sl,cchar **rpp) noex {
	int		rs ;
	if ((rs = strpack_magic(op,sp)) >= 0) ylikely {
...
local int strpack_chunknew(strpack *op,int amount) noex {
