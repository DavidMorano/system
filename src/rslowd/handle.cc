...
/*****************************************************************************
	...
	slp	daemoin's service list pointer 
...
/* can we open and read this file */
	...
	if ((rs = bopen(ifp,tmpfname,"r",0666)) < 0) goto badopen ;
...
/* are we done ? */
	...
	if (sbp->st_size <= offset) goto done ;

	while (((len = breadln(ifp,linebuf,LINELEN)) > 0) &&
	    (! msg_matenv(linebuf,len)) &&
	    (! msg_mathead(linebuf,len,headname)))
	    offset += len ;
...
/* get the service code */
	...
	if (msghvalues[HI_CLEN] != NULL) {
	    ...
	    if (cfdec(msghvalues[HI_CLEN],-1,&jep->clen) < 0)
	        jep->clen = -1 ;
...
handle_noexec:
	...
	jep->wtime = daytime + 10 ;
	jep->state = JOBSTATE_TIMEWAIT ;
	wsp->c[JOBSTATE_TIMEWAIT] += 1 ;
	goto done ;
...
/* run the daemon, if we can */
	...
	if ((rs = handlejob_start(jep,NULL,jsbp, sep)) < 0)
	    goto handle_noexec ;
	...
	logfile_printf(&g.lh,"job started (2)\n") ;
...
int handle_sleep(tlp,jep,delay)
struct sortlist	*tlp ;
struct jobentry	*jep ;
int		delay ;
{
	...
	if (delay < 1) return OK ;

	(void) time(&daytime) ;

	jep->wtime = daytime + delay ;
	jep->state = JOBSTATE_TIMEWAIT ;
	wsp->c[JOBSTATE_TIMEWAIT] += 1 ;
	return OK ;
}
/* end subroutine (handle_sleep) */
