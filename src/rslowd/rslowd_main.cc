...
/*****************************************************************************
	...
	implementation of this program.  This program is the
	server-daemon side of a general purpose transport facility.
	The 'rslow' program is the client half of the thinpip->
	This program waits for an incoming job and then tries to
	execute the job by looking it up in a job service table.
	All, apparently, previous versions of this program were
	really very poor hacks and were susceptible to the many
	little quirks of various UNIX systems or UNIX versions.
	This program has been rewritten a number of times already
	in the hopes of "getting it right" but all of the previous
	rdebugwrites were largely based on the previously bad version
	to start with.  This version is a complete rdebugwrite from
	scratch !!  This version is a hack also but maybe it will
...
/* initialize */
	...
	pip->srvtab = nullptr ;
	pip->directory = nullptr ;
	pip->interrupt = nullptr ;
	pip->lockfile = nullptr ;
	pip->pidfile = nullptr ;
...
#if	CF_DEBUGS
	                    debugprintf("main: got option key w/ value\n") ;
#endif
...
/* read in the configuration file if we have one */
	...
	if (access(configfname,R_OK) >= 0) {
	    ...
	    if ((cf.directory != nullptr) && (pip->directory == nullptr)) {

	        pip->directory = cf.directory ;
	        cf.directory = nullptr ;

	    }

	    if ((cf.interrupt != nullptr) && (pip->interrupt == nullptr)) {

	        pip->interrupt = cf.interrupt ;
	        cf.interrupt = nullptr ;

	    }

	    if ((cf.workdir != nullptr) && (pip->workdir == nullptr)) {
	    ...
	    if ((cf.srvtab != nullptr) && (pip->srvtab == nullptr)) {
	    ...
	    if ((cf.pidfile != nullptr) && (pip->pidfile == nullptr)) {
	    ...
	    if ((cf.logfname != nullptr) && (logfname == nullptr)) {
...
/* check program parameters */
	...
	if ((pip->directory == nullptr) || (pip->directory[0] == '\0'))
	    pip->directory = DEFDIRECTORY ;

	if (pip->workdir == nullptr)
...
/* process the server table file */
	...
	if ((pip->srvtab == nullptr) || (pip->srvtab[0] == '\0')) {
	    ...
	    if (cp == nullptr)
	        cp = filereadable(tmpfname,
	            nullptr,nullptr,DEFSRVFILE1) ;

	    if (cp == nullptr)
	        cp = filereadable(tmpfname,
	            nullptr,nullptr,DEFSRVFILE2) ;
...
/* before we go too far, are we the only one on this PID mutex? */
	...
	if ((pip->pidfile != nullptr) && (pip->pidfile[0] != '\0')) {
...
daemonret:
	...
	if (pip->lfp != nullptr) bclose(pip->lfp) ;
