...
local int	bufdesc_init(BD *) ;
local int	bufdesc_add(BD *,char *,int,int) ;
local int	bufdesc_free(BD *) ;
...
int process(PI *gp,bibdb *bdbp,bfile *ofp,cchar *fname) noex {
	...
	bdbgetname(bdbp,nullptr,&database) ;
	...
	if (gp->debuglevel > 1) {
	    ...
	    bdbcurbegin(bdbp,&cur) ;
	    while (bdbgetname(bdbp,&cur,&cp) >= 0) {
	            ...
	            if (! f_lookopen) {
	                ...
	                if (gp->debuglevel > 1) {
	                    ...
	                    if (database != nullptr)
	                        ...
	                        debugprintf("process: nullptr database\n") ;
...
#if	CF_DEBUG
	                if (gp->debuglevel > 1)
	                    debugprintf("process: read one rs=%d char=%c\n",
				rs,ch1) ;
#endif
...
#if	CF_DEBUG
	                if (gp->debuglevel > 1)
	                    debugprintf("process: read two rs=%d char=%c\n",
				rs,ch2) ;
#endif
...
/* read the response */
				...
				break ;
	                ...
	                if (ch1 == '%') {
	                    ...
	                    if ((len = breadln(lofp,bibbuf,BIBLEN - 1)) < 1) {
	                    ...
	                    bibbuf[0] = ch1 ;
	                    if ((len = 
	                        breadln(lofp,bibbuf + 1,BIBLEN - 2)) < 1) {
...
/* get rid of any remaining trash after our internal buffer limit */
	                ...
	                while ((len > 0) && (bibbuf[len - 1] != '\n'))
	                    len = breadln(lofp,bibbuf,BIBLEN) ;
...
local int bufdesc_add(bdp,buf,len,f_comma)
BD	*bdp ;
char	buf[] ;
int	len, f_comma ;
{
...
/* end subroutine (bufdesc_free) */

local int br_init(brp)
struct br	*brp ;
{
