...
int main(int argc,mainv argv,mainv envv) {
	...
	cchar	*helpfname = nullptr ;
	cchar	*afname = nullptr ;
	cchar	*eigenfname = nullptr ;
	cchar	*ofname = nullptr ;
	cchar	*ignorechars = IGNORECHARS ;
	cchar	*delimiter = " " ;
	cchar	*cp ;
...
/* get some host/user information */
	...
	if ((rs = userinfo(&u,userbuf,USERINFO_LEN,nullptr)) < 0) {
...
/* search locally */
	...
	if ((configfname == nullptr) || (configfname[0] == '\0'))
	    configfname = filereadable(tmpfname,
	        nullptr,ETCDIR1,CONFIGFILE1) ;

	if (configfname == nullptr)
	    configfname = filereadable(tmpfname,
	        nullptr,ETCDIR1,CONFIGFILE2) ;

	if (configfname == nullptr)
	    configfname = filereadable(tmpfname,
	        nullptr,ETCDIR2,CONFIGFILE1) ;
...
/* read in the configuration file if we have one */
	...
	if (u_access(configfname,R_OK) >= 0) {
	    ...
	    if ((cf.root != nullptr) && (g.programroot == nullptr)) {
	    ...
	    if ((cf.helpfname != nullptr) && (helpfname == nullptr)) {
	        ...
	        cf.helpfname = nullptr ;

	    } /* end if (helpfname) */

	    if ((cf.eigenfname != nullptr) && (eigenfname == nullptr)) {
	        ...
	        cf.eigenfname = nullptr ;

	    } /* end if (eigenfname) */

	    if ((cf.tmpdir != nullptr) && (g.tmpdir == nullptr)) {
...
/* clean up the EigenWord dictionary path as we may have it */
	...
	if ((eigenfname == nullptr) || (eigenfname[0] == '\0'))
...
/* the database(s) */
	...
	if (bdbcount(&bdbl) <= 0) {
	    ...
	    if ((cp = getenv("BIBLIOGRAPHY")) != nullptr)
	        bdbloads(&bdbl,cp,-1) ;

	    else if ((cp = getenv("REFER")) != nullptr)
	        bdbloads(&bdbl,cp,-1) ;
...
/* process any files in the argument filename list file */
	...
	if ((rs >= 0) && (afname != nullptr) && (afname[0] != '\0')) {
		...
		char	linebuf[LINEBUFLEN + 1] ;
	        while ((rs = breadln(&afile,linebuf,LINEBUFLEN)) > 0) {
		    if (linebuf[len - 1] == '\n') len -= 1 ;
	            linebuf[len] = '\0' ;
	            cp = linebuf ;
	            if ((cp[0] == '\0') || (cp[0] == '#'))
...
/* is a file readable */
local char *filereadable(tmpfname,dir1,dir2,fname)
