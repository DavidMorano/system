...
/*******************************************************************************
	...
	Description:
	This subroutine processes the input and mails off the message
	to each of the recipients.  The format of the transmitted
	email message is:
	...
	only process one of these at a ctimeere.
...
#include	<getchostname.h>	/* LIBUC */
#include	<estrings.h>		/* LIBUC */
#include	<cksum.h>		/* LIBUC */
#include	<pcsconf.h>		/* LIBUC */
#include	<mailaddr.h>		/* LIBUC */
#include	<recipient.h>		/* LIBUC */
...
/* get the recipients */
	...
	if (rs < 0)
	    ...
	    while ((rs = breadln(ifp,buf,BUFLEN)) > 1) {
	        len = rs ;
...
#if	CF_DEBUG
	                if (DEBUGLEVEL(2))
	                    debugprintf("process: recipient=%r\n",cp,cl) ;
#endif
...
/* process the addresses out as we have them so far */
	...
	while (recipient_hcurenum(&recips,&hc,&hp) >= 0) {
	    RECIPIENT_VAL		*vep ;
...
char		envelope_from[] ;
{
	...
	envelope_from[0] = '\0' ;
	envelope_host[0] = '\0' ;
	if ((rs = breadln(fp,linebuf,LINELEN)) > 1) {
	    len = rs ;
